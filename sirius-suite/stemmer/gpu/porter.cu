...
{  switch (z->b[i])
  {  case 'a': case 'e': case 'i': case 'o': case 'u': return FALSE;
     case 'y': return (i == 0) ? TRUE : !cons1(z, i - 1);
     default: return TRUE;
  }
}
...
__host__ __device__ static int memcmp1(const void *buffer1,const void *buffer2,int count)
{
   if (!count)
      return(0);
...
__host__ __device__ static void r(struct stemmer * z, char * s) { if (m(z) > 0) setto(z, s); }
...
__host__ __device__ static void step2(struct stemmer * z) { switch (z->b[z->k-1])
{
...
__host__ __device__ static void step3(struct stemmer * z) { switch (z->b[z->k])
{
...
__global__ void stem3(struct stemmer **stem_list,int array_size){
  ...
  step1ab(stem_list[tid]);step1c(stem_list[tid]);step2(stem_list[tid]);step3(stem_list[tid]);step4(stem_list[tid]);step5(stem_list[tid]);
  }
}
...
{  
    ...
    struct stemmer **host_stem_list= (struct stemmer **) malloc(ARRAYSIZE * sizeof(struct stemmer *));
    struct stemmer **host2_stem_list= (struct stemmer **) malloc(ARRAYSIZE * sizeof(struct stemmer *));
    ...
    for(int i = 0;i < array_size; i++) {
        ...
        cudaMemcpy(host_z->b, stem_list[i]->b,(i_max+1)*sizeof(char),cudaMemcpyHostToDevice); 
        cudaMemcpy(dev_z,host_z,sizeof(struct stemmer), cudaMemcpyHostToDevice); 
        cudaMemcpy(&(dev_stem_list[i]),&dev_z,sizeof(struct stemmer *), cudaMemcpyHostToDevice);   
    ...
    for(int i = 0;i < array_size;i++)
    {
        ...
        cudaMemcpy(&(stem_list[i]->k),&(host2_stem_list[i]->k),sizeof(int),cudaMemcpyDeviceToHost);
        cudaMemcpy(stem_list[i]->b, host_stem_list[i]->b,(i_max+1)*sizeof(char),cudaMemcpyDeviceToHost);
