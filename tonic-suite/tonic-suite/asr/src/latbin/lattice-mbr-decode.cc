...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "> >\n"
        "for which we use the same I/O routines as for posteriors (type "
        "Posterior).\n"
        "times-wspecifier writes pairs of (start-time, end-time) in frames, "
        "for\n"
        "each sausage position, or for each one-best entry if "
        "--one-best-times=true.\n"
        "Note: use ark:/dev/null or the empty string for unwanted outputs.\n"
        "Note: times will only be very meaningful if you first use "
        "lattice-word-align.\n"
        "If you need ctm-format output, don't use this program but use "
        "lattice-to-ctm-conf\n"
    ...
    po.Register("one-best-times", &one_best_times,
                "If true, output times "
    ...
    std::string lats_rspecifier = po.GetArg(1), trans_wspecifier = po.GetArg(2),
    ...
    for (; !clat_reader.Done(); clat_reader.Next()) {
      ...
      if (trans_wspecifier != "") trans_writer.Write(key, mbr.GetOneBest());
