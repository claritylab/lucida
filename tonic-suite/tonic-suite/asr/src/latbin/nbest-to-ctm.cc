...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "and must be in CompactLattice form where the transition-ids on the "
        "arcs\n"
        "have been aligned with the word boundaries... typically the input "
        "will\n"
        "be a lattice that has been piped through lattice-1best and then\n"
        "lattice-word-align.  It outputs ctm format (with integers in place of "
        "words),\n"
        "assuming the frame length is 0.01 seconds by default (change this "
        "with the\n"
        "--frame-length option).  Note: the output is in the form\n"
        "utterance-id 1 <begin-time> <end-time> <word-id>\n"
        "and you can post-process this to account for segmentation issues and "
        "to \n"
        "convert ints to words; note, the times are relative to start of the "
        "utterance.\n"
        "\n"
        "Usage: nbest-to-ctm [options] <aligned-linear-lattice-rspecifier> "
        "<ctm-wxfilename>\n"
        "e.g.: lattice-1best --acoustic-weight=0.08333 ark:1.lats | \\\n"
        "      lattice-align-words data/lang/phones/word_boundary.int "
        "exp/dir/final.mdl ark:- ark:- | \\\n"
    ...
    std::string lats_rspecifier = po.GetArg(1), ctm_wxfilename = po.GetArg(2);
    ...
    ko.Stream() << std::fixed;  // Set to "fixed" floating point model, where
                                // precision() specifies
    ...
    // We do it this time in order to avoid wrongly printing out a success
    // message
