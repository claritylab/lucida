...
class DeterminizeLatticeTask {
 public:
  ...
  DeterminizeLatticeTask(fst::DeterminizeLatticePrunedOptions &opts,
                         std::string key, BaseFloat acoustic_scale,
                         BaseFloat beam, bool minimize, Lattice *lat,
                         CompactLatticeWriter *clat_writer, int32 *num_warn)
      : opts_(opts),
        key_(key),
  ...
  void operator()() {
    ...
    if (!TopSort(lat_)) {
      KALDI_WARN
          << "Could not topologically sort lattice: this probably means it"
             " has bad properties e.g. epsilon cycles.  Your LM or lexicon "
             "might "
             "be broken, e.g. LM with epsilon cycles or lexicon with empty "
             "words.";
      (*num_warn_)++;
    }
    ...
    if (!DeterminizeLatticePruned(*lat_, beam_, &det_clat_, opts_)) {
      KALDI_WARN
          << "For key " << key_
          << ", determinization did not succeed"
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "acoustic states)\n"
        "for each input-symbol sequence.  This is a version of "
        "lattice-determnize-pruned\n"
        "that accepts the --num-threads option.  These programs do pruning as "
        "part of the\n"
        "determinization algorithm, which is more efficient and prevents "
        "blowup.\n"
        "See http://kaldi.sourceforge.net/lattices.html for more information "
        "on lattices.\n"
        "\n"
        "Usage: lattice-determinize-pruned-parallel [options] "
        "lattice-rspecifier lattice-wspecifier\n"
        " e.g.: lattice-determinize-pruned-parallel --acoustic-scale=0.1 "
        "--beam=6.0 ark:in.lats ark:det.lats\n";
    ...
    fst::DeterminizeLatticePrunedOptions
        determinize_config;  // Options used in DeterminizeLatticePruned--
    // this options class does not have its own Register function as it's viewed
    // as
    // being more part of "fst world", so we register its elements
    // independently.
    ...
    for (; !lat_reader.Done(); lat_reader.Next()) {
      ...
      Lattice *lat =
          lat_reader.Value().Copy();  // will give ownership to "task"
                                      // below

      KALDI_VLOG(2) << "Processing lattice " << key;

      DeterminizeLatticeTask *task = new DeterminizeLatticeTask(
          determinize_config, key, acoustic_scale, beam, minimize, lat,
          &compact_lat_writer, &n_warn);
      sequencer.Run(task);
      n_done++;
    }
    sequencer.Wait();
