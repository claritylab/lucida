...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Read in a Table containing N-best entries from a lattices (i.e. "
        "individual\n"
        "lattices with a linear structure, one for each N-best entry, indexed "
        "by\n"
    ...
    std::string nbest_rspecifier = po.GetArg(1), lats_wspecifier = po.GetArg(2);
    ...
    for (; !compact_nbest_reader.Done(); compact_nbest_reader.Next()) {
      ...
      if (!GetUtteranceId(nbest_id, &utt_id)) {
        KALDI_ERR << "Invalid n-best id " << nbest_id
                  << ": make sure you "
    ...
    if (cur_utt_id != "") compact_lattice_writer.Write(cur_utt_id, cur_union);

    KALDI_LOG << "Done joining n-best into lattices for " << n_utt_done
              << " utterances, with on average "
