...
bool DeterminizeLatticeWrapper(const Lattice &lat, const std::string &key,
                               bool prune, BaseFloat beam, BaseFloat beam_ratio,
                               int32 max_mem, int32 max_loop, BaseFloat delta,
                               int32 num_loops, CompactLattice *clat) {
      ...
      for (; i < num_loops; i++) {
        ...
        if (NumArcs(lat) == NumArcs(pruned_lat)) {
          ...
          KALDI_WARN << "Pruning did not have an effect on the original "
                     << "lattice size; reducing beam to " << cur_beam
                     << " and re-trying.";
  ...
  KALDI_WARN << "Decreased pruning beam --num-loops=" << num_loops
             << " times and was not able to determinize: failed for " << key;
  return false;
}
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        " (see http://kaldi.sourceforge.net/lattices.html for more "
        "explanation)\n"
        " note: this program is tyically only useful if you generated "
        "state-level\n"
        " lattices, e.g. called gmm-latgen-simple with --determinize=false\n"
        "\n"
        "Usage: lattice-determinize [options] lattice-rspecifier "
        "lattice-wspecifier\n"
        " e.g.: lattice-determinize --acoustic-scale=0.1 --beam=15.0 "
        "ark:1.lats ark:det.lats\n";
    ...
    po.Register("prune", &prune,
                "If true, prune determinized lattices "
    ...
    po.Register("max-loop", &max_loop,
                "Option to detect a certain "
    ...
    po.Register("num-loops", &num_loops,
                "Number of times to "
    ...
    for (; !lattice_reader.Done(); lattice_reader.Next()) {
      ...
      CompactLattice clat;
      if (DeterminizeLatticeWrapper(lat, key, prune, beam, beam_ratio, max_mem,
                                    max_loop, delta, num_loops, &clat)) {
        if (minimize) {
