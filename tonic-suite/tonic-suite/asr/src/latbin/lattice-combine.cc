...
bool CompactLatticeNormalize(CompactLattice *clat, BaseFloat weight,
  ...
  if (exp_weights) fst::ScaleLattice(fst::LatticeScale(weight, weight), clat);
  ...
  if (!exp_weights) total_backward_cost -= std::log(weight);
...
void SplitStringToWeights(const string &full, const char *delim,
  ...
  if (tmp.size() != out->size()) {
    KALDI_WARN << "Expecting " << out->size() << " weights, found "
               << tmp.size() << ": using uniform weights.";
    return;
  }
  BaseFloat sum = 0;
  for (vector<BaseFloat>::const_iterator itr = tmp.begin(); itr != tmp.end();
       ++itr) {
    if (*itr < 0.0) {
      KALDI_WARN << "Cannot use negative weight: " << *itr
  ...
  if (sum != 1.0) {
    ...
    for (vector<BaseFloat>::iterator itr = tmp.begin(); itr != tmp.end(); ++itr)
      (*itr) /= sum;
  }
  out->swap(tmp);
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Combine lattices generated by different systems by removing the "
        "total\n"
        "cost of all paths (backward cost) from individual lattices and doing\n"
        "a union of the reweighted lattices.  Note: the acoustic and LM "
        "scales\n"
        "that this program applies are not removed before outputting the "
        "lattices.\n"
        "Intended for use in system combination prior to MBR decoding, see "
        "comments\n"
        "in code.\n"
        "Usage: lattice-combine [options] lattice-rspecifier1 "
        "lattice-rspecifier2"
    ...
    po.Register("lm-scale", &lm_scale,
                "Scaling factor for language model "
                "probabilities");
    po.Register("lat-weights", &weight_str,
                "Colon-separated list of weights "
    ...
    po.Register("exp-weights", &exp_weights,
                "If true, lattice weights are "
    ...
    for (int32 i = 2; i < num_args; ++i) {
      clat_reader_vec[i - 2] =
          new RandomAccessCompactLatticeReader(po.GetArg(i));
    ...
    int32 n_utts = 0, n_total_lats = 0, n_success = 0, n_missing = 0,
          n_other_errors = 0;
    vector<vector<double> > lat_scale =
        fst::LatticeScale(lm_scale, acoustic_scale);
    ...
    for (; !clat_reader1.Done(); clat_reader1.Next()) {
      ...
      bool success =
          CompactLatticeNormalize(&clat1, lat_weights[0], exp_weights);
      if (!success) {
      ...
      for (int32 i = 0; i < num_args - 2; ++i) {
        if (clat_reader_vec[i]->HasKey(key)) {
          ...
          success =
              CompactLatticeNormalize(&clat2, lat_weights[i + 1], exp_weights);
          if (!success) {
