...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "<transcriptions-rspecifier> (<lm-cost-rspecifier>|'') "
        "(<ac-cost-rspecifier>|'') "
        "<nbest-wspecifier>\n"
        "Note: if the rspecifiers for lm-cost or ac-cost are the empty "
        "string,\n"
        "these value will default to zero.\n"
        " e.g.: linear-to-nbest ark:1.ali ark:1.tra ark:1.lmscore "
        "ark:1.acscore "
        "ark:1.nbest\n";
    ...
    std::string ali_rspecifier = po.GetArg(1), trans_rspecifier = po.GetArg(2),
                lm_cost_rspecifier = po.GetArg(3),
                ac_cost_rspecifier = po.GetArg(4),
                lats_wspecifier =
                    po.GetArg(5);  // will probably represent N-best.
    ...
    for (; !ali_reader.Done(); ali_reader.Next()) {
      ...
      BaseFloat ac_cost = (ac_cost_rspecifier == "")
                              ? 0.0
                              : ac_cost_reader.Value(key),
                lm_cost = (lm_cost_rspecifier == "")
                              ? 0.0
                              : lm_cost_reader.Value(key);
      Lattice lat;
