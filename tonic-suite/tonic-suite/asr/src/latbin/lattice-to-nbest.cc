...
// Copyright 2009-2012  Stefan Kombrink  Johns Hopkins University (Author:
// Daniel Povey)
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "you produced lattices with --determinize-lattice=false, i.e. "
        "state-level\n"
        "lattices).\n"
        "Usage: lattice-to-nbest [options] lattice-rspecifier "
        "lattice-wspecifier\n"
        " e.g.: lattice-to-nbest --acoustic-scale=0.1 --n=10 ark:1.lats "
        "ark:nbest.lats\n";
    ...
    for (; !lattice_reader.Done(); lattice_reader.Next()) {
      ...
      std::vector<Lattice> nbest_lats;
      {
        ...
        if (!random) {
          ...
          fst::RandGenOptions<fst::UniformArcSelector<LatticeArc> > opts(
              uniform_selector);
          opts.npath = n;
    ...
    KALDI_LOG << "Done applying N-best algorithm to " << n_done
              << " lattices with n = " << n << ", average actual #paths is "
