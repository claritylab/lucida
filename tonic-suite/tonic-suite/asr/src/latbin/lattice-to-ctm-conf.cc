...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Maximum A Posteriori (but works out the confidences based on "
        "posteriors\n"
        "in the lattice, using the MBR code).  Note: if you don't need "
        "confidences,\n"
    ...
    po.Register("lm-scale", &lm_scale,
                ...
                "probabilities");
    po.Register("decode-mbr", &decode_mbr,
                "If true, do Minimum Bayes Risk "
    ...
    for (; !clat_reader.Done(); clat_reader.Next()) {
      ...
      KALDI_LOG << "For utterance " << key << ", Bayes Risk "
                << mbr.GetBayesRisk() << " and per word, "
                << mbr.GetBayesRisk() / mbr.GetOneBest().size();
      n_done++;
