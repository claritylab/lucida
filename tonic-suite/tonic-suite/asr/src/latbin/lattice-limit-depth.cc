...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Requires an acoustic scale, because forward-backward Viterbi probs "
        "are\n"
        "needed, which will be affected by this.\n"
        "\n"
        "Usage: lattice-limit-depth [options] <lattice-rspecifier> "
        "<lattice-wspecifier>\n"
        "E.g.: lattice-limit-depth --max-arcs-per-frame=1000 "
        "--acoustic-scale=0.1 ark:- ark:-\n";
    ...
    for (; !clat_reader.Done(); clat_reader.Next()) {
      ...
      KALDI_VLOG(2) << "For key " << key << ", depth changed from " << depth_in
                    << " to " << depth_out << " over " << t << " frames.";
