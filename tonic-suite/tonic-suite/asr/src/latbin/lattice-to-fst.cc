...
int main(int argc, char *argv[]) {
  try {
    ...
    std::string lats_rspecifier = po.GetArg(1), fsts_wspecifier = po.GetArg(2);
    ...
    for (; !lattice_reader.Done(); lattice_reader.Next()) {
      ...
      fst::VectorFst<StdArc> fst;
      {
        Lattice lat;
        ConvertLattice(clat,
                       &lat);  // convert to non-compact form.. won't introduce
        // extra states because already removed alignments.
        ConvertLattice(lat,
                       &fst);  // this adds up the (lm,acoustic) costs to get
        // the normal (tropical) costs.
        Project(
            &fst,
            fst::PROJECT_OUTPUT);  // Because in the standard Lattice format,
