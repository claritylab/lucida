...
void ConvertFstToArcLattice(fst::StdVectorFst *net, kaldi::Lattice *lat,
  ...
  for (fst::StateIterator<fst::StdVectorFst> siter(*net); !siter.Done();
       siter.Next()) {
...
void MapTransitionIdsToTransitionStates(kaldi::CompactLattice *lat,
  ...
  for (fst::StateIterator<kaldi::CompactLattice> siter(*lat); !siter.Done();
       siter.Next()) {
    StateId s = siter.Value();
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Compose decoding graph with given lattices to obtain active-arc "
        "lattices.\n"
    ...
    std::string model_filename = po.GetArg(1), fst_in_filename = po.GetArg(2),
                lats_rspecifier = po.GetArg(3), arcs_wspecifier = po.GetArg(4);
    ...
    std::vector<std::pair<int32, int32> >
        arc_map;  // maps state/arc to symbol-id
    ConvertFstToArcLattice(hclg, &graph, &arc_map, &trans_model,
                           true);  // keep weights
    ...
    for (; !lat_reader.Done(); lat_reader.Next()) {
      ...
      fst::Project(&lat_mapped,
                   fst::PROJECT_INPUT);  // keep only transition states
      ...
      if (reverse) {
        ...
        Compose(lat_reverse, graph,
                &lat_composed);  // composed FST contains HCLG arcs
      } else {
        KALDI_LOG << "compose with lattice " << key;
        Compose(lat_mapped, graph,
                &lat_composed);  // composed FST contains HCLG arcs
      }
      if (lattice_wspecifier != "") lat_writer.Write(key, lat_composed);

      CompactLattice clat_determinized;
      if (DeterminizeLattice(lat_composed, &clat_determinized, lat_opts,
                             NULL)) {
        ...
        ArcSort(&fst_det,
                fst::ILabelCompare<fst::StdArc>());  // to improve speed
    ...
    KALDI_LOG << "Done converting " << n_done
              << " lattices, failed: " << n_error;
    return (n_done != 0 ? 0 : 1);
