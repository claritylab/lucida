...
struct LogisticRegressionConfig {
  int32 max_steps, mix_up;
  double normalizer, power;
  LogisticRegressionConfig()
      : max_steps(20), mix_up(0), normalizer(0.0025), power(0.15) {}
  void Register(OptionsItf *po) {
    po->Register("max-steps", &max_steps, "Maximum steps in L-BFGS.");
...
class LogisticRegression {
 ...
 private:
  ...
  BaseFloat DoStep(const Matrix<BaseFloat> &xs, Matrix<BaseFloat> *xw,
                   const std::vector<int32> &ys,
                   OptimizeLbfgs<BaseFloat> *lbfgs, BaseFloat normalizer);
  ...
  BaseFloat GetObjfAndGrad(const Matrix<BaseFloat> &xs,
                           const std::vector<int32> &ys,
                           const Matrix<BaseFloat> &xw, Matrix<BaseFloat> *grad,
