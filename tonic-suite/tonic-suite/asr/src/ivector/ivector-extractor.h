...
class IvectorExtractorUtteranceStats {
 public:
  IvectorExtractorUtteranceStats(int32 num_gauss, int32 feat_dim,
    ...
    if (need_2nd_order_stats) {
      S_.resize(num_gauss);
      for (int32 i = 0; i < num_gauss; i++) S_[i].Resize(feat_dim);
    }
  }
 ...
 protected:
  ...
  Vector<double>
      gamma_;         // zeroth-order stats (summed posteriors), dimension [I]
  Matrix<double> X_;  // first-order stats, dimension [I][D]
  std::vector<SpMatrix<double> >
      S_;  // 2nd-order stats, dimension [I][D][D], if
           // required.
};
...
struct IvectorExtractorOptions {
  ...
  IvectorExtractorOptions()
      : ivector_dim(400), num_iters(2), use_weights(true) {}
  void Register(OptionsItf *po) {
    ...
    po->Register("use-weights", &use_weights,
                 "If true, regress the "
...
class OnlineIvectorEstimationStats {
 public:
  OnlineIvectorEstimationStats(int32 ivector_dim, BaseFloat prior_offset);
...
// It returns the objective function improvement per frame from the "default"
// iVector to
...
struct IvectorExtractorStatsOptions {
  ...
  IvectorExtractorStatsOptions()
      : update_variances(true),
        ...
        cache_size(100) {}
  void Register(OptionsItf *po) {
    po->Register("update-variances", &update_variances,
                 "If true, update the "
                 "Gaussian variances");
    po->Register("compute-auxf", &compute_auxf,
                 "If true, compute the "
...
struct IvectorExtractorEstimationOptions {
  ...
  IvectorExtractorEstimationOptions()
      : variance_floor_factor(0.1), gaussian_min_count(100.0) {}
  void Register(OptionsItf *po) {
    po->Register(
...
class IvectorExtractorStats {
 public:
  ...
  void Write(std::ostream &os,
             bool binary);  // non-const version; relates to cache.
 ...
 protected:
  ...
  double UpdateProjection(const IvectorExtractorEstimationOptions &opts,
                          int32 gaussian, IvectorExtractor *extractor) const;
  ...
  /// This mutex guards num_ivectors_, ivector_sum_ and ivector_scatter_ (for
  /// multi-threaded
  /// update)
  Mutex prior_stats_lock_;

  /// Count of the number of iVectors we trained on.   Need for prior
  /// re-estimation.
 ...
 private:
  IvectorExtractorStats &operator=(
      const IvectorExtractorStats &other);  // Disallow.
};
