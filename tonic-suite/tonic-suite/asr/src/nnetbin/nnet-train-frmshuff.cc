...
int main(int argc, char* argv[]) {
  ...
  try {
    const char* usage =
        "Perform one iteration of Neural Network training by mini-batch "
        "Stochastic Gradient Descent.\n"
        "This version use pdf-posterior as targets, prepared typically by "
        "ali-to-post.\n"
        "Usage:  nnet-train-frmshuff [options] <feature-rspecifier> "
        "<targets-rspecifier> <model-in> [<model-out>]\n"
        "e.g.: \n"
        " nnet-train-frmshuff scp:feature.scp ark:posterior.ark nnet.init "
        "nnet.iter1\n";
    ...
    bool binary = true, crossvalidate = false, randomize = true;
    ...
    po.Register("dropout-retention", &dropout_retention,
                "number between 0..1, saying how many neurons to preserve (0.0 "
...
// Select the GPU
    ...
    while (!feature_reader.Done()) {
      ...
      for (; !feature_reader.Done(); feature_reader.Next()) {
        if (feature_randomizer.IsFull())
          break;  // suspend, keep utt for next loop
        ...
        // correct small length mismatch ... or drop sentence
        {
          ...
          if (max - min < length_tolerance) {
            ...
            KALDI_WARN << utt << ", length mismatch of targets "
                       << targets.size() << " and features " << mat.NumRows();
            num_other_error++;
            continue;
          }
        }
      ...
      for (; !feature_randomizer.Done(); feature_randomizer.Next(),
        ...
        if (kaldi::g_kaldi_verbose_level >= 2) {  // vlog-2
          if ((total_frames / 25000) != ((total_frames + nnet_in.NumRows()) /
                                         25000)) {  // print every 25k frames
    ...
    KALDI_LOG << "Done " << num_done << " files, " << num_no_tgt_mat
              ...
              << time.Elapsed() / 60 << " min, fps"
              << total_frames / time.Elapsed() << "]";
