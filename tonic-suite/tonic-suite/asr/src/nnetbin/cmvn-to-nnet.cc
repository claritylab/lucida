...
int main(int argc, char* argv[]) {
  try {
    ...
    po.Register("tied-normalization", &tied_normalzation,
                "The normalization is tied accross all the input dimensions");
    ...
    for (int32 d = 0; d < cmvn_stats.NumCols() - 1; d++) {
      ...
      if (var <= var_floor) {
        KALDI_WARN << "Very small variance " << var << " flooring to "
                   << var_floor;
        var = var_floor;
      }
      shift(d) = -mean;
