...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    kaldi::int32 max_frames =
        6000;  // Allow segments maximum of 30 seconds by default
    ...
    trn_opts_rbm.learn_rate =
        learn_rate * (1 - momentum);  // keep `effective' learning rate constant
    // pass options to RBM
    ...
    CuMatrix<BaseFloat> feats, feats_transf, pos_hid, pos_hid_aux, neg_vis,
        neg_hid;
    ...
    while (!feature_reader.Done()) {
      ...
      for (; !feature_randomizer.Done(); feature_randomizer.Next()) {
        ...
        // get the dims
        int32 num_frames = pos_vis.NumRows(), dim_hid = rbm.OutputDim();
        ...
        // change the momentum progressively per 0.5million samples of the data
        {
          ...
          int32 n =
              total_frames /
              momentum_step_period;  // change every momentum_step_period data
