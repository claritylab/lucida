...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "Usage:  nnet-train-mmi-sequential [options] <model-in> "
        "<transition-model-in> "
        "<feature-rspecifier> <den-lat-rspecifier> <ali-rspecifier> "
        "[<model-out>]\n"
        "e.g.: \n"
        " nnet-train-mmi-sequential nnet.init trans.mdl scp:train.scp "
...
// Select the GPU
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if ((int32)num_ali.size() != mat.NumRows()) {
        KALDI_WARN << "Numerator alignment has wrong length " << num_ali.size()
                   << " vs. " << mat.NumRows();
        num_other_error++;
        continue;
      }
      ...
      KALDI_VLOG(1) << "Utterance " << utt
                    << ": Average MMI obj. value = " << (mmi_obj / num_frames)
                    << " over " << num_frames << " frames."
                    << " (Avg. den-posterior on ali "
                    << post_on_ali / num_frames << ")";
    ...
    KALDI_LOG << "Done " << num_done << " files, " << num_no_num_ali
              << " with no numerator alignments, " << num_no_den_lat
              << " with no denominator lattices, " << num_other_error
              << " with other errors.";

    KALDI_LOG << "Overall MMI-objective/frame is " << std::setprecision(8)
              << (total_mmi_obj / total_frames) << " over " << total_frames
              << " frames."
