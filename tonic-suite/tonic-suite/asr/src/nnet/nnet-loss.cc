...
void Xent::Eval(const CuMatrixBase<BaseFloat>& net_out,
...
void Xent::Eval(const CuMatrixBase<BaseFloat>& net_out, const Posterior& post,
                CuMatrix<BaseFloat>* diff) {
  int32 num_frames = net_out.NumRows(), num_pdf = net_out.NumCols();
  ...
  for (int32 t = 0; t < post.size(); t++) {
    for (int32 i = 0; i < post[t].size(); i++) {
      ...
      if (pdf >= num_pdf) {
        KALDI_ERR << "Posterior pdf-id out of NN-output dimension, please "
                     "check number of pdfs by 'hmm-info'."
                  << " nn-outputs : " << num_pdf
                  << ", posterior pdf-id : " << pdf;
      }
  ...
  *diff = net_out;
  ...
  // progressive loss reporting
  {
    ...
    if (frames_progress_ > progress_step) {
      KALDI_VLOG(1) << "ProgressLoss[" << frames_progress_ / 100 / 3600 << "h/"
                    << frames_ / 100 / 3600 << "h]: "
                    << (loss_progress_ - entropy_progress_) / frames_progress_
                    << " (Xent)";
      // store
      loss_vec_.push_back((loss_progress_ - entropy_progress_) /
                          frames_progress_);
      // reset
      frames_progress_ = 0;
      loss_progress_ = 0.0;
...
void Xent::EvalVec(const CuMatrixBase<BaseFloat>& net_out,
...
void Mse::Eval(const CuMatrixBase<BaseFloat>& net_out,
               const CuMatrixBase<BaseFloat>& target,
               CuMatrix<BaseFloat>* diff) {
  KALDI_ASSERT(net_out.NumCols() == target.NumCols());
  KALDI_ASSERT(net_out.NumRows() == target.NumRows());
  ...
  *diff = net_out;             // y
  ...
  double mean_square_error =
      0.5 * sum_diff_pow_2_host_.Sum();  // sum over rows (frames)
  ...
  // progressive loss reporting
  {
    ...
    if (frames_progress_ > progress_step) {
      KALDI_VLOG(1) << "ProgressLoss[" << frames_progress_ / 100 / 3600 << "h/"
                    << frames_ / 100 / 3600
                    << "h]: " << loss_progress_ / frames_progress_ << " (Mse)";
      // store
...
void Mse::Eval(const CuMatrixBase<BaseFloat>& net_out, const Posterior& post,
               CuMatrix<BaseFloat>* diff) {
  int32 num_frames = net_out.NumRows(), num_pdf = net_out.NumCols();
  KALDI_ASSERT(num_frames == post.size());
