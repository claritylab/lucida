...
const std::vector<int32>& RandomizerMask::Generate(int32 mask_size) {
  ...
  std::random_shuffle(mask_.begin(),
                      mask_.end());  // with built-in random generator
  return mask_;
}
...
void MatrixRandomizer::AddData(const CuMatrixBase<BaseFloat>& m) {
  ...
  if (data_begin_ > 0) {
    ...
    if (leftover > 0) {
      data_.RowRange(0, leftover)
          .CopyFromMat(data_.RowRange(data_begin_, leftover));
    }
    data_begin_ = 0;
    data_end_ = leftover;
    data_.RowRange(leftover, data_.NumRows() - leftover)
        .SetZero();  // zeroing the rest
  }
  // extend the buffer if necessary
  if (data_.NumRows() < data_end_ + m.NumRows()) {
    CuMatrix<BaseFloat> data_aux(data_);
    data_.Resize(data_end_ + m.NumRows() + 1000,
                 data_.NumCols());  // +1000 row extra
...
void MatrixRandomizer::Randomize(const std::vector<int32>& mask) {
  ...
  // (Here the vector 'mask_in_gpu' is typically shorter than number of rows in
  // 'data_aux_',
  //  because the the buffer 'data_aux_' is larger than capacity
  //  'randomizer_size'.
  //  The extra rows in 'data_aux_' do not contain speech frames and are not
  //  copied
  //  from 'data_aux_', the extra rows in 'data_' are unchanged by
  //  cu::Randomize.)
  cu::Randomize(data_aux_, mask_in_gpu, &data_);
}

void MatrixRandomizer::Next() { data_begin_ += conf_.minibatch_size; }

const CuMatrixBase<BaseFloat>& MatrixRandomizer::Value() {
  KALDI_ASSERT(data_end_ - data_begin_ >=
               conf_.minibatch_size);  // have data for minibatch
...
void VectorRandomizer::AddData(const Vector<BaseFloat>& v) {
  ...
  if (data_begin_ > 0) {
    ...
    data_begin_ = 0;
    data_end_ = leftover;
    data_.Range(leftover, data_.Dim() - leftover)
        .SetZero();  // zeroing the rest
  }
  // extend the buffer if necessary
...
void VectorRandomizer::Next() { data_begin_ += conf_.minibatch_size; }
...
void StdVectorRandomizer<T>::AddData(const std::vector<T>& v) {
  ...
  if (data_begin_ > 0) {
    ...
    KALDI_ASSERT(leftover < data_begin_);  // no overlap
    if (leftover > 0) {
      std::copy(data_.begin() + data_begin_,
                data_.begin() + data_begin_ + leftover, data_.begin());
    }
    data_begin_ = 0;
    data_end_ = leftover;
    // cannot do this, we don't know default value of arbitrary type!
    // data_.RowRange(leftover,data_.NumRows()-leftover).SetZero(); // zeroing
    // the rest
  }
  // extend the buffer if necessary
...
const std::vector<T>& StdVectorRandomizer<T>::Value() {
  ...
  typename std::vector<T>::iterator last =
      data_.begin() + data_begin_ + conf_.minibatch_size;  // not-copied
...
template class StdVectorRandomizer<
    std::vector<std::pair<int32, BaseFloat> > >;  // PosteriorRandomizer
}
}
