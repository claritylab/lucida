...
class ConvolutionalComponent : public UpdatableComponent {
 public:
  ...
  void InitData(std::istream &is) {
    ...
    while (!is.eof()) {
      ...
      else if (token == "<PatchDim>")
        ReadBasicType(is, false, &patch_dim_);
      else if (token == "<PatchStep>")
        ReadBasicType(is, false, &patch_step_);
      else if (token == "<PatchStride>")
        ReadBasicType(is, false, &patch_stride_);
      else
        KALDI_ERR << "Unknown token " << token << ", a typo in config?"
                  << " (ParamStddev|BiasMean|BiasRange|PatchDim|PatchStep|"
                     "PatchStride)";
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in,
    ...
    // backpropagate to vector of matrices (corresponding to position of a
    // filter)
    for (int32 p = 0; p < num_patches; p++) {
      feature_patch_diffs_[p].Resize(num_frames, filter_dim,
                                     kSetZero);  // reset
    ...
    for (int32 p = 0; p < num_patches; p++) {
      for (int32 s = 0; s < num_splice; s++) {
        CuSubMatrix<BaseFloat> src(
            feature_patch_diffs_[p].ColRange(s * patch_dim_, patch_dim_));
        CuSubMatrix<BaseFloat> tgt(
            in_diff->ColRange(p * patch_step_ + s * patch_stride_, patch_dim_));
        tgt.AddMat(1.0, src);  // sum
        // add 1.0 to keep track of target columns in the sum
        in_diff_summands_.Range(p * patch_step_ + s * patch_stride_, patch_dim_)
            .Add(1.0);
      }
    }
    // compensate #summands
 ...
 private:
  int32 patch_dim_,  ///< number of consecutive inputs, 1st dim of patch
      ...
      patch_stride_;  ///< shift for 2nd dim of a patch (i.e. frame length
                      ///before splicing)
