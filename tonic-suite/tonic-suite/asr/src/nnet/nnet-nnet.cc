  ...
  if (NumComponents() == 1) {
    ...
    if (components_[0]->IsUpdatable()) {
      UpdatableComponent *uc =
          dynamic_cast<UpdatableComponent *>(components_[0]);
...
void Nnet::Feedforward(const CuMatrixBase<BaseFloat> &in,
  ...
  for (L++; L <= NumComponents() - 2; L++) {
    components_[L]->Propagate(propagate_buf_[(L - 1) % 2],
                              &propagate_buf_[L % 2]);
  }
      ...
      switch (components_[n]->GetType()) {
        case Component::kAffineTransform: {
          ...
          AffineTransform *aff_t =
              dynamic_cast<AffineTransform *>(components_[n]);
...
void Nnet::SetDropoutRetention(BaseFloat r) {
  for (int32 c = 0; c < NumComponents(); c++) {
    if (GetComponent(c).GetType() == Component::kDropout) {
      ...
      KALDI_LOG << "Setting dropout-retention in component " << c << " from "
                << r_old << " to " << r;
    }
  }
}
...
void Nnet::Read(std::istream &is, bool binary) {
  ...
  while (NULL != (comp = Component::Read(is, binary))) {
    if (NumComponents() > 0 &&
        components_.back()->OutputDim() != comp->InputDim()) {
...
std::string Nnet::Info() const {
  ...
  for (int32 i = 0; i < NumComponents(); i++) {
    ostr << "component " << i + 1 << " : "
         << Component::TypeToMarker(components_[i]->GetType()) << ", input-dim "
         << components_[i]->InputDim() << ", output-dim "
         << components_[i]->OutputDim() << ", " << components_[i]->Info()
         << std::endl;
  }
  return ostr.str();
}
...
void Nnet::Check() const {
  ...
  if (KALDI_ISINF(sum)) {
    KALDI_ERR << "'inf' in network parameters (weight explosion, try lower "
                 "learning rate?)";
  }
  if (KALDI_ISNAN(sum)) {
...
void Nnet::SetTrainOptions(const NnetTrainOptions &opts) {
  ...
  for (int32 l = 0; l < NumComponents(); l++) {
    if (GetComponent(l).IsUpdatable()) {
      dynamic_cast<UpdatableComponent &>(GetComponent(l))
          .SetTrainOptions(opts_);
    }
  }
}
