...
/**
 * Convolutional2DComponent implements convolution over 2-axis (frequency and
 *temporal)
 ...
 * x_patch_dim_,y_patch_dim_     ... temporal and frequency axes sizes of the
 *patch (e.g. (9,9) for 9x9 2D filter)
 * x_patch_step_,y_patch_step_    ... temporal and frequencey sizes of shifts in
 *the convolution (e.g. (1,1) 2D filter with 1 step shift in both axes)
 * x_patch_stride_,y_patch_stride_  ... dimension of the feature (maps if inside
 *convolutional layer) (e.g. (11,32) for 32-band 11 frame spliced spectrogram
 *patch)
 * The type of convolution is controlled by hyperparameters:
 * fmap_x_len_, fmap_y_len_ ... dimension of the feature (maps if inside
 *convolutional layer) (e.g. (11,32) for 32-band 11 frame spliced spectrogram
 *patch)
 * filt_x_len_, filt_y_len_ ... temporal and frequency sizes of the filters
 *(e.g. (9,9) for 9x9 2D filter)
 * filt_x_step_, filt_y_step_ ... temporal and frequency sizes of the filters
 *(e.g. (1,1) for 2D-filter, with 1 step shift in both axes)
 *
 *
...
class Convolutional2DComponent : public UpdatableComponent {
 public:
  ...
  void InitData(std::istream &is) {
    ...
    while (!is.eof()) {
      ...
      else if (token == "<FmapXLen>")
        ReadBasicType(is, false, &fmap_x_len_);
      else if (token == "<FmapYLen>")
        ReadBasicType(is, false, &fmap_y_len_);
      else if (token == "<FiltXLen>")
        ReadBasicType(is, false, &filt_x_len_);
      else if (token == "<FiltYLen>")
        ReadBasicType(is, false, &filt_y_len_);
      else if (token == "<FiltXStep>")
        ReadBasicType(is, false, &filt_x_step_);
      else if (token == "<FiltYStep>")
        ReadBasicType(is, false, &filt_y_step_);
      else if (token == "<ConnectFmap>")
        ReadBasicType(is, false, &connect_fmap_);
      else
    ...
    //
    Matrix<BaseFloat> mat(num_filters,
                          num_input_fmaps * filt_x_len_ * filt_y_len_);
  ...
  void ReadData(std::istream &is, bool binary) {
    ...
    // int32 num_output_fmaps = output_dim_ / (out_fmap_x_len * out_fmap_y_len);
    // KALDI_LOG << "num_output_fmaps " << num_output_fmaps;
    // int32 num_filters = filters_.NumRows(); // this is total num_filters, so
  ...
  void GetParams(Vector<BaseFloat> *wei_copy) const {
    ...
    wei_copy->Range(0, filters_num_elem)
        .CopyRowsFromMat(Matrix<BaseFloat>(filters_));
    wei_copy->Range(filters_num_elem, bias_.Dim())
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in,
    ...
    for (int32 m = 0; m < fmap_x_len_ - filt_x_len_ + 1; m = m + filt_x_step_) {
      for (int32 n = 0; n < fmap_y_len_ - filt_y_len_ + 1;
           n = n + filt_y_step_) {
    ...
    for (int32 p = 0; p < out_fmap_size; p++) {
      ...
      tgt.AddMatMat(1.0, vectorized_feature_patches_[p], kNoTrans, filters_,
                    kTrans, 1.0);
    }
  }
  ...
  void BackpropagateFnc(const CuMatrixBase<BaseFloat> &in,
    ...
    for (int32 p = 0; p < out_fmap_size; p++) {
      ...
      CuSubMatrix<BaseFloat> out_diff_patch(
          out_diff.ColRange(p * num_filters, num_filters));
      feature_patch_diffs_[p].AddMatMat(1.0, out_diff_patch, kNoTrans, filters_,
                                        kNoTrans, 0.0);
    }
        ...
        for (int32 i = 0; i < filt_x_len_; i++) {
          for (int32 j = 0; j < filt_y_len_ * num_input_fmaps; j++) {
            ...
            CuSubMatrix<BaseFloat> src(
                feature_patch_diffs_[out_fmap_cnt].ColRange(
                    i * filt_y_len_ * num_input_fmaps + j,
                    1));  // from which col
            CuSubMatrix<BaseFloat> tgt(
                in_diff->ColRange(c, 1));  // to which col?
            tgt.AddMat(1.0, src);
            // add 1.0
  ...
  void Update(const CuMatrixBase<BaseFloat> &input,
    ...
    */
    ...
    for (int32 p = 0; p < out_fmap_size; p++) {
      CuSubMatrix<BaseFloat> diff_patch(
          diff.ColRange(p * num_filters, num_filters));
      filters_grad_.AddMatMat(1.0, diff_patch, kTrans,
                              vectorized_feature_patches_[p], kNoTrans, 1.0);
 ...
 private:
  int32 fmap_x_len_, fmap_y_len_,  ///< feature maps dimensions (for input x_ is
                                   ///usually splice and y_ is num of fbanks)
                                   ///shift for 2nd dim of a patch (i.e. frame
                                   ///length before splicing)
      filt_x_len_,
      filt_y_len_,  ///< 2D filter dimensions, x_ temporal, y_ spectral
