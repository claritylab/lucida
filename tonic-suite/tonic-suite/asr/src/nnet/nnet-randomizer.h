...
struct NnetDataRandomizerOptions {
  int32 randomizer_size;  // Maximum number of samples we want to have in memory
                          // at once.
  ...
  void Register(OptionsItf* po) {
    po->Register("randomizer-size", &randomizer_size,
                 "Capacity of randomizer, length of concatenated utterances "
                 "which are used for frame-level shuffling (in frames, affects "
                 "memory consumption, max 8000000).");
    po->Register(
...
/// Generates index-mask, which is used to randomize order of datapoints (speech
/// frames)
...
class MatrixRandomizer {
 ...
 private:
  ...
  /// Cursor to beginning of data (row index, moves as mini-batches are
  /// delivered)
  int32 data_begin_;
...
typedef StdVectorRandomizer<std::vector<std::pair<int32, BaseFloat> > >
    PosteriorRandomizer;
