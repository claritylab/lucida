...
class Xent {
 public:
  ...
  void Eval(const CuMatrixBase<BaseFloat> &net_out,
            const CuMatrixBase<BaseFloat> &target, CuMatrix<BaseFloat> *diff);
  ...
  void EvalVec(const CuMatrixBase<BaseFloat> &net_out,
               const std::vector<int32> &target, CuMatrix<BaseFloat> *diff);
...
class Mse {
 public:
  Mse() : frames_(0), loss_(0.0), frames_progress_(0), loss_progress_(0) {}
  ~Mse() {}
