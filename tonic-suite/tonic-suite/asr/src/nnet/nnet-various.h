...
/**
 * Convert basic type to string (try not to overuse as ostringstream creation is
 * slow)
 */
template <typename T>
...
std::string MomentStatistics(const VectorBase<Real> &vec) {
  ...
  ostr << " ( min " << vec.Min() << ", max " << vec.Max() << ", mean " << mean
...
void CheckNanInf(const CuMatrixBase<Real> &mat, const char *msg = "") {
  Real sum = mat.Sum();
  if (KALDI_ISINF(sum)) {
    KALDI_ERR << "'inf' in " << msg;
  }
  if (KALDI_ISNAN(sum)) {
    KALDI_ERR << "'nan' in " << msg;
  }
}
...
class Splice : public Component {
 public:
  Splice(int32 dim_in, int32 dim_out) : Component(dim_in, dim_out) {}
  ~Splice() {}
  ...
  void InitData(std::istream &is) {
    ...
    while (!is.eof()) {
      ...
      /**/ if (token == "<ReadVector>") {
        ...
        // <BuildVector> 1:1:1000 1:1:1000 1 2 3 1:10 </BuildVector> [matlab
        // indexing]
      ...
      for (int32 i = 0; i < build_vector.size(); i++) {
        switch (build_vector[i].size()) {
          ...
          case 3: {  // step can be negative -> flipped min/max
            int32 min = build_vector[i][0], step = build_vector[i][1],
                  max = build_vector[i][2];
...
class CopyComponent : public Component {
 public:
  CopyComponent(int32 dim_in, int32 dim_out) : Component(dim_in, dim_out) {}
  ~CopyComponent() {}
 ...
 public:
  ...
  void InitData(std::istream &is) {
    ...
    while (!is.eof()) {
      ReadToken(is, false, &token);
      /**/ if (token == "<InitParam>")
        ReadBasicType(is, false, &init_param);
      else
        KALDI_ERR << "Unknown token " << token << ", a typo in config?"
                  << " (InitParam)";
      is >> std::ws;  // eat-up whitespace
  ...
  // Data accessors
  const CuVectorBase<BaseFloat> &GetShiftVec() { return shift_data_; }
...
class Rescale : public UpdatableComponent {
 public:
  ...
  // Data accessors
  const CuVectorBase<BaseFloat> &GetScaleVec() { return scale_data_; }
