...
class SentenceAveragingComponent : public UpdatableComponent {
 public:
  ...
  void InitData(std::istream &is) {
    ...
    while (!is.eof()) {
      ...
      /**/ if (token == "<NestedNnetFilename>")
        ReadToken(is, false, &nested_nnet_filename);
      else if (token == "<NestedNnetProto>")
        ReadToken(is, false, &nested_nnet_proto);
      else if (token == "<LearnRateFactor>")
        ReadBasicType(is, false, &learn_rate_factor_);
      else
        KALDI_ERR << "Unknown token " << token << " Typo in config?";
      is >> std::ws;  // eat-up whitespace
    }
    // initialize (read already prepared nnet from file)
    KALDI_ASSERT((nested_nnet_proto != "") ^
                 (nested_nnet_filename != ""));  // xor
  ...
  void PropagateFnc(const CuMatrixBase<BaseFloat> &in,
    ...
    // averaging corresponds to extraction of constant vector code for single
    // sentence,
    int32 num_inputs = in.NumCols(), nnet_outputs = nnet_.OutputDim(),
  ...
  void Update(const CuMatrixBase<BaseFloat> &input,
    ...
    int32 nnet_outputs = nnet_.OutputDim(), num_frames = diff.NumRows();
    // Passing the derivative into the nested network. The loss derivative is
    // averaged:
    // single frame from nested network influenced all the frames in the main
    // network,
    // so to get the derivative w.r.t. single frame from nested network we sum
    // derivatives
    // of all frames from main network (and scale by 1/Nframes constant).
    //
    // In fact all the frames from nested network influenced all the input
    // frames to main nnet,
    // so the loss derivarive w.r.t. nested network output is same for all
    // frames in sentence.
    CuVector<BaseFloat> average_diff(nnet_outputs);
    average_diff.AddRowSumMat(1.0 / num_frames, diff.ColRange(0, nnet_outputs),
                              0.0);
