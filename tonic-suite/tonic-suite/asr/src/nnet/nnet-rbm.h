...
class RbmBase : public Component {
 public:
  typedef enum { Bernoulli, Gaussian } RbmNodeType;

  RbmBase(int32 dim_in, int32 dim_out) : Component(dim_in, dim_out) {}
  ...
  virtual void Reconstruct(const CuMatrixBase<BaseFloat> &hid_state,
                           CuMatrix<BaseFloat> *vis_probs) = 0;
  virtual void RbmUpdate(const CuMatrixBase<BaseFloat> &pos_vis,
                         ...
                         const CuMatrixBase<BaseFloat> &neg_hid) = 0;
  ...
  void SetRbmTrainOptions(const RbmTrainOptions &opts) { rbm_opts_ = opts; }
  /// Get training hyper-parameters from the network
  const RbmTrainOptions &GetRbmTrainOptions() const { return rbm_opts_; }
...
class Rbm : public RbmBase {
 public:
  Rbm(int32 dim_in, int32 dim_out) : RbmBase(dim_in, dim_out) {}
  ~Rbm() {}
  ...
  void InitData(std::istream &is) {
    ...
    std::string vis_type;
    std::string hid_type;
    float vis_bias_mean = 0.0, vis_bias_range = 0.0, hid_bias_mean = 0.0,
          hid_bias_range = 0.0, param_stddev = 0.1;
    std::string vis_bias_cmvn_file;  // initialize biases to logit(p_active)
    // parse config
    std::string token;
    while (!is.eof()) {
      ReadToken(is, false, &token);
      /**/ if (token == "<VisibleType>")
        ReadToken(is, false, &vis_type);
      else if (token == "<HiddenType>")
        ReadToken(is, false, &hid_type);
      else if (token == "<VisibleBiasMean>")
        ReadBasicType(is, false, &vis_bias_mean);
      else if (token == "<VisibleBiasRange>")
        ReadBasicType(is, false, &vis_bias_range);
      else if (token == "<HiddenBiasMean>")
        ReadBasicType(is, false, &hid_bias_mean);
      else if (token == "<HiddenBiasRange>")
        ReadBasicType(is, false, &hid_bias_range);
      else if (token == "<ParamStddev>")
        ReadBasicType(is, false, &param_stddev);
      else if (token == "<VisibleBiasCmvnFilename>")
        ReadToken(is, false, &vis_bias_cmvn_file);
      else
        KALDI_ERR << "Unknown token " << token << " Typo in config?";
    ...
    // initialize
    //
    if (vis_type == "bern" || vis_type == "Bernoulli")
      vis_type_ = RbmBase::Bernoulli;
    else if (vis_type == "gauss" || vis_type == "Gaussian")
      vis_type_ = RbmBase::Gaussian;
    else
      KALDI_ERR << "Wrong <VisibleType>" << vis_type;
    //
    if (hid_type == "bern" || hid_type == "Bernoulli")
      hid_type_ = RbmBase::Bernoulli;
    else if (hid_type == "gauss" || hid_type == "Gaussian")
      hid_type_ = RbmBase::Gaussian;
    else
      KALDI_ERR << "Wrong <HiddenType>" << hid_type;
    ...
    if (vis_bias_cmvn_file == "") {
      ...
      Vector<BaseFloat> p(
          dynamic_cast<AddShift &>(cmvn.GetComponent(0)).GetShiftVec());
      p.Scale(-1.0);
      // compute logit:
  ...
  void RbmUpdate(const CuMatrixBase<BaseFloat> &pos_vis,
    ...
    // in the following section we detect that the weights in Gaussian-Bernoulli
    // RBM
    // are about to explode. The weight explosion is caused by large variance of
    // the
    // reconstructed data, which causes a feed-back loop that keeps increasing
    // the weights.
    //
    // To avoid explosion, the variance of the visible-data and
    // reconstructed-data
    ...
    // 1. scale down the weights and biases by b)/a) (for next mini-batch b)/a)
    // gets 1.0)
    ...
    //
    // An alternative approach is to use smaller values in weight-matrix
    // initialization.
    //
    if (vis_type_ == RbmBase::Gaussian) {
      ...
      if (pos_vis_std * 2 < neg_vis_std) {
        ...
        KALDI_WARN
            ...
            << scale << " b) Lowering learning rate to " << rbm_opts_.learn_rate
            << " [pos_vis_std:" << pos_vis_std << ",neg_vis_std:" << neg_vis_std
            << "]";
        return; /* i.e. don't update weights with current stats, as the update
                   would be too BIG */
      }
    }
    //
  ...
  RbmNodeType VisType() const { return vis_type_; }

  RbmNodeType HidType() const { return hid_type_; }

  void WriteAsNnet(std::ostream &os, bool binary) const {
    // header
    WriteToken(os, binary,
               Component::TypeToMarker(Component::kAffineTransform));
  ...
  /* void WriteParam(std::ostream& os, bool binary) const {
     // header
     WriteBasicType(os, binary, InputDim());
     WriteBasicType(os, binary, OutputDim());
     if(!binary) os << "\n";
