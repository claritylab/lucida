...
class Nnet {
 ...
 public:
  ...
  /// Perform forward pass through the network, don't keep buffers (use it when
  /// not training)
 ...
 private:
  ...
  std::vector<CuMatrix<BaseFloat> >
      propagate_buf_;  ///< buffers for forward pass
  std::vector<CuMatrix<BaseFloat> >
      backpropagate_buf_;  ///< buffers for backward pass
