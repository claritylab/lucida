 ...
 * and backpropagate (BackpropagateFnc: i.e. transform loss derivative w.r.t.
 * output to derivative w.r.t. the input)
 * the formulas are implemented in descendant classes
 * (AffineTransform,Sigmoid,Softmax,...).
 */
class Component {
 ...
 private:
  ...
  static Component *NewComponentOfType(ComponentType t, int32 input_dim,
                                       int32 output_dim);
...
class UpdatableComponent : public Component {
 public:
  ...
  virtual void SetTrainOptions(const NnetTrainOptions &opts) { opts_ = opts; }
...
inline void Component::Backpropagate(const CuMatrixBase<BaseFloat> &in,
  ...
  // Target buffer NULL : backpropagate only through components with nested
  // nnets.
  if (in_diff == NULL) {
