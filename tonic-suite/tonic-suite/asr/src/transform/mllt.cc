...
void MlltAccs::Init(int32 dim,
                    BaseFloat rand_prune) {  // initializes (destroys anything
                                             // that was there before).
  KALDI_ASSERT(dim > 0);
  beta_ = 0;
  rand_prune_ = rand_prune;
  G_.resize(dim);
  for (int32 i = 0; i < dim; i++) G_[i].Resize(dim);  // will zero it too.
}
...
void MlltAccs::Read(std::istream &is, bool binary, bool add) {
  ...
  for (size_t i = 0; i < G_.size(); i++) G_[i].Read(is, binary, add);
  ExpectToken(is, binary, "</MlltAccs>");
}
...
void MlltAccs::Write(std::ostream &os, bool binary) const {
  ...
  if (!binary) os << '\n';
  for (size_t i = 0; i < G_.size(); i++) G_[i].Write(os, binary);
  WriteToken(os, binary, "</MlltAccs>");
  if (!binary) os << '\n';
}

// static version of the Update function.
void MlltAccs::Update(double beta, const std::vector<SpMatrix<double> > &G,
                      MatrixBase<BaseFloat> *M_ptr, BaseFloat *objf_impr_out,
                      BaseFloat *count_out) {
  int32 dim = G.size();
  KALDI_ASSERT(dim != 0 && M_ptr != NULL && M_ptr->NumRows() == dim &&
               M_ptr->NumCols() == dim);
  ...
  for (int32 p = 0; p < num_iters; p++) {
    for (int32 i = 0; i < dim; i++) {  // for each row
      ...
      // optimized by (c.f. Mark Gales's techreport "semitied covariance
      // matrices
...
void MlltAccs::AccumulateFromPosteriors(
    const DiagGmm &gmm, const VectorBase<BaseFloat> &data,
