...
void AffineXformStats::Init(int32 dim, int32 num_gs) {
  if (dim == 0) {
    ...
    G_.resize(num_gs);
    for (int32 i = 0; i < num_gs; i++) G_[i].Resize(dim + 1, kSetZero);
    dim_ = dim;
  }
}
...
void AffineXformStats::SetZero() {
  beta_ = 0.0;
  K_.SetZero();
  for (std::vector<SpMatrix<double> >::iterator it = G_.begin(), end = G_.end();
       it != end; ++it) {
    it->SetZero();
  }
}
...
void AffineXformStats::CopyStats(const AffineXformStats &other) {
  ...
  for (size_t i = 0; i < G_.size(); i++) G_[i].CopyFromSp(other.G_[i]);
}
...
void AffineXformStats::Add(const AffineXformStats &other) {
  ...
  for (size_t i = 0; i < G_.size(); i++) G_[i].AddSp(1.0, other.G_[i]);
}

bool ComposeTransforms(const Matrix<BaseFloat> &a, const Matrix<BaseFloat> &b,
                       bool b_is_affine, Matrix<BaseFloat> *c) {
  ...
  if (a.NumCols() == b.NumRows()) {
    ...
    if (b_is_affine) {  // append 0 0 0 0 ... 1 to b and multiply.
      ...
      b_ext(b.NumRows(), b.NumCols() - 1) =
          1.0;  // so the last row is 0 0 0 0 ... 0 1
      c->Resize(a.NumRows(), b.NumCols());
      c->AddMatMat(1.0, a, kNoTrans, b_ext, kNoTrans, 0.0);  // c = a * b_ext.
    } else {  // extend b by 1 row and column with all zeros except a 1 on
              // diagonal.
      Matrix<BaseFloat> b_ext(b.NumRows() + 1, b.NumCols() + 1);
      SubMatrix<BaseFloat> b_part(b_ext, 0, b.NumRows(), 0, b.NumCols());
      b_part.CopyFromMat(b);
      b_ext(b.NumRows(), b.NumCols()) =
          1.0;  // so the last row is 0 0 0 0 ... 0 1;
    ...
    KALDI_ERR << "ComposeTransforms: mismatched dimensions, a has "
              << a.NumCols() << " columns and b has " << b.NumRows()
              << " rows.";  // this is fatal.
    return false;
  }
}
