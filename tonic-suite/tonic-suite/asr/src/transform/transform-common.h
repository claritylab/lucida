...
class AffineXformStats {
 public:
  ...
  double beta_;
  /// K_ is the summed outer product of [mean times inverse variance] with
  /// [extended data],
  /// scaled by the occupation counts; dimension is dim by (dim+1)
  Matrix<double> K_;
  /// G_ is the outer product of extended-data, scaled by inverse variance, for
  /// each
  ...
  /// be indexed 0 to ((dim)(dim+1))/2.  Each G_[i] is of dimension dim+1 by
  /// dim+1.
  ...
  void Init(int32 dim,
            int32 num_gs);  // num_gs will equal dim for diagonal FMLLR.
  ...
  AffineXformStats(const AffineXformStats &other)
      : beta_(other.beta_), K_(other.K_), G_(other.G_), dim_(other.dim_) {}
