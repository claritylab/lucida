...
/// and a set of global matrices for the full variance.  C.f. \ref
/// transform_hlda
class HldaAccsDiagGmm {
 public:
  ...
  HldaAccsDiagGmm(const AmDiagGmm &model, int32 orig_feat_dim,
  ...
  void Init(const AmDiagGmm &model, int32 orig_feat_dim,
            BaseFloat speedup = 1.0);
  ...
  /// The Update function does the ML update.  It outputs the appropriate
  /// transform
  /// and sets the model's means (in this approach, the model's variances are
  /// unchanged and you would have to run more passes of model re-estimation,
  /// and
  ...
  ///                     transform, of dimension orig-dim x orig-dim (e.g. from
  ///                     LDA),
  ///                     or previous iteration of HLDA.   Will be set at output
  ///                     to
  //                      the full transform of dimension orig-dim x orig-dim.
  ///  @param M [out] The output transform (only accepted rows), should be of
  ///  dimension
  ///                  (feature-dim x orig-dim)
  ///  @param objf_impr_out [out] The objective function improvement
  ///  @param count_out [out] The data-count
  void Update(AmDiagGmm *model, MatrixBase<BaseFloat> *Mfull,
  ...
  void AccumulateFromPosteriors(int32 pdf_id, const DiagGmm &gmm,
                                const VectorBase<BaseFloat> &data,
                                const VectorBase<BaseFloat> &posteriors);

 private:
  std::vector<SpMatrix<double> > S_;  // the S matrices: [model-dim+1] matrices
                                      // of size (feat-dim) x (feat-dim)
  // are used to construct the G matrices.
  std::vector<Vector<double> >
      occs_;  // occupancies for the Gaussians. [num-pdfs][gauss]
