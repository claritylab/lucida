...
class FmllrDiagGmmAccs : public AffineXformStats {
 public:
  ...
  // update.
  FmllrDiagGmmAccs(const FmllrOptions &opts = FmllrOptions()) : opts_(opts) {}
  explicit FmllrDiagGmmAccs(const FmllrDiagGmmAccs &other)
      : AffineXformStats(other),
        single_frame_stats_(other.single_frame_stats_),
        opts_(other.opts_) {}
  explicit FmllrDiagGmmAccs(int32 dim,
  ...
  void AccumulateFromPosteriorsPreselect(
      const DiagGmm &gmm, const std::vector<int32> &gselect,
      const VectorBase<BaseFloat> &data,
      const VectorBase<BaseFloat> &posteriors);

  /// Update
  void Update(const FmllrOptions &opts, MatrixBase<BaseFloat> *fmllr_mat,
 ...
 private:
  ...
  struct SingleFrameStats {
    ...
    SingleFrameStats(const SingleFrameStats &s)
        : x(s.x), a(s.a), b(s.b), count(s.count) {}
    void Init(int32 dim);
  };
  ...
  bool DataHasChanged(
      const VectorBase<BaseFloat> &data) const;  // compares it to the
...
inline void InitFmllr(int32 dim, Matrix<BaseFloat> *out_fmllr) {
...
inline BaseFloat ComputeFmllrLogDet(const Matrix<BaseFloat> &fmllr_mat) {
  KALDI_ASSERT(fmllr_mat.NumRows() != 0 &&
               fmllr_mat.NumCols() == fmllr_mat.NumRows() + 1);
  SubMatrix<BaseFloat> tmp(fmllr_mat, 0, fmllr_mat.NumRows(), 0,
                           fmllr_mat.NumRows());
  return tmp.LogDet();
}
...
BaseFloat ComputeFmllrMatrixDiagGmmDiagonal2(
    const MatrixBase<BaseFloat> &in_xform, const AffineXformStats &stats,
    MatrixBase<BaseFloat> *out_xform);
...
/// This function internally calls ComputeFmllrMatrixDiagGmm{Full, Diagonal,
/// Offset},
...
/// Returns the (diagonal-GMM) FMLLR auxiliary function value given the
/// transform
/// and the stats.
...
/// transform) and will get stats valid for estimating U.  This only works if T
/// is
/// diagonal, because otherwise the standard stats format is not valid.  xform
/// must
