...
class FmllrRawAccs {
 public:
  ...
  FmllrRawAccs(int32 raw_dim, int32 model_dim,
  ...
  void Update(const FmllrRawOptions &opts, MatrixBase<BaseFloat> *raw_fmllr_mat,
 ...
 private:
  struct SingleFrameStats {
    ...
    Vector<BaseFloat> transformed_data;  // [FullDim()] Data times full
  ...
  /// 0 <= i < RawDim(), and j < i, and each element is of dimension RawDim() +
  /// 1
  ...
  double count_;  // The data-count.  Note: in accounting for the determinant,
                  // we will
