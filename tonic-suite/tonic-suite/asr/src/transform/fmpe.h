...
struct FmpeOptions {
  ...
  std::string
      context_expansion;  // This string describes the various contexts...
  ...
  FmpeOptions()
      : context_expansion(
            "0,1.0:-1,1.0:1,1.0:-2,0.5;-3,0.5:2,0.5;3,0.5:-4,0.5;-5,0.5:4,0.5;"
            "5,0.5:-6,0.333;-7,0.333;-8,0.333:6,0.333;7,0.333;8,0.333"),
        post_scale(5.0) {}

  void Register(OptionsItf *po) {
    po->Register("post-scale", &post_scale,
                 "Scaling constant on posterior "
                 "element of offset features, to give it a faster learning "
                 "rate.");
    po->Register("context-expansion", &context_expansion,
                 "Specifies the "
...
struct FmpeStats {
 ...
 private:
  Matrix<BaseFloat>
      deriv;  // contains positive and negative parts of derivatives
