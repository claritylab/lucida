...
// Copyright 2011-2012  Yanmin Qian  Johns Hopkins University (Author: Daniel
// Povey)
...
void Fmpe::ComputeC() {
  ...
  for (int32 pdf = 0; pdf < ngmm.NumGauss(); pdf++) {
    ...
    x2_stats.AddDiagVec(ngmm.weights_(pdf),
                        ngmm.vars_.Row(pdf));  // add diagonal
  ...
  C_.Resize(dim);
  try {
    TpMatrix<double> Ctmp(dim);
    Ctmp.Cholesky(x2_stats);
    C_.CopyFromTp(Ctmp);
  } catch (...) {
    KALDI_ERR
        << "Error initializing fMPE object: cholesky of "
...
void Fmpe::ApplyContext(const MatrixBase<BaseFloat> &intermed_feat,
  ...
  int32 dim = FeatDim(), ncontexts = NumContexts(), T = intermed_feat.NumRows();
  ...
  for (int32 i = 0; i < ncontexts; i++) {
    ...
    SubMatrix<BaseFloat> this_intermed_feat(intermed_feat, 0, T, dim * i, dim);
...
void Fmpe::ApplyContextReverse(
  ...
  int32 dim = FeatDim(), ncontexts = NumContexts(), T = feat_deriv.NumRows();
  ...
  for (int32 i = 0; i < ncontexts; i++) {
    ...
    for (int32 j = 0; j < static_cast<int32>(contexts_[i].size()); j++) {
      ...
      for (int32 t_out = 0; t_out < T; t_out++) {  // t_out indexes the output
        int32 t_in = t_out + t_offset;             // t_in indexes the input.
        if (t_in >= 0 && t_in < T)  // Discard frames outside range.
          this_intermed_feat_deriv.Row(t_in)
              .AddVec(weight, feat_deriv.Row(t_out));
...
void Fmpe::ApplyProjection(const MatrixBase<BaseFloat> &feat_in,
  ...
  if (!optimize) {  // Why do we keep this un-optimized code around?
    ...
    for (size_t i = 0; i < all_posts.size(); i++) {
      ...
      input_chunk.Range(0, dim).AddVecVec(
          -this_post, gmm_.means_invvars().Row(gauss), this_stddev, 0.0);
      ...
      input_chunk.Range(0, dim)
          .AddVecDivVec(this_post, this_feat, this_stddev, 1.0);
      ...
      this_intermed_feat.AddMatVec(
          1.0, projT_.Range(gauss * (dim + 1), dim + 1, 0, dim * ncontexts),
    ...
    while (i < all_posts.size()) {
      ...
      for (int32 j = 0; j < batch_size; j++) {  // set up "input_chunks".
        // To understand this code, first examine code and comments in
        // "non-optimized"
        ...
        this_input_chunk.Range(0, dim)
            .AddVecVec(-this_post, this_mean_invvar, this_stddev, 0.0);
        this_input_chunk.Range(0, dim)
            .AddVecDivVec(this_post, this_feat, this_stddev, 1.0);
      ...
      intermed_temp.AddMatMat(1.0, input_chunks, kNoTrans, this_projT_chunk,
                              kNoTrans, 0.0);
...
void Fmpe::ApplyProjectionReverse(
    const MatrixBase<BaseFloat> &feat_in,
    const std::vector<std::vector<int32> > &gselect,
  ...
  for (size_t i = 0; i < all_posts.size(); i++) {
    ...
    SubMatrix<BaseFloat> plus_chunk(*proj_deriv_plus, gauss * (dim + 1),
                                    dim + 1, 0, dim * ncontexts),
        minus_chunk(*proj_deriv_minus, gauss * (dim + 1), dim + 1, 0,
                    dim * ncontexts);
    ...
    // proj_deriv_minus.
    AddOuterProductPlusMinus(static_cast<BaseFloat>(1.0), input_chunk,
                             this_intermed_feat_deriv, &plus_chunk,
                             &minus_chunk);
  }
}
...
void Fmpe::ComputeFeatures(const MatrixBase<BaseFloat> &feat_in,
  ...
  Matrix<BaseFloat> intermed_feat(feat_in.NumRows(), dim * NumContexts());
...
void Fmpe::AccStats(
  ...
  if (indirect_feat_deriv != NULL)
    fmpe_stats->AccumulateChecks(feat_in, direct_feat_deriv,
                                 *indirect_feat_deriv);

  Matrix<BaseFloat> feat_deriv(
      direct_feat_deriv);  // "feat_deriv" is initially direct+indirect.
  if (indirect_feat_deriv != NULL) feat_deriv.AddMat(1.0, *indirect_feat_deriv);
  ...
  ApplyProjectionReverse(feat_in, gselect, intermed_feat_deriv, &stats_plus,
                         &stats_minus);
}
...
Fmpe::Fmpe(const DiagGmm &gmm, const FmpeOptions &config)
    : gmm_(gmm), config_(config) {
...
BaseFloat Fmpe::Update(const FmpeUpdateOptions &config,
  ...
  KALDI_ASSERT(SameDim(proj_deriv_plus, projT_) &&
               SameDim(proj_deriv_minus, projT_));
  KALDI_ASSERT(proj_deriv_plus.Min() >= 0);
  KALDI_ASSERT(proj_deriv_minus.Min() >= 0);
  BaseFloat learning_rate = config.learning_rate, l2_weight = config.l2_weight;
  ...
  for (int32 i = 0; i < projT_.NumRows(); i++) {
    for (int32 j = 0; j < projT_.NumCols(); j++) {
      ...
      // which we set to zero and solve for z, to get z = x +
      // learning_rate.(p-n)/(p+n)
      // At this point we add regularization, a term of the form -l2_weight *
      // z^2.
      ...
      //      z = ((p-n) + x (p+n)/learning_rate) / (2.l2_weight +
      //      (p+n)/learning_rate)

      BaseFloat z = ((p - n) + x * (p + n) / learning_rate) /
                    (2 * l2_weight + (p + n) / learning_rate);
      // z is the new parameter value.

      tot_linear_objf_impr +=
          (z - x) * (p - n);  // objf impr based on linear assumption.
      projT_(i, j) = z;
      if (z * x < 0) changed++;
    }
  }
...
void Fmpe::Write(std::ostream &os, bool binary) const {
  if (gmm_.NumGauss() == 0) KALDI_ERR << "Fmpe::Write, object not initialized.";
...
BaseFloat ComputeAmGmmFeatureDeriv(const AmDiagGmm &am_gmm,
  ...
  for (size_t i = 0; i < posterior.size(); i++) {
    for (size_t j = 0; j < posterior[i].size(); j++) {
      ...
      ans += weight * gmm.ComponentPosteriors(this_feat, &gauss_posteriors);
      ...
      if (model_diff != NULL &&
          weight > 0.0) {  // We need to get the indirect diff.
...
SubMatrix<BaseFloat> FmpeStats::DerivPlus()
    const {  // const-ness not preserved.
  KALDI_ASSERT(deriv.NumRows() != 0);
  int32 proj_num_rows = deriv.NumRows(), proj_num_cols = deriv.NumCols() / 2;
  return SubMatrix<BaseFloat>(deriv, 0, proj_num_rows, 0, proj_num_cols);
}
SubMatrix<BaseFloat> FmpeStats::DerivMinus()
    const {  // const-ness not preserved.
  KALDI_ASSERT(deriv.NumRows() != 0);
  int32 proj_num_rows = deriv.NumRows(), proj_num_cols = deriv.NumCols() / 2;
  return SubMatrix<BaseFloat>(deriv, 0, proj_num_rows, proj_num_cols,
                              proj_num_cols);
}
...
void FmpeStats::AccumulateChecks(const MatrixBase<BaseFloat> &feats,
  ...
  KALDI_ASSERT(direct_deriv.NumRows() == T && direct_deriv.NumCols() == dim &&
               indirect_deriv.NumRows() == T &&
               indirect_deriv.NumCols() == dim);
...
void FmpeStats::DoChecks() {
  ...
  Vector<double> shift_check(dim), shift_check2(dim), scale_check(dim),
      scale_check2(dim);
  for (int32 d = 0; d < dim; d++) {
    // shiftnumerator = direct+indirect deriv-- should be zero.
    double shift_num =
               checks(0, d) - checks(1, d) + checks(2, d) - checks(3, d),
           shift_den =
               checks(0, d) + checks(1, d) + checks(2, d) + checks(3, d),
           shift_den2 = fabs(checks(0, d) - checks(1, d)) +
                        fabs(checks(2, d) - checks(3, d));
    shift_check(d) = shift_num / shift_den;
    shift_check2(d) = shift_num / shift_den2;
    double scale_num =
               checks(4, d) - checks(5, d) + checks(6, d) - checks(7, d),
           scale_den =
               checks(4, d) + checks(5, d) + checks(6, d) + checks(7, d),
           scale_den2 = fabs(checks(4, d) - checks(5, d)) +
                        fabs(checks(6, d) - checks(7, d));
  ...
  KALDI_LOG << "Scale-check is as follows (should be in range +- 0.01 or less)."
            << scale_check;
  KALDI_LOG << "Shift-check(2) is as follows: most elements should be in range "
               "+-0.1: " << shift_check2;
  KALDI_LOG << "Scale-check(2) is as follows: most elements should be in range "
               "+-0.1: " << scale_check2;
}
