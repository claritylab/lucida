...
void CompressedAffineXformStats::CopyFromAffineXformStats(
  ...
  Matrix<double> K_corrected(
      input.K_);  // This K_corrected matrix is a version of the
  ...
  for (int32 i = 0; i < dim; i++) {
    ...
    new_g_row.CopyRowFromSp(Gi_tmp,
                            i);  // i'th row of compressed+reconstructed G_i.
    // The auxiliary function for the i'th row of the transform, v_i, is as
    // follows
    // [ignoring the determinant], where/ k_i is the i'th row of K:
    //  v_i . k_i - 0.5 v_i^T G_i u_i.
    // Let u_i be the unit vector in the i'th dimension.  This is the "default"
    // value
    // of v_i.  The derivative of the auxf w.r.t. v_i, taken around this point,
    // is:
    // k_i - G_i u_i
...
void CompressedAffineXformStats::PrepareOneG(const SpMatrix<double> &Gi,
  ...
  SubVector<double> linearized_matrix((*linearized), 1,
                                      ((dim + 1) * (dim + 2)) / 2);
  TpMatrix<double> C(dim + 1);
  C.Cholesky(Gi);  // Get the Cholesky factor: after we compress and uncompress
  // this and re-create Gi, it's bound to be +ve semidefinite, which is a Good
  // Thing.
...
void CompressedAffineXformStats::ExtractOneG(
    const SubVector<double> &linearized, double beta, SpMatrix<double> *Gi) {
  int32 dim = Gi->NumRows() - 1;
  KALDI_ASSERT(dim > 0);
