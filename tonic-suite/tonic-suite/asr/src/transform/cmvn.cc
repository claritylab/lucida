...
void AccCmvnStats(const VectorBase<BaseFloat> &feats, BaseFloat weight,
...
void AccCmvnStats(const MatrixBase<BaseFloat> &feats,
  ...
  for (int32 i = 0; i < num_frames; i++) {
    ...
    if (weight != 0.0) AccCmvnStats(this_frame, weight, stats);
  }
}

void ApplyCmvn(const MatrixBase<double> &stats, bool var_norm,
               MatrixBase<BaseFloat> *feats) {
  KALDI_ASSERT(feats != NULL);
  int32 dim = stats.NumCols() - 1;
  if (stats.NumRows() > 2 || stats.NumRows() < 1 || feats->NumCols() != dim) {
    KALDI_ERR << "Dim mismatch in ApplyCmvn: cmvn " << stats.NumRows() << 'x'
              << stats.NumCols() << ", feats " << feats->NumRows() << 'x'
              << feats->NumCols();
  }
  ...
  for (int32 d = 0; d < dim; d++) {
    ...
    if (!var_norm) {
      ...
      double var = (stats(1, d) / count) - mean * mean, floor = 1.0e-20;
      if (var < floor) {
