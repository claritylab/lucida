  ...
  if (!binary) {  // Text-mode read, different "human-readable" format.
    ...
    while (!(is >> token).fail()) {
      if (token == "</Topology>") {
        ...
        KALDI_ERR << "Reading HmmTopology object, expected </Topology> or "
        ...
        while (1) {
          is >> s;
          if (is.fail())
            KALDI_ERR << "Reading HmmTopology object, unexpected end of file "
          ...
          if (s == "</ForPhones>")
            ...
            if (!ConvertStringToInteger(s, &phone))
              KALDI_ERR << "Reading HmmTopology object, expected integer, got "
                           "instead " << s;
        ...
        while (token != "</TopologyEntry>") {
          if (token != "<State>")
            KALDI_ERR << "Expected </TopologyEntry> or <State>, got instead "
                      << token;
          int32 state;
...
void HmmTopology::Write(std::ostream &os, bool binary) const {
  ...
  if (!binary) {  // Text-mode write.
    ...
    for (size_t i = 0; i < entries_.size(); i++) {
      ...
      for (size_t j = 0; j < entries_[i].size(); j++) {
        WriteBasicType(os, binary, entries_[i][j].pdf_class);
        WriteBasicType(os, binary,
                       static_cast<int32>(entries_[i][j].transitions.size()));
        for (size_t k = 0; k < entries_[i][j].transitions.size(); k++) {
          WriteBasicType(os, binary, entries_[i][j].transitions[k].first);
          WriteBasicType(os, binary, entries_[i][j].transitions[k].second);
        }
...
void HmmTopology::Check() {
  ...
  for (size_t i = 0; i < entries_.size(); i++) {
    ...
    if (num_states <= 1)
      KALDI_ERR
          << "HmmTopology::Check(), cannot only have one state (i.e., must "
    ...
    for (int32 j = 1; j < num_states; j++)
      if (!has_trans_in[j])
        KALDI_ERR << "HmmTopology::Check, state " << (j)
...
const HmmTopology::TopologyEntry &HmmTopology::TopologyForPhone(
    int32 phone) const {  // Will throw if phone not covered.
  if (static_cast<size_t>(phone) >= phone2idx_.size() ||
      phone2idx_[phone] == -1) {
...
HmmTopology GetDefaultTopology(const std::vector<int32> &phones_in) {
  ...
  topo_string << "<Topology>\n"
                 "<TopologyEntry>\n"
                 "<ForPhones> ";
  for (size_t i = 0; i < phones.size(); i++) topo_string << phones[i] << " ";
