...
bool PosteriorHolder::Write(std::ostream &os, bool binary, const T &t) {
  ...
  try {
    if (binary) {
      ...
      for (Posterior::const_iterator iter = t.begin(); iter != t.end();
           ++iter) {
        int32 sz2 = iter->size();
        WriteBasicType(os, binary, sz2);
        for (std::vector<std::pair<int32, BaseFloat> >::const_iterator iter2 =
                 iter->begin();
...
bool PosteriorHolder::Read(std::istream &is) {
  ...
  try {
    if (is_binary) {
      ...
      if (sz < 0) KALDI_ERR << "Reading posteriors: got negative size";
      t_.resize(sz);
      for (Posterior::iterator iter = t_.begin(); iter != t_.end(); ++iter) {
        int32 sz2;
        ReadBasicType(is, true, &sz2);
        if (sz2 < 0) KALDI_ERR << "Reading posteriors: got negative size";
        iter->resize(sz2);
        for (std::vector<std::pair<int32, BaseFloat> >::iterator iter2 =
                 iter->begin();
             iter2 != iter->end(); iter2++) {
      ...
      if (is.fail()) {
        KALDI_WARN << "holder of Posterior: error reading line "
                   << (is.eof() ? "[eof]" : "");
      ...
      while (1) {
        ...
        if (str != "[")
          KALDI_ERR << "Reading Posterior object: expecting [, got " << str
                    << " (if this is an integer, possibly "
        ...
        while (1) {
          ...
          line_is >> i >> p;
          if (line_is.fail())
            KALDI_ERR << "Error reading Posterior object (could not get data "
                         "after \"[\");";
...
bool GaussPostHolder::Write(std::ostream &os, bool binary, const T &t) {
  ...
  try {
    ...
    for (GaussPost::const_iterator iter = t.begin(); iter != t.end(); ++iter) {
      int32 sz2 = iter->size();
      WriteBasicType(os, binary, sz2);
      for (std::vector<std::pair<int32, Vector<BaseFloat> > >::const_iterator
               iter2 = iter->begin();
           iter2 != iter->end(); iter2++) {
        WriteBasicType(os, binary, iter2->first);
...
bool GaussPostHolder::Read(std::istream &is) {
  ...
  try {
    ...
    for (GaussPost::iterator iter = t_.begin(); iter != t_.end(); ++iter) {
      ...
      if (sz2 < 0) KALDI_ERR << "Reading posteriors: got negative size";
      iter->resize(sz2);
      for (std::vector<std::pair<int32, Vector<BaseFloat> > >::iterator iter2 =
               iter->begin();
           iter2 != iter->end(); iter2++) {
...
bool PosteriorEntriesAreDisjoint(
  ...
  for (size_t i = 0; i < post_elem1.size(); i++)
    set1.insert(post_elem1[i].first);
...
// we are using).
int32 MergePosteriors(const Posterior &post1, const Posterior &post2,
                      bool merge, bool drop_frames, Posterior *post) {
  ...
  for (size_t i = 0; i < post->size(); i++) {
    ...
    (*post)[i].insert((*post)[i].end(), post1[i].begin(), post1[i].end());
    (*post)[i].insert((*post)[i].end(), post2[i].begin(), post2[i].end());
...
void ConvertPosteriorToPdfs(const TransitionModel &tmodel,
                            const Posterior &post_in, Posterior *post_out) {
  post_out->clear();
  ...
  for (size_t i = 0; i < post_out->size(); i++) {
    ...
    for (size_t j = 0; j < post_in[i].size(); j++) {
      int32 tid = post_in[i][j].first, pdf_id = tmodel.TransitionIdToPdf(tid);
    ...
    for (unordered_map<int32, BaseFloat>::const_iterator iter =
             pdf_to_post.begin();
         iter != pdf_to_post.end(); ++iter) {
      if (iter->second != 0.0)
        (*post_out)[i].push_back(std::make_pair(iter->first, iter->second));
    }
  }
}
...
void WeightSilencePost(const TransitionModel &trans_model,
                       ...
                       BaseFloat silence_scale, Posterior *post) {
  for (size_t i = 0; i < post->size(); i++) {
...
void WeightSilencePostDistributed(const TransitionModel &trans_model,
  ...
  for (size_t i = 0; i < post->size(); i++) {
    ...
    KALDI_ASSERT(sil_weight >= 0.0 &&
                 nonsil_weight >= 0.0);  // This "distributed"
...
BaseFloat VectorToPosteriorEntry(
    const VectorBase<BaseFloat> &log_likes, int32 num_gselect,
    BaseFloat min_post, std::vector<std::pair<int32, BaseFloat> > *post_entry) {
  ...
  std::nth_element(temp_post.begin(), temp_post.begin() + num_gselect,
                   temp_post.end(), compare);
  std::sort(temp_post.begin(), temp_post.begin() + num_gselect, compare);

  post_entry->clear();
  post_entry->insert(post_entry->end(), temp_post.begin(),
                     temp_post.begin() + num_gselect);
  ...
  for (size_t i = 0; i < size; i++) tot += (*post_entry)[i].second;
  BaseFloat inv_tot = 1.0 / tot;
  for (size_t i = 0; i < size; i++) (*post_entry)[i].second *= inv_tot;
  return ans;
}
