...
// transition-index.
// We associate with each (transition-state, transition-index) a unique
// transition-id.
// Each individual probability estimated by the transition-model is asociated
// with a
// transition-id.
//
...
struct MleTransitionUpdateConfig {
  ...
  bool share_for_pdfs;  // If true, share all transition parameters that have
                        // the same pdf.
  MleTransitionUpdateConfig(BaseFloat floor = 0.01, BaseFloat mincount = 5.0,
...
struct MapTransitionUpdateConfig {
  ...
  void Register(OptionsItf *po) {
    po->Register("transition-tau", &tau,
                 "Tau value for MAP estimation of transition "
                 "probabilities.");
    po->Register("share-for-pdfs", &share_for_pdfs,
                 "If true, share all transition parameters where the states "
                 "have the same pdf.");
...
class TransitionModel {
 public:
  ...
  void Read(std::istream &is, bool binary);  // note, no symbol table: topo
                                             // object always read/written w/o
                                             // symbols.
  ...
  int32 SelfLoopOf(int32 trans_state)
      const;  // returns the self-loop transition-id, or zero if
  ...
  bool IsFinal(int32 trans_id)
      const;  // returns true if this trans_id goes to the final state
  // (which is bound to be nonemitting).
  bool IsSelfLoop(int32 trans_id)
      const;  // return true if this trans_id corresponds to a self-loop.
  ...
  void MleUpdate(const Vector<double> &stats,
                 const MleTransitionUpdateConfig &cfg, BaseFloat *objf_impr_out,
                 BaseFloat *count_out);
  ...
  void MapUpdate(const Vector<double> &stats,
                 const MapTransitionUpdateConfig &cfg, BaseFloat *objf_impr_out,
                 BaseFloat *count_out);

  /// Print will print the transition model in a human-readable way, for
  /// purposes of human
  /// inspection.  The "occs" are optional (they are indexed by pdf-id).
  void Print(std::ostream &os, const std::vector<std::string> &phone_names,
             const Vector<double> *occs = NULL);

  void InitStats(Vector<double> *stats) const {
    stats->Resize(NumTransitionIds() + 1);
  }
 ...
 private:
  ...
  void MapUpdateShared(const Vector<double> &stats,
                       ...
                       BaseFloat *objf_impr_out, BaseFloat *count_out);
  void ComputeTriples(const ContextDependency &ctx_dep);  // called from
                                                          // constructor.
                                                          // initializes
                                                          // triples_.
  void ComputeDerived();  // called from constructor and Read function: computes
                          // state2id_ and id2state_.
  void ComputeDerivedOfProbs();  // computes quantities derived from log-probs
                                 // (currently just
  ...
  /// the triples are in sorted order which allows us to do the reverse mapping
  /// from
  ...
  /// the
  /// tree (but the tree numbers pdfs contiguously from zero so this is the
  /// number
  /// of pdfs).
  int32 num_pdfs_;
...
/// phones from
///          just this set (false if they may be shared with phones outside this
///          set).
