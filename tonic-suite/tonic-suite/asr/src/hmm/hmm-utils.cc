...
fst::VectorFst<fst::StdArc> *GetHmmAsFst(
    std::vector<int32> phone_window, const ContextDependencyInterface &ctx_dep,
    const TransitionModel &trans_model, const HTransducerConfig &config,
    HmmCacheType *cache) {
  using namespace fst;

  if (config.reverse)
    ReverseVector(&phone_window);  // phone_window represents backwards
  ...
  if (phone == 0) {  // error.  Error message depends on whether reversed.
    if (config.reverse)
      KALDI_ERR
          << "phone == 0.  Possibly you are trying to get a reversed "
  ...
  for (int32 pdf_class = 0; pdf_class < static_cast<int32>(pdfs.size());
       pdf_class++) {
  ...
  for (int32 hmm_state = 0; hmm_state < static_cast<int32>(entry.size());
       hmm_state++) {
    ...
    for (trans_idx = 0;
      ...
      if (pdf_class == kNoPdf) {
        ...
        int32 trans_id = trans_model.PairToTransitionId(trans_state, trans_idx);
  ...
  if (cache != NULL) (*cache)[cache_index] = ans;
  return ans;
}
...
fst::VectorFst<fst::StdArc> *GetHmmAsFstSimple(
  ...
  for (int32 hmm_state = 0; hmm_state < static_cast<int32>(entry.size());
    ...
    int32 pdf_class = entry[hmm_state].pdf_class, pdf;
    if (pdf_class == kNoPdf)
      pdf = kNoPdf;  // nonemitting state; not generally used.
    else {
...
fst::VectorFst<fst::StdArc> *GetHTransducer(
  ...
  if (ilabel_info.size() > 0)
    KALDI_ASSERT(ilabel_info[0].size() ==
                 0);  // make sure epsilon is epsilon...

  for (int32 j = 1; j < static_cast<int32>(ilabel_info.size());
       j++) {  // zero is eps.
    ...
    if (ilabel_info[j].size() == 1 &&
      ...
      fst->AddArc(0,
                  Arc(disambig_sym_left, disambig_sym_left, Weight::One(), 1));
      fsts[j] = fst;
    } else {  // Real phone-in-context.
      std::vector<int32> phone_window = ilabel_info[j];

      VectorFst<Arc> *fst =
          GetHmmAsFst(phone_window, ctx_dep, trans_model, config, &cache);
      fsts[j] = fst;
    }
  }
...
void GetIlabelMapping(const std::vector<std::vector<int32> > &ilabel_info_old,
  ...
  std::map<std::pair<int32, std::vector<int32> >, int32> pair_to_physical;
  ...
  for (int32 i = 1; i < num_syms_old; i++) {
    ...
    if (vec.size() == 1 && vec[0] <= 0) {  // disambig.
      ...
      int32 num_pdf_classes =
          trans_model.GetTopo().NumPdfClasses(central_phone);
      ...
      if (iter ==
          pair_to_physical.end()) {  // first time we saw something like this.
  ...
  for (int32 i = 0; i < num_syms_old; i++) seen[old2old_map[i]] = true;
  ...
  for (int32 i = 0; i < num_syms_old; i++)
    if (seen[i]) (*old2new_map)[i] = cur_id++;
...
fst::VectorFst<fst::StdArc> *GetPdfToTransitionIdTransducer(
  ...
  for (int32 tid = 1; tid <= trans_model.NumTransitionIds(); tid++) {
    ...
    ans->AddArc(0, Arc(pdf + 1, tid, Weight::One(),
                       0));  // note the offset of 1 on the pdfs.
...
class TidToTstateMapper {
 public:
  ...
  // Its point is to provide an equivalence class on labels that's relevant to
  // what
  // the self-loop will be on the following (or preceding) state.
  TidToTstateMapper(const TransitionModel &trans_model,
                    const std::vector<int32> &disambig_syms)
      : trans_model_(trans_model), disambig_syms_(disambig_syms) {}
...
static void AddSelfLoopsBefore(const TransitionModel &trans_model,
  ...
  KALDI_ASSERT(state_in[fst->Start()] == kNoStateId ||
  ...
  for (StateId s = 0; s < static_cast<StateId>(state_in.size()); s++) {
    if (state_in[s] >
        0) {  // defined, and not eps or a disambiguation symbol...
...
static void AddSelfLoopsAfter(const TransitionModel &trans_model,
  ...
  for (StateId s = 0; s < num_states; s++) {
    ...
    if (fst->Final(s) != Weight::Zero()) {
      KALDI_ASSERT(my_trans_state == kNoLabel ||
                   my_trans_state ==
...
// IsReordered returns true if the transitions were possibly reordered.  This
// reordering
...
// The function returns false (no reordering) if there is not enough information
// in
// the alignment to tell (i.e. no self-loop were taken), and in this case the
// calling
...
static bool SplitToPhonesInternal(
    ...
    bool reordered, std::vector<std::vector<int32> > *split_output) {
    ...
    if (trans_model.IsFinal(trans_id)) {  // is final-prob
      if (!reordered)
        ...
        while (i + 1 < alignment.size() &&
          ...
          KALDI_ASSERT(
              trans_model.TransitionIdToTransitionState(alignment[i]) ==
      ...
      if (this_phone != next_phone) {
        // The phone changed, but this is an error-- we should have detected
        // this via the
        // IsFinal check.
        was_ok = false;
        end_points.push_back(i + 1);
  ...
  for (size_t i = 0; i < end_points.size(); i++) {
    ...
    int32 pdf_class =
        trans_model.GetTopo().TopologyForPhone(phone)[0].pdf_class;
...
bool SplitToPhones(const TransitionModel &trans_model,
  ...
  return SplitToPhonesInternal(trans_model, alignment, is_reordered,
                               split_alignment);
}
...
bool ConvertAlignment(const TransitionModel &old_trans_model,
  ...
  if (!SplitToPhones(old_trans_model, old_alignment, &split)) return false;
  ...
  int32 N = new_ctx_dep.ContextWidth(), P = new_ctx_dep.CentralPosition();

  // by starting at -N and going to split.size()+N, we're
  // being generous and not bothering to work out the exact
  // array bounds.
  for (int32 win_start = -N; win_start < static_cast<int32>(split.size() + N);
    ...
    if (static_cast<size_t>(central_pos) < split.size()) {
      ...
      int32 num_pdf_classes =
          new_trans_model.GetTopo().NumPdfClasses(central_phone);
      std::vector<int32> state_seq(num_pdf_classes);  // Indexed by pdf-class
      for (int32 pdf_class = 0; pdf_class < num_pdf_classes; pdf_class++) {
        if (!new_ctx_dep.Compute(phone_window, pdf_class,
                                 &(state_seq[pdf_class]))) {
          std::ostringstream ss;
      ...
      for (size_t j = 0; j < split[central_pos].size(); j++) {
        ...
        int32 trans_idx =
            old_trans_model.TransitionIdToTransitionIndex(old_tid);
        if (static_cast<size_t>(pdf_class) >= state_seq.size())
          KALDI_ERR << "ConvertAlignment: error converting alingment, possibly "
  ...
  for (StateIterator<VectorFst<StdArc> > siter(*fst); !siter.Done();
    ...
    for (MutableArcIterator<VectorFst<StdArc> > aiter(fst, siter.Value());
      ...
      if (l >= 1 && l <= num_tids) {  // a transition-id.
        BaseFloat scaled_log_prob = GetScaledTransitionLogProb(
...
void AddTransitionProbs(const TransitionModel &trans_model,
  ...
  for (fst::StateIterator<Lattice> siter(*lat); !siter.Done(); siter.Next()) {
    for (MutableArcIterator<Lattice> aiter(lat, siter.Value()); !aiter.Done();
         aiter.Next()) {
...
bool ConvertPhnxToProns(const std::vector<int32> &phnx,
  ...
  while (i < phnx.size()) {
    ...
    if (phnx[i] == word_start_sym) {  // start a word...
      ...
      if (!(i > 0 && phnx[i - 1] == word_end_sym)) return false;
