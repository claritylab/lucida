...
void TransitionModel::ComputeTriples(const ContextDependency &ctx_dep) {
  ...
  for (int32 pdf = 0; pdf < static_cast<int32>(pdf_info.size()); pdf++) {
    for (size_t j = 0; j < pdf_info[pdf].size(); j++) {
      int32 phone = pdf_info[pdf][j].first, pdf_class = pdf_info[pdf][j].second;
  ...
  // now triples_ is populated with all possible triples of (phone, hmm_state,
  // pdf).
  std::sort(triples_.begin(),
            triples_.end());  // sort to enable reverse lookup.
...
void TransitionModel::ComputeDerived() {
  ...
  for (int32 tstate = 1; tstate <= static_cast<int32>(triples_.size());
       tstate++)
...
void TransitionModel::InitializeProbs() {
  ...
  for (int32 trans_id = 1; trans_id <= NumTransitionIds(); trans_id++) {
    ...
    const HmmTopology::TopologyEntry &entry =
        topo_.TopologyForPhone(triple.phone);
...
int32 TransitionModel::NumTransitionIndices(int32 trans_state) const {
  ...
  return static_cast<int32>(state2id_[trans_state + 1] -
...
int32 TransitionModel::PairToTransitionId(int32 trans_state,
  ...
  KALDI_ASSERT(static_cast<size_t>(trans_state) <= triples_.size());
  KALDI_ASSERT(trans_index <
               state2id_[trans_state + 1] - state2id_[trans_state]);
...
int32 TransitionModel::NumPhones() const {
  ...
  for (int32 i = 0; i < num_trans_state; ++i) {
    if (triples_[i].phone > max_phone_id) max_phone_id = triples_[i].phone;
  }
  return max_phone_id;
}
...
int32 TransitionModel::SelfLoopOf(
    int32 trans_state) const {  // returns the self-loop transition-id,
  ...
  for (int32 tstate = 1; tstate <= NumTransitionStates(); tstate++) {
    ...
    if (tid == 0) {                            // no self-loop
      ...
      non_self_loop_log_probs_(tstate) =
          log(non_self_loop_prob);  // will be negative.
    }
  }
}
  ...
  for (int32 tstate = 1; tstate <= NumTransitionStates(); tstate++) {
    ...
    if (n > 1) {  // no point updating if only one transition...
      ...
      if (tstate_tot < cfg.mincount) {
        ...
        for (int32 i = 0; i < 3;
             i++) {  // keep flooring+renormalizing for 3 times..
        ...
        for (int32 tidx = 0; tidx < n; tidx++) {
          ...
          double objf_change =
              counts(tidx) * (log(new_probs(tidx)) - log(old_probs(tidx)));
...
void TransitionModel::MapUpdate(const Vector<double> &stats,
  ...
  for (int32 tstate = 1; tstate <= NumTransitionStates(); tstate++) {
    ...
    if (n > 1) {  // no point updating if only one transition...
      ...
      for (int32 tidx = 0; tidx < n; tidx++)
        new_probs(tidx) =
            (counts(tidx) + cfg.tau * old_probs(tidx)) / (cfg.tau + tstate_tot);
      // Compute objf change
      for (int32 tidx = 0; tidx < n; tidx++) {
        double objf_change =
            counts(tidx) * (log(new_probs(tidx)) - log(old_probs(tidx)));
...
void TransitionModel::MleUpdateShared(const Vector<double> &stats,
  ...
  for (map_iter = pdf_to_tstate.begin(); map_iter != pdf_to_tstate.end();
       ++map_iter) {
...
void TransitionModel::MapUpdateShared(const Vector<double> &stats,
  ...
  for (map_iter = pdf_to_tstate.begin(); map_iter != pdf_to_tstate.end();
    ...
    if (n > 1) {  // Only update if >1 transition...
      ...
      for (int32 tidx = 0; tidx < n; tidx++)
        new_probs(tidx) =
            (counts(tidx) + old_probs(tidx) * cfg.tau) / (pdf_tot + cfg.tau);
      // Compute objf change
      for (int32 tidx = 0; tidx < n; tidx++) {
        double objf_change =
            counts(tidx) * (log(new_probs(tidx)) - log(old_probs(tidx)));
...
void TransitionModel::Print(std::ostream &os,
  ...
  if (occs != NULL) KALDI_ASSERT(occs->Dim() == NumPdfs());
  for (int32 tstate = 1; tstate <= NumTransitionStates(); tstate++) {
...
bool GetPdfsForPhones(const TransitionModel &trans_model,
  ...
  pdfs->clear();
  for (int32 tstate = 1; tstate <= trans_model.NumTransitionStates();
       tstate++) {
