...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "Usage: sgmm2-acc-stats-gpost [options] <model-in> "
        "<feature-rspecifier> "
        "<gpost-rspecifier> <stats-out>\n"
        "e.g.: sgmm2-acc-stats-gpost 1.mdl 1.ali scp:train.scp ark, s, cs:- "
        "1.acc\n";
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!gpost_reader.HasKey(utt) ||
        ...
        KALDI_WARN << "No Gaussian-posterior information for utterance " << utt
                   << " (or wrong size).";
        num_err++;
        continue;
      }
