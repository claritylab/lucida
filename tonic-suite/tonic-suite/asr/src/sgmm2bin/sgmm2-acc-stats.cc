...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "e.g.: sgmm2-acc-stats --gselect=ark:gselect.ark 1.mdl 1.ali "
        "scp:train.scp 'ark:ali-to-post 1.ali ark:-|' 1.acc\n"
    ...
    {  // this anonymous scope is to ensure deallocation of unnecessary stuff
      ...
      RandomAccessBaseFloatVectorReaderMapped spkvecs_reader(
          spkvecs_rspecifier, utt2spk_rspecifier);
      ...
      for (; !feature_reader.Done(); feature_reader.Next()) {
        ...
        for (size_t i = 0; i < posterior.size(); i++) {
          ...
          for (size_t j = 0; j < pdf_posterior[i].size(); j++) {
            ...
            tot_like_this_file +=
                sgmm_accs.Accumulate(am_sgmm, per_frame_vars, pdf_id, weight,
                                     &spk_vars) *
                weight;
            tot_weight += weight;
          }
