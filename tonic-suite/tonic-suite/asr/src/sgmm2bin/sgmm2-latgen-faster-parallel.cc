...
void ProcessUtterance(
    const AmSgmm2 &am_sgmm, const TransitionModel &trans_model,
    ...
    double *like_sum, int64 *frame_sum, int32 *num_done, int32 *num_err,
  ...
  vector<vector<int32> > *gselect =
      new std::vector<vector<int32> >(gselect_reader.Value(utt));
  ...
  DecodableAmSgmm2Scaled *sgmm_decodable =
      new DecodableAmSgmm2Scaled(am_sgmm, trans_model, new_feats, gselect,
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Decode features using SGMM-based model.  This version accepts the "
        "--num-threads\n"
        "option but otherwise behaves identically to sgmm2-latgen-faster\n"
        "Usage:  sgmm2-latgen-faster-parallel [options] <model-in> "
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) ==
        kNoRspecifier) {  // a single FST.
      SequentialBaseFloatMatrixReader feature_reader(feature_rspecifier);
      // It's important that we initialize decode_fst after feature_reader, as
      // it
...

      {
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          LatticeFasterDecoder *decoder =
              new LatticeFasterDecoder(*decode_fst, decoder_opts);

          ProcessUtterance(am_sgmm, trans_model, log_prune, acoustic_scale,
                           features, gselect_reader, spkvecs_reader, word_syms,
                           utt, determinize, allow_partial, &alignment_writer,
                           &words_writer, &compact_lattice_writer,
      ...
      for (; !fst_reader.Done(); fst_reader.Next()) {
        ...
        // constructors, one of which takes ownership and one of which does
        // not).
        LatticeFasterDecoder *decoder =
            new LatticeFasterDecoder(decoder_opts, fst);
    ...
    KALDI_LOG << "Done " << num_done << " utterances, failed for " << num_err;
