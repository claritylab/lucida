...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Compute SGMM model projection that only models a part of a pre-LDA "
        "space.\n"
        "Used in predictive SGMMs.  Takes as input an LDA+MLLT transform,\n"
        "and outputs a transform from the pre-LDA+MLLT space to the space "
        "that\n"
        "we want to model\n"
        "Usage: sgmm2-project [options] <model-in> <lda-mllt-mat-in> "
        "<model-out> <new-projection-out>\n"
        "e.g.: sgmm2-project --start-dim=0 --end-dim=52 final.mdl "
        "final.inv_full_mat final_proj1.mdl proj1.mat\n";
    ...
    po.Register("start-dim", &start_dim,
                "Starting dimension to keep in "
                "pre-LDA-MLLT space.");
    po.Register("end-dim", &end_dim,
                "Ending dimension to keep in "
    ...
    std::string model_rxfilename = po.GetArg(1),
                ...
                model_wxfilename = po.GetArg(3), proj_wxfilename = po.GetArg(4);
    ...
    sgmm_project.ComputeProjection(am_sgmm, inv_lda_mllt_mat, start_dim,
                                   end_dim, &projection);

    Matrix<BaseFloat> total_projection(projection.NumRows(),
                                       projection.NumCols());
    total_projection.AddMatMat(1.0, projection, kNoTrans, inv_lda_mllt_mat,
                               kNoTrans, 0.0);
