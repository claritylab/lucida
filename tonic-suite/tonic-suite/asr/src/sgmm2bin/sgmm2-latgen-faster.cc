...
bool ProcessUtterance(
    LatticeFasterDecoder &decoder, const AmSgmm2 &am_sgmm,
    const TransitionModel &trans_model, double log_prune, double acoustic_scale,
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage:  sgmm2-latgen-faster [options] <model-in> "
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) ==
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          double like;
          if (ProcessUtterance(
                  decoder, am_sgmm, trans_model, log_prune, acoustic_scale,
                  features, gselect_reader, spkvecs_reader, word_syms, utt,
      ...
      delete decode_fst;  // only safe to do this after decoder goes out of
                          // scope.
      ...
      for (; !fst_reader.Done(); fst_reader.Next()) {
        ...
        if (ProcessUtterance(decoder, am_sgmm, trans_model, log_prune,
