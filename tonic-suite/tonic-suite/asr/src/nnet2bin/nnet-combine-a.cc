...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "This is a \"special case\" of neural net combination.  It takes a "
        "previous\n"
        "iteration's model, and N other models that have been trained in\n"
        "parallel with SGD on different batches.  If there are L updatable "
        "components,\n"
        "it first uses the validation set to train L parameters \\alpha_l, "
        "consisting of step-lengths\n"
        "along the direction (old-model) -> (average of trained models).  "
        "There is a threshold\n"
        "\"valid-impr-thresh\" (default 0.5).  If the validation-set "
        "improvement is more than\n"
        "this, we skip the next step.  The next step is to \"overshoot\" by a "
        "specified factor,\n"
        "e.g. 1.8.  (This should be strictly less than 2).  Once we have the "
        "resulting parameters\n"
        "\\alpha_l, we multiply the per-layer learning rates by those factors, "
        "subject to sanity-preserving\n"
        "limits on the changes and a minimum learning-rate (see the other "
        "options)\n"
        "\n"
        "Usage:  nnet-combine-a [options] <old-model> <new-model1> "
        "<new-model2> ... <new-modelN> <valid-examples-in> <model-out>\n"
        "\n"
        "e.g.:\n"
    ...
    CombineNnetsA(combine_config, validation_set, nnets, &(am_nnet1.GetNnet()));
