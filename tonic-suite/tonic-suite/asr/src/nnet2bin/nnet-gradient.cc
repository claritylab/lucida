...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Compute neural net gradient using backprop.  Can use multiple "
        "threads\n"
        "using --num-threads option.   Note: this is in addition to any "
        "BLAS-level\n"
        "multi-threading.  Note: the model gradient is written in the same "
        "format\n"
        "as the model, with the transition-model included.\n"
        "\n"
        "Usage:  nnet-gradient [options] <model-in> <training-examples-in> "
        "<model-gradient-out>\n"
        "\n"
    ...
    DoBackpropParallel(am_nnet.GetNnet(), minibatch_size, &example_reader,
                       &num_examples, &(am_gradient.GetNnet()));
    ...
    KALDI_LOG << "Finished computing gradient, processed " << num_examples
              << " training examples.  Wrote model to " << gradient_wxfilename;
