...
void SetPriors(const TransitionModel &tmodel,
               const Vector<double> &transition_accs, double prior_floor,
               AmNnet *am_nnet) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Train the transition probabilities of a neural network acoustic "
        "model\n"
        "\n"
        "Usage:  nnet-train-transitions [options] <nnet-in> "
        "<alignments-rspecifier> <nnet-out>\n"
        "e.g.:\n"
    ...
    po.Register("set-priors", &set_priors,
                "If true, also set priors in neural "
    ...
    std::string nnet_rxfilename = po.GetArg(1), ali_rspecifier = po.GetArg(2),
