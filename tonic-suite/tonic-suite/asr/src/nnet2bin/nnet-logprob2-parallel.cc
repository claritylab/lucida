...
struct NnetLogprobTask {
  NnetLogprobTask(const AmNnet &am_nnet, const Vector<BaseFloat> &inv_priors,
                  const std::string &key, const Matrix<BaseFloat> &feats,
                  BaseFloatMatrixWriter *prob_writer_nodiv,
                  BaseFloatMatrixWriter *logprob_writer_divided)
      : am_nnet_(am_nnet),
        inv_priors_(inv_priors),
        key_(key),
        feats_(feats),
  ...
  void operator()() {
    ...
    bool pad_input = true;
    NnetComputation(am_nnet_.GetNnet(), feats_, pad_input, &log_probs_);
  }

  ~NnetLogprobTask() {  // Produces output.  Run sequentially.
    // at this point they are probabilities, not log-probs, without prior
    // division.
    prob_writer_nodiv_->Write(key_, Matrix<BaseFloat>(log_probs_));

    log_probs_.MulColsVec(
        inv_priors_);  // scales each column by the corresponding element
    // of inv_priors.
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "output\n"
        "matrix of logprobs.  This version of the program outputs to two "
        "tables,\n"
        "one table of probabilities without prior division and one table of\n"
        "log-probs with prior division.  It is intended for use in "
        "discriminative\n"
        "training.  This version supports multi-threaded operation "
        "(--num-threads\n"
        "option)\n"
        "\n"
        "Usage: nnet-logprob2-parallel [options] <model-in> "
        "<features-rspecifier> "
        "<probs-wspecifier-not-divided> <logprobs-wspecifier-divided>\n"
        "\n"
        "e.g.: nnet-logprob2-parallel 1.nnet \"$feats\" ark:- "
        "\"ark:|logprob-to-post ark:- 1.post\" ark:- \\"
