...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "held-out),\n"
        "produce an output model that will generally be the same as the new "
        "model,\n"
        "but in cases where changes a particular component led to an objective "
        "function\n"
        "change per example that's less than -t for a particular threshold "
        "t>=0\n"
        "(default --threshold=0.0001), will regress towards the old model.  "
        "Does this\n"
        "by repeatedly downscaling by a scale (default --scale=0.75) until we "
        "satisfy\n"
        "the criterion.\n"
        "Usage:  nnet-limit-degradation [options] <old-model-in> "
        "<new-model-in> <training-examples-in> <model-out>\n"
        "e.g.: nnet-limit-degradation 1.nnet 2.nnet ark:valid.egs 2new.nnet\n";

    ParseOptions po(usage);

    bool binary_write = false;
    BaseFloat scale = 0.75;  // Each time we find we went too far, we multiply
                             // the parameter-change by this scale.
    BaseFloat threshold = 0.0001;  // Maximum degradation.
    po.Register("binary", &binary_write,
                "If true, write model in binary format.");
    po.Register("scale", &scale,
                "Scale factor we multiply by each time we detect "
                "excess degradation");
    po.Register("threshold", &threshold,
                "Threshold of amount of loglike-per-frame "
    ...
    for (int32 iter = 0; iter < max_iter; iter++) {
      ...
      double objf_per_frame =
          ComputeNnetGradient(avg_nnet, examples, batch_size, &nnet_gradient);
      ...
      if (diff.Min() > -threshold) {
        ...
        for (int32 i = 0; i < nu; i++) old_scales(i) = 1.0 - scales(i);
