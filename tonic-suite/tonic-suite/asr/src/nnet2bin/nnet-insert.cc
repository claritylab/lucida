...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "This is mostly intended for adding new hidden layers to neural "
        "networks.\n"
        "You can either specify the option --insert-at=n (specifying the index "
        "of\n"
        "the component after which you want your neural network inserted), or "
        "by\n"
        "default this program will insert it just before the component before "
        "the\n"
        "softmax component.  CAUTION: It will also randomize the parameters of "
        "the\n"
        "component before the softmax (typically AffineComponent), with stddev "
        "equal\n"
        "to the --stddev-factor option (default 0.1), times the inverse square "
        "root\n"
        "of the number of inputs to that component.\n"
        "Set --randomize-next-component=false to turn this off.\n"
        "\n"
        "Usage:  nnet-insert [options] <nnet-in> <raw-nnet-to-insert-in> "
        "<nnet-out>\n"
        "e.g.:\n"
    ...
    po.Register("insert-at", &insert_at,
                "Inserts new components before the "
                "specified component (note: indexes are zero-based).  If <0, "
                "inserts before the component before the softmax.");
    po.Register("stddev-factor", &stddev_factor,
                "Factor on the standard "
    ...
    if (insert_at == -1) {
      if ((insert_at = IndexOfSoftmaxLayer(am_nnet.GetNnet())) == -1)
        KALDI_ERR
            << "We don't know where to insert the new components: "
    ...
    InsertComponents(src_nnet, insert_at, &(am_nnet.GetNnet()));
    ...
    if (randomize_next_component) {
      ...
      BaseFloat stddev =
          stddev_factor / std::sqrt(static_cast<BaseFloat>(uc->InputDim()));
