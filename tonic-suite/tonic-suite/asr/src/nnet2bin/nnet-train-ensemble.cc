...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "gradient descent using minibatches.  Modified version of "
        "nnet-train-simple.\n"
        "Implements parallel gradient descent with a term that encourages the "
        "nnets to\n"
        "produce similar outputs.\n"
        "\n"
        "Usage:  nnet-train-ensemble [options] <model-in-1> <model-in-2> ... "
        "<model-in-n>"
        "  <training-examples-in> <model-out-1> <model-out-2> ... "
        "<model-out-n> \n"
        "\n"
        "e.g.:\n"
    ...
    {
      if (zero_stats) {
        for (int32 n = 1; n < num_nnets; n++) nnets[n]->ZeroStats();
      }
      {  // want to make sure this object deinitializes before
        // we write the model, as it does something in the destructor.
        NnetEnsembleTrainer trainer(train_config, nnets);

        SequentialNnetExampleReader example_reader(examples_rspecifier);

        for (; !example_reader.Done(); example_reader.Next(), num_examples++)
          trainer.TrainOnExample(
              example_reader.Value());  // It all happens here!
      }
