...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "The file <index-list> contains a series of line, each containing a "
        "list\n"
        "of integer indexes.  For each line we create a transform of the same "
        "type\n"
        "as nnet-get-feature-transform would produce, taking as input just "
        "the\n"
        "listed feature dimensions.  The output transform will be the "
        "concatenation\n"
        "of all these transforms.  The output-dim will be the number of "
        "integers in\n"
        "the file <index-list> (the individual transforms are not "
        "dimension-reducing).\n"
        "Do not set the --dim option."
        "Usage:  nnet-get-feature-transform-multi [options] <index-list> "
        "<lda-acc-1> <lda-acc-2> ... <lda-acc-n> <matrix-out>\n";
