...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-train-simple-perturbed [options] <model-in> "
        "<training-examples-in> <model-out>\n"
        "note: the option --within-covar=<file> is needed\n"
        "\n"
        "e.g.:\n"
        "nnet-train-simple-perturbed --within-covar=within.spmat "
        "--target-objf-change=0.2 \\\n"
...

    {
      ...
      {  // want to make sure this object deinitializes before
        ...
        NnetPerturbedTrainer trainer(train_config, within_covar,
                                     &(am_nnet.GetNnet()));
