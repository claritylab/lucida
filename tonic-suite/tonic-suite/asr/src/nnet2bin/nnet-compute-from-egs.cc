...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Does the neural net computation, taking as input the nnet-training "
        "examples\n"
        "(typically an archive with the extension .egs), ignoring the labels; "
        "it\n"
        "outputs as a matrix the result.  Used mostly for debugging.\n"
        "\n"
        "Usage:  nnet-compute-from-egs [options] <raw-nnet-in> "
        "<egs-rspecifier> "
        "<feature-wspecifier>\n"
        "e.g.:  nnet-compute-from-egs 'nnet-to-raw-nnet final.mdl -|' "
        "egs.10.1.ark ark:-\n";
    ...
    for (; !example_reader.Done(); example_reader.Next()) {
      ...
      int32 basic_dim = eg.input_frames.NumCols(), spk_dim = eg.spk_info.Dim(),
            dim = basic_dim + spk_dim;
      Matrix<BaseFloat> input_frames(eg.input_frames),
          input_block(context, dim);
      input_block.Range(0, context, 0, basic_dim)
          .CopyFromMat(input_frames.Range(start_offset, context, 0, basic_dim));
      if (spk_dim != 0) {
        input_block.Range(0, context, basic_dim, spk_dim)
            .CopyRowsFromVec(eg.spk_info);
      }
