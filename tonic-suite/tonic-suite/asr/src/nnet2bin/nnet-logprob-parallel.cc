...
struct NnetLogprobTask {
  NnetLogprobTask(const AmNnet &am_nnet, const CuVector<BaseFloat> &inv_priors,
                  const std::string &key, const CuMatrix<BaseFloat> &feats,
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage: nnet-logprob-parallel [options] <model-in> "
        "<features-rspecifier> "
        "<logprobs-wspecifier>\n"
        "\n"
        "e.g.: nnet-logprob-parallel 1.nnet \"$feats\" ark:- | "
