...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-shuffle-egs-discriminative [options] <egs-rspecifier> "
        "<egs-wspecifier>\n"
        "\n"
        "nnet-shuffle-egs-discriminative --srand=1 ark:train.degs "
        "ark:shuffled.degs\n";
    ...
    if (buffer_size == 0) {  // Do full randomization
      ...
      for (; !example_reader.Done(); example_reader.Next())
        egs.push_back(new DiscriminativeNnetExample(example_reader.Value()));
