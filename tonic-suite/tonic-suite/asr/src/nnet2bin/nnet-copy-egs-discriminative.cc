...
void AverageConstPart(int32 const_feat_dim, DiscriminativeNnetExample *eg) {
  if (eg->spk_info.Dim() != 0) {  // already has const part.
    ...
    int32 dim = eg->input_frames.NumCols(), basic_dim = dim - const_feat_dim;
    ...
    eg->spk_info.AddRowSumMat(
        1.0 / mat.NumRows(),
        mat.Range(0, mat.NumRows(), basic_dim, const_feat_dim), 0.0);
  }
}
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-copy-egs-discriminative [options] <egs-rspecifier> "
        "<egs-wspecifier1> [<egs-wspecifier2> ...]\n"
        "\n"
        "e.g.\n"
    ...
    for (int32 i = 0; i < num_outputs; i++)
      example_writers[i] =
          new DiscriminativeNnetExampleWriter(po.GetArg(i + 2));
