...
/*
  ...
  specific classes]) const;
    Read(std::istream &, bool binary, [possibly extra optional args for specific
  classes]);
  ..
  }
  ...
  the
  type WriteMyTypedefName.  This is to avoid introducing confusing templated
  functions;
  we could easily create templated functions to handle most of these cases but
  they
  ...
  class.
  We provide special functions for manipulating these.  These special tokens
  must
  ...
  the strings
  contain data rather than "special tokens" that are whitespace-free and
  nonempty).  This is
  because Kaldi is structured in such a way that strings don't appear, except as
  OpenFst symbol
