...
void UnitTestGcdLcmTpl() {
  for (I a = 1; a < 15; a++) {  // a is min gcd.
    ...
    I h = Lcm(b * a, c * a);
    KALDI_ASSERT(h != 0 && (h % (b * a)) == 0 && (h % (c * a)) == 0);
  }
}
  ...
  for (int i = 1; i < 10; i++) {
    ...
    {  // test WithProb().
      ...
      }
      {
        int32 tot = 0, n = 10000;
        BaseFloat p = 0.5;
        for (int32 i = 0; i < n; i++) tot += WithProb(p);
    ...
    {  // test RandInt().
      ...
      for (int j = 0;; j++) {
        ...
        if (std::abs((float)sum) < 0.5 * sqrt((double)j) * (maxint - minint))
          break;
      }
    }
...
void UnitTestLogAddSub() {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    KALDI_ASSERT(std::abs(add - add1) < thresh &&
                 std::abs(add - add2) < thresh);
...
void UnitTestDefines() {  // Yes, we even unit-test the preprocessor statements.
  ...
  KALDI_ASSERT(1.0 + DBL_EPSILON != 1.0 && 1.0 + (0.5 * DBL_EPSILON) == 1.0 &&
               "If this test fails, you can probably just comment it out-- may "
               "mean your CPU exceeds expected floating point precision");
  KALDI_ASSERT(1.0f + FLT_EPSILON != 1.0f &&
               1.0f + (0.5f * FLT_EPSILON) == 1.0f &&
               "If this test fails, you can probably just comment it out-- may "
               "mean your CPU exceeds expected floating point precision");
...
void UnitTestApproxEqual() {
  ...
  KALDI_ASSERT(ApproxEqual(-std::numeric_limits<float>::infinity(),
                           -std::numeric_limits<float>::infinity()));
  KALDI_ASSERT(!ApproxEqual(-std::numeric_limits<float>::infinity(), 0));
  KALDI_ASSERT(!ApproxEqual(-std::numeric_limits<float>::infinity(), 1));
}
