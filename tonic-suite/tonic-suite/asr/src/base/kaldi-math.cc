...
int32 RandInt(int32 min_val, int32 max_val,
              struct RandomState *state) {  // This is not exact.
  ...
  if (RAND_MAX > (max_val - min_val) * 8) {
    ...
    if ((unsigned int)(RAND_MAX * RAND_MAX) >
      ...
      throw std::runtime_error(
          std::string() + "rand_int failed because we do not support " +
          "such large random numbers. " + "(Extend this function).");
    }
  }
#else
  return min_val +
