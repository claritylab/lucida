...
struct OnlineIvectorExtractionConfig {
  ...
  void Register(OptionsItf *po) {
    po->Register("lda-matrix", &lda_mat_rxfilename,
                 "Filename of LDA matrix, "
    ...
    po->Register("num-gselect", &num_gselect,
                 "Number of Gaussians to select "
                 "for iVector extraction");
    po->Register("min-post", &min_post,
                 "Threshold for posterior pruning in "
                 "iVector extraction");
    po->Register(
        "posterior-scale", &posterior_scale,
        "Scale for posteriors in "
        "iVector extraction (may be viewed as inverse of prior scale)");
    po->Register("use-most-recent-ivector", &use_most_recent_ivector,
                 "If true, "
    ...
    po->Register("max-remembered-frames", &max_remembered_frames,
                 "The maximum "
...
struct OnlineIvectorExtractorAdaptationState {
  ...
  /// This constructor initializes adaptation-state with no prior speaker
  /// history.
...
/// OnlineIvectorFeature is an online feature-extraction class that's
/// responsible
...
class OnlineIvectorFeature : public OnlineFeatureInterface {
 public:
  ...
  // Objective improvement per frame from iVector estimation, versus default
  // iVector
