...
class OnlineFeatInputItf {
 public:
  ...
  //            we're at the beginning of the file and some buffering is going
  //            on.
  //            In that case you should try again.  The function will return
  //            "false"
  ...
  //       considering time out handling to be implementation detail, and if
  //       needed
  ...
  virtual int32 Dim()
      const = 0;  // Return the output dimension of these features.
...
class OnlineCmnInput : public OnlineFeatInputItf {
 public:
  ...
  //                mean, at the start of the file.  Adds latency but only at
  //                the
  //                start
  OnlineCmnInput(OnlineFeatInputItf *input, int32 cmn_window, int32 min_window)
      : input_(input),
        ...
        history_(cmn_window + 1, input->Dim()),
        t_in_(0),
        t_out_(0),
        sum_(input->Dim()) {
 ...
 private:
  ...
  void AcceptFrame(
      const VectorBase<BaseFloat> &input);          // Accept the next frame
  ...
  Vector<double>
      sum_;  // Sum of the frames from t_out_ - HistoryLength(t_out_),
             // to t_out_ - 1.
...
class OnlineLdaInput : public OnlineFeatInputItf {
 public:
  OnlineLdaInput(OnlineFeatInputItf *input, const Matrix<BaseFloat> &transform,
 ...
 private:
  ...
  // a contiguous window of size "context_window" of input frames.  The
  // dimension
  ...
  static void SpliceFrames(const MatrixBase<BaseFloat> &input1,
                           ...
                           int32 context_window, Matrix<BaseFloat> *output);
...
class OnlineFeInput : public OnlineFeatInputItf {
 public:
  ...
  OnlineFeInput(OnlineAudioSourceItf *au_src, E *fe, const int32 frame_size,
...
template <class E>
bool OnlineFeInput<E>::Compute(Matrix<BaseFloat> *output) {
...
struct OnlineFeatureMatrixOptions {
  ...
  int32 num_tries;   // number of tries of getting no output and timing out,
                     // before we give up.
  OnlineFeatureMatrixOptions() : batch_size(27), num_tries(5) {}
