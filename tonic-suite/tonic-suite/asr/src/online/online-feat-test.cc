 ...
 public:
  ...
  virtual bool Compute(Matrix<BaseFloat> *output) {
    ...
    if (num_frames_return == 0) {
      ...
      output->CopyFromMat(
          feats_.Range(position_, num_frames_return, 0, feats_.NumCols()));
    }
    position_ += num_frames_return;
    if (position_ == feats_.NumRows())
      return false;
    else
      return true;
  }
...
template <class Real>
static void AssertEqual(const Matrix<Real> &A, const Matrix<Real> &B,
                        float tol = 0.001) {
...
void TestOnlineFeatureMatrix() {
  ...
  for (int32 frame = 0; frame < num_frames; frame++) {
    ...
    KALDI_ASSERT(online_feature_matrix.GetFrame(frame)
                     .ApproxEqual(input_feats.Row(frame)));
  }
...
void TestOnlineLdaInput() {
  ...
  Matrix<BaseFloat> transform(lda_output_dim,
                              lda_input_dim + (have_offset ? 1 : 0));
  transform.SetRandn();
  Matrix<BaseFloat> input_feats(num_frames, dim);
  input_feats.SetRandn();

  OnlineMatrixInput matrix_input(input_feats);
  OnlineLdaInput lda_input(&matrix_input, transform, left_context,
                           right_context);
  ...
  if (!have_offset) {
    ...
    SubMatrix<BaseFloat> linear_part(transform, 0, transform.NumRows(), 0,
                                     transform.NumCols() - 1);
    output_feats2.AddMatMat(1.0, temp_feats, kNoTrans, linear_part, kTrans,
                            0.0);
...
void TestOnlineCmnInput() {    // We're also testing OnlineCacheInput here.
  ...
  OnlineCmnInput cmn_input(&matrix_input, cmn_window, min_window);
  ...
  for (int32 i = 0; i < output_feats2.NumRows(); i++) {
    ...
    if (i == 0 && min_window == 0)
      ...
      SubMatrix<BaseFloat> this_block(input_feats, 0, window_nframes, 0, dim);
      ...
      Vector<BaseFloat> this_sum(dim);
      SubMatrix<BaseFloat> this_block(input_feats, i - window_nframes,
                                      window_nframes, 0, dim);
      this_sum.AddRowSumMat(1.0, this_block, 0.0);
      this_row.AddVec(-1.0 / window_nframes, this_sum);
    }
  }
  ...
  for (int32 i = 0; i < output_feats2.NumRows(); i++) {
    if (!output_feats1.Row(i).ApproxEqual(output_feats2.Row(i))) {
      KALDI_ERR << "Rows differ " << i << ", " << input_feats.Row(i)
                << output_feats1.Row(i) << output_feats2.Row(i);
    }
  }
  KALDI_ASSERT(output_feats1.ApproxEqual(output_feats2));
