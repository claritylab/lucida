...
struct OnlineFasterDecoderOpts : public FasterDecoderOptions {
  ...
  int32 max_utt_len_;   // if utt. is longer, we accept shorter silence as utt.
                        // separators
...
class OnlineFasterDecoder : public FasterDecoder {
 public:
  ...
  enum DecodeState {
    ...
    kEndUtt =
        2,  // End of utterance, caused by e.g. a sufficiently long silence
 ...
 private:
  ...
  void MakeLattice(const Token *start, const Token *end,
  ...
  const TransitionModel &
      trans_model_;            // needed for trans-id -> phone conversion
