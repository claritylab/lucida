...
void OnlineFasterDecoder::MakeLattice(
  ...
  if (this_cost != std::numeric_limits<double>::infinity()) is_final = true;
...
void OnlineFasterDecoder::UpdateImmortalToken() {
  ...
  for (const Elem *e = toks_.GetList(); e != NULL; e = e->tail) {
    ...
    while (tok->arc_.ilabel == 0)  // deal with non-emitting ones ...
      tok = tok->prev_;
    if (tok != NULL) emitting.insert(tok);
  }
  Token *the_one = NULL;
  while (1) {
    if (emitting.size() == 1) {
...
bool OnlineFasterDecoder::PartialTraceback(
...
void OnlineFasterDecoder::FinishTraceBack(
    fst::MutableFst<LatticeArc> *out_fst) {
  Token *best_tok = NULL;
...
void OnlineFasterDecoder::TracebackNFrames(
  ...
  bool is_final = false;
  double this_cost =
      best_tok->cost_ + fst_.Final(best_tok->arc_.nextstate).Value();

  if (this_cost != std::numeric_limits<double>::infinity()) is_final = true;
  std::vector<LatticeArc> arcs_reverse;  // arcs in reverse order.
  for (Token *tok = best_tok; (tok != NULL) && (nframes > 0);
       tok = tok->prev_) {
...
bool OnlineFasterDecoder::EndOfUtterance() {
  ...
  for (size_t i = 0; i < split.size(); i++) {
    ...
    if (silence_set_.count(phone) == 0) return false;
  }
  return true;
}

OnlineFasterDecoder::DecodeState OnlineFasterDecoder::Decode(
  ...
  for (; !decodable->IsLastFrame(frame_ - 1) && batch_frame < opts_.batch_size;
    ...
    if (batch_frame != 0 && (frame_ % 200) == 0)
      ...
      KALDI_VLOG(3) << "Beam: " << effective_beam_ << "; Speed: "
                    << ((timer.Elapsed() - tstart_batch) * 1000) /
                           (batch_frame * 10) << " xRT";
