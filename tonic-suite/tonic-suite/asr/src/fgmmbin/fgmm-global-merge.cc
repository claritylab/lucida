...
void MergeFullGmm(const FullGmm &src, FullGmm *dst) {
  ...
  means_invcovars.Range(0, dst_num_mix, 0, dim)
      .CopyFromMat(dst_copy.means_invcovars());
  means_invcovars.Range(dst_num_mix, src_num_mix, 0, dim)
      .CopyFromMat(src.means_invcovars());
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: fgmm-global-merge [options] fgmm-out sizes-file-out fgmm-in1 "
        "fgmm-in2 ...\n";
