...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: \n"
        " fgmm-gselect [options] <model-in> <feature-rspecifier> "
        "<gselect-wspecifier>\n"
        "The --gselect option (which takes an rspecifier) limits selection to "
        "a subset\n"
        "of indices:\n"
        "e.g.: fgmm-gselect \"--gselect=ark:gunzip -c bigger.gselect.gz|\" "
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (gselect_rspecifier !=
        ...
        for (int32 i = 0; i < mat.NumRows(); i++)
          tot_like_this_file += fgmm.GaussianSelectionPreselect(
