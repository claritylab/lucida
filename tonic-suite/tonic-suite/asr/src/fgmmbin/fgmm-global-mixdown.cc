...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "\"good\" pairs of Gaussians to consider merging (pairs that most "
        "often\n"
        ...
        "e.g.: fgmm-global-mixdown --gselect=gselect.1 --mixdown-target=120 "
        "1.ubm 2.ubm\n"
    ...
    BaseFloat power =
        0.75;  // Power used in choosing pairs; between 0.5 and 1 make sense.
    ParseOptions po(usage);
    po.Register("binary", &binary_write, "Write output in binary mode");
    po.Register("gselect", &gselect_rspecifier,
                "Gaussian-selection info, used "
                "to select most promising pairs");
    po.Register("num-pairs", &num_pairs,
                "Number of pairs of Gaussians to try merging "
    ...
    po.Register("power", &power,
                "Power used in choosing pairs from gselect (should be between "
                "0.5 and 1)");
    ...
    KALDI_ASSERT(mixdown_target >= 0 &&
                 "--mixdown-target option is required and must be >0.");
    ...
    if (gselect_rspecifier == "") {  // use all pairs.
      ...
      unordered_map<std::pair<int32, int32>, int32, PairHasher<int32> >
          counts;  // co-occurrence map:
      ...
      for (unordered_map<std::pair<int32, int32>, int32,
                         PairHasher<int32> >::iterator iter = counts.begin();
           iter != counts.end(); ++iter) {
        int32 idx1 = iter->first.first, idx2 = iter->first.second,
              count = iter->second;
        if (idx1 != idx2) {
          BaseFloat x = counts[std::make_pair(idx1, idx1)] *
                        counts[std::make_pair(idx2, idx2)];
    ...
    KALDI_LOG << "Mixed down GMM from " << orig_ngauss << " to " << cur_ngauss
              << ", likelihood change was " << like_change;
