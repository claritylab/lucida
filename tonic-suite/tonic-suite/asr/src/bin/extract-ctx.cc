...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Given the tree stats and the resulting tree, output a mapping of "
        "phones\n"
        "in context (and pdf-class) to the pdf-id.  This can be used to link "
        "the\n"
        "acoustic model parameters to their phonetic meaning.  Outputs lines "
        "such as\n"
        "  \"<pdf-id> <pdf-class> <left> <center> <right>\" in case of "
        "tri-phones\n"
        "e.g.: \n"
    ...
    po.Register(
        "non-sil-pdf-classes", &nonsilpdfclasses,
    ...
    if (mono) {
      ...
      if (phones_symtab != NULL) {
        ...
        if (maxs != (int32)(ns - 1)) {
          KALDI_WARN << "specified highest symbol (" << maxs
                     << ") not equal to size of symtab (" << (ns - 1)
                     << "), adjusting ";
          maxs = (ns - 1);
        }
      }
      ...
      for (int32 p = 1; p <= maxs; ++p) {
        int32 mpdf =
            (silset.find(p) == silset.end() ? nonsilpdfclasses : silpdfclasses);
        ...
        for (int i = 0; i < mpdf; ++i) {
          ...
          EventAnswerType ans;
          if (map.Map(et, &ans)) {
            std::cout << ans << " " << EventTypeToString(et, phones_symtab,
                                                         addpos) << std::endl;
          } else {
      ...
      // build-tree-questions.h:typedef std::vector<std::pair<EventType,
      // Clusterable*> > BuildTreeStatsType
      ...
      for (size_t i = 0; i < stats.size(); ++i) {
        ...
        if (map.Map(stats[i].first, &ans)) {
          ...
          KALDI_WARN << "Unable to get pdf-id for stats "
                     << EventTypeToString(stats[i].first, phones_symtab,
                                          addpos);
        }
      }
    }
