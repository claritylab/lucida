...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "This program applies a linear or affine transform to individual "
        "vectors, e.g.\n"
        "iVectors.  It is transform-feats, except it works on vectors rather "
        "than matrices,\n"
        "and expects a single transform matrix rather than possibly a table of "
        "matrices\n"
        "\n"
        "Usage: transform-vec [options] <transform-rxfilename> "
        "<feats-rspecifier> <feats-wspecifier>\n"
    ...
    for (; !vec_reader.Done(); vec_reader.Next()) {
      ...
      if (transform_cols == vec_dim) {
        ...
        vec_out.AddMatVec(1.0,
                          transform.Range(0, transform.NumRows(), 0, vec_dim),
                          kNoTrans, vec, 1.0);
      }
