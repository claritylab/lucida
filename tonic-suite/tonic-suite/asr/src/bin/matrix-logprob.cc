...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Compute the log-prob of a particular (e.g.) pdf sequence, derived "
        "from a pdf alignment,\n"
        "given log-probs in a matrix.  The log-probs are computed over the "
        "whole data and printed\n"
        "as a logging message. Optionally also write out the original matrix.  "
        "This is for use in\n"
        "neural net discriminative training (for computing objective "
        "functions).\n"
        "\n"
        "Usage: matrix-logprob [options] <matrix-rspecifier> "
        "<pdf-ali-rspecifier> [<matrix-wspecifier1>]\n";
    ...
    for (; !matrix_reader.Done(); matrix_reader.Next()) {
      ...
      if (pdf_ali_reader.HasKey(key)) {
        ...
        if (num_frames != logprob.NumRows()) {
          KALDI_WARN << "Alignment has wrong size " << num_frames << " vs. "
                     << logprob.NumRows() << " for utterance " << key;
          num_err++;
          continue;
        }
        for (int32 t = 0; t < num_frames; t++) {
          int32 pdf_id = ali[t];
          if (pdf_id < 0 ||
              pdf_id > logprob.NumCols())  // I'm letting this be an error.
    ...
    KALDI_LOG << "Average log-prob per frame is "
              << (tot_log_prob / tot_num_frames) << " over " << tot_num_frames
              << " frames.";
