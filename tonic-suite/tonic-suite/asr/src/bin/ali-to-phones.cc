...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Convert model-level alignments to phone-sequences (in integer, not "
        "text, form)\n"
        "Usage:  ali-to-phones  [options] <model> <alignments-rspecifier> "
        "<phone-transcript-wspecifier>\n"
        "e.g.: \n"
    ...
    po.Register("per-frame", &per_frame,
                "If true, write out the frame-level phone alignment (else "
                "phone sequence)");
    ...
    Int32PairVectorWriter pair_writer(write_lengths ? phones_wspecifier
                                                    : empty);
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      if (!write_lengths) {
        ...
        for (size_t i = 0; i < split.size(); i++) {
          ...
          if (per_frame)
            for (int32 j = 0; j < num_repeats; j++) phones.push_back(phone);
          else
