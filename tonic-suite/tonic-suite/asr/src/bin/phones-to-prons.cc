...
void CreatePhonesAltFst(const std::vector<int32> &phones, int32 word_start_sym,
                        int32 word_end_sym, fst::VectorFst<fst::StdArc> *ofst) {
  using fst::StdArc;
  ...
  for (size_t i = 0; i < phones.size(); i++) {
    ...
    ofst->AddArc(cur_s, StdArc(phones[i], phones[i], Weight::One(), next_s));
    cur_s = next_s;
  }
  for (StateId s = 0; s <= cur_s; s++) {
    ofst->AddArc(s, StdArc(word_end_sym, word_end_sym, Weight::One(), s));
    ofst->AddArc(s, StdArc(word_start_sym, word_start_sym, Weight::One(), s));
  }
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "<word-end-sym> <phones-rspecifier> <words-rspecifier> "
        "<prons-wspecifier>\n"
        "e.g.: \n"
    ...
    if (!ConvertStringToInteger(word_end_sym_str, &word_end_sym) ||
      ...
      KALDI_ERR << "Invalid word end symbol (expecting integer >= 0"
                << ", different from word start symbol): " << word_end_sym_str;
    ...
    for (; !phones_reader.Done(); phones_reader.Next()) {
      ...
      if (phnx2word.Start() == fst::kNoStateId) {
        ...
        if (g_kaldi_verbose_level >= 2) {
          ...
          fst::FstPrinter<StdArc> fstprinter(phn2word, NULL, NULL, NULL, false,
                                             true);
      ...
      if (!fst::GetLinearSymbolSequence(phnx2word_best, &phnx, &words2,
                                        &garbage))
        KALDI_ERR << "phnx2word is not a linear transducer (code error?)";
      if (words2 != words) KALDI_ERR << "words have changed! (code error?)";
      ...
      if (!ConvertPhnxToProns(phnx, words, word_start_sym, word_end_sym,
                              &prons)) {
    ...
    KALDI_LOG << "Done " << n_done << " utterances; " << n_err
              << " had errors.";
