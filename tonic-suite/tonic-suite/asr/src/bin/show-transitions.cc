...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage:  show-transitions phones-symbol-table transition/model-file "
        "[occs-file]\n"
        "e.g.: \n"
    ...
    for (size_t i = 0; i < syms->NumSymbols(); i++) names[i] = syms->Find(i);
    ...
    trans_model.Print(std::cout, names,
