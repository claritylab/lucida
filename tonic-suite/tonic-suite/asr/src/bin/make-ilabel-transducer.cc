...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Make transducer that de-duplicates context-dependent ilabels that map "
        "to the same state\n"
        "Usage:   make-ilabel-transducer ilabel-info-right tree-file "
        "transition-gmm/model ilabel-info-left [mapping-fst-out]\n"
        "e.g.: \n"
        " make-ilabel-transducer old_ilabel_info 1.tree 1.mdl new_ilabel_info "
        "> convert.fst\n";
    ...
    po.Register("old-to-new-mapping", &old2new_map_wxfilename,
    ...
    std::string ilabel_info_rxfilename = po.GetArg(1),
                tree_filename = po.GetArg(2), model_filename = po.GetArg(3),
    ...
    GetIlabelMapping(old_ilabels, ctx_dep, trans_model, &old2new_mapping);
    ...
    new_ilabels.resize(
        1 + *std::max_element(old2new_mapping.begin(), old2new_mapping.end()));
