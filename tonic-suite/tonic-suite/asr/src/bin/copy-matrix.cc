...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "  or: copy-matrix [options] <matrix-in-rxfilename> "
    ...
    std::string matrix_in_fn = po.GetArg(1), matrix_out_fn = po.GetArg(2);

    // all these "fn"'s are either rspecifiers or filenames.

    bool in_is_rspecifier =
             (ClassifyRspecifier(matrix_in_fn, NULL, NULL) != kNoRspecifier),
         out_is_wspecifier = (ClassifyWspecifier(matrix_out_fn, NULL, NULL,
                                                 NULL) != kNoWspecifier);
