...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Generate lattices, reading log-likelihoods as matrices, using "
        "multiple decoding threads\n"
        " (model is needed only for the integer mappings in its "
        "transition-model)\n"
        "Usage: latgen-faster-mapped-parallel [options] trans-model-in "
...

      {
        for (; !loglike_reader.Done(); loglike_reader.Next()) {
          ...
          DecodableMatrixScaledMapped *decodable =
              new DecodableMatrixScaledMapped(trans_model, acoustic_scale,
                                              loglikes);
