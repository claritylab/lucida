...
/** @brief Sums the pdf vectors to counts, this is used to obtain prior counts
 * for hybrid decoding.
*/
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "(eg. for example to get pdf-counts to estimate DNN-output priros, or "
        "data analysis)\n"
        ...
        " ali-to-phone --per-frame=true ark:1.ali ark:- | analyze-counts "
        "--verbose=1 ark:- - >/dev/null\n";
    ...
    std::string alignments_rspecifier = po.GetArg(1), wxfilename = po.GetArg(2);
    ...
    // We need at least one occurence for each tgt, so there is no nan during
    // decoding
    std::vector<uint64> counts_nozero(counts);
    for (size_t i = 0; i < counts.size(); i++) {
      if (counts_nozero[i] == 0) {
        KALDI_WARN
            << "Zero count for element " << i << ", force setting to one."
    ...
    if (symbol_table_filename != "" || (kaldi::g_kaldi_verbose_level >= 1)) {
      ...
      os << "\n#total " << sum << " (" << static_cast<float>(sum) / 100 / 3600
         << "h)" << std::endl;
      KALDI_LOG << os.str();
    }
