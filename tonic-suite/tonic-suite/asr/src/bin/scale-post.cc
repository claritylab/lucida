...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        " each utterance.\n"
        "Usage: scale-post <post-rspecifier> (<scale-rspecifier>|<scale>) "
        "<post-wspecifier>\n";
    ...
    if (ClassifyRspecifier(scale_or_scale_rspecifier, NULL, NULL) ==
        kNoRspecifier) {
    ...
    for (; !posterior_reader.Done(); posterior_reader.Next()) {
      ...
      if (scale_or_scale_rspecifier != "" && !scale_reader.HasKey(key)) {
        ...
        BaseFloat post_scale =
            (scale_or_scale_rspecifier == "" ? global_scale
