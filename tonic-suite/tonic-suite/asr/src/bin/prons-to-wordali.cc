...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "    \"ark:ali-to-phones --write-lengths 1.mdl ark:1.ali ark:-|\" "
        "ark:1.wali\n";

    ParseOptions po(usage);
    bool per_frame = false;
    po.Register("per-frame", &per_frame,
                "If true, write out the frame-level word alignment (else word "
                "sequence)");
    po.Read(argc, argv);
    ...
    for (; !prons_reader.Done(); prons_reader.Next()) {
      ...
      if (!per_frame) {
        ...
        for (size_t i = 0; i < word_alignment.size(); i++) {
          int32 word = word_alignment[i].first, len = word_alignment[i].second;
          for (int32 j = 0; j < len; j++) word_per_frame.push_back(word);
        }
