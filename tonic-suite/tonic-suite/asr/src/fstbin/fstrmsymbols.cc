...
/* some test examples:
 ...
 ( echo "0 0 1 1"; echo " 0 0 3 2"; echo "0 0"; ) | fstcompile | fstrmsymbols
 /tmp/in.list | fstprint
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Replaces a subset of symbols with epsilon, wherever they appear on "
        "the input side\n"
    ...
    po.Register("remove-from-output", &remove_from_output,
                "If true, remove these symbols from "
    ...
    if (!ReadIntegerVectorSimple(disambig_rxfilename, &disambig_in))
      KALDI_ERR << "fstrmsymbols: Could not read disambiguation symbols from "
                << (disambig_rxfilename == "" ? "standard input"
                                              : disambig_rxfilename);
