...
static void CheckOrthogonal(CuMatrixBase<BaseFloat> *N, bool quiet = false,
...
void OnlinePreconditioner::Init(const CuMatrixBase<BaseFloat> &R0) {
  ...
  KALDI_ASSERT(D > 1);
  if (rank_ >= D) {
    KALDI_WARN << "Rank " << rank_ << " of online preconditioner is >= dim "
               << D << ", setting it to " << (D - 1)
  ...
  if (rho_t_ < floor_val) rho_t_ = floor_val;
  d_t_.CopyFromVec(L);
...
void OnlinePreconditioner::PreconditionDirections(
    CuMatrixBase<BaseFloat> *R_t, CuVectorBase<BaseFloat> *row_prod,
    BaseFloat *scale) {
  if (row_prod == NULL) {
...
void OnlinePreconditioner::ReorthogonalizeXt1(const VectorBase<BaseFloat> &d_t1,
                                              BaseFloat rho_t1,
...
void OnlinePreconditioner::PreconditionDirectionsInternal(
    const int32 t, const BaseFloat rho_t, const Vector<BaseFloat> &d_t,
    CuMatrixBase<BaseFloat> *WJKL_t, CuMatrixBase<BaseFloat> *R_t,
    CuVectorBase<BaseFloat> *row_prod, BaseFloat *scale) {
  ...
  CuSubMatrix<BaseFloat> J_t(*WJKL_t, R, R, 0, D), L_t(*WJKL_t, 0, R, D, R),
      K_t(*WJKL_t, R, R, D, R), WJ_t(*WJKL_t, 0, 2 * R, 0, D),
  ...
  if (!locked) {
    ...
    BaseFloat gamma_t = (tr_Pt_PtT == 0.0 ? 1.0 : sqrt(tr_Rt_RtT / tr_Pt_PtT));
    *scale = gamma_t;
    return;
  }
  ...
  SubMatrix<BaseFloat> L_t_cpu(LK_cpu, 0, R, 0, R), K_t_cpu(LK_cpu, R, R, 0, R);
  if (!compute_lk_together) {
  ...
  if (nf > 0) must_reorthogonalize = true;
  if (nf > 0 && self_debug_) {
    KALDI_WARN << "Floored " << nf << " elements of C_t.";
  }
  BaseFloat tr_Rt_RtT_check;
  if (self_debug_) tr_Rt_RtT_check = TraceMatMat(*R_t, *R_t, kTrans);

  R_t->AddMatMat(-1.0, H_t, kNoTrans, W_t, kNoTrans,
                 1.0);  // P_t = R_t - H_t W_t
  ...
  for (int32 i = 0; i < R; i++) tr_Rt_RtT += L_t_cpu(i, i) * (2.0 - e_t(i));
  if (self_debug_) {
  ...
  *scale = gamma_t;
  ...
  if (must_reorthogonalize) {
    ...
    ReorthogonalizeXt1(d_t1, rho_t1, &W_t1, &J_t, &L_t);
  }
...
void OnlinePreconditioner::ComputeWt1(int32 N, const VectorBase<BaseFloat> &d_t,
                                      const VectorBase<BaseFloat> &d_t1,
                                      BaseFloat rho_t, BaseFloat rho_t1,
    ...
    for (int32 i = 0; i < R; i++) {
      for (int32 j = 0; j < R; j++) {
        ...
        if ((i == j && fabs(elem - 1.0) > 0.1) ||
          ...
          KALDI_WARN
              << "Failed to verify W_{t+1}, the following should be unit: "
              << W_t1_prod_cpu;
        }
      }
    }
...
void OnlinePreconditioner::ComputeZt(int32 N, BaseFloat rho_t,
  ...
  BaseFloat etaN = eta / N, eta1 = 1.0 - eta, etaN_sq = etaN * etaN,
            eta1_sq = eta1 * eta1, etaN_eta1 = etaN * eta1;
  int32 R = d_t.Dim();
  for (int32 i = 0; i < R; i++) {
    ...
    for (int32 j = 0; j <= i; j++) {
      ...
      (*Z_t)(i, j) =
          etaN_sq * inv_sqrt_e_t_i * K_t_i_j * inv_sqrt_e_t_j +
...
void OnlinePreconditioner::ComputeEt(
    const VectorBase<BaseFloat> &d_t, BaseFloat beta_t,
  ...
  for (int32 i = 0; i < D; i++) e[i] = 1.0 / (beta_t / d[i] + 1);
...
OnlinePreconditioner::OnlinePreconditioner(const OnlinePreconditioner &other)
    : rank_(other.rank_),
      ...
      delta_(other.delta_),
      t_(other.t_),
      num_updates_skipped_(other.num_updates_skipped_),
...
void OnlinePreconditioner::SetNumSamplesHistory(BaseFloat num_samples_history) {
  KALDI_ASSERT(num_samples_history > 0.0 && num_samples_history < 1.0e+6);
