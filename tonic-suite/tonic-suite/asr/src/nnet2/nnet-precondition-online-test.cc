...
void OnlinePreconditionerSimple::PreconditionDirections(
    CuMatrixBase<BaseFloat> *R, CuVectorBase<BaseFloat> *row_prod,
    BaseFloat *scale) {
  ...
  PreconditionDirectionsCpu(&R_cpu_dbl, &row_prod_cpu_dbl, scale);
...
void OnlinePreconditionerSimple::PreconditionDirectionsCpu(
    MatrixBase<double> *R_t, VectorBase<double> *row_prod, BaseFloat *scale) {
  if (X_t_.NumRows() == 0) Init(*R_t);
  ...
  double rho_t1 = (1.0 / (D - R)) *
                  (eta * S_t.Trace() + (1.0 - eta) * (D * rho_t_ + d_t_.Sum()) -
                   sqrt_c_t.Sum());
...
void UnitTestApproxEigsOfProduct() {
  int32 dimM = 10 + Rand() % 50, dimN = 10 + Rand() % 50;
  ...
  CuMatrix<BaseFloat> P_approx(rank, product_dim), P_exact(rank, product_dim);
  CuVector<BaseFloat> s_approx(rank), s_exact(rank);
