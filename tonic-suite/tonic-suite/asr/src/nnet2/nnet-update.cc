...
NnetUpdater::NnetUpdater(const Nnet &nnet, Nnet *nnet_to_update)
...
double NnetUpdater::ComputeObjfAndDeriv(const std::vector<NnetExample> &data,
...
double NnetUpdater::ComputeTotAccuracy(
  ...
  for (int32 i = 0; i < output.NumRows(); i++) {
    for (size_t j = 0; j < data[i].labels.size(); j++) {
      int32 ref_pdf_id = data[i].labels[j].first, hyp_pdf_id = best_pdf_cpu[i];
...
void NnetUpdater::Backprop(CuMatrix<BaseFloat> *deriv) const {
  ...
  for (int32 c = nnet_.NumComponents() - 1; c >= nnet_.LastUpdatableComponent();
       c--) {
    const Component &component = nnet_.GetComponent(c);
    Component *component_to_update =
        (nnet_to_update_ == NULL ? NULL : &(nnet_to_update_->GetComponent(c)));
    const CuMatrix<BaseFloat> &input = forward_data_[c],
...
double DoBackprop(const Nnet &nnet, const std::vector<NnetExample> &examples,
                  Nnet *nnet_to_update, double *tot_accuracy) {
...
double ComputeNnetGradient(const Nnet &nnet,
  ...
  for (int32 start_pos = 0;
    ...
    tot_objf += DoBackprop(nnet, batch, gradient);
  }
...
double ComputeNnetObjf(const Nnet &nnet,
                       const std::vector<NnetExample> &validation_set,
                       int32 batch_size, double *tot_accuracy) {
  double tot_accuracy_tmp;
  if (tot_accuracy) *tot_accuracy = 0.0;
  std::vector<NnetExample> batch;
  batch.reserve(batch_size);
  double tot_objf = 0.0;
  for (int32 start_pos = 0;
    ...
    tot_objf += ComputeNnetObjf(
        nnet, batch, tot_accuracy != NULL ? &tot_accuracy_tmp : NULL);
    if (tot_accuracy) *tot_accuracy += tot_accuracy_tmp;
  }
  return tot_objf;
}
