...
void InsertComponents(
  ...
  for (int32 c = c_to_insert; c < dest_nnet->NumComponents(); c++)
    components[c + src_nnet.NumComponents()] =
        dest_nnet->GetComponent(c).Copy();
...
void ReplaceLastComponents(const Nnet &src_nnet, int32 num_to_remove,
                           Nnet *dest_nnet) {
  KALDI_ASSERT(num_to_remove >= 0 &&
               num_to_remove <= dest_nnet->NumComponents());
