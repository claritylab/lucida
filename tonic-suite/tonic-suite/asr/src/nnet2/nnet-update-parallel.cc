...
class DoBackpropParallelClass : public MultiThreadable {
 public:
  ...
  DoBackpropParallelClass(const Nnet &nnet, ExamplesRepository *repository,
                          double *tot_weight_ptr, double *log_prob_ptr,
                          Nnet *nnet_to_update, bool store_separate_gradients)
      : nnet_(nnet),
  ...
  ~DoBackpropParallelClass() {
    if (nnet_to_update_orig_ != nnet_to_update_) {
      ...
      // template function, *and* store_separate_gradients_ has been set to
      // true.
...
#if HAVE_CUDA == 1
double DoBackpropSingleThreaded(const Nnet &nnet, int32 minibatch_size,
                                SequentialNnetExampleReader *examples_reader,
                                double *tot_weight_out, Nnet *nnet_to_update) {
...
double DoBackpropParallel(const Nnet &nnet, int32 minibatch_size,
  ...
  *tot_weight = 0.0;
  ...
  DoBackpropParallelClass c(nnet, &repository, tot_weight, &tot_log_prob,
                            nnet_to_update, store_separate_gradients);
...
double DoBackpropSingleThreaded(const Nnet &nnet, int32 minibatch_size,
  ...
  *tot_weight = TotalNnetTrainingWeight(egs);
  for (size_t i = 0; i < egs.size(); i += minibatch_size) {
    ...
    std::vector<NnetExample> this_egs(egs.begin() + i, end_iter);
...
double DoBackpropParallel(const Nnet &nnet, int32 minibatch_size,
  ...
  if (num_threads == 1)  // support GPUs: special case for 1 thread.
    return DoBackpropSingleThreaded(nnet, minibatch_size, egs, tot_weight,
                                    nnet_to_update);
  ...
  *tot_weight = 0;
...

  {
    ...
    for (int32 offset = 0; offset < num_egs; offset += minibatch_size) {
      ...
      std::vector<NnetExample> examples(
          egs.begin() + offset, egs.begin() + offset + this_minibatch_size);
  ...
  KALDI_VLOG(2) << "Did backprop on " << *tot_weight
                << " examples, average log-prob "
                << "per frame is " << (tot_log_prob / *tot_weight);
  return tot_log_prob;
}
