...
struct NnetCombineAconfig {
  ...
  BaseFloat min_learning_rate;  // 0.0001 by default; we don't allow learning
                                // rate to go below
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register(
        ...
        "we start using the \"overshoot\" mechanism to keep learning rates "
        "high.");
    po->Register(
        "overshoot", &overshoot,
        "Factor by which we overshoot the step "
