...
static void AddDirection(const Nnet &orig_nnet, const Nnet &direction,
                         const VectorBase<BaseFloat> &scales, Nnet *dest) {
  *dest = orig_nnet;
...
static BaseFloat ComputeObjfAndGradient(
    ...
    const Vector<double> &scale_params, const Nnet &orig_nnet,
    const Nnet &direction, Vector<double> *gradient) {
  ...
  BaseFloat ans = ComputeNnetGradient(nnet_combined, validation_set, batch_size,
                                      &nnet_gradient);
...
void CombineNnetsA(const NnetCombineAconfig &config,
                   const std::vector<NnetExample> &validation_set,
                   const std::vector<Nnet> &nnets, Nnet *nnet_out) {
  ...
  zero_objf = ComputeObjfAndGradient(validation_set, scale_params, nnets[0],
                                     direction, &gradient);
  KALDI_LOG << "Objective function at old parameters is " << zero_objf;

  scale_params.Set(
      1.0);  // start optimization from the average of the parameters.
  ...
  OptimizeLbfgs<double> lbfgs(scale_params, lbfgs_options);

  for (int32 i = 0; i < config.num_bfgs_iters; i++) {
    scale_params.CopyFromVec(lbfgs.GetProposedValue());
    objf = ComputeObjfAndGradient(validation_set, scale_params, nnets[0],
                                  direction, &gradient);
  ...
  KALDI_LOG
      << "Combining nnets, after BFGS, validation objf per frame changed from "
      << zero_objf << " (no change), or " << initial_objf
      << " (default change), "
  ...
  if (objf_change < config.valid_impr_thresh) {
    ...
    BaseFloat overshoot = config.overshoot,
              overshoot_max =
                  config.valid_impr_thresh / objf_change;  // >= 1.0.
    if (overshoot_max < overshoot) {
      KALDI_LOG << "Limiting overshoot from " << overshoot << " to "
                << overshoot_max << " since the objf-impr " << objf_change
                << " is close to "
                << "--valid-impr-thresh=" << config.valid_impr_thresh;
      overshoot = overshoot_max;
    }
    KALDI_ASSERT(overshoot < 2.0 &&
                 "--valid-impr-thresh must be < 2.0 or "
    ...
    KALDI_LOG << "Combining nnets, after overshooting, validation objf changed "
              << "to " << objf
              << ".  Note: (zero, start, optimized) objfs were " << zero_objf
              << ", " << initial_objf << ", " << optimized_objf;
    if (objf < zero_objf) {
      // Note: this should not happen according to a quadratic approximation,
      // and we
      // expect this branch to be taken only rarely if at all.
      KALDI_WARN << "After overshooting, objf was worse than not updating; not "
                    "doing the "
                 << "overshoot. ";
