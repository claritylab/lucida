...
/**
   ...
   a transform T, it turns out that we have to multiply the gradients by
  something
   ...
   recognition so no need to keep features in the range [0, 1].  Given a
  training
   ...
   a scalar length parameter (e.g. D = 0.1), and ||.|| is the 2-norm.  This
  means
   that we want to perturb the training example in the most damaging way
  possible,
   ...
   Anyway, we make a simplifying assumption that the loss function for a
  particular
   sample is just a linear function of the input, and when we get to the space
  of
   ...
   derivative to x we get a derivative \nabla_t of the objective function (e.g.
  a
  ...
  of
   ...
   so  \hat{\nabla}_t = T^{-T} \nabla_t.
   (this is not the formal way of getting these derivatives, it's just how I
  remember).
   Anyway, we now have
   ...
   so
     k_t = D / sqrt(\nabla_t^T T^{-1} T^{-T} \nabla_t)
         = D / sqrt(\nabla_t^T C C^T \nabla_t)
         = D / sqrt(\nabla_t^T W \nabla_t)
   Now, we actually want the update in terms of the parameter x instead of
  \hat{x},
   so multiplying the definition of \hat{x}'_t above by T^{-1} on the left, we
  have:
  ...
  FixedAffineTransform) instead.  We'll use the name x for that space, and
  forget
  ...
  D accordingly.  We'll use a simple heuristic that D should change
  proportionally
  ...
  for speech tasks with small amounts of data (say, <30 hours) and a couple
  thousand
  classes
  we typically see objective values like: training-set -0.6 and valdiation-set
  -1.1.
  These are avearage log-probabilities per frame, of the correct class.
  The two numbers are quite different because there is substantial overtraining.
  Note: for Karel's
  nnet1 setup, the difference is typically smaller, more like -0.8 vs. -1.0, as
  that setup monitors the validation-set objective and decreases the learning
  rate
  when it starts to degrade.  Now, for much larger training sets, we might
  see smaller differences in training-set versus validation-set objective
  function:
  for instance: say, -1.40 versus -1.45.  (For larger training sets the
  objectives tend
  to be more negative simply because we have more leaves).  We measure these
  values each
  iteration: see the files compute_prob_train.*.log and compute_prob_valid.*.log
  produced
  by the example scripts.   The reason why I discuss these values
  is that if the training-set and validation-set objective functions are very
  close, then
  it means that there is not much overtraining going on and we don't want to
  apply this
  method too strongly; on the other hand, if they are very different, it means
  we are
  overtraining badly and we may want to apply this method more.

  So we plan to set target-objf-change to the following value, at the script
  level:

   target-objf-change = target-multiplier * (training-objf - validation-objf))

  (e.g. target-multiplier = 1.0).
  Note that if target-objf-change is less than a specified
  min-target-objf-change
  (e.g. 0.1) then we won't apply the perturbed training at all, which will save
  time.  The method is intended to help generalization, and if we're
  generalizing
...
struct NnetPerturbedTrainerConfig {
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("minibatches-per-phase", &minibatches_per_phase,
                 ...
                 "objective.");
    po->Register("target-objf-change", &target_objf_change,
                 "Target objective "
                 "function change from feature perturbation, used to set "
                 "feature distance parameter D");
    po->Register("initial-d", &initial_d,
                 "Initial value of parameter D "
...
class NnetPerturbedTrainer {
 ...
 private:
  ...
  double logprob_this_phase_;  // Needed for accumulating train log-prob on each
                               // phase.
...
/// a class NnetPerturbedUpdater that's defined in train-nnet-perturbed.cc, but
/// we
...
/// within_class_covar (actually a symmetric matrix, but represented as
/// CuMatrix),
