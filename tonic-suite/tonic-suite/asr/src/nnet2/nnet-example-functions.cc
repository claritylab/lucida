...
bool LatticeToDiscriminativeExample(const std::vector<int32> &alignment,
                                    ...
                                    BaseFloat weight, int32 left_context,
                                    int32 right_context,
  ...
  eg->input_frames.Resize(left_context + num_frames + right_context, feat_dim);
  eg->input_frames.Range(left_context, num_frames, 0, feat_dim)
      .CopyFromMat(feats);
  ...
  for (int32 t = 0; t < right_context; t++)
    eg->input_frames.Row(left_context + num_frames + t)
        .CopyFromVec(feats.Row(num_frames - 1));
...
/**
   ...
   Time taken to process segment of a certain length: [must be sub-linear.]
      [use quadratic function that's max at specified segment length and zero at
   zero.]

   No penalty for processing frames we don't need to process (already implicit
   in
   segment-processing time above.)

   Penalty for splitting where we should not split.  [Make it propto
   log(#paths).]
...
class DiscriminativeExampleSplitter {
 ...
 private:
  ...
  int32 RightContext() {
    return eg_.input_frames.NumRows() - NumFrames() - eg_.left_context;
  }
  ...
  struct FrameInfo {
    ...
    FrameInfo()
        : den_state_count(0),
          ...
          num_den_overlap(false),
          nonzero_derivative(false),
          can_excise_frame(false),
          start_state(std::numeric_limits<int32>::max()),
          end_state(0) {}
  };
  ...
  Lattice
      lat_;  // lattice generated from eg_.den_lat, with epsilons removed etc.
  ...
  for (int32 state = 0; state < num_states; state++) {
    ...
    if (t < num_frames)
      frame_info_[t + 1].start_state =
          std::min(state, frame_info_[t + 1].start_state);
    frame_info_[t].end_state = std::max(state, frame_info_[t].end_state);
  }

  for (int32 i = 1; i <= NumFrames(); i++)
    frame_info_[i].end_state =
        std::max(frame_info_[i - 1].end_state, frame_info_[i].end_state);
  for (int32 i = NumFrames() - 1; i >= 0; i--)
    frame_info_[i].start_state =
        std::min(frame_info_[i + 1].start_state, frame_info_[i].start_state);
  ...
  for (int32 t = 0; t < num_frames; t++) {
    ...
    if (config_.criterion == "mpfe" || config_.criterion == "smbr") {
      ...
      if (config_.drop_frames) {
        ...
        frame_info.nonzero_derivative =
            frame_info.num_den_overlap && frame_info.den_state_count > 1;
      } else {
    ...
    if (config_.criterion == "mpfe") {
      frame_info.can_excise_frame =
          !frame_info.nonzero_derivative && !frame_info.multiple_transition_ids;
...
void DiscriminativeExampleSplitter::DoExcise(SplitExampleStats *stats) {
  ...
  for (int32 t = 0; t < num_frames; t++) {
    ...
    if (!can_excise[t]) need_some_frame = true;
  }
  if (!need_some_frame) {  // We don't need any frame within this file, so
                           // simply
                           // delete the segment.
  ...
  stats->longest_segment_after_excise =
      std::max(stats->longest_segment_after_excise, num_frames_kept);

  int32 num_frames_kept_plus = num_frames_kept + left_context + right_context;
  eg_out.input_frames.Resize(num_frames_kept_plus, eg_.input_frames.NumCols());
  ...
  for (int32 i = 0; i < right_context; i++) {
    ...
    SubVector<BaseFloat> src(eg_.input_frames, end_t + left_context + i);
    dst.CopyFromVec(src);
  }
  ...
  for (int32 t = start_t; t < end_t; t++) {
    if (!will_excise[t]) {
      SubVector<BaseFloat> dst(eg_out.input_frames, left_context + dst_t);
      SubVector<BaseFloat> src(eg_.input_frames, left_context + t);
      dst.CopyFromVec(src);
      dst_t++;
    }
  }
...
void DiscriminativeExampleSplitter::DoSplit(SplitExampleStats *stats) {
  ...
  {  // A "split" is a pair of successive split points.  Work out for each split
    ...
    for (size_t s = 0; s < is_kept.size(); s++) {
      ...
      for (int32 t = start; t < end; t++)
        if (frame_info_[t].nonzero_derivative) keep_this_split = true;
  ...
  for (int32 t = 0; t < num_frames; t++)
    if (frame_info_[t].nonzero_derivative) stats->num_frames_must_keep++;
...
void SplitExampleStats::Print() {
  ...
  double percent_needed = num_frames_must_keep * 100.0 / num_frames_orig,
         percent_after_split =
             num_frames_kept_after_split * 100.0 / num_frames_orig,
         percent_after_excise =
             num_frames_kept_after_excise * 100.0 / num_frames_orig;

  KALDI_LOG << "Needed to keep " << percent_needed
            << "% of frames, after split "
            << "kept " << percent_after_split
            << "%, after excising frames kept " << percent_after_excise << "%.";
...
void DiscriminativeExampleSplitter::OutputOneSplit(int32 seg_begin,
  ...
  eg_out.num_ali.insert(eg_out.num_ali.end(), eg_.num_ali.begin() + seg_begin,
                        eg_.num_ali.begin() + seg_end);

  CreateOutputLattice(seg_begin, seg_end, &(eg_out.den_lat));

  eg_out.input_frames =
      eg_.input_frames.Range(seg_begin, seg_end - seg_begin + tot_context, 0,
...
void DiscriminativeExampleSplitter::CreateOutputLattice(
    int32 seg_begin, int32 seg_end, CompactLattice *clat_out) {
  Lattice lat_out;
  ...
  for (StateId s = frame_info_[seg_begin].start_state;
    ...
    for (fst::ArcIterator<Lattice> aiter(lat_, s); !aiter.Done();
         aiter.Next()) {
      const Arc &arc = aiter.Value();
      StateId next_state =
          GetOutputStateId(arc.nextstate, &state_map, &lat_out);
      KALDI_ASSERT(arc.ilabel != 0 &&
                   arc.ilabel == arc.olabel);  // We expect no epsilons.
      lat_out.AddArc(this_state,
...
void ExciseDiscriminativeExample(
    ...
    const TransitionModel &tmodel, const DiscriminativeNnetExample &eg,
    std::vector<DiscriminativeNnetExample> *egs_out,
    SplitExampleStats *stats_out) {
  DiscriminativeExampleSplitter splitter(config, tmodel, eg, egs_out);
...
void UpdateHash(const TransitionModel &tmodel,
                const DiscriminativeNnetExample &eg, std::string criterion,
                bool drop_frames, Matrix<double> *hash, double *num_weight,
                double *den_weight, double *tot_t) {
  int32 feat_dim = eg.input_frames.NumCols(), left_context = eg.left_context,
  ...
  *tot_t += num_frames;
  ...
  ExampleToPdfPost(tmodel, silence_phones, criterion, drop_frames, eg, &post);

  Vector<BaseFloat> avg_feat(feat_dim);

  for (int32 t = 0; t < num_frames; t++) {
    SubMatrix<BaseFloat> context_window(eg.input_frames, t, context_width, 0,
                                        feat_dim);
...
void ExampleToPdfPost(const TransitionModel &tmodel,
                      ...
                      std::string criterion, bool drop_frames,
                      const DiscriminativeNnetExample &eg, Posterior *post) {
  ...
  if (criterion == "mpfe" || criterion == "smbr") {
    ...
    LatticeForwardBackwardMmi(tmodel, lat, eg.num_ali, drop_frames,
                              convert_to_pdf_ids, cancel, post);
  }
...
void AppendDiscriminativeExamples(
  ...
  int32 dim = eg0.input_frames.NumCols() + eg0.spk_info.Dim(),
        left_context = eg0.left_context, num_frames = eg0.num_ali.size(),
  ...
  output->input_frames.Range(0, eg0.input_frames.NumRows(), 0,
                             eg0.input_frames.NumCols())
      .CopyFromMat(eg0.input_frames);
  ...
  for (size_t i = 1; i < input.size(); i++) {
    ...
    output->input_frames.Range(feat_offset, eg_i.input_frames.NumRows(), 0,
                               eg_i.input_frames.NumCols())
        .CopyFromMat(eg_i.input_frames);
    if (eg_i.spk_info.Dim() != 0) {
      output->input_frames.Range(feat_offset, eg_i.input_frames.NumRows(),
                                 eg_i.input_frames.NumCols(),
                                 eg_i.spk_info.Dim())
          .CopyRowsFromVec(eg_i.spk_info);
      KALDI_ASSERT(eg_i.input_frames.NumCols() + eg_i.spk_info.Dim() == dim);
    }

    output->num_ali.insert(output->num_ali.end(), inter_segment_ali.begin(),
                           inter_segment_ali.end());
    output->num_ali.insert(output->num_ali.end(), eg_i.num_ali.begin(),
                           eg_i.num_ali.end());
...
void CombineDiscriminativeExamples(
    int32 max_length, const std::vector<DiscriminativeNnetExample> &input,
  ...
  std::vector<std::vector<size_t> > groups;
  SolvePackingProblem(max_length, costs, &groups);
  output->clear();
