...
class NnetDiscriminativeUpdater {
 public:
  ...
  void Update() {
    Propagate();
    LatticeComputations();
    if (nnet_to_update_ != NULL) Backprop();
  }
...
NnetDiscriminativeUpdater::NnetDiscriminativeUpdater(
    ...
    const NnetDiscriminativeUpdateOptions &opts,
    const DiscriminativeNnetExample &eg, Nnet *nnet_to_update,
    NnetDiscriminativeStats *stats)
    : am_nnet_(am_nnet),
      tmodel_(tmodel),
      opts_(opts),
...
SubMatrix<BaseFloat> NnetDiscriminativeUpdater::GetInputFeatures() const {
  ...
  int32 eg_left_context = eg_.left_context,
        eg_right_context =
            eg_.input_frames.NumRows() - num_frames_output - eg_left_context;
  ...
  SubMatrix<BaseFloat> ans(eg_.input_frames, offset, num_output_frames, 0,
...
void NnetDiscriminativeUpdater::Propagate() {
  ...
  if (spk_dim == 0) {
    ...
    forward_data_[0]
        .Range(0, input_feats.NumRows(), 0, input_feats.NumCols())
        .CopyFromMat(input_feats);
    forward_data_[0]
        .Range(0, input_feats.NumRows(), input_feats.NumCols(), spk_dim)
        .CopyRowsFromVec(eg_.spk_info);
  }
  ...
  for (int32 c = 0; c < nnet.NumComponents(); c++) {
    ...
    const Component *prev_component =
        (c == 0 ? NULL : &(nnet.GetComponent(c - 1)));
...
void NnetDiscriminativeUpdater::LatticeComputations() {
  ...
  TopSort(
      &lat_);  // Topologically sort (required by forward-backward algorithms)

  if (opts_.criterion == "mmi" && opts_.boost != 0.0) {
    BaseFloat max_silence_error = 0.0;
    LatticeBoost(tmodel_, eg_.num_ali, silence_phones_, opts_.boost,
                 max_silence_error, &lat_);
  }
  ...
  for (StateId s = 0; s < num_states; s++) {
    ...
    for (fst::ArcIterator<Lattice> aiter(lat_, s); !aiter.Done();
      ...
      if (arc.ilabel !=
          0) {  // input-side has transition-ids, output-side empty
  ...
  for (index = 0; index < answers.size(); index++) {
    ...
    BaseFloat pseudo_loglike =
        log(post / priors(pdf_id)) * opts_.acoustic_scale;
...
double NnetDiscriminativeUpdater::GetDiscriminativePosteriors(Posterior *post) {
  if (opts_.criterion == "mpfe" || opts_.criterion == "smbr") {
    ...
    return LatticeForwardBackwardMmi(tmodel_, lat_, eg_.num_ali,
                                     opts_.drop_frames, convert_to_pdfs, cancel,
                                     post);
  }
}
...
void NnetDiscriminativeUpdate(const AmNnet &am_nnet,
                              ...
                              Nnet *nnet_to_update,
                              NnetDiscriminativeStats *stats) {
  NnetDiscriminativeUpdater updater(am_nnet, tmodel, opts, eg, nnet_to_update,
                                    stats);
  updater.Update();
}
...
void NnetDiscriminativeStats::Print(std::string criterion) {
  ...
  if (criterion == "mmi") {
    double num_objf = tot_num_objf / tot_t_weighted,
           den_objf = tot_den_objf / tot_t_weighted, objf = num_objf - den_objf;
    KALDI_LOG << "MMI objective function is " << num_objf << " - " << den_objf
              << " = " << objf << " per frame, over " << tot_t_weighted
              << " frames.";
  } else if (criterion == "mpfe") {
    double objf = tot_den_objf / tot_t_weighted;  // this contains the actual
                                                  // summed objf
    KALDI_LOG << "MPFE objective function is " << objf << " per frame, over "
              << tot_t_weighted << " frames.";
  } else {
    double objf = tot_den_objf / tot_t_weighted;  // this contains the actual
                                                  // summed objf
    KALDI_LOG << "SMBR objective function is " << objf << " per frame, over "
              << tot_t_weighted << " frames.";
  }
}
