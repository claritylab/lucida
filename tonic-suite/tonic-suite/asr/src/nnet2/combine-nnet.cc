...
static int32 GetInitialModel(const std::vector<NnetExample> &validation_set,
  ...
  for (int32 n = 0; n < num_nnets; n++) {
    BaseFloat objf =
        ComputeNnetObjf(nnets[n], validation_set, minibatch_size) / tot_frames;
  ...
  {  // Now try a version where all the neural nets have the same weight.
    ...
    BaseFloat objf =
        ComputeNnetObjf(average_nnet, validation_set, minibatch_size) /
        tot_frames;
...
static void GetInitialScaleParams(
    ...
    const std::vector<Nnet> &nnets, Vector<double> *scale_params) {
...
static double ComputeObjfAndGradient(
    const std::vector<NnetExample> &validation_set,
    const Vector<double> &scale_params, const std::vector<Nnet> &nnets,
    bool debug, Vector<double> *gradient) {
  ...
  double ans = ComputeNnetGradient(nnet_combined, validation_set, batch_size,
                                   &nnet_gradient);
  ...
  if (debug) {
    ...
    for (int32 i = 0; i < scale_params.Dim(); i++) {
      ...
      if (fg * delta < 1.0e-05) delta = 1.0e-05 / fg;

      Vector<double> scale_params_temp(scale_params);
      scale_params_temp(i) += delta;
      double new_ans = ComputeObjfAndGradient(validation_set, scale_params_temp,
                                              nnets, false, NULL);
...
void CombineNnets(const NnetCombineConfig &combine_config,
  ...
  GetInitialScaleParams(combine_config, validation_set, nnets, &scale_params);
  ...
  for (int32 i = 0; i < combine_config.num_bfgs_iters; i++) {
    ...
    objf = ComputeObjfAndGradient(validation_set, scale_params, nnets,
                                  combine_config.test_gradient, &gradient);
