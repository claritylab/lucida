...
struct NnetEnsembleTrainerConfig {
  ...
  NnetEnsembleTrainerConfig()
      : minibatch_size(500), minibatches_per_phase(50), beta(0.5) {}
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register(
        ...
        "between the output posteriors and the averaged posteriors from other "
        "nets.");
  }
};
...
class NnetEnsembleTrainer {
 ...
 private:
  ...
  // ratio of the supervision, when interpolating the supervision with the
  // averaged posteriors.
  double beta_;
  double avg_logprob_this_phase_;  // Needed for accumulating train log-prob on
                                   // each phase.
