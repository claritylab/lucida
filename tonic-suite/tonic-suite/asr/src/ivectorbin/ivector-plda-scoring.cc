...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "(if either key could not be found, the dot-product field in the "
        "output\n"
        "will be absent, and this program will print a warning)\n"
        "For training examples, the input is the iVectors averaged over "
        "speakers;\n"
        "a separate archive containing the number of utterances per speaker "
        "may be\n"
        "optionally supplied using the --num-utts option; this affects the "
        "PLDA\n"
        "scoring (if not supplied, it defaults to 1 per speaker).\n"
        "\n"
        "Usage: ivector-plda-scoring <plda> <train-ivector-rspecifier> "
        "<test-ivector-rspecifier>\n"
        " <trials-rxfilename> <scores-wxfilename>\n"
        "\n"
        "e.g.: ivector-plda-scoring --num-utts=ark:exp/train/num_utts.ark plda "
        "ark:exp/train/spk_ivectors.ark ark:exp/test/ivectors.ark trials "
        "scores\n"
    ...
    po.Register("num-utts", &num_utts_rspecifier,
                "Table to read the number of "
    ...
    SequentialBaseFloatVectorReader train_ivector_reader(
        train_ivector_rspecifier);
    SequentialBaseFloatVectorReader test_ivector_reader(
        test_ivector_rspecifier);
    ...
    for (; !train_ivector_reader.Done(); train_ivector_reader.Next()) {
      ...
      tot_train_renorm_scale += plda.TransformIvector(
          plda_config, ivector, num_examples, transformed_ivector);
      train_ivectors[spk] = transformed_ivector;
      num_train_ivectors++;
    }
    KALDI_LOG << "Read " << num_train_ivectors << " training iVectors, "
              << "errors on " << num_train_errs;
    if (num_train_ivectors == 0) KALDI_ERR << "No training iVectors present.";
    ...
    for (; !test_ivector_reader.Done(); test_ivector_reader.Next()) {
      ...
      int32 num_examples =
          1;  // this value is always used for test (affects the
              // length normalization in the TransformIvector
              // function).
      Vector<BaseFloat> *transformed_ivector = new Vector<BaseFloat>(dim);

      tot_test_renorm_scale += plda.TransformIvector(
          plda_config, ivector, num_examples, transformed_ivector);
      test_ivectors[utt] = transformed_ivector;
      num_test_ivectors++;
    }
    KALDI_LOG << "Read " << num_test_ivectors << " test iVectors.";
    if (num_test_ivectors == 0) KALDI_ERR << "No test iVectors present.";
    ...
    while (std::getline(ki.Stream(), line)) {
      ...
      BaseFloat score = plda.LogLikelihoodRatio(
          train_ivector_dbl, num_train_examples, test_ivector_dbl);
      sum += score;
      sumsq += score * score;
      num_trials_done++;
    ...
    KALDI_LOG << "Processed " << num_trials_done << " trials, "
              << num_trials_err << " had errors.";
