...
struct LatticeTrackingDecoderConfig {
  ...
  BaseFloat
      max_beam;  // maximum beam (in case tracked tokens go too far from beam)
  fst::DeterminizeLatticePhonePrunedOptions det_opts;

  LatticeTrackingDecoderConfig()
      : beam(16.0),
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("prune-interval", &prune_interval,
...
/**
   ...
   (for complementarity with the forward-in-time one), and for the
   forward-in-time
   HCLG graph, we work out which arcs in the graph each lattice arc corresponds
   to...
   this is done in a shortest-path sense, in that we work out for each path
   through
   ...
   The lattice that we give to the decoder to help the search, has a format
   where
   the ilabels contain the state in HCLG (corresponding to the preceding-state
   of
   the arc in the lattice), and the olabels contain the arc index corresponding
   to
   that transition in the lattice, i.e. the offset into the list of arcs
   transitioning
   from that state in HCLG.

   Since this decocer is somewhat special-purpose, and is not widely used in the
   recipes, we are not updating it to the "new interface" with
   AdvanceDecoding().
 */
class LatticeTrackingDecoder {
 public:
  ...
  LatticeTrackingDecoderConfig GetOptions() { return config_; }
  ...
  /// a normal lattice (typically from decoding in an opposite direction in
  /// time),
 ...
 private:
  ...
  struct Token {
    ...
    StateId lat_state;  // current state in graph arc lattice from first pass
                        // decoding
