...
struct BiglmFasterDecoderOptions : public FasterDecoderOptions {
  BiglmFasterDecoderOptions() { min_active = 200; }
};
...
class BiglmFasterDecoder {
 public:
  ...
  // as the decoding graph.  Instead of having everything indexed by the state
  // in
  ...
  ~BiglmFasterDecoder() { ClearToks(toks_.Clear()); }

  void Decode(DecodableInterface *decodable) {
    // clean up from last time:
    ClearToks(toks_.Clear());
    PairId start_pair = ConstructPair(fst_.Start(), lm_diff_fst_->Start());
    Arc dummy_arc(0, 0, Weight::One(),
                  fst_.Start());  // actually, the last element of
  ...
  bool ReachedFinal() {
    for (const Elem *e = toks_.GetList(); e != NULL; e = e->tail) {
      ...
      if (this_weight != Weight::Zero()) return true;
    }
    return false;
  }
  ...
  bool GetBestPath(fst::MutableFst<LatticeArc> *fst_out,
    ...
    Token *best_tok = NULL;
    Weight best_final;  // only set if is_final == true.  The final-prob
                        // corresponding
    // to the best final token (i.e. the one with best weight best_weight,
    // below).
    bool is_final = ReachedFinal();
    if (!is_final) {
      for (const Elem *e = toks_.GetList(); e != NULL; e = e->tail)
        if (best_tok == NULL || *best_tok < *(e->val)) best_tok = e->val;
    } else {
 ...
 private:
  ...
  BaseFloat GetCutoff(Elem *list_head, size_t *tok_count,
    ...
    if (opts_.max_active == std::numeric_limits<int32>::max() &&
      ...
      if (tmp_array_.size() > static_cast<size_t>(opts_.min_active)) {
        if (opts_.min_active == 0)
          min_active_cutoff = best_weight;
        else {
          std::nth_element(
              tmp_array_.begin(), tmp_array_.begin() + opts_.min_active,
      ...
      if (max_active_cutoff <
          beam_cutoff) {  // max_active is tighter than beam.
        if (adaptive_beam)
  ...
  BaseFloat ProcessEmitting(DecodableInterface *decodable, int frame) {
      ...
      for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
        ...
        if (arc.ilabel != 0) {          // we'd propagate..
          ...
          BaseFloat ac_cost = -decodable->LogLikelihood(frame, arc.ilabel),
                    new_weight =
                        arc.weight.Value() + tok->weight_.Value() + ac_cost;
    ...
    // 'owned' is a complex thing here; the point is we need to call
    // toks_.Delete(e)
    // on each elem 'e' to let toks_ know we're done with them.
    for (Elem *e = last_toks, *e_tail; e != NULL;
        ...
        for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
          ...
          if (arc.ilabel != 0) {  // propagate.
            ...
            BaseFloat new_weight =
                arc.weight.Value() + tok->weight_.Value() + ac_weight.Value();
  ...
  void ProcessNonemitting(BaseFloat cutoff) {
    ...
    while (!queue_.empty()) {
      ...
      if (tok->weight_.Value() >
          cutoff) {  // Don't bother processing successors.
        continue;
      }
