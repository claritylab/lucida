...
LatticeTrackingDecoder::LatticeTrackingDecoder(
  ...
  for (int32 frame = 1; !decodable->IsLastFrame(frame - 2); frame++) {
    ...
    if (decodable->IsLastFrame(frame - 1))
      ...
      PruneActiveTokens(frame,
                        config_.lattice_beam * 0.1);  // use larger delta.
  }
...
bool LatticeTrackingDecoder::GetRawLattice(fst::MutableFst<LatticeArc> *ofst,
  ...
  for (int32 f = 0; f <= num_frames; f++) {
    ...
    if (f == 0 && ofst->NumStates() > 0) ofst->SetStart(ofst->NumStates() - 1);
  }
  KALDI_VLOG(3) << "init:" << num_toks_ / 2 + 3
  ...
  for (int32 f = 0; f <= num_frames; f++) {
    for (Token *tok = active_toks_[f].toks; tok != NULL;
...
bool LatticeTrackingDecoder::GetLattice(
    fst::MutableFst<CompactLatticeArc> *ofst, bool use_final_probs) const {
  Lattice raw_fst;
  GetRawLattice(&raw_fst, use_final_probs);
  if (raw_fst.NumStates() == 0) return false;
  Invert(&raw_fst);        // make it so word labels are on the input.
  if (!TopSort(&raw_fst))  // topological sort makes lattice-determinization
                           // more efficient
...
void LatticeTrackingDecoder::PruneForwardLinks(int32 frame,
...
void LatticeTrackingDecoder::PruneForwardLinksFinal(int32 frame) {
  ...
  BaseFloat best_cost_final = infinity, best_cost_nofinal = infinity;
  ...
  for (Elem *e = cur_toks; e != NULL; e = e->tail) {
    ...
    if ((tok->lat_state != fst::kNoStateId) && (final_cost != infinity)) {
      ...
      // best_tracked_final = std::min(best_tracked_final, tok->tot_cost +
      // final_cost);
      // worst_tracked_final = std::max(worst_tracked_final, tok->tot_cost +
      // final_cost);
    }
    best_cost_final = std::min(best_cost_final, tok->tot_cost + final_cost);
    // worst_cost_final = std::max(worst_cost_final, tok->tot_cost +
    // final_cost);
  ...
  while (changed) {
    ...
    for (Token *tok = active_toks_[frame].toks; tok != NULL; tok = tok->next) {
      ...
      if (tok_extra_cost > config_.lattice_beam) tok_extra_cost = infinity;
  ...
  for (Token *tok = active_toks_[frame].toks; tok != NULL; tok = tok->next) {
    if (tok->extra_cost != infinity) {
      ...
      if (final_active_) {
        BaseFloat final_cost = tok_to_final_cost[tok];
        if (final_cost != infinity) final_costs_[tok] = final_cost;
      } else {
        final_costs_[tok] = 0;
      }
    }
...
// know that we can safely ignore them because the frame after them was
// unchanged.
// delta controls when it considers a cost to have changed enough to continue
// going backward and propagating the change.
// for a larger delta, we will recurse less far back
void LatticeTrackingDecoder::PruneActiveTokens(int32 cur_frame,
                                               BaseFloat delta) {
...
void LatticeTrackingDecoder::PruneActiveTokensFinal(int32 cur_frame) {
  ...
  KALDI_VLOG(3) << "PruneActiveTokensFinal: pruned tokens from "
  ...
  if (config_.max_active == std::numeric_limits<int32>::max()) {
    ...
    for (Elem *e = list_head; e != NULL; e = e->tail, count++) {
      ...
      if (e->val->lat_state != fst::kNoStateId) {  // tracked token
        worst_tracked =
            std::max(worst_tracked, static_cast<BaseFloat>(e->val->tot_cost));
      }
    }
...
void LatticeTrackingDecoder::ProcessEmitting(DecodableInterface *decodable,
                                             int32 frame) {
  ...
  for (Elem *e = last_toks, *e_tail; e != NULL; e = e_tail) {
    ...
    if ((tok->tot_cost <= cur_cutoff) || (tok->lat_state != fst::kNoStateId)) {
      ...
      std::vector<std::pair<Label, StateId> >
          lat_arcs;  // arc number and next state
      if (tok->lat_state != fst::kNoStateId) {
        ...
        // do final states correspond? (lat_state and HCLG state should be
        // final)
        ...
        for (; !lat_iter.Done(); lat_iter.Next()) {
          ...
          // assume arc_graph arcs are sorted by arc_num, each arc_num occurs
          // once
...
void LatticeTrackingDecoder::ProcessNonemitting(int32 frame) {
  ...
  // if (frame > 0) KALDI_ASSERT(worst_tracked > 0.0); // track at least one
  // token
  ...
  while (!queue_.empty()) {
    ...
    tok->links = NULL;
    // GetArcStateMap(&arc_map, state, tok->lat_state); // in case of tracked
    // tokens, contains nextstates
    std::vector<std::pair<Label, StateId> >
        lat_arcs;  // arc number and next state
    if (tok->lat_state != fst::kNoStateId) {
      // in case of tracked tokens, iterate all outgoing arcs from arc_graph_
    ...
    for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
      ...
      if (arc.ilabel == 0) {  // propagate nonemitting only...
        ...
        if ((tot_cost < cutoff) || (lat_nextstate != fst::kNoStateId)) {
          ...
          Token *new_tok = FindOrAddToken(arc.nextstate, lat_nextstate, frame,
                                          tot_cost, &changed);
...
void LatticeTrackingDecoder::ClearActiveTokens() {  // a cleanup routine, at utt
                                                    // end/begin
...
bool DecodeUtteranceLatticeTracking(
    ...
    const fst::StdVectorFst &
        arc_graph,  // contains graph arcs from forward pass lattice
