...
bool LatticeSimpleDecoder::GetLattice(CompactLattice *ofst,
                                      bool use_final_probs) const {
  Lattice raw_fst;
...
inline LatticeSimpleDecoder::Token *LatticeSimpleDecoder::FindOrAddToken(
    StateId state, int32 frame, BaseFloat tot_cost, bool emitting,
    bool *changed) {
...
void LatticeSimpleDecoder::PruneForwardLinks(int32 frame,
  ...
  *links_pruned = false;
  ...
  while (changed) {
    ...
    for (Token *tok = active_toks_[frame].toks; tok != NULL; tok = tok->next) {
      ...
      if (fabs(tok_extra_cost - tok->extra_cost) > delta) changed = true;
      tok->extra_cost =
...
void LatticeSimpleDecoder::FinalizeDecoding() {
  ...
  KALDI_VLOG(3) << "pruned tokens from " << num_toks_begin << " to "
                << num_toks_;
}
  ...
  while (!queue.empty()) {
    ...
    for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
      ...
      if (arc.ilabel == 0) {  // propagate nonemitting only...
        BaseFloat graph_cost = arc.weight.Value(), cur_cost = tok->tot_cost,
        ...
        if (tot_cost < cutoff) {
          ...
          if (changed) queue.push_back(arc.nextstate);
        }
      }
    }
  }
...
void LatticeSimpleDecoder::PruneCurrentTokens(
  ...
  for (unordered_map<StateId, Token *>::iterator iter = toks->begin();
       iter != toks->end(); ++iter) {
    best_cost =
        std::min(best_cost, static_cast<BaseFloat>(iter->second->tot_cost));
  }
  std::vector<StateId> retained;
  BaseFloat cutoff = best_cost + beam;
  for (unordered_map<StateId, Token *>::iterator iter = toks->begin();
       iter != toks->end(); ++iter) {
    if (iter->second->tot_cost < cutoff) retained.push_back(iter->first);
  }
