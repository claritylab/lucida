...
class LatticeBiglmFasterDecoder {
 public:
  ...
  // A PairId will be constructed as: (StateId in fst) + (StateId in
  // lm_diff_fst) << 32;
  ...
  bool GetBestPath(fst::MutableFst<LatticeArc> *ofst,
    ...
    // fst::FstPrinter<LatticeArc> fstprinter(fst, NULL, NULL, NULL, false,
    // true);
 ...
 private:
  inline PairId ConstructPair(StateId fst_state, StateId lm_state) {
    return static_cast<PairId>(fst_state) +
  ...
  inline Token *FindOrAddToken(PairId state_pair, int32 frame,
                               BaseFloat tot_cost, bool emitting,
                               bool *changed) {
  ...
  void PruneForwardLinks(int32 frame, bool *extra_costs_changed,
    ...
    // extra_costs_changed is set to true if extra_cost was changed for any
    // token
  ...
  void PruneForwardLinksFinal(int32 frame) {
    ...
    for (Elem *e = cur_toks, *e_tail; e != NULL; e = e_tail) {
      ...
      BaseFloat final_cost =
          fst_.Final(state).Value() + lm_diff_fst_->Final(lm_state).Value();
  ...
  // where hash toks_ are (so we do not want to mess with it because these
  // tokens
  ...
  void PruneActiveTokens(int32 cur_frame, BaseFloat delta) {
    ...
    for (int32 frame = cur_frame - 1; frame >= 0; frame--) {
      ...
      if (active_toks_[frame].must_prune_forward_links) {
        ...
        if (extra_costs_changed &&
            frame > 0)  // any token has changed extra_cost
  ...
  BaseFloat GetCutoff(Elem *list_head, size_t *tok_count,
    ...
    if (config_.max_active == std::numeric_limits<int32>::max()) {
      ...
      if (tmp_array_.size() <= static_cast<size_t>(config_.max_active)) {
        ...
        if (adaptive_beam)
          *adaptive_beam =
              std::min(config_.beam, ans - best_weight + config_.beam_delta);
        return ans;
      }
    }
  }
      ...
      if (tok->tot_cost <= cur_cutoff) {
        for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
          ...
          if (arc_ref.ilabel != 0) {  // propagate..
            ...
            Token *next_tok =
                FindOrAddToken(next_pair, frame, tot_cost, true, NULL);
  ...
  void ProcessNonemitting(int32 frame) {
    ...
    while (!queue_.empty()) {
      ...
      Token *tok =
          toks_.Find(state_pair)->val;  // would segfault if state not in
      ...
      for (fst::ArcIterator<fst::Fst<Arc> > aiter(fst_, state); !aiter.Done();
        ...
        if (arc_ref.ilabel == 0) {  // propagate nonemitting only...
          ...
          if (tot_cost < cutoff) {
            bool changed;
            PairId next_pair = ConstructPair(arc.nextstate, next_lm_state);
            Token *new_tok = FindOrAddToken(next_pair, frame, tot_cost, false,
                                            &changed);  // false: non-emit
