...
template <typename Real>
void SpMatrix<Real>::SymPosSemiDefEig(VectorBase<Real> *s, MatrixBase<Real> *P,
...
void SpMatrix<Real>::CopyFromMat(const MatrixBase<Real> &M,
  ...
  switch (copy_type) {
    case kTakeMeanAndCheck: {
      ...
      for (MatrixIndexT i = 0; i < D; i++) {
        for (MatrixIndexT j = 0; j < i; j++) {
          Real a = M(i, j), b = M(j, i), avg = 0.5 * (a + b),
               diff = 0.5 * (a - b);
          (*this)(i, j) = avg;
    ...
    case kTakeLower: {  // making this one a bit more efficient.
      const Real *src = M.Data();
      Real *dest = this->data_;
      MatrixIndexT stride = M.Stride();
      for (MatrixIndexT i = 0; i < D; i++) {
        for (MatrixIndexT j = 0; j <= i; j++) dest[j] = src[j];
        dest += i + 1;
        src += stride;
      }
    } break;
    case kTakeUpper:
      for (MatrixIndexT i = 0; i < D; i++)
        for (MatrixIndexT j = 0; j <= i; j++) (*this)(i, j) = M(j, i);
      break;
    default:
...
Real SpMatrix<Real>::Trace() const {
  ...
  Real ans = 0.0;
  for (int32 i = 1; i <= num_rows; i++, data += i) ans += *data;
  return ans;
}
...
void SpMatrix<Real>::AddDiagVec(const Real alpha,
  ...
  Real *dst = this->data_;
  if (alpha == 1.0)
    for (int32 i = 1; i <= num_rows; i++, src++, dst += i) *dst += *src;
  else
    for (int32 i = 1; i <= num_rows; i++, src++, dst += i) *dst += alpha **src;
}

// instantiate the template above.
template void SpMatrix<float>::AddDiagVec(const float alpha,
                                          const VectorBase<double> &v);

template void SpMatrix<double>::AddDiagVec(const double alpha,
    ...
    if (logdet != NULL || det_sign != NULL) {
      ...
      for (int i = 0; i < (int)this->num_rows_; i++) {
        if (p_ipiv[i] > 0) {  // not a 2x2 block...
          ...
          // The following line is more complex than it looks: there are 2
          // offsets of
          // 1 that cancel.
          prod *= thisdet;
        }
        if (i == (int)(this->num_rows_ - 1) || fabs(prod) < 1.0e-10 ||
            fabs(prod) > 1.0e+10) {
          if (prod < 0) {
            prod = -prod;
            sign *= -1;
...
#else
// in the ATLAS case, these are not implemented using a library and we back off
// to something else.
template <typename Real>
void SpMatrix<Real>::Invert(Real *logdet, Real *det_sign, bool need_inverse) {
  ...
  if (need_inverse)
    for (MatrixIndexT i = 0; i < this->NumRows(); i++)
      for (MatrixIndexT j = 0; j <= i; j++) (*this)(i, j) = M(i, j);
}
#endif
...
void SpMatrix<Real>::InvertDouble(Real *logdet, Real *det_sign,
  ...
  dmat.Invert(logdet ? &logdet_tmp : NULL, det_sign ? &det_sign_tmp : NULL,
              inverse_needed);
...
Real TraceSpSp(const SpMatrix<Real> &A, const SpMatrix<OtherReal> &B) {
  ...
  for (row = 0; row < R; row++) {
    for (col = 0; col < row; col++) ans += 2.0 * *(Aptr++) * *(Bptr++);
    ans += *(Aptr++) * *(Bptr++);  // Diagonal.
  }
  return ans;
}

template float TraceSpSp<float, double>(const SpMatrix<float> &A,
                                        const SpMatrix<double> &B);

template double TraceSpSp<double, float>(const SpMatrix<double> &A,
...
template double TraceSpMat(const SpMatrix<double> &A,
...
template float TraceMatSpMat(const MatrixBase<float> &A,
                             MatrixTransposeType transA,
                             const SpMatrix<float> &B,
                             const MatrixBase<float> &C,
                             MatrixTransposeType transC);
template double TraceMatSpMat(const MatrixBase<double> &A,
                              MatrixTransposeType transA,
                              const SpMatrix<double> &B,
...
Real TraceMatSpMatSp(const MatrixBase<Real> &A, MatrixTransposeType transA,
  ...
  KALDI_ASSERT(
      (transA == kTrans ? A.NumCols() : A.NumRows() == D.NumCols()) &&
      (transA == kTrans ? A.NumRows() : A.NumCols() == B.NumRows()) &&
...
template float TraceMatSpMatSp(const MatrixBase<float> &A,
                               MatrixTransposeType transA,
                               const SpMatrix<float> &B,
                               const MatrixBase<float> &C,
                               MatrixTransposeType transC,
                               const SpMatrix<float> &D);
template double TraceMatSpMatSp(const MatrixBase<double> &A,
                                MatrixTransposeType transA,
                                const SpMatrix<double> &B,
                                const MatrixBase<double> &C,
                                MatrixTransposeType transC,
...
bool SpMatrix<Real>::IsUnit(Real cutoff) const {
  ...
  for (MatrixIndexT i = 0; i < R; i++)
    for (MatrixIndexT j = 0; j <= i; j++)
      max = std::max(max, static_cast<Real>(
                              std::abs((*this)(i, j) - (i == j ? 1.0 : 0.0))));
  return (max <= cutoff);
}
...
bool SpMatrix<Real>::IsTridiagonal(Real cutoff) const {
  ...
  for (MatrixIndexT i = 0; i < R; i++)
    for (MatrixIndexT j = 0; j <= i; j++) {
      if (j + 1 < i)
        max_abs_offdiag = std::max(max_abs_offdiag, std::abs((*this)(i, j)));
      else
        max_abs_2diag = std::max(max_abs_2diag, std::abs((*this)(i, j)));
    }
...
bool SpMatrix<Real>::ApproxEqual(const SpMatrix<Real> &other, float tol) const {
  if (this->NumRows() != other.NumRows())
    KALDI_ERR << "SpMatrix::AproxEqual, size mismatch, " << this->NumRows()
              << " vs. " << other.NumRows();
  SpMatrix<Real> tmp(*this);
  tmp.AddSp(-1.0, other);
  return (tmp.FrobeniusNorm() <=
          tol * std::max(this->FrobeniusNorm(), other.FrobeniusNorm()));
}
...
template <typename Real>
MatrixIndexT SpMatrix<Real>::LimitCond(
    Real maxCond, bool invert) {  // e.g. maxCond = 1.0e+05.
  MatrixIndexT Dim = this->NumRows();
  Vector<Real> s(Dim);
  Matrix<Real> P(Dim, Dim);
  ...
  P.MulColsVec(s);
  (*this).AddMat2(1.0, P, kNoTrans, 0.0);  // (*this) = P*P^T.  ... (*this) = P
                                           // * floor(s) * P^T  ... if P was
                                           // original P.
  return nfloored;
}

void SolverOptions::Check() const { KALDI_ASSERT(K > 10 && eps < 1.0e-10); }
...
double SolveQuadraticProblem(const SpMatrix<double> &H,
  ...
  Vector<double> l(dim);
  H.SymPosSemiDefEig(&l, &U);  // does svd H = U L V^T and checks that H == U L
                               // U^T to within a tolerance.
  // floor l.
  ...
  if (nfloored != 0 && opts.print_debug_output) {
    KALDI_LOG << "Solving quadratic problem for " << opts.name << ": floored "
              << nfloored << " eigenvalues. ";
  }
  Vector<double> tmp(dim);
  tmp.AddMatVec(1.0, U, kTrans, gbar, 0.0);  // tmp = U^T \bar{g}
  tmp.DivElements(
      l);  // divide each element of tmp by l: tmp = \tilde{L}^{-1} U^T \bar{g}
  Vector<double> delta(dim);
  delta.AddMatVec(1.0, U, kNoTrans, tmp,
                  0.0);  // delta = U tmp = U \tilde{L}^{-1} U^T \bar{g}
  ...
  if (auxf_after < auxf_before) {  // Reject change.
    if (auxf_after < auxf_before - 1.0e-10 && opts.print_debug_output)
      KALDI_WARN << "Optimizing vector auxiliary function for " << opts.name
                 << ": auxf decreased " << auxf_before << " to " << auxf_after
                 << ", change is " << (auxf_after - auxf_before);
    return 0.0;
  } else {
    x->CopyFromVec(xhat);
...
// Maximizes the auxiliary function   Q(x) = tr(M^T SigmaInv Y) - 0.5
// tr(SigmaInv M Q M^T).
...
Real SolveQuadraticMatrixProblem(const SpMatrix<Real> &Q,
  ...
  if (opts.diagonal_precondition) {
    ...
    SolverOptions new_opts(opts);
    new_opts.diagonal_precondition = false;
    ans = SolveQuadraticMatrixProblem(Q_scaled, Y_scaled, SigmaInv, new_opts,
                                      &M_scaled);
  ...
  Vector<Real> l(cols);
  Q.SymPosSemiDefEig(&l, &U);  // does svd Q = U L V^T and checks that Q == U L
                               // U^T to within a tolerance.
  // floor l.
  ...
  if (nfloored != 0 && opts.print_debug_output)
    KALDI_LOG << "Solving matrix problem for " << opts.name << ": floored "
              << nfloored << " eigenvalues. ";
  Matrix<Real> tmpDelta(rows, cols);
  tmpDelta.AddMatMat(1.0, Ybar, kNoTrans, U, kNoTrans,
                     0.0);  // tmpDelta = Ybar * U.
  l.InvertElements();
  KALDI_ASSERT(1.0 / l.Max() !=
               0);         // check not infinite.  eps should take care of this.
  tmpDelta.MulColsVec(l);  // tmpDelta = Ybar * U * \tilde{L}^{-1}

  Matrix<Real> Delta(rows, cols);
  Delta.AddMatMat(1.0, tmpDelta, kNoTrans, U, kTrans,
                  0.0);  // Delta = Ybar * U * \tilde{L}^{-1} * U^T

  Real auxf_before, auxf_after;
  SpMatrix<Real> MQM(rows);
  Matrix<Real> &SigmaInvY(tmpDelta);
  {
    Matrix<Real> SigmaInvFull(SigmaInv);
    SigmaInvY.AddMatMat(1.0, SigmaInvFull, kNoTrans, Y, kNoTrans, 0.0);
  }
  {  // get auxf_before.      Q(x) = tr(M^T SigmaInv Y) - 0.5 tr(SigmaInv M Q
     // M^T).
    MQM.AddMat2Sp(1.0, *M, kNoTrans, Q, 0.0);
    auxf_before = TraceMatMat(*M, SigmaInvY, kaldi::kTrans) -
                  0.5 * TraceSpSp(SigmaInv, MQM);
  }
  ...
  {  // get auxf_after.
    ...
    auxf_after = TraceMatMat(Mhat, SigmaInvY, kaldi::kTrans) -
                 0.5 * TraceSpSp(SigmaInv, MQM);
  }

  if (auxf_after < auxf_before) {
    if (auxf_after < auxf_before - 1.0e-10)
      KALDI_WARN << "Optimizing matrix auxiliary function for " << opts.name
                 << ", auxf decreased " << auxf_before << " to " << auxf_after
                 << ", change is " << (auxf_after - auxf_before);
    return 0.0;
  } else {
...
template <typename Real>
Real SolveDoubleQuadraticMatrixProblem(
    const MatrixBase<Real> &G, const SpMatrix<Real> &P1,
    const SpMatrix<Real> &P2, const SpMatrix<Real> &Q1,
    const SpMatrix<Real> &Q2, const SolverOptions &opts, MatrixBase<Real> *M) {
  ...
  for (MatrixIndexT n = 0; n < rows; n++) {
    ...
    try {
      QsumInv.Invert();
      Real old_objf =
          VecVec(mdash_n, gdash_n) - 0.5 * VecSpVec(mdash_n, Qsum, mdash_n);
      mdash_n.AddMatVec(1.0, QsumInv, kNoTrans, gdash_n,
                        0.0);  // m'_n := g'_n * (Q_1 + d_n Q_2)^{-1}
      Real new_objf =
          VecVec(mdash_n, gdash_n) - 0.5 * VecSpVec(mdash_n, Qsum, mdash_n);
      if (new_objf < old_objf) {
        if (new_objf < old_objf - 1.0e-05) {
          KALDI_WARN << "In double quadratic matrix problem: objective "
                        "function decreasing during optimization of "
                     << opts.name << ", " << old_objf << "->" << new_objf
                     << ", change is " << (new_objf - old_objf);
...
template <>
template <>
void SpMatrix<double>::AddVec2(const double alpha,
                               const VectorBase<double> &v) {
...
// instantiate the template above.
template void SpMatrix<float>::AddVec2(const float alpha,
                                       const VectorBase<double> &v);
template void SpMatrix<double>::AddVec2(const double alpha,
                                        const VectorBase<float> &v);
...
template float VecSpVec(const VectorBase<float> &v1, const SpMatrix<float> &M,
                        const VectorBase<float> &v2);
template double VecSpVec(const VectorBase<double> &v1,
                         const SpMatrix<double> &M,
                         const VectorBase<double> &v2);

template <typename Real>
void SpMatrix<Real>::AddMat2Sp(const Real alpha, const MatrixBase<Real> &M,
                               MatrixTransposeType transM,
                               const SpMatrix<Real> &A, const Real beta) {
  if (transM == kNoTrans) {
  ...
  if (transM == kNoTrans) {
    for (MatrixIndexT r = 0; r < dim; r++, p_row_data += r) {
      cblas_Xspmv(A.NumRows(), 1.0, p_A_data, M.RowData(r), 1, 0.0,
                  tmp_vec_data, 1);
    ...
    for (MatrixIndexT r = 0; r < dim; r++, p_row_data += r) {
      cblas_Xspmv(A.NumRows(), 1.0, p_A_data, M.Data() + r, M.Stride(), 0.0,
                  tmp_vec_data, 1);
...
template <typename Real>
void SpMatrix<Real>::AddSmat2Sp(const Real alpha, const MatrixBase<Real> &M,
                                MatrixTransposeType transM,
                                const SpMatrix<Real> &A, const Real beta) {
...
template <typename Real>
void SpMatrix<Real>::AddMat2Vec(const Real alpha, const MatrixBase<Real> &M,
                                MatrixTransposeType transM,
...
void SpMatrix<Real>::AddMat2(const Real alpha, const MatrixBase<Real> &M,
  ...
  Matrix<Real> temp_mat(
      *this);  // wastefully copies upper triangle too, but this
  // doesn't dominate O(N) time.

  // This function call is hard-coded to update the lower triangle.
  cblas_Xsyrk(transM, this_dim, m_other_dim, alpha, M.Data(), M.Stride(), beta,
              temp_mat.Data(), temp_mat.Stride());
...
// Instantiate the template above.
template double TraceSpSpLower(const SpMatrix<double> &A,
                               const SpMatrix<double> &B);
template float TraceSpSpLower(const SpMatrix<float> &A,
                              const SpMatrix<float> &B);
...
template float SolveDoubleQuadraticMatrixProblem(
    const MatrixBase<float> &G, const SpMatrix<float> &P1,
    const SpMatrix<float> &P2, const SpMatrix<float> &Q1,
    const SpMatrix<float> &Q2, const SolverOptions &opts, MatrixBase<float> *M);
