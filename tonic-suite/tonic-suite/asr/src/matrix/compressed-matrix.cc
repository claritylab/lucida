...
void CompressedMatrix::CopyFromMat(const MatrixBase<Real> &mat) {
  ...
  if (global_header.range <= 0.0) global_header.range = 1.0e-05;
  ...
  if (num_rows >= 5) {
    ...
    if (num_rows > 1)
      header->percentile_25 = std::max<uint16>(
          FloatToUint16(global_header, sdata[1]), header->percentile_0 + 1);
    else
      header->percentile_25 = header->percentile_0 + 1;
    if (num_rows > 2)
      header->percentile_75 = std::max<uint16>(
          FloatToUint16(global_header, sdata[2]), header->percentile_25 + 1);
    else
...
// static
inline unsigned char CompressedMatrix::FloatToChar(float p0, float p25,
...
inline float CompressedMatrix::CharToFloat(float p0, float p25, float p75,
                                           float p100, unsigned char value) {
  if (value <= 64) {
...
// This is not compressed.
    ...
    Matrix<BaseFloat> temp_mat(this->NumRows(), this->NumCols(), kUndefined);
    ...
    if (data_ == NULL) {
      ...
      os << h.min_value << ' ' << h.range << ' ' << h.num_rows << ' '
         << h.num_cols << '\n';
      ...
      for (int32 i = 0; i < h.num_cols; i++, per_col_header++) {
        os << per_col_header->percentile_0 << ' '
           << per_col_header->percentile_25 << ' '
  ...
  if (os.fail()) KALDI_ERR << "Error writing compressed matrix to stream.";
}
...
void CompressedMatrix::Read(std::istream &is, bool binary) {
  ...
  if (binary) {  // Binary-mode read.
    ...
    if (peekval == 'C') {
      ...
      if (is.fail()) KALDI_ERR << "Failed to read header";
      ...
      *(reinterpret_cast<GlobalHeader *>(data_)) = h;
      ...
      Matrix<BaseFloat> M;
      M.Read(is, binary);  // This will crash if it was not a Matrix.  This
                           // might happen,
    ...
    *(reinterpret_cast<GlobalHeader *>(data_)) = h;
    ...
    PerColHeader *per_col_header = reinterpret_cast<PerColHeader *>(
        static_cast<char *>(data_) + sizeof(GlobalHeader));
    unsigned char *c =
        reinterpret_cast<unsigned char *>(per_col_header + h.num_cols);
    for (int32 i = 0; i < h.num_cols; i++, per_col_header++) {
  ...
  if (is.fail()) KALDI_ERR << "Failed to read data.";
}
...
// Instantiate the template for float and double.
template void CompressedMatrix::CopyToMat(MatrixBase<float> *mat) const;
template void CompressedMatrix::CopyToMat(MatrixBase<double> *mat) const;
...
template <typename Real>
void CompressedMatrix::CopyToMat(int32 row_offset, int32 column_offset,
  ...
  // number of columns
  start_of_subcol +=
      column_offset * num_rows;  // skip appropriate number of rows
