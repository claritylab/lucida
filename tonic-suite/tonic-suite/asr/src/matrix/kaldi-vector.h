...
class VectorBase {
 public:
  ...
  /// Sets each element of *this to the tanh of the corresponding element of
  /// "src".
  ...
  /// If trans == kNoTrans, solves M x = b, where b is the value of *this at
  /// input
  ...
  /// as you would expect.
  void AddDiagMatMat(Real alpha, const MatrixBase<Real> &M,
                     MatrixTransposeType transM, const MatrixBase<Real> &N,
                     MatrixTransposeType transN, Real beta = 1.0);
...
class Vector : public VectorBase<Real> {
 public:
  ...
  template <typename OtherReal>
  explicit Vector(const VectorBase<OtherReal> &v)
      : VectorBase<Real>() {
    Resize(v.Dim(), kUndefined);
    this->CopyFromVec(v);
  }
...
template <typename Real>
bool ApproxEqual(const VectorBase<Real> &a, const VectorBase<Real> &b,
                 Real tol = 0.01) {
