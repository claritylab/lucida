...
Real OptimizeLbfgs<Real>::RecentStepLength() const {
  ...
  if (n == 0)
    ...
    Real avg = 0.0;
    for (size_t i = 0; i < n; i++) avg += step_lengths_[i] / n;
    return avg;
  }
}
  ...
  if (k_ == 0) {
    if (H_.Dim() == 0) {
      ...
      if (opts_.first_step_length > 0.0) {  // this takes
        ...
        learning_rate =
            (gradient_length > 0.0 ? opts_.first_step_length / gradient_length
                                   : 1.0);
...
template <typename Real>
void OptimizeLbfgs<Real>::ComputeNewDirection(
    Real function_value, const VectorBase<Real> &gradient) {
  ...
  // 7.4 of N&W.
  Vector<Real> &q(deriv_),
      &r(new_x_);  // Use deriv_ as a temporary place to put
  ...
  if (&q != &gradient) q.CopyFromVec(gradient);  // q <-- \nabla f_k.
  Vector<Real> alpha(m);
  // for i = k - 1, k - 2, ... k - m
  for (SignedMatrixIndexT i = k - 1;
       i >= std::max(k - m, static_cast<SignedMatrixIndexT>(0)); i--) {
    alpha(i % m) =
        rho_(i % m) * VecVec(S(i), q);  // \alpha_i <-- \rho_i s_i^T q.
    q.AddVec(-alpha(i % m), Y(i));      // q <-- q - \alpha_i y_i
  }
  r.SetZero();
  r.AddVecVec(1.0, H_, q, 0.0);  // r <-- H_k^{(0)} q.
  // for k = k - m, k - m + 1, ... , k - 1
  for (SignedMatrixIndexT i =
           std::max(k - m, static_cast<SignedMatrixIndexT>(0));
       i < k; i++) {
  ...
  // any backtracking (or acceptance of that step) occurs inside
  // StepSizeIteration.
  // We're still within iteration k; we haven't yet finalized the step size.
  new_x_.Scale(-1.0);
  new_x_.AddVec(1.0, x_);
  if (&deriv_ != &gradient) deriv_.CopyFromVec(gradient);
  f_ = function_value;
  d_ = opts_.d;
...
bool OptimizeLbfgs<Real>::AcceptStep(Real function_value,
  ...
  if ((opts_.minimize && prod <= 1.0e-20) ||
      (!opts_.minimize && prod >= -1.0e-20) || len == 0.0)
    return false;  // This will force restart.

  KALDI_VLOG(3) << "Accepted step; length was " << len << ", prod was " << prod;
...
template <typename Real>
void OptimizeLbfgs<Real>::Restart(const VectorBase<Real> &x, Real f,
                                  const VectorBase<Real> &gradient) {
...
void OptimizeLbfgs<Real>::StepSizeIteration(Real function_value,
                                            const VectorBase<Real> &gradient) {
  KALDI_VLOG(3) << "In step size iteration, function value changed " << f_
                << " to " << function_value;
  ...
  bool wolfe_i_ok;
  if (opts_.minimize)
    wolfe_i_ok = (function_value <= temp);
  else
    wolfe_i_ok = (function_value >= temp);
  ...
  enum {
    kDecrease,
    kNoChange
  } d_action;  // What do do with d_: leave it alone,
  ...
  if (wolfe_i_ok && wolfe_ii_ok) {
    ...
    if (last_failure_type_ ==
        kWolfeII) {  // Last time we failed it was Wolfe ii).
    ...
    if (last_failure_type_ ==
        kWolfeI)             // switching between wolfe i and ii failures->
  ...
  if (d_action == kDecrease) d_ = std::sqrt(d_);
  ...
  if (iteration_action == kDecreaseStep || iteration_action == kIncreaseStep) {
    ...
    if (new_x_.ApproxEqual(temp_, 1.0e-08) &&
        std::abs(f_ - function_value) < 1.0e-08 * std::abs(f_) &&
        iteration_action == kDecreaseStep) {
  ...
  if (iteration_action == kRestart) {
    ...
    bool use_newx;
    if (opts_.minimize)
      use_newx = (function_value < f_);
    else
      use_newx = (function_value > f_);
    KALDI_VLOG(3) << "Restarting computation.";
    if (use_newx)
      Restart(new_x_, function_value, gradient);
    else
      Restart(x_, f_, deriv_);
  }
}
...
template <typename Real>
void OptimizeLbfgs<Real>::DoStep(
    Real function_value, const VectorBase<Real> &gradient,
...
template <typename Real>
const VectorBase<Real> &OptimizeLbfgs<Real>::GetValue(Real *objf_value) const {
...
int32 LinearCgd(const LinearCgdOptions &opts, const SpMatrix<Real> &A,
  ...
  Real r_cur_norm_sq = VecVec(r, r), r_initial_norm_sq = r_cur_norm_sq,
       r_recompute_norm_sq = r_cur_norm_sq;

  Real max_error_sq = std::max<Real>(opts.max_error * opts.max_error,
                                     std::numeric_limits<Real>::min()),
       residual_factor =
           opts.recompute_residual_factor * opts.recompute_residual_factor;
  ...
  for (; k < M + M / 2 + 5 && k != opts.max_iters; k++) {
    ...
    // Check if converged.
    if (r_next_norm_sq <= max_error_sq) break;
  ...
  if (r_cur_norm_sq > r_initial_norm_sq) {
    KALDI_WARN << "Doing linear CGD in dimension " << A.NumRows() << ", after "
               << k << " iterations the squared residual has got worse, "
...
template class OptimizeLbfgs<float>;
template class OptimizeLbfgs<double>;

template int32 LinearCgd<float>(const LinearCgdOptions &opts,
                                const SpMatrix<float> &A,
                                const VectorBase<float> &b,
                                VectorBase<float> *x);

template int32 LinearCgd<double>(const LinearCgdOptions &opts,
