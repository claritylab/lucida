...
void PackedMatrix<Real>::Scale(Real alpha) {
  size_t nr = num_rows_, sz = (nr * (nr + 1)) / 2;
  cblas_Xscal(sz, alpha, data_, 1);
}

template <typename Real>
void PackedMatrix<Real>::AddPacked(const Real alpha,
                                   const PackedMatrix<Real> &rMa) {
...
void PackedMatrix<Real>::SetRandn() {
  ...
  for (size_t i = 0; i < size; i++) data[i] = RandGauss();
}
...
void PackedMatrix<Real>::Resize(MatrixIndexT r, MatrixResizeType resize_type) {
  ...
  if (resize_type == kCopyData) {
    if (this->data_ == NULL || r == 0)
      resize_type = kSetZero;  // nothing to copy.
    else if (this->num_rows_ == r) {
      return;
    }  // nothing to do.
    else {
      ...
      memset(static_cast<void *>(ptr + mem_size_min), 0,
             mem_size_full - mem_size_min);
      tmp.Swap(this);
      return;
    }
  }
...
void PackedMatrix<Real>::CopyFromPacked(const PackedMatrix<OtherReal> &orig) {
  ...
  if (sizeof(Real) == sizeof(OtherReal)) {
    ...
    size_t nr = NumRows(), size = (nr * (nr + 1)) / 2;
    for (size_t i = 0; i < size; i++, dst++, src++) *dst = *src;
  }
}

// template instantiations.
template void PackedMatrix<float>::CopyFromPacked(
    const PackedMatrix<double> &orig);
template void PackedMatrix<double>::CopyFromPacked(
    const PackedMatrix<float> &orig);
template void PackedMatrix<double>::CopyFromPacked(
    const PackedMatrix<double> &orig);
template void PackedMatrix<float>::CopyFromPacked(
    const PackedMatrix<float> &orig);
...
void PackedMatrix<Real>::CopyFromVec(const SubVector<OtherReal> &vec) {
  ...
  if (sizeof(Real) == sizeof(OtherReal)) {
    ...
    for (MatrixIndexT i = 0; i < size; i++, dst++, src++) *dst = *src;
  }
}

// template instantiations.
template void PackedMatrix<float>::CopyFromVec(const SubVector<double> &orig);
template void PackedMatrix<double>::CopyFromVec(const SubVector<float> &orig);
template void PackedMatrix<double>::CopyFromVec(const SubVector<double> &orig);
template void PackedMatrix<float>::CopyFromVec(const SubVector<float> &orig);
...
void PackedMatrix<Real>::SetUnit() {
  memset(data_, 0, SizeInBytes());
  for (MatrixIndexT row = 0; row < num_rows_; row++) (*this)(row, row) = 1.0;
}

template <typename Real>
Real PackedMatrix<Real>::Trace() const {
  Real ans = 0.0;
  for (MatrixIndexT row = 0; row < num_rows_; row++) ans += (*this)(row, row);
  return ans;
}
...
void PackedMatrix<Real>::Write(std::ostream &os, bool binary) const {
  ...
  if (binary) {
    ...
    // We don't use the built-in Kaldi write routines for the floats, as they
    // are
...
void PackedMatrix<Real>::Read(std::istream &is, bool binary, bool add) {
  if (add) {
    ...
    if (this->NumRows() == 0)
      this->Resize(tmp.NumRows());
    else {
      if (this->NumRows() != tmp.NumRows()) {
        if (tmp.NumRows() == 0)
          return;  // do nothing in this case.
        else
          KALDI_ERR << "PackedMatrix::Read, size mismatch " << this->NumRows()
                    << " vs. " << tmp.NumRows();
      }
    }
...
bad:
  KALDI_ERR << "Failed to read packed matrix from stream. "
            << specific_error.str() << " File position at start is "
            << pos_at_start << ", currently " << is.tellg();
}

// Instantiate PackedMatrix for float and double.
template class PackedMatrix<float>;

template class PackedMatrix<double>;
