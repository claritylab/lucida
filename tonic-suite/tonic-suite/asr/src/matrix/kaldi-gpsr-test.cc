...
static void UnitTestGpsr() {
  for (int32 i = 0; i < 5; i++) {
    ...
    H.AddMat2(1.0, M, kTrans, 0.0);  // H = M^T M
                                     //    InitRand(&H);
    //    KALDI_LOG << "dim 1 " << dim1 << "; dim 2 " << dim2 << " LD " <<
    //    H.LogDet()
    //              << " Cond " << H.Cond() << "\nH " << H;
    ...
    Real objf_old =
        0.5 * VecSpVec(x, H, x) - VecVec(x, g) + opts.gpsr_tau * x.Norm(1.0);
    GpsrBasic(opts, H, g, &x);
    Real objf_new =
        0.5 * VecSpVec(x, H, x) - VecVec(x, g) + opts.gpsr_tau * x.Norm(1.0);
    KALDI_ASSERT(objf_old >= objf_new);  // since we are minimizing
    KALDI_LOG << "GPSR-basic: objf old = " << objf_old
              << "; new = " << objf_new;
    Vector<Real> x2(x);
    GpsrBB(opts, H, g, &x);
    Real objf_new_bb =
    ...
    KALDI_LOG << "GPSR-BB: objf old = " << objf_old
              << "; new = " << objf_new_bb;
  }
}
}
