...
class EigenvalueDecomposition {
 ...
 public:
  ...
  void GetV(MatrixBase<Real> *
                V_out) {  // V is what we call P externally; it's the matrix of
    // eigenvectors.
  ...
  void GetRealEigenvalues(VectorBase<Real> *r_out) {
    ...
    for (int i = 0; i < n_; i++) (*r_out)(i) = d_[i];
  }
  void GetImagEigenvalues(VectorBase<Real> *i_out) {
    ...
    for (int i = 0; i < n_; i++) (*i_out)(i) = e_[i];
  }
 ...
 private:
  ...
  // complex division
  inline static void cdiv(Real xr, Real xi, Real yr, Real yi, Real *cdivr,
                          Real *cdivi) {
    Real r, d;
...
template <typename Real>
void EigenvalueDecomposition<Real>::Tred2() {
...
template <typename Real>
void EigenvalueDecomposition<Real>::Tql2() {
  ...
  for (int l = 0; l < n_; l++) {
    ...
    if (m > l) {
      ...
      do {
        ...
        Real r = Hypot(p, static_cast<Real>(1.0));  // This is a Kaldi version
                                                    // of hypot that works with
                                                    // templates.
        if (p < 0) {
          r = -r;
        }
        ...
        for (int i = m - 1; i >= l; i--) {
          ...
          g = c * e_[i];
          h = c * p;
          r = Hypot(p, e_[i]);  // This is a Kaldi version of Hypot that works
                                // with templates.
          e_[i + 1] = s * r;
          s = e_[i] / r;
          c = p / r;
...
template <typename Real>
void EigenvalueDecomposition<Real>::Hqr2() {
        ...
        if (e_[i] < 0.0) {
          ...
          if (e_[i] == 0) {
            ...
            if (std::abs(x) > (std::abs(z) + std::abs(q))) {
              ...
              cdiv(-r - y * H(i, n - 1), -s - y * H(i, n), z, q, &cdivr,
                   &cdivi);
              H(i + 1, n - 1) = cdivr;
              H(i + 1, n) = cdivi;
            }
...
EigenvalueDecomposition<Real>::EigenvalueDecomposition(
  ...
  if (A.IsSymmetric(0.0)) {
    for (int i = 0; i < n_; i++)
      for (int j = 0; j < n_; j++)
        V(i, j) = A(i, j);  // Note that V(i, j) is a member function; A(i, j)
                            // is an operator
    // of the matrix A.
    // Tridiagonalize.
    Tred2();
    ...
    for (int i = 0; i < n_; i++)
      for (int j = 0; j < n_; j++)
        H(i, j) = A(i, j);  // as before: H is member function, A(i, j) is
                            // operator of matrix.
