...
//                      Yanmin Qian;  Saarland University;  Johns Hopkins
//                      University (Author: Daniel Povey)
...
void ComplexFt(const VectorBase<Real> &in, VectorBase<Real> *out,
               bool forward) {
  ...
  Real exp1N_re,
      exp1N_im;  //  forward -> exp(-2pi / N), backward -> exp(2pi / N).
  Real fraction = exp_sign * M_2PI /
                  static_cast<Real>(N);  // forward -> -2pi/N, backward->-2pi/N
  ...
  for (int two_m = 0; two_m < twoN; two_m += 2) {  // For each output component.
    ...
    Real sum_re = 0.0,
         sum_im = 0.0;  // complex output for index m (the sum expression)
    for (int two_n = 0; two_n < twoN; two_n += 2) {
      ComplexAddProduct(data_in[two_n], data_in[two_n + 1], expmn_re, expmn_im,
                        &sum_re, &sum_im);
...
template void ComplexFt(const VectorBase<float> &in, VectorBase<float> *out,
                        bool forward);
template void ComplexFt(const VectorBase<double> &in, VectorBase<double> *out,
                        bool forward);
    ...
    for (; data_thisblock != data_end;
      ...
      for (int qd = 0; qd < Q; qd++) {
        Real pdQ_qd_re = qd_re,
             pdQ_qd_im = qd_im;  // 1^((p'Q+q') / N) == 1^((p'/P) + (q'/N))
...
/* Equations for ComplexFftRecursive.
   ...
   the output (Fourier transform) be d_k, k = 0..N-1.  We use these letters
   because
   ...
   That is, we let p, q, p', q' range over these indices and observe that this
   way we
   ...
   The point here is to separate the index q.  Now we can expand out the
   remaining
   ...
   Here, the sequence of C_{p, q'} over q'=0..Q-1, is just the DFT of the
   sequence
   ...
   one index of type q present, but q' is just a more natural variable name to
   use
   since we use q' elsewhere to subscript c and C.

   Rewriting (9), we have:
         c_(pQ+q')  = \sum_q 1^(q'q (P/N)) b_(pQ+q) (11)
    which is the DFT computed by the recursive call to this function [after
   computing
    the b's by rearranging the a's].  From the c's we want to compute the d's.
    Taking (6), substituting in the sum (7), and using (10) to write it as an
   array,
    we have:
         d_(p'Q+q') =  \sum_p 1^(p'pQ/N) 1^(q'p/N)  c_(pQ+q') (12)
    This sum is independent for different values of q'.  Note that d overwrites
   c
    in memory.  We compute this in  a direct way, using a little array of size P
   to
    store the computed d values for one value of q' (we reuse the array for each
   value
    of q').
    ...
    - Using (8), we rearrange the data in memory so that we have b not a in
   memory
       (this is the block "do the rearrangement").
       The pseudocode for this is as follows.  For simplicity we use a temporary
   array.
        ...
        The reason this accomplishes (8) is that we want pQ+q and qP+p to be
   swapped
        over for each p, q, and the "if m > n" is a convenient way of ensuring
   that
        this swapping happens only once (otherwise it would happen twice, since
   pQ+q
        and qP+p both range over the entire set of numbers 0..N-1).

    - We do the DFT on the smaller block size to compute c from b (this eq eq.
   (11)).
      Note that this is actually multiple DFTs, one for each value of p, but
   this
      goes to the "nffts" argument of the function call, which we have ignored
   up to now.
...
void ComplexFft(VectorBase<Real> *v, bool forward, Vector<Real> *tmp_in) {
  ...
  ComplexFftRecursive(v->Data(), 1, N, factor_beg, factor_beg + factors.size(),
                      forward, (tmp_in ? tmp_in : &tmp));
}
...
void RealFftInefficient(VectorBase<Real> *v, bool forward) {
  ...
  if (forward) {
    ...
    for (MatrixIndexT i = 0; i < N; i++) (*v)(i) = vtmp(i * 2);
  }
}
...
template void ComplexFft(VectorBase<float> *v, bool forward,
                         Vector<float> *tmp_in);
template void ComplexFft(VectorBase<double> *v, bool forward,
...
/* Notes for real FFTs.
   We are using the same convention as above, 1^x to mean exp(-2\pi x) for the
forward transform.
   Actually, in a slight abuse of notation, we use this meaning for 1^x in both
the forward and
   backward cases because it's more convenient in this section.

   Suppose we have real data a[0...N-1], with N even, and want to compute its
Fourier transform.
   We can make do with the first N/2 points of the transform, since the
remaining ones are complex
   conjugates of the first.  We want to compute:
       for k = 0...N/2-1,
       A_k = \sum_{n = 0}^{N-1}  a_n 1^(kn/N)                 (1)

   We treat a[0..N-1] as a complex sequence of length N/2, i.e. a sequence
b[0..N/2 - 1].
   Viewed as sequences of length N/2, we have:
       b = c + i d,
   where c = a_0, a_2 ... and d = a_1, a_3 ...

   We can recover the length-N/2 Fourier transforms of c and d by doing FT on b
and
   then doing the equations below.  Derivation is marked by (*) in a comment
below (search
...
so: re(D_k)= 1/2 (im(B_k) + im(B_{N/2-k}))                             (z2)
    ...
    To recover the FT A from C and D, we write, rearranging (1):
       ...
       A_k = \sum_{n = 0, 2, ..., N-2} a_n 1^(kn/N)
           ...
           = \sum_{n = 0, 1, ..., N/2-1} a_n 1^(2kn/N)  + a_{n+1} 1^(2kn/N)
1^(k/N)
           = \sum_{n = 0, 1, ..., N/2-1} c_n 1^(2kn/N)  + d_n  1^(2kn/N) 1^(k/N)
       A_k =  C_k + 1^(k/N) D_k (a0)

    This equation is valid for k = 0...N/2-1, which is the range of the
sequences B_k and
    C_k.  We don't use is for k = 0, which is a special case considered below.
For
    ...
    By symmetry this gives us the Fourier components for N/2+1, ... N, if we
want
    them.  However, it doesn't give us the value for exactly k = N/2.  For k = 0
and k = N/2, it
    is easiest to argue directly about the meaning of the A_k, B_k and C_k in
terms of
    sums of points.
       A_0 and A_{N/2} are both real, with A_0=\sum_n a_n, and A_1 an
alternating sum
       A_1 = a_0 - a_1 + a_2 ...
     It's easy to show that
              A_0 = B_0 + C_0            (a1)
              A_{N/2} = B_0 - C_0.       (a2)
     Since B_0 and C_0 are both real, B_0 is the real coefficient of D_0 and C_0
is the
     imaginary coefficient.

     *REVERSING THE PROCESS*

     Next we want to reverse this process.  We just need to work out C_k and D_k
from the
     sequence A_k.  Then we do the inverse complex fft and we get back where we
started.
     ...
     and we use
         D_0 = B_0 + i C_0
     to get the 1st complex coefficient of D.  This is exactly the same as the
forward process
     except with an extra factor of 1/2.

     Consider equations (a0) and (a0b).  We want to work out C_k and D_k from
A_k and A_k'.  Remember
     k' = N/2 - k.
     ...
     Write down
      ...
      Next, we want to compute B_k and B_k' from C_k and D_k.  C.f. (z0)..(z3),
and remember
     ...
     We would like to make the equations (p0) ... (p3) look like the forward
equations (z0), (z1),
     (a0) and (a0b) so we can reuse the code.  Define E_k = -i 1^(k/N) D_k.
Then write down (p0)..(p3).
     We have
             C_k  =            0.5 (A_k + A_k'^*)                    (p0')
             E_k  =       -0.5 i   (A_k - A_k'^*)                    (p1')
             B_k  =    C_k - 1^(-k/N) E_k                            (p2')
             B_k' =    C_k + 1^(-k/N) E_k                            (p3')
     So these are exactly the same as (z0), (z1), (a0), (a0b) except replacing
1^(k/N) with
     -1^(-k/N) .  Remember that we defined 1^x above to be exp(-2pi x/N), so the
signs here
     might be opposite to what you see in the code.

     MODIFICATION: we need to take care of a factor of two.  The complex FFT we
implemented
     does not divide by N in the reverse case.  So upon inversion we get larger
by N/2.
     However, this is not consistent with normal FFT conventions where you get a
factor of N.
...
/*
   ...
   letters here (A, B, C and N) are all distinct from the same letters used in
the
...
void ComputeDctMatrix(Matrix<Real> *M) {
  ...
  for (MatrixIndexT k = 1; k < K; k++)
    for (MatrixIndexT n = 0; n < N; n++)
      (*M)(k, n) =
          normalizer * std::cos(static_cast<double>(M_PI) / N * (n + 0.5) * k);
}
...
void MatrixExponential<Real>::Compute(const MatrixBase<Real> &M,
  ...
  P_.Resize(dim, dim);
  P_.CopyFromMat(M);
  P_.Scale(std::pow(static_cast<Real>(0.5), static_cast<Real>(N_)));
  ...
  for (MatrixIndexT i = 0; i < dim; i++) (*X)(i, i) += 1.0;
};
...
template <typename Real>
void MatrixExponential<Real>::ComputeTaylor(const MatrixBase<Real> &P,
                                            MatrixBase<Real> *B0) {
  ...
  Matrix<Real> Pn(P),                  // Pn = P^n
      B0cur(dim, dim), tmp(dim, dim);  // use B0cur to test whether B0 changed.
  std::vector<Matrix<Real> *>
      powers_tmp;  // list of stored powers of P, starting
  ...
  if (n == max_iter)
    KALDI_WARN << "Reached maximum iteration computing Taylor expansion of "
                  "matrix [serious problem]";

  powers_.resize(powers_tmp.size());
  for (MatrixIndexT i = 0; i < static_cast<MatrixIndexT>(powers_tmp.size());
       i++) {
...
void MatrixExponential<Real>::Backprop(const MatrixBase<Real> &hX,
  ...
  for (MatrixIndexT i = N_ - 1; i != (static_cast<MatrixIndexT>(0) -
                                      static_cast<MatrixIndexT>(1));  // i >= 0
       i--) {
...
void MatrixExponential<Real>::BackpropTaylor(const MatrixBase<Real> &hB0,
  ...
  // The variable \hat{B}_0 (hB0) represents df/dX, where f is a scalar
  // function.
  // Note that there is no transpose in our notation for derivative: hB0(i, j)
  // is
  // d/df of E(i, j).
  // Imagine that f is \tr(hB0^T B0) (since this varies linearly with B0 in the
  // same
  // way that the real f does).  We want d/dP (hB0^T B0); call this hP (for
  // \hat{P}).
  //   hP = d/dP  \sum_{i = 1}^n (1/i!) hB0^T P^i
  // Taking each individual term P in this expression and treating the others as
  // constants,
  // and noting that whenever we have something like tr(A B), then B^T is the
  // derivative
  // of this expression w.r.t. A (in our notation),
  //  hP = hB0 + (1/2!) (hB0 P^T + P^T hB0) + (1/3!) (hB0 P^T P^T + P^T hB0 P^T
  //  + P^T P^T hB0)
  //          + (1/4!) ....   (1)
  // We can compute this with the following recursion by which we get each of
  // the terms
  // in this series in turn:
  //  hP_1 <-- hB0
  //  hP <-- hP_1
  //  for n = 2 .. infinity:  # we only carry this on for as many terms as we
  //  used
  ...
  MatrixIndexT dim = P.NumRows();
  KALDI_ASSERT(P.NumCols() == dim && hB0.NumRows() == dim &&
               hB0.NumCols() == dim && hP->NumRows() == dim &&
               hP->NumCols() == dim);
  hP->SetZero();
  Matrix<Real> hPn1(hB0), hPn(dim, dim);
  ...
  hPn.AddMatMat(1.0 / n, hPn1, kNoTrans, P, kTrans,
                0.0);  // hP_n <-- (1/n) hP_{n-1} P^T
  hPn.AddMatMat(1.0 / nfact, P, kTrans, hB0, kNoTrans,
                1.0);    // hP_n += (1/n!) P^T^(n-1) hB0
  hP->AddMat(1.0, hPn);  // add in second term in (1)
  hPn.Swap(&hPn1);

  for (MatrixIndexT i = 0; i < static_cast<MatrixIndexT>(powers_.size()); i++) {
    n++;
    nfact *= n;
...
void ComputePca(const MatrixBase<Real> &X, MatrixBase<Real> *U,
                MatrixBase<Real> *A, bool print_eigs, bool exact) {
  ...
  if (D < N) {                         // Do conventional PCA.
    ...
    for (MatrixIndexT g = 0; g < G; g++) U->Row(g).CopyColFromMat(Utmp, g);
    if (print_eigs)
      KALDI_LOG << (exact ? "" : "Retained ") << "PCA eigenvalues are " << l;
    if (A != NULL) A->AddMatMat(1.0, X, kNoTrans, *U, kTrans, 0.0);
    ...
    for (MatrixIndexT g = 0; g < G; g++) {
      ...
      if (A != NULL)
        for (MatrixIndexT n = 0; n < N; n++) (*A)(n, g) = sqrtlg * Vtmp(g, n);
    }
    // Now orthogonalize.  This is mainly useful in
    // case there were zero eigenvalues, but we do it
    // for all of them.
    U->OrthogonalizeRows();
    if (print_eigs) KALDI_LOG << "(inner-product) PCA eigenvalues are " << l;
  }
}

template void ComputePca(const MatrixBase<float> &X, MatrixBase<float> *U,
                         MatrixBase<float> *A, bool print_eigs, bool exact);

template void ComputePca(const MatrixBase<double> &X, MatrixBase<double> *U,
                         MatrixBase<double> *A, bool print_eigs, bool exact);
  ...
  for (int32 i = 0; i < nrows; i++) {
    ...
    if (multiple > 0.0) {
      for (int32 j = 0; j < ncols; j++, plusdata++, minusdata++, btmp++) {
        if (*btmp > 0.0)
          *plusdata += multiple **btmp;
        else
          *minusdata -= multiple **btmp;
      }
    } else {
      for (int32 j = 0; j < ncols; j++, plusdata++, minusdata++, btmp++) {
        if (*btmp < 0.0)
...
template void AddOuterProductPlusMinus<float>(float alpha,
                                              const VectorBase<float> &a,
                                              const VectorBase<float> &b,
                                              MatrixBase<float> *plus,
                                              MatrixBase<float> *minus);
template void AddOuterProductPlusMinus<double>(double alpha,
