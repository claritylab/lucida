...
SplitRadixComplexFft<Real>::~SplitRadixComplexFft() {
  delete[] brseed_;
  if (tab_ != NULL) {
    for (MatrixIndexT i = 0; i < logn_ - 3; i++) delete[] tab_[i];
    delete[] tab_;
  }
}

template <typename Real>
void SplitRadixComplexFft<Real>::Compute(Real *xr, Real *xi,
                                         bool forward) const {
...
void SplitRadixComplexFft<Real>::Compute(Real *x, bool forward,
  ...
  if (temp_buffer->size() != N_) temp_buffer->resize(N_);
  ...
  memcpy(static_cast<void *>(x + N_), static_cast<void *>(temp_ptr),
         sizeof(Real) * N_);

  Compute(x, x + N_, forward);
  // Now change the format back to interleaved.
  memcpy(static_cast<void *>(temp_ptr), static_cast<void *>(x + N_),
         sizeof(Real) * N_);
...
template <typename Real>
void SplitRadixComplexFft<Real>::BitReversePermute(Real *x,
                                                   MatrixIndexT logn) const {
...
template <typename Real>
void SplitRadixComplexFft<Real>::ComputeRecursive(Real *xr, Real *xi,
                                                  MatrixIndexT logn) const {
  MatrixIndexT m, m2, m4, m8, nel, n;
  ...
  /* Check range of logn */
  if (logn < 0) KALDI_ERR << "Error: logn is out of bounds in SRFFT";
...
void SplitRadixRealFft<Real>::Compute(Real *data, bool forward,
  ...
  if (!forward) {  // call to base class
    ...
    for (MatrixIndexT i = 0; i < N; i++) data[i] *= 2.0;
