...
class MatrixBase {
 public:
  ...
  /// Eigenvalue Decomposition of a square NxN matrix into the form (*this) = P
  /// D
  ...
  void TestUninitialized()
      const;  // This function is designed so that if any element
  ...
  void GroupPnormDeriv(const MatrixBase<Real> &input,
                       const MatrixBase<Real> &output, Real power);
  ...
  */
  ...
  friend Real kaldi::TraceMatMat<Real>(const MatrixBase<Real> &A,
                                       ...
                                       MatrixTransposeType trans);  // tr (A B)
  ...
  void AddMatMat(const Real alpha, const MatrixBase<Real> &A,
  ...
  void AddMatMatDivMat(const MatrixBase<Real> &A, const MatrixBase<Real> &B,
                       const MatrixBase<Real> &C);

  /// A version of AddMatMat specialized for when the second argument
  /// contains a lot of zeroes.
  void AddMatSmat(const Real alpha, const MatrixBase<Real> &A,
                  MatrixTransposeType transA, const MatrixBase<Real> &B,
                  MatrixTransposeType transB, const Real beta);

  /// A version of AddMatMat specialized for when the first argument
  /// contains a lot of zeroes.
  void AddSmatMat(const Real alpha, const MatrixBase<Real> &A,
                  MatrixTransposeType transA, const MatrixBase<Real> &B,
                  MatrixTransposeType transB, const Real beta);

  /// this <-- beta*this + alpha*A*B*C.
  void AddMatMatMat(const Real alpha, const MatrixBase<Real> &A,
                    MatrixTransposeType transA, const MatrixBase<Real> &B,
                    MatrixTransposeType transB, const MatrixBase<Real> &C,
                    MatrixTransposeType transC, const Real beta);
  ...
  void AddTpMat(const Real alpha, const TpMatrix<Real> &A,
                MatrixTransposeType transA, const MatrixBase<Real> &B,
                MatrixTransposeType transB, const Real beta) {
    Matrix<Real> M(A);
  ...
  /// this <-- beta*this + alpha*A*B.
  void AddMatSp(const Real alpha, const MatrixBase<Real> &A,
                MatrixTransposeType transA, const SpMatrix<Real> &B,
                const Real beta) {
    Matrix<Real> M(B);
    return AddMatMat(alpha, A, transA, M, kNoTrans, beta);
  }
  /// this <-- beta*this + alpha*A*B*C.
  void AddSpMatSp(const Real alpha, const SpMatrix<Real> &A,
                  const MatrixBase<Real> &B, MatrixTransposeType transB,
                  const SpMatrix<Real> &C, const Real beta) {
    Matrix<Real> M(A), N(C);
    return AddMatMatMat(alpha, M, kNoTrans, B, transB, N, kNoTrans, beta);
  }
  /// this <-- beta*this + alpha*A*B.
  void AddMatTp(const Real alpha, const MatrixBase<Real> &A,
                MatrixTransposeType transA, const TpMatrix<Real> &B,
                MatrixTransposeType transB, const Real beta) {
    Matrix<Real> M(B);
  ...
  /// this <-- beta*this + alpha*A*B.
  void AddTpTp(const Real alpha, const TpMatrix<Real> &A,
               MatrixTransposeType transA, const TpMatrix<Real> &B,
               MatrixTransposeType transB, const Real beta) {
    Matrix<Real> M(A), N(B);
  ...
  void AddSpSp(const Real alpha, const SpMatrix<Real> &A,
...
// Below is internal methods for Svd, user does not have to know about this.
 ...
 protected:
  // destroys *this!
  bool JamaSvd(VectorBase<Real> *s, MatrixBase<Real> *U, MatrixBase<Real> *V);
...
class Matrix : public MatrixBase<Real> {
 public:
  ...
  template <typename OtherReal>
  explicit Matrix(const SpMatrix<OtherReal> &M)
      : MatrixBase<Real>() {
...
class SubMatrix : public MatrixBase<Real> {
 public:
  ...
  // classes.  Be careful!
  SubMatrix(Real *data, MatrixIndexT num_rows, MatrixIndexT num_cols,
            MatrixIndexT stride);
...
template <typename Real>
bool ApproxEqual(const MatrixBase<Real> &A, const MatrixBase<Real> &B,
                 Real tol = 0.01) {
...
template <typename Real>
void SortSvd(VectorBase<Real> *s, MatrixBase<Real> *U,
             MatrixBase<Real> *Vt = NULL, bool sort_on_absolute_value = true);
...
void CreateEigenvalueMatrix(const VectorBase<Real> &real,
                            const VectorBase<Real> &imag, MatrixBase<Real> *D);
