...
// This routine is a modified form of jama_svd.h which is part of the TNT
// distribution.
...
bool MatrixBase<Real>::JamaSvd(VectorBase<Real> *s_in, MatrixBase<Real> *U_in,
  ...
  for (k = 0; k < std::max(nct, nrt); k++) {
    ...
    for (j = k + 1; j < n; j++) {
      if ((k < nct) && (s(k) != 0.0)) {
        ...
        /*for (i = k; i < m; i++) {
          t += adata[i*astride + k]*adata[i*astride + j];  //   A(i, k)*A(i, j);
          // 3
          }*/
        t = -t / A(k, k);
        ...
        /*for (i = k; i < m; i++) {
          adata[i*astride + j] += t*adata[i*astride + k];  // A(i, j) += t*A(i,
          k); // 5
          }*/
      }
      ...
      if ((k + 1 < m) & (e(k) != 0.0)) {
        ...
        for (j = k + 1; j < n; j++) {
          for (i = k + 1; i < m; i++) {
            workdata[i] +=
                edata[j] *
                adata[i * astride + j];  // work(i) += e(j)*A(i, j); // 5
          }
        }
        for (j = k + 1; j < n; j++) {
          Real t(-e(j) / e(k + 1));
          ...
          /*
          for (i = k+1; i < m; i++) {
            adata[i*astride + j] += t*workdata[i];  // A(i, j) += t*work(i); //
          5
            }*/
        }
      }
  ...
  if (wantu) {
    ...
    for (k = nct - 1; k >= 0; k--) {
      if (s(k) != 0.0) {
        for (j = k + 1; j < nu; j++) {
          Real t = cblas_Xdot(m - k, udata + k * ustride + k, ustride,
                              udata + k * ustride + j, ustride);
          // for (i = k; i < m; i++) {
          //  t += udata[i*ustride + k]*udata[i*ustride + j];  // t += U(i,
          //  k)*U(i, j); // 8
          // }
          t = -t / U(k, k);
          cblas_Xaxpy(m - k, t, udata + ustride * k + k, ustride,
                      udata + k * ustride + j, ustride);
          /*for (i = k; i < m; i++) {
            udata[i*ustride + j] += t*udata[i*ustride + k];  // U(i, j) +=
            t*U(i, k); // 4
            }*/
        }
        for (i = k; i < m; i++) {
  ...
  if (wantv) {
    for (k = n - 1; k >= 0; k--) {
      if ((k < nrt) & (e(k) != 0.0)) {
        for (j = k + 1; j < nu; j++) {
          Real t = cblas_Xdot(n - (k + 1), vdata + (k + 1) * vstride + k,
                              vstride, vdata + (k + 1) * vstride + j, vstride);
          /*Real t (0.0);
          for (i = k+1; i < n; i++) {
            t += vdata[i*vstride + k]*vdata[i*vstride + j];  // t += V(i,
          k)*V(i, j); // 7
            }*/
          t = -t / V(k + 1, k);
          ...
          /*for (i = k+1; i < n; i++) {
            vdata[i*vstride + j] += t*vdata[i*vstride + k];  // V(i, j) +=
            t*V(i, k); // 7
            }*/
        }
      }
      ...
      case 3: {
        ...
        for (j = k; j < p - 1; j++) {
          ...
          if (wantv) {
            ...
            /*for (i = 0; i < n; i++) {
              t = cs*vdata[i*vstride + j] + sn*vdata[i*vstride + j+1];  // t =
              cs*V(i, j) + sn*V(i, j+1);         // 13
              vdata[i*vstride + j+1] = -sn*vdata[i*vstride + j] +
              cs*vdata[i*vstride + j+1];  // V(i, j+1) = -sn*V(i, j) + cs*V(i,
              j+1); // 5
          ...
          if (wantu && (j < m - 1)) {
            ...
            /*for (i = 0; i < m; i++) {
              t = cs*udata[i*ustride + j] + sn*udata[i*ustride + j+1];  // t =
              cs*U(i, j) + sn*U(i, j+1); // 7
              udata[i*ustride + j+1] = -sn*udata[i*ustride + j]
              +cs*udata[i*ustride + j+1];  // U(i, j+1) = -sn*U(i, j) + cs*U(i,
              j+1); // 8
