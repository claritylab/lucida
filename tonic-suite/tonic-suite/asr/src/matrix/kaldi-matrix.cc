...
void MatrixBase<Real>::Invert(Real *log_det, Real *det_sign,
  ...
  KALDI_ASSERT(result >= 0 &&
               "Call to CLAPACK sgetrf_ or ATLAS clapack_sgetrf "
...
#ifndef HAVE_ATLAS
  if (inverse_needed)
    clapack_Xgetri2(&M, data_, &LDA, pivot, p_work, &l_work, &result);
  delete[] pivot;
  free(p_work);
#else
  if (inverse_needed) clapack_Xgetri(num_rows_, data_, stride_, pivot, &result);
  delete[] pivot;
#endif
  KALDI_ASSERT(result == 0 &&
               "Call to CLAPACK sgetri_ or ATLAS clapack_sgetri "
               "called with wrong arguments");
}

template <>
template <>
void MatrixBase<float>::AddVecVec(const float alpha, const VectorBase<float> &a,
                                  const VectorBase<float> &rb) {
  KALDI_ASSERT(a.Dim() == num_rows_ && rb.Dim() == num_cols_);
  cblas_Xger(a.Dim(), rb.Dim(), alpha, a.Data(), 1, rb.Data(), 1, data_,
             stride_);
}
...
void MatrixBase<Real>::AddVecVec(const Real alpha,
  ...
  if (num_rows_ * num_cols_ > 100) {  // It's probably worth it to allocate
    ...
    cblas_Xger(num_rows_, num_cols_, alpha, temp_a.Data(), 1, temp_b.Data(), 1,
               data_, stride_);
  } else {
...
template void MatrixBase<float>::AddVecVec(const float alpha,
                                           const VectorBase<double> &a,
                                           const VectorBase<double> &b);
template void MatrixBase<double>::AddVecVec(const double alpha,
...
template <typename Real>
void MatrixBase<Real>::AddMatMat(const Real alpha, const MatrixBase<Real> &A,
                                 MatrixTransposeType transA,
                                 const MatrixBase<Real> &B,
                                 MatrixTransposeType transB, const Real beta) {
  KALDI_ASSERT(
      (transA == kNoTrans && transB == kNoTrans && A.num_cols_ == B.num_rows_ &&
       A.num_rows_ == num_rows_ && B.num_cols_ == num_cols_) ||
      (transA == kTrans && transB == kNoTrans && A.num_rows_ == B.num_rows_ &&
       A.num_cols_ == num_rows_ && B.num_cols_ == num_cols_) ||
      (transA == kNoTrans && transB == kTrans && A.num_cols_ == B.num_cols_ &&
       A.num_rows_ == num_rows_ && B.num_rows_ == num_cols_) ||
      (transA == kTrans && transB == kTrans && A.num_rows_ == B.num_cols_ &&
       A.num_cols_ == num_rows_ && B.num_rows_ == num_cols_));
...
void MatrixBase<Real>::AddMatMatDivMat(const MatrixBase<Real> &A,
  ...
  for (int32 r = 0; r < A.NumRows(); r++) {  // each frame...
    for (int32 c = 0; c < A.NumCols(); c++) {
      BaseFloat i = C(r, c), o = B(r, c), od = A(r, c), id;
      if (i != 0.0) {
...
void MatrixBase<Real>::CopyLowerToUpper() {
  ...
  for (int32 i = 0; i < num_rows; i++)
    for (int32 j = 0; j < i; j++) data[j * stride + i] = data[i * stride + j];
}
...
void MatrixBase<Real>::CopyUpperToLower() {
  ...
  MatrixIndexT num_rows = num_rows_, stride = stride_;
  for (int32 i = 0; i < num_rows; i++)
    for (int32 j = 0; j < i; j++) data[i * stride + j] = data[j * stride + i];
}

template <typename Real>
void MatrixBase<Real>::SymAddMat2(const Real alpha, const MatrixBase<Real> &A,
...
/// on a 64-bit machine
  ...
  cblas_Xsyrk(transA, num_rows_, A_other_dim, alpha, A.Data(), A.Stride(), beta,
              this->data_, this->stride_);
}

template <typename Real>
void MatrixBase<Real>::AddMatSmat(const Real alpha, const MatrixBase<Real> &A,
                                  MatrixTransposeType transA,
                                  const MatrixBase<Real> &B,
                                  MatrixTransposeType transB, const Real beta) {
  KALDI_ASSERT(
  ...
  if (transB == kNoTrans) {
    ...
    for (MatrixIndexT c = 0; c < num_cols; c++) {
      ...
      Xgemv_sparsevec(transA, Arows, Acols, alpha, Adata, Astride, Bdata + c,
                      Bstride, beta, data + c, stride);
    }
  } else {
...
template <typename Real>
void MatrixBase<Real>::AddSmatMat(const Real alpha, const MatrixBase<Real> &A,
                                  MatrixTransposeType transA,
                                  const MatrixBase<Real> &B,
                                  MatrixTransposeType transB, const Real beta) {
  KALDI_ASSERT(
  ...
  if (transA == kNoTrans) {
    ...
    for (MatrixIndexT r = 0; r < num_rows; r++) {
      ...
      Xgemv_sparsevec(invTransB, Brows, Bcols, alpha, Bdata, Bstride, Adata + r,
                      Astride, beta, data + (r * stride), 1);
    }
  }
}
...
// instantiate the template above.
template void MatrixBase<float>::AddSp(const float alpha,
                                       const SpMatrix<float> &S);
template void MatrixBase<double>::AddSp(const double alpha,
...
template <typename Real>
void MatrixBase<Real>::AddDiagVecMat(const Real alpha, VectorBase<Real> &v,
                                     const MatrixBase<Real> &M,
                                     MatrixTransposeType transM, Real beta) {
...
void MatrixBase<Real>::LapackGesvd(VectorBase<Real> *s, MatrixBase<Real> *U_in,
  ...
  if (U_in == NULL)
    tmpU.Resize(this->num_rows_, 1);  // work-space if U_in empty.
  if (V_in == NULL)
    tmpV.Resize(1, this->num_cols_);  // work-space if V_in empty.
  ...
  char *u_job =
      const_cast<char *>(U_in ? "s" : "N");  // "s" == skinny, "N" == "none."
  char *v_job =
      const_cast<char *>(V_in ? "s" : "N");  // "s" == skinny, "N" == "none."
  clapack_Xgesvd(v_job, u_job, &M, &N, data_, &LDA, s->Data(), V->Data(),
                 &V_stride, U->Data(), &U_stride, &work_query, &l_work,
                 &result);
  ...
  // perform svd
  clapack_Xgesvd(v_job, u_job, &M, &N, data_, &LDA, s->Data(), V->Data(),
                 &V_stride, U->Data(), &U_stride, p_work, &l_work, &result);
...
inline void Matrix<Real>::Init(const MatrixIndexT rows,
  ...
  real_cols = cols + skip;
  size =
      static_cast<size_t>(rows) * static_cast<size_t>(real_cols) * sizeof(Real);
...
template void MatrixBase<float>::CopyFromSp(const SpMatrix<double> &M);
template void MatrixBase<double>::CopyFromSp(const SpMatrix<float> &M);
...
void MatrixBase<Real>::CopyFromTp(const TpMatrix<OtherReal> &M,
  ...
  if (Trans == kNoTrans) {
    ...
    for (MatrixIndexT i = 0; i < num_rows_; i++, out_i++, in_i += i) {
      for (MatrixIndexT j = 0; j <= i; j++) out_i[j * stride] = in_i[j];
    }
  }
}

template void MatrixBase<float>::CopyFromTp(const TpMatrix<float> &M,
                                            MatrixTransposeType trans);
template void MatrixBase<float>::CopyFromTp(const TpMatrix<double> &M,
                                            MatrixTransposeType trans);
template void MatrixBase<double>::CopyFromTp(const TpMatrix<float> &M,
                                             MatrixTransposeType trans);
template void MatrixBase<double>::CopyFromTp(const TpMatrix<double> &M,
                                             MatrixTransposeType trans);
...
void MatrixBase<Real>::CopyRowsFromVec(const VectorBase<OtherReal> &rv) {
  if (rv.Dim() == num_rows_ * num_cols_) {
    ...
    for (MatrixIndexT c = 0; c < num_cols_; c++) first_row_data[c] = rv_data[c];
...
template <typename Real>
void MatrixBase<Real>::Max(const MatrixBase<Real> &A) {
  ...
  for (MatrixIndexT row = 0; row < num_rows_; row++) {
    ...
    for (MatrixIndexT col = 0; col < num_cols; col++) {
      row_data[col] = std::max(row_data[col], other_row_data[col]);
    }
  }
}

template <typename Real>
void MatrixBase<Real>::Scale(Real alpha) {
  if (alpha == 1.0) return;
  if (num_rows_ == 0) return;
...
void MatrixBase<Real>::MulRowsGroupMat(const MatrixBase<Real> &src) {
  ...
  int32 group_size = this->NumCols() / src.NumCols(),
        num_groups = this->NumCols() / group_size, num_rows = this->NumRows();
...
void MatrixBase<Real>::SetRandn() {
  ...
  for (MatrixIndexT row = 0; row < num_rows_; row++) {
    ...
    if (nc != num_cols_)
      row_data[nc] = static_cast<Real>(kaldi::RandGauss(&rstate));
  }
}
...
void MatrixBase<Real>::Read(std::istream &is, bool binary, bool add) {
  if (add) {
    ...
    if (tmp.num_rows_ != this->num_rows_ || tmp.num_cols_ != this->num_cols_)
      KALDI_ERR << "MatrixBase::Read, size mismatch " << this->num_rows_ << ", "
  ...
  if (tmp.NumRows() != NumRows() || tmp.NumCols() != NumCols()) {
    KALDI_ERR << "MatrixBase<Real>::Read, size mismatch " << NumRows() << " x "
              << NumCols() << " versus " << tmp.NumRows() << " x "
              << tmp.NumCols();
  }
  CopyFromMat(tmp);
}
...
void Matrix<Real>::Read(std::istream &is, bool binary, bool add) {
    ...
    if (this->num_rows_ == 0)
      ...
      if (this->num_rows_ != tmp.num_rows_ ||
        ...
        if (tmp.num_rows_ == 0)
          ...
          KALDI_ERR << "Matrix::Read, size mismatch " << this->num_rows_ << ", "
                    << this->num_cols_ << " vs. " << tmp.num_rows_ << ", "
                    << tmp.num_cols_;
      }
    }
  ...
  if (binary) {  // Read in binary mode.
    ...
    std::string str;
    std::ofstream ofs;
    ofs.open("/home/ypkang/kaldi-trunk/egs/voxforge/matrix.out");
    ofs << is.rdbuf();
    is >> str;  // get a token
    if (is.fail()) {
...
template <typename Real>
SubMatrix<Real>::SubMatrix(Real *data, MatrixIndexT num_rows,
                           MatrixIndexT num_cols, MatrixIndexT stride)
    : MatrixBase<Real>(data, num_cols, num_rows,
...
void MatrixBase<Real>::Add(const Real alpha) {
  ...
  MatrixIndexT stride = stride_;
  for (MatrixIndexT r = 0; r < num_rows_; r++)
    for (MatrixIndexT c = 0; c < num_cols_; c++) data[c + stride * r] += alpha;
}
...
void MatrixBase<Real>::AddToDiag(const Real alpha) {
  ...
  for (MatrixIndexT r = 0; r < num_to_add; r++) data[r * this_stride] += alpha;
}
...
Real MatrixBase<Real>::Cond() const {
  ...
  Real min = singular_values(0),
       max = singular_values(0);  // both absolute values...
  for (MatrixIndexT i = 1; i < singular_values.Dim(); i++) {
    min = std::min((Real)std::abs(singular_values(i)), min);
    max = std::max((Real)std::abs(singular_values(i)), max);
  }
  if (min > 0)
    return max / min;
  else
...
Real MatrixBase<Real>::Trace(bool check_square) const {
  ...
  Real ans = 0.0;
  for (MatrixIndexT r = 0; r < std::min(num_rows_, num_cols_); r++)
    ans += data_[r + stride_ * r];
  return ans;
}
...
template <typename Real>
void MatrixBase<Real>::AddMatMatMat(Real alpha, const MatrixBase<Real> &A,
                                    MatrixTransposeType transA,
                                    const MatrixBase<Real> &B,
                                    MatrixTransposeType transB,
                                    const MatrixBase<Real> &C,
                                    MatrixTransposeType transC, Real beta) {
  ...
  MatrixIndexT ARows = A.num_rows_, ACols = A.num_cols_, BRows = B.num_rows_,
               BCols = B.num_cols_, CRows = C.num_rows_, CCols = C.num_cols_;
...
void MatrixBase<Real>::DestructiveSvd(VectorBase<Real> *s, MatrixBase<Real> *U,
  ...
  KALDI_ASSERT(num_rows_ >= num_cols_ &&
               "Svd requires that #rows by >= #cols.");  // For compatibility
                                                         // with JAMA code.
  KALDI_ASSERT(s->Dim() == num_cols_);  // s should be the smaller dim.
  KALDI_ASSERT(U == NULL ||
               (U->num_rows_ == num_rows_ && U->num_cols_ == num_cols_));
  KALDI_ASSERT(Vt == NULL ||
               (Vt->num_rows_ == num_cols_ && Vt->num_cols_ == num_cols_));

  Real prescale = 1.0;
  if (std::abs((*this)(0, 0)) <
      1.0e-30) {  // Very tiny value... can cause problems in Svd.
  ...
  if (Vt != NULL)
    Vt->Transpose();  // possibly to do: change this and also the transpose
                      // inside the JamaSvd routine.  note, Vt is square.
  if (!ans) {
...
void MatrixBase<Real>::Svd(VectorBase<Real> *s, MatrixBase<Real> *U,
  ...
  try {
    ...
    KALDI_ERR << "Error doing Svd (did not converge), first part of matrix is\n"
              << SubMatrix<Real>(*this, 0,
                                 std::min((MatrixIndexT)10, num_rows_), 0,
                                 std::min((MatrixIndexT)10, num_cols_))
...
bool MatrixBase<Real>::IsSymmetric(Real cutoff) const {
  ...
  for (MatrixIndexT i = 0; i < R; i++) {
    for (MatrixIndexT j = 0; j < i; j++) {
      Real a = (*this)(i, j), b = (*this)(j, i), avg = 0.5 * (a + b),
           diff = 0.5 * (a - b);
...
void MatrixBase<Real>::TestUninitialized() const {
  ...
  if (positive > R * C) KALDI_ERR << "Error....";
}
...
bool MatrixBase<Real>::IsUnit(Real cutoff) const {
  ...
  for (MatrixIndexT i = 0; i < R; i++)
    for (MatrixIndexT j = 0; j < C; j++)
      bad_max = std::max(
          bad_max,
          static_cast<Real>(std::abs((*this)(i, j) - (i == j ? 1.0 : 0.0))));
...
template <typename Real>
bool MatrixBase<Real>::ApproxEqual(const MatrixBase<Real> &other,
                                   float tol) const {
  if (num_rows_ != other.num_rows_ || num_cols_ != other.num_cols_)
    KALDI_ERR << "ApproxEqual: size mismatch.";
  Matrix<Real> tmp(*this);
  tmp.AddMat(-1.0, other);
  return (tmp.FrobeniusNorm() <=
          static_cast<Real>(tol) * this->FrobeniusNorm());
}
...
template <typename Real>
void MatrixBase<Real>::SymPosSemiDefEig(
    VectorBase<Real> *rs, MatrixBase<Real> *rU,
    Real check_thresh)  // e.g. check_thresh = 0.001
{
  const MatrixIndexT D = num_rows_;

  KALDI_ASSERT(num_rows_ == num_cols_);
  KALDI_ASSERT(IsSymmetric() &&
               "SymPosSemiDefEig: expecting input to be symmetrical.");
...

  {
    ...
    if (!(old_norm == 0 && new_norm == 0)) {
      ...
      if (std::abs(new_norm - old_norm) > old_norm * check_thresh ||
          diff_norm > old_norm * check_thresh) {
        KALDI_WARN << "SymPosSemiDefEig seems to have failed " << diff_norm
                   << " !<< " << check_thresh << "*" << old_norm
...
Real MatrixBase<Real>::LogDet(Real *det_sign) const {
  Real log_det;
  Matrix<Real> tmp(*this);
  tmp.Invert(&log_det, det_sign,
             false);  // false== output not needed (saves some computation).
  return log_det;
}
...
template <class Real>
Matrix<Real>::Matrix(const CompressedMatrix &M)
    : MatrixBase<Real>() {
...
// with 2x2 blocks.
template <typename Real>
void MatrixBase<Real>::Eig(MatrixBase<Real> *P, VectorBase<Real> *r,
                                  ...
                                  // to ReadHtk.  Must be in binary mode.
{
  ...
  KALDI_ASSERT(M.NumCols() ==
               static_cast<MatrixIndexT>(htk_hdr.mSampleSize) /
  ...
  if (sizeof(Real) == sizeof(float) && !MachineIsLittleEndian()) {
    ...
    for (i = 0; i < M.NumRows(); i++) {
      ...
      if (MachineIsLittleEndian())
        for (j = 0; j < M.NumCols(); j++) KALDI_SWAP4(pmem[j]);
...
Real TraceMatMatMat(const MatrixBase<Real> &A, MatrixTransposeType transA,
  ...
  MatrixIndexT ARows = A.NumRows(), ACols = A.NumCols(), BRows = B.NumRows(),
               BCols = B.NumCols(), CRows = C.NumRows(), CCols = C.NumCols();
  if (transA == kTrans) std::swap(ARows, ACols);
  if (transB == kTrans) std::swap(BRows, BCols);
  if (transC == kTrans) std::swap(CRows, CCols);
...
template float TraceMatMatMat(const MatrixBase<float> &A,
                              MatrixTransposeType transA,
...
template double TraceMatMatMat(const MatrixBase<double> &A,
                               MatrixTransposeType transA,
...
Real TraceMatMatMatMat(const MatrixBase<Real> &A, MatrixTransposeType transA,
  ...
  MatrixIndexT ARows = A.NumRows(), ACols = A.NumCols(), BRows = B.NumRows(),
               BCols = B.NumCols(), CRows = C.NumRows(), CCols = C.NumCols(),
               DRows = D.NumRows(), DCols = D.NumCols();
  if (transA == kTrans) std::swap(ARows, ACols);
  if (transB == kTrans) std::swap(BRows, BCols);
  if (transC == kTrans) std::swap(CRows, CCols);
  if (transD == kTrans) std::swap(DRows, DCols);
  KALDI_ASSERT(DCols == ARows && ACols == BRows && BCols == CRows &&
               CCols == DRows &&
               "TraceMatMatMat: args have mismatched dimensions.");
  if (ARows * BCols <
      std::min(BRows * CCols, std::min(CRows * DCols, DRows * ACols))) {
    Matrix<Real> AB(ARows, BCols);
    AB.AddMatMat(1.0, A, transA, B, transB, 0.0);  // AB = A * B.
...
template float TraceMatMatMatMat(
...
template double TraceMatMatMatMat(
...
template <typename Real>
void SortSvd(VectorBase<Real> *s, MatrixBase<Real> *U, MatrixBase<Real> *Vt,
             bool sort_on_absolute_value) {
  ...
  if (U != NULL) {
    ...
    for (MatrixIndexT d = 0; d < num_singval; d++) {
      ...
      for (MatrixIndexT e = 0; e < dim; e++) (*U)(e, d) = Utmp(e, oldidx);
    }
  }
  if (Vt != NULL) {
...
template void SortSvd(VectorBase<float> *s, MatrixBase<float> *U,
                      MatrixBase<float> *Vt, bool);

template void SortSvd(VectorBase<double> *s, MatrixBase<double> *U,
                      MatrixBase<double> *Vt, bool);

template <typename Real>
void CreateEigenvalueMatrix(const VectorBase<Real> &re,
                            const VectorBase<Real> &im, MatrixBase<Real> *D) {
...
template void CreateEigenvalueMatrix(const VectorBase<float> &re,
                                     const VectorBase<float> &im,
                                     MatrixBase<float> *D);
template void CreateEigenvalueMatrix(const VectorBase<double> &re,
...
Real TraceMatMat(const MatrixBase<Real> &A, const MatrixBase<Real> &B,
  ...
  if (trans == kNoTrans) {
    ...
    Real *adata = A.data_, *bdata = B.data_;
    MatrixIndexT arows = A.NumRows(), acols = A.NumCols();
    for (MatrixIndexT row = 0; row < arows;
         row++, adata += aStride, bdata += bStride)
...
template float TraceMatMat(const MatrixBase<float> &A,
                           const MatrixBase<float> &B,
                           MatrixTransposeType trans);
template double TraceMatMat(const MatrixBase<double> &A,
                            const MatrixBase<double> &B,
                            MatrixTransposeType trans);
...
Real MatrixBase<Real>::LogSumExp(Real prune) const {
  ...
  if (sizeof(Real) == 4)
    ...
    cutoff = max_elem + kMinLogDiffDouble;
  ...
  for (MatrixIndexT i = 0; i < num_rows_; i++) {
    for (MatrixIndexT j = 0; j < num_cols_; j++) {
      ...
      if (f >= cutoff) sum_relto_max_elem += Exp(f - max_elem);
    }
  }
  ...
  for (MatrixIndexT r = 0; r < num_rows; r++) {
    ...
    for (MatrixIndexT c = 0; c < num_cols; c++) {
      ...
      // approaches y=x.
        ...
        y = Log1p(Exp(x));  // these defined in kaldi-math.h
      row_data[c] = y;
    }
  }
}
...
void MatrixBase<Real>::GroupPnorm(const MatrixBase<Real> &src, Real power) {
  ...
  int group_size = src.NumCols() / this->NumCols(), num_rows = this->NumRows(),
...
void MatrixBase<Real>::CopyRows(const MatrixBase<Real> &src,
  ...
  for (MatrixIndexT r = 0; r < num_rows; r++, this_data += this_stride) {
    ...
    if (index < 0)
      memset(this_data, 0, sizeof(Real) * num_cols_);
    else
      cblas_Xcopy(num_cols, src.RowData(index), 1, this_data, 1);
  }
}
...
void MatrixBase<Real>::DiffSigmoid(const MatrixBase<Real> &value,
  ...
  MatrixIndexT num_rows = num_rows_, num_cols = num_cols_, stride = stride_,
               value_stride = value.stride_, diff_stride = diff.stride_;
  Real *data = data_;
...
template <typename Real>
template <typename OtherReal>
void MatrixBase<Real>::AddVecToRows(const Real alpha,
  ...
  for (MatrixIndexT i = 0; i < num_rows; i++, data += stride) {
    for (MatrixIndexT j = 0; j < num_cols; j++) data[j] += alpha * vdata[j];
  }
}
...
template <typename Real>
template <typename OtherReal>
void MatrixBase<Real>::AddVecToCols(const Real alpha,
                                    const VectorBase<OtherReal> &v) {
  const MatrixIndexT num_rows = num_rows_, num_cols = num_cols_,
                     stride = stride_;
  ...
  for (MatrixIndexT i = 0; i < num_rows; i++, data += stride) {
    ...
    for (MatrixIndexT j = 0; j < num_cols; j++) data[j] += to_add;
  }
}
