...
//                       Ondrej Glembek;  Saarland University (Author: Arnab
//                       Ghoshal);
...
template <typename Real>
static void InitRand(TpMatrix<Real> *M) {
  // start:
...
template <typename Real>
static void InitRand(Vector<Real> *v) {
...
start:
  ...
  if (M->NumRows() != 0 && M->Cond() > 100) {
    printf(
        "Condition number of random matrix large %f, trying again (this is "
        "normal)\n",
        (float)M->Cond());
    goto start;
  }
...
start:
  for (MatrixIndexT i = 0; i < M->NumRows(); i++)
    for (MatrixIndexT j = 0; j <= i; j++) (*M)(i, j) = RandGauss();
  if (M->NumRows() != 0 && M->Cond() > 100) goto start;
}
...
template<typename Real> static void AssertEqual(const Matrix<Real> &A,
  ...
  for (MatrixIndexT i = 0;i < A.NumRows();i++)
    for (MatrixIndexT j = 0;j < A.NumCols();j++) {
      KALDI_ASSERT(std::abs(A(i, j)-B(i, j)) < tol*std::max(1.0, (double)
(std::abs(A(i, j))+std::abs(B(i, j)))));
    }
}
...
/* was:
   ...
   for (MatrixIndexT i = 0;i < A.NumRows();i++)
   for (MatrixIndexT j = 0;j<=i;j++)
   if (std::abs(A(i, j)-B(i, j)) > tol*std::max(1.0, (double) (std::abs(A(i,
   j))+std::abs(B(i, j))))) return false;
   return true;
   }
*/
...
/*
  HERE
template<typename Real> static void AssertEqual(Vector<Real> &A, Vector<Real>
&B, float tol = 0.001) {
...
template<typename Real> static bool ApproxEqual(Vector<Real> &A, Vector<Real>
&B, float tol = 0.001) {
  KALDI_ASSERT(A.Dim() == B.Dim());
  for (MatrixIndexT i = 0;i < A.Dim();i++)
...
static void UnitTestAddToDiagMatrix() {
  for (int p = 0; p < 2; p++) {
    ...
    M.AddToDiag(alpha);
    for (MatrixIndexT i = 0; i < dimM && i < dimN; i++) Mcopy(i, i) += alpha;
    AssertEqual(M, Mcopy);
  }
}
...
static void UnitTestAddDiagVecMat() {
  for (int p = 0; p < 2; p++) {
    ...
    for (int32 r = 0; r < dimM; r++) {
      ...
      if (trans == kTrans)
        Nrow.CopyColFromMat(N, r);
      else
        Nrow.CopyFromVec(N.Row(r));
...
static void UnitTestSpAddDiagVec() {
  for (MatrixIndexT i = 0; i < 10; i++) {
    ...
    for (MatrixIndexT i = 0; i < dimM; i++) T(i, i) += alpha * v(i);
    AssertEqual(S, T);
  }
}
...
static void UnitTestSetRandn() {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    for (MatrixIndexT pow = 1; pow < 5; pow++) {
      ...
      // but it involves the gamma function and it's too much hassle to
      // implement.
      Real lower_bound = expected_moment - allowed_deviation,
           upper_bound = expected_moment + allowed_deviation;
      KALDI_ASSERT(observed_moment >= lower_bound &&
                   observed_moment <= upper_bound);
    }
  }
}
...
template <typename Real>
static void UnitTestNorm() {  // test some simple norm properties: scaling.
                              // also ApproxEqual test.
  ...
  for (MatrixIndexT p = 0; p < 10; p++) {
    ...
    Real Mnorm = M.FrobeniusNorm(), Snorm = S.FrobeniusNorm(),
         Vnorm1 = V.Norm(1.0), Vnorm2 = V.Norm(2.0), Vnorm3 = V.Norm(3.0);
    M.Scale(scalar);
    S.Scale(scalar);
    V.Scale(scalar);
    ...
    KALDI_ASSERT(S.ApproxEqual(S2, 0.15));
    Matrix<Real> M2(M);
    M2.Scale(1.1);
    KALDI_ASSERT(!M.ApproxEqual(M2));
    KALDI_ASSERT(M.ApproxEqual(M2, 0.15));
    Vector<Real> V2(V);
    V2.Scale(1.1);
    KALDI_ASSERT(!V.ApproxEqual(V2));
    KALDI_ASSERT(V.ApproxEqual(V2, 0.15));
  }
}
...
static void UnitTestCopyRows() {
  for (MatrixIndexT p = 0; p < 10; p++) {
    MatrixIndexT num_rows1 = 10 + Rand() % 10, num_rows2 = 10 + Rand() % 10,
                 num_cols = 10 + Rand() % 10;
...
static void UnitTestCopyCols() {
  for (MatrixIndexT p = 0; p < 10; p++) {
    MatrixIndexT num_cols1 = 10 + Rand() % 10, num_cols2 = 10 + Rand() % 10,
                 num_rows = 10 + Rand() % 10;
...
template <typename Real>
static void
UnitTestSimpleForMat() {  // test some simple operates on all kinds of matrix
  ...
  for (MatrixIndexT p = 0; p < 10; p++) {
    ...
    }
    {
      ...
      for (MatrixIndexT m = 0; m < dimM; m++)
        for (MatrixIndexT n = 0; n < dimN; n++) N(m, n) -= 2.0;
      AssertEqual(M, N);
    }
  ...
  for (MatrixIndexT p = 0; p < 10;
       p++) {  // test for sp and tp matrix's AddSp() and AddTp() function
    MatrixIndexT dimM = 10 + Rand() % 10;
  ...
  for (MatrixIndexT i = 0; i < 10;
       i++) {  // test for sp matrix's AddVec2() function
    MatrixIndexT dimM = 10 + Rand() % 10;
    ...
    for (MatrixIndexT i = 0; i < dimM; i++)
      for (MatrixIndexT j = 0; j < dimM; j++) {
        KALDI_ASSERT(std::abs(M(i, j) - (Sorig(i, j) + 0.5 * V(i) * V(j))) <
                     0.001);
        KALDI_ASSERT(std::abs(Md(i, j) - (Sorig(i, j) + 0.5 * V(i) * V(j))) <
                     0.001);
      }
  }
}
...
template <typename Real>
static void UnitTestDeterminant() {  // also tests matrix axpy and IsZero() and
                                     // TraceOfProduct{, T}
  for (MatrixIndexT iter = 0; iter < 5;
       iter++) {  // First test the 2 det routines are the same
    int dimM = 10 + Rand() % 10;
    Matrix<Real> M(dimM, dimM), N(dimM, dimM);
    InitRand(&M);
    N.AddMatMat(1.0, M, kNoTrans, M, kTrans, 0.0);  // N:=M*M^T.
    for (MatrixIndexT i = 0; i < (MatrixIndexT)dimM; i++)
      N(i, i) += 0.0001;  // Make sure numerically +ve det-- can by chance be
                          // almost singular the way we initialized it (I think)
    SpMatrix<Real> S(dimM);
    S.CopyFromMat(N);  // symmetric so should not crash.
    ...
    KALDI_ASSERT(sign2 == 1.0 && sign3 == 1.0 &&
                 std::abs(logdet2 - logdet) < 0.1 &&
                 std::abs(logdet2 - logdet3) < 0.1);
    ...
    Real a = TraceSpSp(S, S), b = TraceMatMat(N, N),
         c = TraceMatMat(N, N, kTrans);
...
static void UnitTestDeterminantSign() {
  for (MatrixIndexT iter = 0; iter < 20;
       iter++) {  // First test the 2 det routines are the same
    int dimM = 10 + Rand() % 10;
    Matrix<Real> M(dimM, dimM), N(dimM, dimM);
    InitRand(&M);
    ...
    logdet3 = S.LogDet(&sign3);
    KALDI_ASSERT(sign2 == 1.0 && sign3 == 1.0 &&
                 std::abs(logdet2 - logdet) < 0.01 &&
                 std::abs(logdet2 - logdet3) < 0.01);

    MatrixIndexT num_sign_changes = Rand() % 5;
    for (MatrixIndexT change = 0; change < num_sign_changes; change++) {
      // Change sign of S's det by flipping one eigenvalue, and N by flipping
      // one row.
      {
        Matrix<Real> M(S);
...
static void UnitTestSherman() {
  for (MatrixIndexT iter = 0; iter < 1; iter++) {
    ...
    InitRand(&V);
    for (MatrixIndexT i = 0; i < (MatrixIndexT)dimM; i++)
      A(i, i) += 0.0001;  // Make sure numerically +ve det-- can by chance be
                          // almost singular the way we initialized it (I think)
    ...
    tt.AddMatMat(1.0, V, kTrans, invA, kNoTrans,
                 0.0);  // tt0 = trans(V) *inv(A)
...
static void UnitTestSvdBad() {
  ...
  for (MatrixIndexT i = 0; i < N; i++)
    for (MatrixIndexT j = 0; j <= i; j++) M(i, j) = 1591.3614306764898;
  M(0, 0) *= 1.00001;
  M(10, 10) *= 1.00001;
  Matrix<Real> U(N, N);
  Vector<Real> l(N);
...
static void UnitTestSvdZero() {
  ...
  for (MatrixIndexT iter = 0; iter < Iter; iter++) {
    MatrixIndexT dimM = Base + Rand() % Rand_,
                 dimN = Base + Rand() % Rand_;  // M>=N.
    Matrix<Real> M(dimM, dimN);
    Matrix<Real> U(dimM, dimM), Vt(dimN, dimN);
    Vector<Real> v(std::min(dimM, dimN));
    if (iter % 2 == 0)
      M.SetZero();
    else
      M.Unit();
...
static void UnitTestSvdNodestroy() {
  MatrixIndexT Base = 3, Rand_ = 2, Iter = 25;
  for (MatrixIndexT iter = 0; iter < Iter; iter++) {
    MatrixIndexT dimN = Base + Rand() % Rand_,
                 dimM =
                     dimN + Rand() % Rand_;  // M>=N, as required by JAMA Svd.
    MatrixIndexT minsz = std::min(dimM, dimN);
    Matrix<Real> M(dimM, dimN);
    Matrix<Real> U(dimM, minsz), Vt(minsz, dimN);
    Vector<Real> v(minsz);
    InitRand(&M);
    if (iter < 2) KALDI_LOG << "M " << M;
    M.Svd(&v, &U, &Vt);
    ...
    for (MatrixIndexT it = 0; it < 2; it++) {
      ...
      for (MatrixIndexT i = 0; i + 1 < v.Dim();
           i++)  // check SortSvd is working.
...
/*
  template<typename Real> static void UnitTestSvdVariants() {  // just make sure
  it doesn't crash if we call it but don't want left or right singular vectors.
  there are KALDI_ASSERTs inside the Svd.
  #ifndef HAVE_ATLAS
  ...
  Matrix<Real> M(dimM, dimN);
  Matrix<Real> U(dimM, dimM), Vt(dimN, dimN); Vector<Real> v(std::min(dimM,
  dimN));
  Matrix<Real> Utmp(dimM, 1); Matrix<Real> Vttmp(1, dimN);
  InitRand(&M);
  M.Svd(v, U, Vttmp, "A", "N");
  M.Svd(v, Utmp, Vt, "N", "A");
  Matrix<Real> U2(dimM, dimM), Vt2(dimN, dimN); Vector<Real> v2(std::min(dimM,
  dimN));
  M.Svd(v, U2, Vt2, "A", "A");
...
template <typename Real>
static void UnitTestSvdJustvec() {  // Making sure gives same answer if we get
                                    // just the vector, not the eigs.
  MatrixIndexT Base = 10, Rand_ = 5, Iter = 25;
  for (MatrixIndexT iter = 0; iter < Iter; iter++) {
    MatrixIndexT dimM = Base + Rand() % Rand_,
                 dimN = Base + Rand() % Rand_;  // M>=N.
...
static void UnitTestEigSymmetric() {
  for (MatrixIndexT iter = 0; iter < 5; iter++) {
    ...
    Matrix<Real> tmp(P);
    tmp.MulColsVec(real_eigs);  // P * eigs
    Matrix<Real> M2(dimM, dimM);
    M2.AddMatMat(1.0, tmp, kNoTrans, P, kTrans,
                 0.0);   // M2 = tmp * Pinv = P * eigs * P^T
...
static void UnitTestEig() {
  for (MatrixIndexT iter = 0; iter < 5; iter++) {
    ...
    {  // Check that the eigenvalues match up with the determinant.
      ...
      for (MatrixIndexT i = 0; i < dimM; i++)
        logdet_check += 0.5 * log(real_eigs(i) * real_eigs(i) +
                                  imag_eigs(i) * imag_eigs(i));
    ...
    Matrix<Real> M2(dimM, dimM);
    M2.AddMatMat(1.0, tmp, kNoTrans, Pinv, kNoTrans,
                 0.0);  // M2 = tmp * Pinv = P * D * Pinv.
  ...
  for (MatrixIndexT iter = 0; iter < 100; iter++) {
    ...
    switch (iter % 5) {
      ...
      case 1:  // general random symmetric matrix.
        InitRand(&S);
        break;
      default: {  // start with a random matrix; do decomposition; change the
                  // eigenvalues to
...
// TEMP!
template <typename Real>
static Real NonOrthogonality(const MatrixBase<Real> &M,
...
static Real NonDiagonalness(const SpMatrix<Real> &S) {
  ...
  for (MatrixIndexT i = 0; i < S.NumRows(); i++)
    for (MatrixIndexT j = 0; j <= i; j++) {
      if (i == j) {
        max_diag = std::max(max_diag, std::abs(S(i, j)));
      } else {
        max_offdiag = std::max(max_offdiag, std::abs(S(i, j)));
      }
    }
  if (max_diag == 0.0) {
    if (max_offdiag == 0.0)
      return 0.0;  // perfectly diagonal.
    else
      return 1.0;  // perfectly non-diagonal.
  } else {
...
static void UnitTestTridiagonalize() {
  {
    float tmp[5];
    tmp[4] = 1.0;
    cblas_Xspmv(1, 0.0, tmp + 2, tmp + 1, 1, 0.0, tmp + 4, 1);
...
static void UnitTestMmul() {
  for (MatrixIndexT iter = 0; iter < 5; iter++) {
    MatrixIndexT dimM = 20 + Rand() % 10, dimN = 20 + Rand() % 10,
                 dimO = 20 + Rand() % 10;  // dims between 10 and 20.
...
static void UnitTestMmulSym() {
  ...
  for (MatrixIndexT iter = 0; iter < 5; iter++) {
    ...
    Matrix<Real> A(dimM, dimM), B(dimM, dimM), C(dimM, dimM), tmp(dimM, dimM),
        tmp2(dimM, dimM);
    ...
    tmp.CopyFromMat(A);
    A.AddMat(1.0, tmp, kTrans);
    tmp.CopyFromMat(B);
    B.AddMat(1.0, tmp, kTrans);
    tmp.CopyFromMat(C);
    C.AddMat(1.0, tmp, kTrans);
...
static void UnitTestAddVecVec() {
  for (int32 i = 0; i < 20; i++) {
    ...
    for (int32 j = 0; j < 20; j++) {
      ...
      AssertEqual(M(dimX, dimY), N(dimX, dimY) + alpha * v(dimX) * w(dimY));
    }
  }
}
...
static void UnitTestVecmul() {
  for (MatrixIndexT iter = 0; iter < 5; iter++) {
    ...
    MatrixIndexT dimM = 20 + Rand() % 10,
                 dimN = 20 + Rand() % 10;  // dims between 10 and 20.
...
static void UnitTestSpLogExp() {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    B.AddMat2(1.0, M, kNoTrans,
              0.0);  // B = M*M^T -> positive definite (since M nonsingular).
...
static void UnitTestResize() {
  for (size_t i = 0; i < 10; i++) {
    MatrixIndexT dimM1 = Rand() % 10, dimN1 = Rand() % 10, dimM2 = Rand() % 10,
                 dimN2 = Rand() % 10;
    ...
    for (MatrixIndexT j = 0; j < 3; j++) {
      ...
      if (resize_type == kSetZero) {
        ...
        for (MatrixIndexT i = 0; i < dimM2; i++) {
          if (i < dimM1)
            AssertEqual(v(i), vcopy(i));
          else
            KALDI_ASSERT(v(i) == 0);
          for (MatrixIndexT j = 0; j < dimN2; j++) {
            if (i < dimM1 && j < dimN1)
              AssertEqual(M(i, j), Mcopy(i, j));
            else
              AssertEqual(M(i, j), 0.0);
          }
          for (MatrixIndexT i2 = 0; i2 < dimM2; i2++) {
            if (i < dimM1 && i2 < dimM1)
              AssertEqual(S(i, i2), Scopy(i, i2));
            else
              AssertEqual(S(i, i2), 0.0);
          }
        }
      }
    }
...
static void UnitTestAddDiagMat2() {
  for (MatrixIndexT iter = 0; iter < 4; iter++) {
    MatrixIndexT dimM = 10 + Rand() % 3, dimN = 1 + Rand() % 4;
    Vector<Real> v(dimM);
    InitRand(&v);
    Vector<Real> w(v);
    if (iter % 2 == 1) {
...
static void UnitTestAddDiagMatMat() {
  for (MatrixIndexT iter = 0; iter < 4; iter++) {
    ...
    MatrixIndexT dimM = 10 + Rand() % 3, dimN = 5 + Rand() % 4;
    Vector<Real> v(dimM);
...
static void UnitTestTpInvert() {
  for (MatrixIndexT i = 0; i < 30; i++) {
    ...
    for (MatrixIndexT i = 0; i < M.NumRows(); i++) {
      for (MatrixIndexT j = 0; j < i; j++) M(i, j) = RandGauss();
      M(i, i) = 20 * std::max((Real)0.1, (Real)RandGauss());  // make sure
                                                              // invertible by
                                                              // making it
                                                              // diagonally
                                                              // dominant (-ish)
...
static void UnitTestLimitCondInvert() {
  for (MatrixIndexT i = 0; i < 10; i++) {
    ...
    InitRand(&X);
    B.AddMat2(1.0, X, kNoTrans, 0.0);  // B = X*X^T -> positive definite (almost
                                       // certainly), since N > M.
    ...
    Matrix<Real> I(dimM, dimM);
    I.AddMatMat(1.0, Bf, kNoTrans, B2f, kNoTrans, 0.0);
...
static void UnitTestFloorChol() {
  for (MatrixIndexT i = 0; i < 10; i++) {
    ...
    for (MatrixIndexT j = 0; j < 10; j++) {
      ...
      if (i < 3)
        KALDI_LOG << "alpha = " << alpha << ", ip_a = " << ip_a
                  << " ip_b = " << ip_b << " ip_c = " << ip_c << '\n';
...
static void UnitTestFloorUnit() {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    Vector<Real> s2(dimM);
    Matrix<Real> P2(dimM, dimM);
    BFloored.SymPosSemiDefEig(&s2, &P2);

    KALDI_ASSERT((s.Min() >= floor && std::abs(s2.Min() - s.Min()) < 0.01) ||
                 std::abs(s2.Min() - floor) < 0.01);
  }
}
...
static void UnitTestSimple() {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    InitRand(&S);
    Matrix<Real> N(S);
    KALDI_ASSERT(!N.IsDiagonal());  // technically could be diagonal, but almost
                                    // infinitely unlikely.
    ...
    {
      Vector<Real> V2(V);
      for (MatrixIndexT i = 0; i < V2.Dim(); i++) V2(i) = exp(V2(i));
      V.ApplyExp();
      AssertEqual(V, V2);
    }
    {
      ...
      for (MatrixIndexT i = 0; i < N.NumRows(); i++)
        for (MatrixIndexT j = 0; j < N.NumCols(); j++) N2(i, j) = exp(N2(i, j));
      N3.ApplyExp();
      AssertEqual(N2, N3);
    }
...
static void UnitTestIo() {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    if (i == 0) {
      dimM = 0;
      dimN = 0;  // test case when both are zero.
    }
    Matrix<Real> M(dimM, dimN);
    InitRand(&M);
    Matrix<Real> N;
...
template <typename Real>
static void UnitTestIoCross() {  // across types.

  typedef typename OtherReal<Real>::Real
      Other;  // e.g. if Real == float, Other == double.
  for (MatrixIndexT i = 0; i < 5; i++) {
    MatrixIndexT dimM = Rand() % 10 + 1;
    MatrixIndexT dimN = Rand() % 10 + 1;
    bool binary = (i % 2 == 0);
...
template <typename Real>
static void UnitTestRange() {  // Testing SubMatrix class.
  ...
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    MatrixIndexT dimMEnd = dimMStart + 1 + (Rand() % 10);
    if (dimMEnd > dimM) dimMEnd = dimM;
    MatrixIndexT dimNEnd = dimNStart + 1 + (Rand() % 10);
    if (dimNEnd > dimN) dimNEnd = dimN;

    SubMatrix<Real> sub(M, dimMStart, dimMEnd - dimMStart, dimNStart,
                        dimNEnd - dimNStart);

    KALDI_ASSERT(sub.Sum() ==
                 M.Range(dimMStart, dimMEnd - dimMStart, dimNStart,
                         dimNEnd - dimNStart).Sum());
  ...
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    MatrixIndexT lenEnd = lenStart + 1 + (Rand() % 10);
...
static void UnitTestAddMatSmat() {
  for (MatrixIndexT i = 0; i < 6; i++) {
    MatrixIndexT dimM = (Rand() % 10) + 1, dimN = (Rand() % 10 + 1),
                 dimO = (Rand() % 10 + 1);
    MatrixTransposeType transB = (i % 2 == 0 ? kTrans : kNoTrans),
                        transC = (i % 3 == 0 ? kTrans : kNoTrans);
    Matrix<Real> A(dimM, dimN), B(dimM, dimO), C(dimO, dimN);
    A.SetRandn();
    B.SetRandn();
    C.SetRandn();
...
static void UnitTestAddMat2() {
  ...
  for (MatrixIndexT i = 0; i < 5; i++) {
    MatrixIndexT dimM = (Rand() % 10) + extra, dimN = (Rand() % 10) + extra;
    Real alpha = 0.2 * (Rand() % 6), beta = 0.2 * (Rand() % 6);
    SpMatrix<Real> S(dimM);
    S.SetRandn();
    MatrixTransposeType trans = (i % 2 == 1 ? kTrans : kNoTrans),
  ...
  for (MatrixIndexT i = 0; i < 5; i++) {
...

    {
      Real a1 = TraceMatMat(M2, G, kTrans),
           ...
           a4 = a1 - 0.5 * a2 - 0.5 * a3, b4 = b1 - 0.5 * b2 - 0.5 * b3;
...
static void UnitTestMax2() {
  for (MatrixIndexT i = 0; i < 2; i++) {
    ...
    for (MatrixIndexT r = 0; r < M; r++)
      for (MatrixIndexT c = 0; c < N; c++) C(r, c) = std::max(A(r, c), B(r, c));
    D.CopyFromMat(A);
    D.Max(B);
    AssertEqual(C, D);
  }
...
static void UnitTestLbfgs() {
  ...
  for (MatrixIndexT iter = 0; iter < 3; iter++) {
    ...
    Real c = 0.01;
    Real sign =
        (minimize ? -1.0 : 1.0);  // function has a maximum not minimum..
    ...
    KALDI_VLOG(2) << "L-BFGS finished after " << num_iters
...
static void UnitTestLinearCgd() {
  for (int i = 0; i < 20; i++) {
    ...
    if (Rand() % 2 == 0) opts.max_iters = 1 + Rand() % 10;
    if (Rand() % 2 == 0)
...
static void UnitTestTrace() {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    Real t1 = TraceMat(ABC), t2 = ABC.Trace(), t3 = TraceMatMat(A, BC),
         t4 = TraceMatMat(AT, BC, kTrans), t5 = TraceMatMat(BC, AT, kTrans),
    ...
    ABC2.AddMatMatMat(0.25, AT, kTrans, B, kNoTrans, C, kNoTrans,
                      2.0);  // the extra 1.0 means another 0.25.
    ...
    Real tol = 0.001;
    KALDI_ASSERT((std::abs(t1 - t2) < tol) && (std::abs(t2 - t3) < tol) &&
  ...
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    Real u2 =
        TraceMatMatMatMat(M, kNoTrans, tM, kNoTrans, O, kTrans, sM, kNoTrans);
...
static void UnitTestSplitRadixComplexFft() {
  ...
  for (MatrixIndexT N_ = 0; N_ < 30; N_ += 3) {
    ...
    for (MatrixIndexT p = 0; p < 3; p++) {
      Vector<Real> v(twoN), w_base(twoN), w_alg(twoN), x_base(twoN),
          x_alg(twoN);
...
static void UnitTestSplitRadixRealFft() {
  for (MatrixIndexT p = 0; p < 30; p++) {
    MatrixIndexT logn = 2 + Rand() % 11, N = 1 << logn;
...
template <typename Real>
static void UnitTestMatrixExponential() {
  for (MatrixIndexT p = 0; p < 10; p++) {
    ...
    {  // work out largest eig.
      ...
      for (MatrixIndexT i = 0; i < dim; i++) {
        Real abs_eig = std::sqrt(real_eigs(i) * real_eigs(i) +
                                 imag_eigs(i) * imag_eigs(i));
    ...
    for (MatrixIndexT i = 0; i < 52;
         i++) {  // since max-eig = 0.5 and 2**52 == dbl eps.
...
static void UnitTestMatrixExponentialBackprop() {
  for (MatrixIndexT p = 0; p < 10; p++) {
    ...
    double f_diff2 =
        TraceMatMat(Mdash, delta, kTrans);  // computed "analytically"
...
static void UnitTestPca(bool full_test) {
  ...
  for (MatrixIndexT i = 0; i < 10; i++) {
    bool exact = i % 2 == 0;
    MatrixIndexT true_dim = (full_test ? 200 : 50) +
                            Rand() % 5,  // dim of subspace points live in
        feat_dim = true_dim + Rand() % 5,    // dim of feature space
        num_points = true_dim + Rand() % 5,  // number of training points.
        G = std::min(feat_dim,
                     std::min(num_points, static_cast<MatrixIndexT>(
                                              true_dim + Rand() % 5)));
...
static void UnitTestPca2(bool full_test) {
  for (MatrixIndexT i = 0; i < 5; i++) {
    ...
    MatrixIndexT feat_dim = (full_test ? 600 : 100),
                 num_points =
                     (i % 2 == 0 ? feat_dim * 2 : feat_dim / 2);  // test
...

    {
      ...
      AssertEqual(ScatterProjU.Trace(), ScatterProjU2.Trace(),
                  0.1);  // Algorithm is
...
static void UnitTestSvdSpeed() {
  ...
  for (size_t i = 0; i < sizes.size(); i++) {
    MatrixIndexT size = sizes[i];
    {
      ...
      KALDI_LOG << "For size " << size << ", Eig without eigenvectors took "
                << diff << " seconds.";
    }
    {
      start = time(NULL);
      ...
      double diff = difftime(end, start);
      KALDI_LOG << "For size " << size << ", Eig with eigenvectors took "
                << diff << " seconds.";
    }
    {
      start = time(NULL);
      ...
      double diff = difftime(end, start);
      KALDI_LOG << "For size " << size << ", SVD without eigenvectors took "
                << diff << " seconds.";
    }
    {
      start = time(NULL);
      ...
      double diff = difftime(end, start);
      KALDI_LOG << "For size " << size << ", SVD with eigenvectors took "
                << diff << " seconds.";
    }
  }
}
...
static void UnitTestCompressedMatrix() {
  ...
  for (MatrixIndexT n = 0; n < num_tot; n++) {
    ...
    if (Rand() % 2 == 0 &&
        num_rows != 0) {  // set one row to all the same value,
    ...
    {  // Check that when compressing a matrix that has already been compressed,
      ...
      if (!M2.ApproxEqual(M3, 1.0e-05)) {
        ...
        KALDI_ERR << "Matrices differ " << M2 << " vs. " << M3
                  << ", M2 range is " << M2.Min() << " to " << M2.Max()
                  << ", M3 range is " << M3.Min() << " to " << M3.Max();
      }
    }
    ...
    if (num_rows != 0 && num_cols != 0) {
      MatrixIndexT sub_row_offset =
                       (num_rows == 1 ? 0 : Rand() % (num_rows - 1)),
                   sub_col_offset =
                       (num_cols == 1 ? 0 : Rand() % (num_cols - 1));
    ...
    KALDI_LOG << "Compressed matrix, tot = " << tot << ", diff = " << err;
    if (err > 0.01 * tot) {
...
static void UnitTestRandCategorical() {
  int32 N = 1 + Rand() % 10;
  Vector<Real> vec(N);
  for (int32 n = 0; n < N; n++) vec(n) = Rand() % 3;
  if (vec.Sum() == 0) vec(0) = 2.0;
  Real sum = vec.Sum();
  int32 num_samples = 100000;
  std::vector<int32> counts(N, 0);
  for (int32 i = 0; i < num_samples; i++) counts[vec.RandCategorical()]++;
  for (int32 n = 0; n < N; n++) {
    Real a = counts[n] / (1.0 * num_samples), b = vec(n) / sum;
    KALDI_LOG << "a = " << a << ", b = " << b;
    KALDI_ASSERT(fabs(a - b) <=
                 0.1);  // pretty arbitrary.  Will increase #samp if fails.
  }
}
...
static void UnitTestTopEigs() {
  for (MatrixIndexT i = 0; i < 2; i++) {
    ...
    for (MatrixIndexT i = 0; i < dim; i++)
      for (MatrixIndexT j = 0; j <= i; j++) mat(i, j) = RandGauss();
    ...
    // Now check that eigs are close to real top eigs.
    {
      ...
      KALDI_LOG << "real sum is " << real_sum
