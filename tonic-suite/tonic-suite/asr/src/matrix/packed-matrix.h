...
class PackedMatrix {
 ...
 public:
  ...
  explicit PackedMatrix(const PackedMatrix<OtherReal> &orig)
      : data_(NULL) {
  ...
  Real Max() const {
    ...
    return *(std::max_element(data_,
                              data_ + ((num_rows_ * (num_rows_ + 1)) / 2)));
  }

  Real Min() const {
    KALDI_ASSERT(num_rows_ > 0);
    return *(std::min_element(data_,
                              data_ + ((num_rows_ * (num_rows_ + 1)) / 2)));
  }
 ...
 private:
  /// Init assumes the current contents of the class are is invalid (i.e. junk
  /// or
