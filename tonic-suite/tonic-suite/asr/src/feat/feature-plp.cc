...
Plp::~Plp() {
  ...
  for (std::map<BaseFloat, Vector<BaseFloat> *>::iterator iter =
           equal_loudness_.begin();
...
void Plp::Compute(const VectorBase<BaseFloat> &wave, BaseFloat vtln_warp,
                  Matrix<BaseFloat> *output,
                  Vector<BaseFloat> *wave_remainder) {
  const MelBanks *mel_banks = GetMelBanks(vtln_warp);
  const Vector<BaseFloat> *equal_loudness = GetEqualLoudness(vtln_warp);
  ComputeInternal(wave, *mel_banks, *equal_loudness, output, wave_remainder);
}
...
void Plp::Compute(const VectorBase<BaseFloat> &wave, BaseFloat vtln_warp,
  ...
  const Vector<BaseFloat> *equal_loudness =
      GetEqualLoudness(vtln_warp, *mel_banks, &must_delete_equal_loudness);

  ComputeInternal(wave, *mel_banks, *equal_loudness, output, wave_remainder);

  if (must_delete_mel_banks) delete mel_banks;
  if (must_delete_equal_loudness) delete equal_loudness;
}
...
void Plp::ComputeInternal(const VectorBase<BaseFloat> &wave,
  ...
  KALDI_ASSERT(opts_.num_ceps <=
               opts_.lpc_order + 1);      // our num-ceps includes C0.
  for (int32 r = 0; r < rows_out; r++) {  // r is frame index..
    ...
    energy = std::max(energy, std::numeric_limits<BaseFloat>::min());
    ...
    if (opts_.cepstral_scale != 1.0) final_cepstrum.Scale(opts_.cepstral_scale);
