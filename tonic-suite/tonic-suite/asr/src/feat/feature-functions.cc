...
int32 NumFrames(int32 nsamp, const FrameExtractionOptions &opts) {
...
void ExtractWindow(const VectorBase<BaseFloat> &wave,
  ...
  if (opts.snip_edges) {
    ...
    int32 mid = frame_shift * (f + 0.5), begin = mid - frame_length / 2,
          end = begin + frame_length, begin_limited = std::max<int32>(0, begin),
  ...
  if (window->Dim() != frame_length_padded) window->Resize(frame_length_padded);
  ...
  if (opts.preemph_coeff != 0.0) Preemphasize(&window_part, opts.preemph_coeff);
...
void ExtractWaveformRemainder(const VectorBase<BaseFloat> &wave,
  ...
  if (remaining_len > 0)
    wave_remainder->CopyFromVec(
        SubVector<BaseFloat>(wave, offset, remaining_len));
}
...
void ComputePowerSpectrum(VectorBase<BaseFloat> *waveform) {
  ...
  // KALDI_ASSERT(dim > 0 && (dim & (dim-1) == 0));  // make sure a power of
  // two.. actually my FFT code
  // does not require this (dan) but this is better in case we use different
  // code [dan].

  // RealFft(waveform, true);  // true == forward (not inverse) FFT; makes no
  // difference here,
  ...
  BaseFloat first_energy = (*waveform)(0) * (*waveform)(0),
            last_energy =
                (*waveform)(1) * (*waveform)(1);  // handle this special case
  ...
  (*waveform)(half_dim) =
      last_energy;  // Will actually never be used, and anyway
  // if the signal has been bandlimited sensibly this should be zero.
}

DeltaFeatures::DeltaFeatures(const DeltaFeaturesOptions &opts) : opts_(opts) {
  KALDI_ASSERT(opts.order >= 0 &&
               opts.order < 1000);  // just make sure we don't get binary junk.
  // opts will normally be 2 or 3.
  KALDI_ASSERT(opts.window > 0 &&
               opts.window < 1000);  // again, basic sanity check.
  ...
  for (int32 i = 1; i <= opts.order; i++) {
    Vector<BaseFloat> &prev_scales = scales_[i - 1], &cur_scales = scales_[i];
...
void DeltaFeatures::Process(const MatrixBase<BaseFloat> &input_feats,
  ...
  int32 num_frames = input_feats.NumRows(), feat_dim = input_feats.NumCols();
  KALDI_ASSERT(static_cast<int32>(output_frame->Dim()) ==
               feat_dim * (opts_.order + 1));
  ...
  for (int32 i = 0; i <= opts_.order; i++) {
    ...
    for (int32 j = -max_offset; j <= max_offset; j++) {
      ...
      if (scale != 0.0) output.AddVec(scale, input_feats.Row(offset_frame));
    }
  }
}

ShiftedDeltaFeatures::ShiftedDeltaFeatures(
    const ShiftedDeltaFeaturesOptions &opts)
    : opts_(opts) {
...
void ComputeDeltas(const DeltaFeaturesOptions &delta_opts,
  ...
  output_features->Resize(input_features.NumRows(),
                          input_features.NumCols() * (delta_opts.order + 1));
...
void ComputeShiftedDeltas(const ShiftedDeltaFeaturesOptions &delta_opts,
                          const MatrixBase<BaseFloat> &input_features,
                          Matrix<BaseFloat> *output_features) {
  output_features->Resize(
      input_features.NumRows(),
      input_features.NumCols() * (delta_opts.num_blocks + 1));
...
BaseFloat ComputeLpc(const VectorBase<BaseFloat> &autocorr_in,
  ...
  BaseFloat ans = Durbin(n, autocorr_in.Data(), lpc_out->Data(), tmp.Data());
  if (ans <= 0.0) KALDI_WARN << "Zero energy in LPC computation";
...
void SpliceFrames(const MatrixBase<BaseFloat> &input_features,
  ...
  if (T == 0 || D == 0) KALDI_ERR << "SpliceFrames: empty input";
  ...
  for (int32 t = 0; t < T; t++) {
    ...
    for (int32 j = 0; j < N; j++) {
      ...
      if (t2 < 0) t2 = 0;
      if (t2 >= T) t2 = T - 1;
      SubVector<BaseFloat> dst(dst_row, j * D, D), src(input_features, t2);
      dst.CopyFromVec(src);
    }
  }
}
...
void ReverseFrames(const MatrixBase<BaseFloat> &input_features,
                   Matrix<BaseFloat> *output_features) {
  int32 T = input_features.NumRows(), D = input_features.NumCols();
  if (T == 0 || D == 0) KALDI_ERR << "ReverseFrames: empty input";
...
void SlidingWindowCmnOptions::Check() const {
  ...
  if (center) KALDI_ASSERT(min_window > 0 && min_window <= cmn_window);
...
void SlidingWindowCmnInternal(const SlidingWindowCmnOptions &opts,
  ...
  for (int32 t = 0; t < num_frames; t++) {
    ...
    if (!opts.center) {
      if (window_end > t) window_end = std::max(t + 1, opts.min_window);
    }
    ...
    if (last_window_start == -1) {
      SubMatrix<double> input_part(input, window_start,
                                   window_end - window_start, 0, dim);
      ...
      if (window_start > last_window_start) {
        ...
        if (opts.normalize_variance) cur_sumsq.AddVec2(-1.0, frame_to_remove);
      }
      if (window_end > last_window_end) {
        ...
        if (opts.normalize_variance) cur_sumsq.AddVec2(1.0, frame_to_add);
      }
    }
    ...
    if (opts.normalize_variance) {
      if (window_frames == 1) {
        ...
        if (num_floored > 0 && num_frames > 1) {
          KALDI_WARN << "Flooring variance When normalizing variance, floored "
