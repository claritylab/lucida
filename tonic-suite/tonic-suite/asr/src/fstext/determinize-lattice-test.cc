...
void TestLatticeStringRepository() {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    vector<IntType> prefix, prefix2(kaldi::Rand() % 10), prefix3;
    ...
    if (str.size() > prefix.size()) assert(!sr.IsPrefixOf(e, g));
  }
}
...
void TestDeterminizeLattice() {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    try {
      ...
      if (!DeterminizeLattice<TropicalWeight, int32>(*fst, &det_fst, lat_opts,
                                                     NULL))
      ...
      {
        FstPrinter<CompactArc> fstprinter(compact_fst, NULL, NULL, NULL, false,
                                          true);
        fstprinter.Print(&std::cout, "standard output");
      }
      if (kaldi::Rand() % 2 == 1)
        ConvertLattice<Weight, Int>(det_fst, &compact_det_fst, false);
      else if (!DeterminizeLattice<TropicalWeight, int32>(
                   *fst, &compact_det_fst, lat_opts, NULL))
        throw std::runtime_error("could not determinize");

      std::cout << "Compact version of determinized FST is:\n";
      {
        FstPrinter<CompactArc> fstprinter(compact_det_fst, NULL, NULL, NULL,
                                          false, true);
        fstprinter.Print(&std::cout, "standard output");
      }

      assert(RandEquivalent(compact_det_fst, compact_fst, 5 /*paths*/,
