...
inline typename Arc::Weight ComputeTotalWeight(ExpandedFst<Arc> &fst,
  ...
  while (!q.empty()) {
    ...
    for (ArcIterator<Fst<Arc> > aiter(fst, s); !aiter.Done(); aiter.Next()) {
      ...
      cur_delta[arc.nextstate] =
          Plus(Weight(cur_delta[arc.nextstate]), next_weight).Value();
      if (!queued[arc.nextstate] &&
          !ApproxEqual(Weight(cur_tot[arc.nextstate]),
                       Plus(Weight(cur_delta[arc.nextstate]),
                            Weight(cur_tot[arc.nextstate])),
                       delta)) {
    ...
    if (final != Weight::Zero()) {
      ...
      if (total_final.Value() < max_weight.Value()) {  // Note: this means that
                                                       // total_final is MORE
                                                       // THAN max_weight.
...
inline LogWeight RescaleToStochastic(MutableFst<LogArc> *fst,
                                     float approx_delta, float delta) {
  ...
  while (1) {
    ...
    std::cerr << "Current rescaling factor is " << cur_rescale
              << ", total is: " << cur_tot << '\n';
    ...
    if (ApproxEqual(cur_tot, Weight::One()))
      return cur_rescale;                // Succeeded.
      ...
      // cur_tot != max.
      if (last_tot == Weight::Zero() ||
          last_tot == max) {  // No derivative information.
        if (cur_tot.Value() > 0)
          factor = Weight(-0.01);  // enough to approximate derivative.
        else
          factor = Weight(0.01);
      } else {
        ...
        if (dtot_drescale < 1.0) {
          LOG(WARN) << "Rescale: derivative does not have expected value: "
                    << dtot_drescale;
          if (cur_tot.Value() > 0)
            factor = Weight(-0.01);  // enough to approximate derivative.
          else
          ...
          factor = Weight(-cur_tot.Value() /
                          dtot_drescale);  // want to get the value to zero.
    ...
    if (have_lower && have_upper) {
      float oldval = cur_rescale.Value(), newval = oldval + factor.Value();
