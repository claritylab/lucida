...
class StringRepository {
  ...
  // StringId
  // representation of sequences of Labels.  It is to save memory, and to save
  // compute.
 ...
 public:
  ...
  class VectorEqual {  // Equality-operator function object.
   public:
    size_t operator()(const vector<Label> *vec1,
                      const vector<Label> *vec2) const {
      return (*vec1 == *vec2);
    }
  };

  typedef unordered_map<const vector<Label> *, StringId, VectorKey, VectorEqual>
      MapType;
  ...
  StringRepository() {
    ...
    // compilation so make them
    // class variables.  Due to the brokenness of <limits>, they can't be
    // accessed as constants.
    string_end = (numeric_limits<StringId>::max() / 2) -
                 1;  // all hash values must be <= this.
    no_symbol =
        (numeric_limits<StringId>::max() / 2);  // reserved for empty sequence.
    single_symbol_start = (numeric_limits<StringId>::max() / 2) + 1;
    single_symbol_range = numeric_limits<StringId>::max() - single_symbol_start;
  }
  void Destroy() {
    for (typename vector<vector<Label> *>::iterator iter = vec_.begin();
         iter != vec_.end(); ++iter)
      delete *iter;
...
class DeterminizerStar {
 ...
 private:
  ...
  void EpsilonClosure(const vector<Element> &input_subset,
    ...
    typedef typename std::map<InputStateId, Element>::iterator MapIter;
    {
      ...
      for (size_t i = 0; i < input_subset.size(); i++) {
        std::pair<const InputStateId, Element> pr(input_subset[i].state,
                                                  input_subset[i]);
    ...
    while (queue.size() != 0) {
      ...
      for (ArcIterator<Fst<Arc> > aiter(*ifst_, elem.state); !aiter.Done();
        ...
        if (arc.ilabel == 0) {  // Epsilon transition.
          ...
          typename std::map<InputStateId, Element>::iterator iter =
          ...
          if (iter == cur_subset.end()) {
            ...
            if (iter->second.string != next_elem.string) {
              std::cerr << "DeterminizerStar: FST was not functional -> not "
                           "determinizable\n";
            ...
            if (!ApproxEqual(weight, iter->second.weight,
                             delta_)) {  // add extra part of weight to queue.
              queue.push_back(next_elem);
              replaced_elems = true;
            }
  ...
  void ProcessTransition(OutputStateId state, Label ilabel,
    ...
    // sorted
    // order).  This function removes duplicates from "subset", normalizes it,
    // and adds
    // a transition to the dest. state (possibly affecting Q_ and hash_, if
    // state did not
    // exist).
    ...
    {  // This block makes the subset have one unique Element per state, adding
      ...
      while (cur_in != end) {  // while we have more elements to process.
        // At this point, cur_out points to location of next place we want to
        // put an element,
  ...
  // sorting them
  // using a lexicographical ordering, and calling ProcessTransition for each
  // range
  ...
  // output
  // fst.  This is a hash lookup; if no such state exists, it adds a new state
  // to the hash
  ...
  OutputStateId SubsetToStateId(
      const vector<Element> &subset) {  // may add the subset to the queue.
    ...
    if (iter == hash_.end()) {  // was not there.
      ...
      if (allow_partial_ == false) {
        ...
        Q_.push_front(
            pair<vector<Element> *, OutputStateId>(new_subset, new_state_id));
      } else {
  ...
  // necessary.
  // The first stage is "EpsilonClosure" (follow epsilons to get a possibly
  // larger set
  // of (states, weights)).  After that we ignore epsilons.  We process the
  // final-weight
  // of the state, and then handle transitions out (this may add more
  // determinized states
  // to the queue).
  ...
  void Debug() {  // this function called if you send a signal
    ...
    std::cerr << "Traceback below (or on standard error) in format ilabel "
                 "(olabel olabel) ilabel (olabel) ...\n";
    for (ssize_t i = traceback.size() - 1; i >= 0; i--) {
      std::cerr << traceback[i].first << ' ' << "( ";
      vector<Label> seq;
      repository_.SeqOfId(traceback[i].second, &seq);
      for (size_t j = 0; j < seq.size(); j++) std::cerr << seq[j] << ' ';
      std::cerr << ") ";
    }
    std::cerr << '\n';
    exit(1);
  ...
  deque<pair<vector<Element> *, OutputStateId> >
...
template <class Arc>
bool DeterminizeStar(Fst<Arc> &ifst, MutableFst<Arc> *ofst, float delta,
