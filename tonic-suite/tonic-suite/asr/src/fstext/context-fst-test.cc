...
// GenAcceptorFromSequence generates a linear acceptor (identical input+output
// symbols) that has this
// sequence of symbols, and
template <class Arc>
static VectorFst<Arc> *GenAcceptorFromSequence(
    const vector<typename Arc::Label> &symbols, float cost) {
  ...
  {  // compute split_cost.  it must sum to "cost".
    ...
    while (indices.size() < num_indices)
      indices.insert(kaldi::Rand() % split_cost.size());
    for (std::set<int32>::iterator iter = indices.begin();
...
template <class Arc>
static float CheckPhones(
    const VectorFst<Arc> &linear_fst,
    ...
    const vector<vector<typename Arc::Label> > &ilabel_info, int N, int P) {
  ...
  Weight tot_cost;
  bool ans =
      GetLinearSymbolSequence(linear_fst, &input_syms, &output_syms, &tot_cost);
  ...
  for (size_t i = 0; i < input_syms.size(); i++) {
    ...
    if ((ilabel_info[isym].size() == 1 && ilabel_info[isym][0] <= 0))
      continue;  // disambig.
    input_syms_long.push_back(ilabel_info[isym]);
  }

  for (size_t i = 0; i < input_syms_long.size(); i++) {
    vector<int32> phone_context_window(
        N);  // phone at pos i will be at pos P in this window.
    int pos =
        ((int)i) - P;  // pos of first phone in window [ may be out of range] .
    for (int j = 0; j < N; j++, pos++) {
      if (static_cast<size_t>(pos) < phone_seq.size())
        phone_context_window[j] = phone_seq[pos];
      else
        phone_context_window[j] =
            0;  // 0 is a special symbol that context-dep-itf expects to see
      // when no phone is present due to out-of-window.  context-fst knows about
      // this too.
    }
...
template <class Arc>
static VectorFst<Arc> *GenRandPhoneSeq(
    vector<typename Arc::Label> &phone_syms,
    vector<typename Arc::Label> &disambig_syms,
    typename Arc::Label subsequential_symbol, int num_subseq_syms,
    float seq_prob, vector<typename Arc::Label> *phoneseq_out) {
  ...
  size_t len =
      (kaldi::Rand() % 4) * (kaldi::Rand() % 3);  // up to 3*2=6 phones.
  float disambig_prob = 0.33;
  phoneseq_out->clear();
  vector<Label> syms;  // the phones
  for (size_t i = 0; i < len; i++) {
    while (kaldi::RandUniform() < disambig_prob)
      syms.push_back(disambig_syms[kaldi::Rand() % disambig_syms.size()]);
    Label phone_id = phone_syms[kaldi::Rand() % phone_syms.size()];
    phoneseq_out->push_back(
        phone_id);  // record in output the underlying phone sequence.
...
static void TestContextFst(bool verbose, bool use_matcher) {
  ...
  while (phones_set.size() < num_phones)
    phones_set.insert(
        1 + kaldi::Rand() % (num_phones + 5));  // don't use 0 [== epsilon]
  vector<int32> phones;
  ...
  ContextFst<Arc> cfst(subsequential_symbol, phones, disambig_syms, N, P);

  bool test_vec = (kaldi::Rand() % 2 == 0);
  VectorFst<Arc> *cfst_vec = NULL;
  if (test_vec) {
    cfst_vec = new VectorFst<Arc>(cfst);  // fully expand it.
    cfst_vec->SetInputSymbols(
        cfst.InputSymbols());  // because isymbols get changed
    // as it gets constructed.
  }

  if (verbose) {  // Try to print the fst.
    FstPrinter<Arc> fstprinter(cfst, cfst.InputSymbols(), cfst.OutputSymbols(),
                               NULL, false, true);
  ...
  */
  ...
  for (size_t p = 0; p < 10; p++) {
    ...
    int num_subseq =
        N - P -
        1;  // zero if P == N-1, i.e. P is last element, i.e. left-context only.
    float tot_cost = 20.0 * kaldi::RandUniform();
    VectorFst<Arc> *f =
        GenRandPhoneSeq<Arc>(phone_syms, disambig_syms, subsequential_symbol,
                             num_subseq, tot_cost, &phone_seq);
    if (verbose) {
      std::cout << "Sequence FST is:\n";
      {  // Try to print the fst.
        FstPrinter<Arc> fstprinter(*f, f->InputSymbols(), f->OutputSymbols(),
                                   NULL, false, true);
        fstprinter.Print(&std::cout, "standard output");
      }
    }
    ...
    if (use_matcher)
      ComposeContextFst(cfst, *f, &fst_composed);
    else
      Compose(cfst, *f, &fst_composed);

    if (test_vec) {
      Compose(*cfst_vec, *f, &fst_composed_vec);
      assert(RandEquivalent(fst_composed, fst_composed_vec, 5 /*paths*/,
    ...
    // Testing WriteILabelInfo and ReadILabelInfo.
    {
      ...
      WriteILabelInfo(kaldi::Output("tmpf", binary).Stream(), binary,
                      cfst.ILabelInfo());

      bool binary_in;
      vector<vector<int32> > ilabel_info;
      kaldi::Input ki("tmpf", &binary_in);
      ReadILabelInfo(ki.Stream(), binary_in, &ilabel_info);
    ...
    // because it creates a copy of the input symbols of cfst *as they existed
    // at the start*.
    // They get modified during the composition (assuming we didn't already
    // print out the FST)
    // because
    ...
    if (verbose) {
      ...
      {  // Try to print the fst.
        FstPrinter<Arc> fstprinter(fst_composed, fst_composed.InputSymbols(),
                                   fst_composed.OutputSymbols(), NULL, false,
                                   true);
        fstprinter.Print(&std::cout, "standard output");
      }
    }
    ...
    float tot_cost_check =
        CheckPhones<Arc>(fst_composed, phone_syms, disambig_syms, phone_seq,
                         cfst.ILabelInfo(), N, P);
