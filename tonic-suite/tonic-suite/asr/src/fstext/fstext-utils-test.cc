...
template <class Arc>
void TestSafeDeterminizeWrapper() {  // also tests
  ...
  int n_syms = 2 + kaldi::Rand() % 5, n_states = 3 + kaldi::Rand() % 10,
      n_arcs = 5 + kaldi::Rand() % 30,
      n_final = 1 + kaldi::Rand() % 3;  // Up to 2 unique symbols.
  cout << "Testing pre-determinize with " << n_syms << " symbols, " << n_states
       << " states and " << n_arcs << " arcs and " << n_final
       << " final states.\n";
  ...
  if (fst->Start() != kNoStateId) {  // "Connect" did not make it empty....
    ...
    // PushInLog<REWEIGHT_TO_INITIAL>(fst_det, kPushLabels);  // will always
    // succeed.
    KALDI_LOG << "Num states [orig]: " << fst->NumStates() << "[det]"
              << fst_det->NumStates();
    assert(RandEquivalent(*fst, *fst_det, 5 /*paths*/, 0.01 /*delta*/,
...
void TestPushInLog() {  // also tests SafeDeterminizeMinimizeWrapper().
  ...
  PushInLog<REWEIGHT_TO_INITIAL>(&fst2, kPushLabels | kPushWeights,
                                 0.01);  // speed it up using large delta.
  assert(RandEquivalent(*fst, fst2, 5 /*paths*/, 0.01 /*delta*/,
                        kaldi::Rand() /*seed*/, 100 /*path length-- max?*/));
...
// MakeLoopFstCompare is as MakeLoopFst but implmented differently [ less
// efficiently
// but more clearly], so we can check for equivalence.
template <class Arc>
VectorFst<Arc> *MakeLoopFstCompare(
    const vector<const ExpandedFst<Arc> *> &fsts) {
...
void TestMakeLoopFst() {
  ...
  vector<const ExpandedFst<Arc> *> fsts(num_fsts,
  ...
  VectorFst<Arc> *fst1 = MakeLoopFst(fsts), *fst2 = MakeLoopFstCompare(fsts);
...
void TestRemoveUselessArcs() {
  for (size_t i = 0; i < 4; i++) {
    ...
    VectorFst<Arc> orig_composed, nouseless_composed;
    ...
    VectorFst<Arc> orig_bestpath, nouseless_bestpath;
    ...
    GetLinearSymbolSequence<Arc, int>(nouseless_bestpath, NULL, NULL,
                                      &wnouseless);
    assert(ApproxEqual(worig, wnouseless, kDelta));

    // assert(RandEquivalent(orig_bestpath, nouseless_bestpath, 5/*paths*/,
    // 0.01/*delta*/, Rand()/*seed*/, 100/*path length-- max?*/));
    delete fst;
  }
}
...
int main() {
  for (int i = 0; i < 5; i++) {
    fst::TestMakeLinearAcceptor<fst::StdArc,
                                int>();  // this also tests
