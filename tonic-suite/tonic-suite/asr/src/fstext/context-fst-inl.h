...
ContextFstImpl<Arc, LabelT>::FindLabel(const vector<LabelT> &label_vec) {
  // Finds ilabel corresponding to this information.. Creates new ilabel if
  // necessary.
...
template <class Arc, class LabelT>
typename ContextFstImpl<Arc, LabelT>::StateId
ContextFstImpl<Arc, LabelT>::Start() {
...
ContextFstImpl<Arc, LabelT>::ContextFstImpl(const ContextFstImpl &other)
    : phone_syms_(other.phone_syms_), disambig_syms_(other.disambig_syms_) {
  std::cerr << "ContextFst copying not yet supported [not hard, but would have "
               "to test.]";
  exit(1);
}

template <class Arc, class LabelT>
ContextFstImpl<Arc, LabelT>::ContextFstImpl(
    Label subsequential_symbol,           // epsilon not allowed.
  ...
  {  // This block checks the inputs.
    ...
    if (phone_syms.empty())
      KALDI_WARN << "Context FST created but there are no phone symbols: "
  ...
  SetType("context");
  assert(subsequential_symbol_ !=
         0);  // it's OK to be kNoLabel though, if it never appears in ifst.

  assert(disambig_syms_.count(subsequential_symbol_) == 0 &&
         phone_syms_.count(subsequential_symbol_) == 0);
  ...
  assert(eps_id ==
         0);  // doing this in the initializer should guarantee it is zero.

  if (N > P + 1 && !disambig_syms_.empty()) {
    // We add in a symbol whose sequence representation is [ 0 ], and whose
    // symbol-id
    // is 1.  This is treated as a disambiguation symbol, we call it #-1 in
    // printed
    // form.  It is necessary to ensure that all determinizable LG's will have
    // determinizable
    // CLG's.  The problem it fixes is quite subtle-- it relates to reordering
    // of
    // disambiguation symbols (they appear earlier in CLG than in LG, relative
    // to phones),
    // and the fact that if a disambig symbol appears at the very start of a
    // sequence in
    // CLG, it's not clear exatly where it appeared on the corresponding
    // sequence at
    // the input of LG.
    vector<LabelT> pseudo_eps_vec;
    pseudo_eps_vec.push_back(0);
    pseudo_eps_symbol_ =
        FindLabel(pseudo_eps_vec);  // this function will add it to the input
    // symbol table, if necessary.
...
template <class Arc, class LabelT>
typename ContextFstImpl<Arc, LabelT>::Weight ContextFstImpl<Arc, LabelT>::Final(
    StateId s) {
  assert(static_cast<size_t>(s) <
         state_seqs_.size());  // make sure state exists already.
  if (!this->HasFinal(s)) {    // Work out final-state weight.
    ...
    if (P_ < N_ - 1) {
      /* Note that P_ (in zero based indexing) is the "central position", and
         for arcs out of
         this state the thing at P_ will be the one we expand.  If this is the
         subsequential symbol,
         it means we will output nothing (and will obviously never output
         anything).  Thus we make
         this state the final state.
      */
      final_ok = (seq[P_] == subsequential_symbol_);
    } else {
      /* If P_ == N_-1, then the "central phone" is the last one in the list (we
         have a left-context system).
         In this case everything is output immediately and there is no need for
         a subsequential symbol.
...
size_t ContextFstImpl<Arc, LabelT>::NumArcs(StateId s) {
  ...
  if (!seq.empty() && seq.back() == subsequential_symbol_) {
    // State is not a "normal" state because it just saw the subsequential
    // symbol,
    ...
    // For normal states, in general there is potentially an arc for each phone
    // and an arc
...
template <class Arc, class LabelT>
void ContextFstImpl<Arc, LabelT>::InitArcIterator(StateId s,
...
template <class Arc, class LabelT>
void ContextFstImpl<Arc, LabelT>::CreateDisambigArc(
    StateId s, Label olabel,
  ...
  label_info.push_back(
      -olabel);  // olabel is a disambiguation symbol.  Use its negative
...
template <class Arc, class LabelT>
bool ContextFstImpl<Arc, LabelT>::CreatePhoneOrEpsArc(
    StateId src, StateId dst, Label olabel, const vector<LabelT> &phone_seq,
    Arc *oarc) {
  ...
  // returns false if it could not create an arc due to the decision-tree
  // returning false
  // [this only happens if opts_.behavior_on_failure ==
  // ContextFstOptions::kNoArc].
...
// otherwise.
template <class Arc, class LabelT>
bool ContextFstImpl<Arc, LabelT>::CreateArc(StateId s, Label olabel,
                                            Arc *oarc) {
  ...
  if (IsDisambigSymbol(
          olabel)) {  // Disambiguation-symbol arcs.. create self-loop.
    CreateDisambigArc(s, olabel, oarc);
    return true;
  } else if (IsPhoneSymbol(olabel) || olabel == subsequential_symbol_) {
    // If all is OK, we shift the old sequence left by 1 and push on the new
    // phone.
    ...
    if (olabel == subsequential_symbol_ &&
      ...
      // to
      // accept any more, or we'd be making the subsequential symbol the central
      // phone.
      return false;
    }
    ...
    for (int i = 1; i < N_; i++)
      if (phoneseq[i] == subsequential_symbol_)
        phoneseq[i] = 0;  // don't put subseq. symbol on
    // the output arcs, just 0.
    return CreatePhoneOrEpsArc(s, nextstate, olabel, phoneseq, oarc);
  } else {
    std::cerr << "ContextFst: CreateArc, invalid olabel supplied [confusion "
...
template <class Arc, class LabelT>
void ContextFstImpl<Arc, LabelT>::Expand(
    StateId s) {  // expands arcs only [not final state weight].
  assert(static_cast<size_t>(s) <
         state_seqs_.size());  // make sure state exists already.
  ...
  for (typename kaldi::ConstIntegerSet<Label>::iterator iter =
           phone_syms_.begin();
...
template <class Arc, class LabelT>
ContextFst<Arc, LabelT>::ContextFst(const ContextFst<Arc, LabelT> &fst,
                                    bool reset) {
  if (reset) {
    ...
    // (e.g. then you could delete original one).  Might be useful in
    // transcription
...
bool ContextMatcher<Arc, LabelT>::Find(typename Arc::Label match_label) {
  ...
  // we know at this point that match_type_ == MATCH_OUTPUT.  we are matching
  // output.
  ...
  if (match_label == kNoLabel) {
    ...
    const ContextFst<Arc, LabelT> *cfst =
        static_cast<const ContextFst<Arc, LabelT> *>(
            fst_);  // we checked in initializer, that it is.
...
SymbolTable *CreateILabelInfoSymbolTable(
    const vector<vector<I> > &info, const SymbolTable &phones_symtab,
  ...
  for (size_t i = 1; i < info.size(); i++) {
    ...
    if (info[i].size() == 1 && info[i][0] <= 0) {
      ...
      for (size_t j = 0; j < info[i].size(); j++) {
        ...
        if (phonesym == "") {
          std::cerr << "CreateILabelInfoSymbolTable: symbol " << info[i][j]
      ...
      if (s != static_cast<int64>(i)) {
        std::cerr << "CreateILabelInfoSymbolTable: some problem with duplicate "
                     "symbols.";
        exit(1);
      }
    }
...
inline void ComposeContext(vector<int32> &disambig_syms_in, int N, int P,
  ...
  int32 subseq_sym = 1;
  if (!all_syms.empty()) subseq_sym = std::max(subseq_sym, all_syms.back() + 1);
  ...
  if (P != N - 1) AddSubsequentialLoop(subseq_sym, ifst);
