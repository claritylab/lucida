...
class LatticeDeterminizer {
 ...
 private:
      ...
      for (ArcIterator<Fst<Arc> > aiter(*ifst_, elem.state); !aiter.Done();
        ...
        if (arc.ilabel == 0 &&
          ...
          if (iter == cur_subset.end()) {
            ...
            // determinization we'd
            // add the weights.  Here, we find which one has the better weight,
            // and
  ...
  // ilabel,
  // and creates a new transition in the determinized FST for each unique
  // ilabel.
...
// to pre-size the hashes so we're not constantly rebuilding them.
    ...
    if (start_id != kNoStateId) {
      /* Insert determinized-state corresponding to the start state into hash
         and
         queue.  Unlike all the other states, we don't "normalize" the
         representation
