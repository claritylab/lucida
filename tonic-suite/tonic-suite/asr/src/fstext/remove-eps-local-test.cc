...
static void TestRemoveEpsLocal() {
  ...
  std::cout << "Number of states 0 = " << num_states_0
            << ", 1 = " << num_states_1 << '\n';

  assert(RandEquivalent(fst, fst_copy1, 5 /*paths*/, 0.01 /*delta*/,
...
static void TestRemoveEpsLocalSpecial() {
  ...
  {  // Make the FST stochastic.
    for (StateId s = 0; s < logfst->NumStates(); s++) {
      ...
      if (w != Weight::Zero()) {
        ...
        for (MutableArcIterator<Fst> aiter(logfst, s); !aiter.Done();
             aiter.Next()) {
          Arc a = aiter.Value();
  ...
  RemoveEpsLocalSpecial(
      &fst);  // removes eps in std-arc but keep stochastic in log-arc
  // make sure equivalent.
  assert(RandEquivalent(fst, fst_copy, 5 /*paths*/, 0.01 /*delta*/,
                        kaldi::Rand() /*seed*/, 100 /*path length-- max?*/));
