...
Weight WalkSinglePath(StdVectorFst *ifst,
  ...
  while (ifst->Final(isrc) == Weight::Zero()) {  // while not final
    ...
    if (dfst->GetArc(dsrc, iarc.olabel, &oarc)) {
      ...
      // cout << "  Matched label "<<iarc.olabel<<" at summed cost
      // "<<cost<<endl;
      totalCost = Times(totalCost, cost);
    } else {
      cout << "  Can't match arc [" << iarc.ilabel << "," << iarc.olabel << ","
           << iarc.weight << "] from " << isrc << endl;
      exit(1);
    }
    isrc = iarc.nextstate;
...
void TestBackoffAndCache() {
  ...
  for (StateIterator<StdVectorFst> riter(*rfst); !riter.Done(); riter.Next()) {
    ...
    for (ArcIterator<StdVectorFst> aiter(*rfst, rsrc); !aiter.Done();
         aiter.Next()) {
      ...
      if (dfst1.GetArc(rsrc, rarc.ilabel, &darc)) {
        ...
        cerr << "  Got same arc at state " << rsrc << ": " << rarc.ilabel << " "
             << darc.ilabel << endl;
      } else {
        cerr << "Couldn't find arc " << rarc.ilabel << " for state " << rsrc
             << endl;
        exit(1);
      }
    }
...
void TestCompose() {
  ...
  {  // Mostly checking for compilation errors here.
    ...
    KALDI_ASSERT(b && arc.nextstate == 1 && arc.ilabel == 100 &&
                 arc.olabel == 100 && arc.weight.Value() == 0.25);
  }
}
}
