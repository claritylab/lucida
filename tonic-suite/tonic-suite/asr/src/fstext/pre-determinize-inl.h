...
/*
  ...
  symbols on the input
  side as necessary for disambiguation.  Note that we do not treat epsilon as a
  real symbol
  when measuring determinizability in this sense.   The extra symbols are added
  to the vocabulary,
  on the input side; these are of the form (prefix)1, (prefix)2, and so on
  without limit, where
  (prefix) is some prefix the user provides, e.g. '#' (the function checks that
  this will not
  lead to conflicts with symbols already in the FST).  The function tells us how
  many such
  symbols it created.

   Note that there is a paper "Generalized optimization algorithm for speech
  recognition
   transducers" by Allauzen and Mohri, that deals with a similar issue, but this
  is a very
   different algorithm that only aims to ensure determinizability, but not
  *compact*
   determinizability.

   Our algorithm is slightly heuristic, and probably not optimal, but does
  ensure that the
   output is compactly determinizable, possibly at the expense of inserting
  unnecessary
   symbols.  We considered more sophisticated algorithms, but these were
  extremely
   complicated and would give the same output for the kinds of inputs that we
  envisage.

   Suppose the input FST is T.  We want to ensure that in det(T), if we consider
  the
   states of det(T) as weighted subsets of states of T, each state of T only
  appears once
   in any given subset.  This ensures that det(T) is no larger than T in an
  appropriate
   sense.  The way we do this is as follows.  We identify all states in T that
  have
   multiple input transitions (counting "being an initial state" as an input
  transition).
   Let's call these "problematic" states.  For a problematic state p we
  stipulate that it
   can never appear in any state of det(T) unless that state equals (p, \bar{1})
  [i.e. p,
   unweighted].  In order to ensure this, we insert input symbols on the
  ...
  to tell us
   the path through T (and we insert extra symbols sufficient to make this so).
  This is to
   simplify the algorithm, so that we don't have to consider the output symbols
  or weights
  ...
  states t
           that are reachable via sequences of arcs a such that i[a]=epsilon and
  n[a] is
           not problematic.
  ...
  table compiled into
         it, and a prefix (e.g. #) for symbols to be added.  We check that the
  input FST is trim,
         and that it does not have any symbols that appear on its arcs, that are
  equal to the prefix
         followed by digits.

    (ii) Outputs: The algorithm modifies the FST that is given to it, and
  returns the number of
         the highest numbered "extra symbol" inserted.  The extra symbols are
  numbered #1, #2 and
         so on without limit (as integers).  They are inserted into the symbol
  table in a sequential
  ...
  below, which is true
         if s is problematic.  This also requires, for efficiency, that the arcs
  be sorted on input
         label.
            ...
            Algorithm:
                ...
                while Q is nonempty:
                  ...
                  for each transition a from state s with epsilon on the input
  label [we can
  ...
  transitions as necessary).
           (c) Sort arcs on input label (so epsilon arcs are at the start of arc
  lists).
  ...
  array indexed by
           states, i.e.
             p(s)
           which is true if the state is problematic.  We can do this by
  constructing an array
           t(s) to store the number of transitions into each state [adding one
  for the initial state],
           and then setting p(s) = true if t(s) > 1.

           Also create a boolean array d(s), defined for states, and set d(s) =
  false.
           This array is purely for sanity-checking that we are processing each
  state exactly once.

     (iii) Set up an array of integers m(a), indexed by arcs (how exactly we
  store these is
           implementation-dependent, but this will probably be a hash from
  (state, arc-index) to
           integers.  m(a) will store the extra symbol, if any, to be added to
  that arc (or -1
           if no such symbol; we can also simply have the arc not present in the
  hash).  The
           initial value of m(a) is -1 (if array), or undefined (if hash).

      (iv) Initialize a set of sets-of-states S, and a queue of pairs Q, as
  follows.
            The pairs in Q are a pair of (set-of-states, integer), where the
  integer
  ...
  rather than
            using std::set.  We use a sorted vector of StateId's.  And in S, we
  index them
            by the lowest-numbered state-id.  Because each state is supposed to
  only ever
            be a member of one set, if there is an attempt to add another,
  different set
  ...
  i.e.:
               S_t = \{ a \in E[A]: i[a] = t \}
               We further define T_t and U_t as the subsets of S where the
  destination
  ...
  indexed by
               t that maps to a list of pairs (state, arc-offset) that stores
  S_t.
  ...
  (extra symbols
            start from zero, in this numbering which doesn't correspond to the
  symbol-table
            numbering).  Here we add n+1 extra symbols to the symbol table and
  store
  ...
  dest-state]:
                  create a new state t with a transition to n[a], with
  input-label #m(a) and
...
// set of states
// that are reachable from S via sequences of arcs a such that i[a]=epsilon and
// n[a] is
// not problematic.  We assume that the fst is sorted on input label (so epsilon
// arcs first)
// The algorithm is described in section (C) above.  We use the same variable
// for S and T.
template <class Arc>
  ...
  while (Q.size() != 0) {
    ...
    for (ArcIterator<MutableFst<Arc> > aiter(*fst, s); !aiter.Done();
      ...
      if (!pVec[arc.nextstate]) {  // Next state is not problematic -> we can
                                   // use this transition.
        pair<typename std::set<StateId>::iterator, bool> p =
...
void PreDeterminize(MutableFst<Arc> *fst, typename Arc::Label first_new_sym,
  ...
  {  // (D)(i)(a): check is trim (i.e. connected, in OpenFST parlance).
    ...
    if (props != (kAccessible | kCoAccessible)) {  // All states are not both
                                                   // accessible and
                                                   // co-accessible...
  ...
  {  // D(ii): computing the array p. ["problematic states, i.e. states with >1
    ...
    vector<bool> seen_vec(max_state + 1, false);  // rather than counting
                                                  // incoming transitions we
                                                  // just have a bool that says
                                                  // we saw at least one.
  ...
  // return
  // or throw an exception from this function, later than this point, without
  // cleaning up!
  ...
  {
    vector<StateId> all_seed_states;  // all "problematic" states, plus initial
                                      // state (if not problematic).
    ...
    for (size_t idx = 0; idx < all_seed_states.size(); idx++) {
      ...
      if (ptr != NULL) {  // was inserted.
        ...
        assert(
            !"Error: PreDeterminize failed in initialization\n");  // conceptual
                                                                   // bug or
                                                                   // programming
                                                                   // error.
        exit(1);
  ...
  while (Q.size() != 0) {
    ...
    // end-state).
    // Here, arc-offset reflects the order in which we accessed the arc using
    // the
    ...
    {  // This block sets up arc_hash
      for (size_t idx = 0; idx < A.size(); idx++) {
        ...
        for (ArcIterator<MutableFst<Arc> > aiter(*fst, s); !aiter.Done();
          ...
          pair<pair<StateId, ArcId>, StateId> this_pair(
              pair<StateId, ArcId>(s, arc_id), arc.nextstate);
    ...
    if (arc_hash.count(0) == 1) {  // We have epsilon transitions out.
      ...
      typedef typename set<pair<pair<StateId, ArcId>, StateId> >::iterator
          set_iter_t;
    ...
    // (D)(v)(e)
    {
      typedef typename map<Label,
                           set<pair<pair<StateId, ArcId>, StateId> > >::iterator
          map_iter_t;
      typedef typename set<pair<pair<StateId, ArcId>, StateId> >::iterator
          set_iter_t2;
      for (map_iter_t miter = arc_hash.begin(); miter != arc_hash.end();
        ...
        if (t != 0) {        // For t != epsilon,
          set<StateId> V_t;  // set of destination non-problem states.  Will
                             // create this set now.
          ...
          // problematic states)
          // The if-statement if (|S_t|>1) is pushed inside the loop, as the
          // loop also computes
          // the set V_t.
  ...
  {  // (D)(vii): compute symbol-table ID's.
    ...
    int64 n = -1;
    for (typename map<pair<StateId, ArcId>, size_t>::iterator m_iter =
             m_map.begin();
...
void AddSelfLoops(MutableFst<Arc> *fst, vector<typename Arc::Label> &isyms,
  ...
  for (StateIterator<MutableFst<Arc> > siter(*fst); !siter.Done();
    ...
    for (ArcIterator<MutableFst<Arc> > aiter(*fst, state); !aiter.Done();
      ...
      // already had the symbols
      // we are inserting.  This is contrary to the preconditions of this
      // algorithm.
...
typename Arc::StateId CreateSuperFinal(MutableFst<Arc> *fst) {
  ...
  if (final_states.size() == 1) {
    if (fst->Final(final_states[0]) == Weight::One()) {
      ...
      if (iter.Done()) {  // already have a final state w/ no transitions out
                          // and unit weight.  So don.
