...
class MapInputSymbolsMapper {
 public:
  Arc operator()(const Arc &arc_in) {
    ...
    if (ans.ilabel > 0 &&
        ans.ilabel <
            static_cast<typename Arc::Label>((*symbol_mapping_).size()))
  ...
  MapInputSymbolsMapper(const vector<I> &to_remove, bool copy) {
    KALDI_ASSERT_IS_INTEGER_TYPE(I);
    if (copy)
      symbol_mapping_ = new vector<I>(to_remove);
    else
...
bool GetLinearSymbolSequences(const Fst<Arc> &fst,
  ...
  for (ArcIterator<Fst<Arc> > aiter(fst, start_state); !aiter.Done();
       aiter.Next(), n++) {
    ...
    while (1) {
      if (fst.Final(cur_state) != Weight::Zero()) {
        (*weights_out)[n] = Times((*weights_out)[n], fst.Final(cur_state));
        if (fst.NumArcs(cur_state) != 0) error = true;
        break;
      } else {
  ...
  for (size_t i = 0; i < bad_states.size(); i++) {
    ...
    for (size_t j = 0; j < my_arcs.size(); j++) {
      ...
      if (arc.ilabel != 0) {
        ...
        // bad state.
        // Not as optimal as it could be, but does avoid some complicated
        // weight-pushing
        // issues in which, to maintain stochasticity, we would have to know
        // which semiring
...
void ComposeDeterministicOnDemand(const Fst<Arc> &fst1,
  ...
  typedef unordered_map<StatePair, StateId, kaldi::PairHasher<StateId> >
      MapType;
  ...
  while (!state_queue.empty()) {
    ...
    for (ArcIterator<Fst<Arc> > aiter(fst1, q1); !aiter.Done(); aiter.Next()) {
      ...
      if (arc1.olabel == 0) {
        fst_composed->AddArc(state_map[q], Arc(0, 0, arc1.weight, next_state));
      } else {
...
template <class Arc>
bool IsStochasticFst(const Fst<Arc> &fst, float delta,
...
template <>
bool IsStochasticFst(const Fst<LogArc> &fst, float delta,
                     LogArc::Weight *min_sum, LogArc::Weight *max_sum) {
  typedef LogArc Arc;
...
bool IsStochasticFstInLog(const VectorFst<StdArc> &fst, float delta,
                          StdArc::Weight *min_sum, StdArc::Weight *max_sum) {
