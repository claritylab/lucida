...
/* This header defines a context FST "C" which transduces from symbols
   representing phone
...
/// \addtogroup context_fst_group "Classes and functions related to context
/// expansion"
/// @{
...
template <class Arc,
          class LabelT = int32>  // make the vector<Label> things actually
                                 // vector<int32> for
 ...
 public:
  ...
  typedef unordered_map<vector<LabelT>, StateId, kaldi::VectorHasher<LabelT> >
      VectorToStateType;
  typedef unordered_map<vector<LabelT>, Label, kaldi::VectorHasher<LabelT> >
      VectorToLabelType;
 ...
 private:
  ...
  inline void CreateDisambigArc(StateId s, Label olabel,
  ...
  int N_;
  int P_;
  int pseudo_eps_symbol_;  // This is the symbol we put on epsilon arcs at the
                           // start
  // of the graph.  If we have "real" disambiguation symbols AND N > P+1, then
  // this cannot be
  // epsilon or there is a danger of non-determinizable output.  It's because in
  // this case,
  // the disambiguation symbols are shifted left w.r.t. the phones, and there
  // becomes
  // and ambiguity if a disambiguation symbol appears at the start of a sequence
  // onthe
  // input of CLG, whether it was at the very start of the input of LG, or just
  // after, say,
  // the first real phone.  What we do if we need pseudo_eps_symbol_ to be not
  // epsilon,
  // we create a special symbol with symbol-id 1 and sequence representation
  // (ilabels entry)
...
/*
   ...
   to phones.  It is an on-demand FST.  However, it does not create itself in
   the usual
   way by expanding states by enumerating all their arcs.  This is possible to
   enable
   iterating over arcs, but it is not recommended.  Instead, we define a special
   Matcher class that knows how to request the specific arc corresponding to a
   particular
   output label.
   ...
   enumerate all output symbols (if we want to access it in an inefficient way),
   and
   also to distinguish between phones and disambiguation symbols.
*/

template <class Arc,
          class LabelT = int32>  // make the vector<LabelT> things actually
                                 // vector<int32> for
                                 // easier compatibility with Kaldi code.
 ...
 public:
  ...
  ContextFst(
      ...
      int32 P)
      :  // Pos of "central" phone in ctx window, from 0..N-1.
        impl_(new ContextFstImpl<Arc, LabelT>(subsequential_symbol, phones,
                                              disambig_syms, N, P)) {
...
// Just directs to use the one from CacheFst.
template <class A>
class ArcIterator<ContextFst<A> > : public CacheArcIterator<ContextFst<A> > {
 public:
...
inline void ContextFst<A, I>::InitStateIterator(
...
// using arc iterators (which would force a call to Expand in ContextFstImpl,
// which
...
// type of the FST using FstType(), and verifies that the left hand FST is a
// context
// FST, and then uses a static cast to ContextFst.  [We can't make it a template
// argument, as the template for ComposeFstOptions is only templated on a single
// FST
...
class ContextMatcher
    : public MatcherBase<
          Arc> {  // CAREFUL: templated on arc, not on FST like normal Matcher.
 public:
  typedef Fst<Arc> FST;  // basic FST type that we get passed
  // because this is used in composition, typically one side will be a
  // ContextFst,
  ...
  virtual ~ContextMatcher() { delete fst_; }
...
*/
template <class Arc, class LabelT>
void ComposeContextFst(const ContextFst<Arc, LabelT> &ifst1,
...
/**
   ...
   subsequential loop.
 */
inline void ComposeContext(vector<int32> &disambig_syms, int N, int P,
