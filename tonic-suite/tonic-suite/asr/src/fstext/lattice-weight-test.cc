...
LatticeWeight RandomLatticeWeight() {
  if (kaldi::Rand() % 3 == 0) {
    ...
    return LatticeWeight(
        2, 1);  // this tests more thoroughly certain properties...
  } else {
...
CompactLatticeWeight RandomCompactLatticeWeight() {
  ...
  if (w == LatticeWeight::Zero()) {
    ...
    vector<int32> str;
    for (int32 i = 0; i < len; i++) str.push_back(kaldi::Rand() % 10 + 1);
...
void LatticeWeightTest() {
  for (int32 i = 0; i < 100; i++) {
    ...
    BaseFloat f1 = l1.Value1() + l1.Value2(), f2 = l2.Value1() + l2.Value2(),
              f3 = l3.Value1() + l3.Value2(), f4 = l4.Value1() + l4.Value2();
    ...
    KALDI_ASSERT(Times(l1, l2) == Times(l2, l1));  // commutativity of Times
                                                   // (true for this semiring,
                                                   // not always)
    KALDI_ASSERT(Plus(l3, LatticeWeight::Zero()) == l3);  // x + 0 = x
    KALDI_ASSERT(Times(l3, LatticeWeight::One()) == l3);  // x * 1 = x
    KALDI_ASSERT(Times(l3, LatticeWeight::Zero()) ==
                 LatticeWeight::Zero());  // x * 0 = 0
    ...
    KALDI_ASSERT(Compare(l1, Plus(l1, l2)) !=
                 1);  // so do not have l1 > l1 + l2
    ...
    if (l2 != LatticeWeight::Zero())
      KALDI_ASSERT(ApproxEqual(Divide(Times(l1, l2), l2),
                               l1));  // (a*b) / b = a if b != 0
...
void CompactLatticeWeightTest() {
  for (int32 i = 0; i < 100; i++) {
    ...
    KALDI_ASSERT(Times(l3, CompactLatticeWeight::Zero()) ==
                 CompactLatticeWeight::Zero());  // x * 0 = 0
    ...
    CompactLatticeWeight l5 = RandomCompactLatticeWeight(),
                         l6 = RandomCompactLatticeWeight();
    KALDI_ASSERT(Times(Plus(l1, l2), Plus(l5, l6)) ==
                 Plus(Times(l1, l5),
                      Plus(Times(l1, l6),
                           Plus(Times(l2, l5),
                                Times(l2, l6)))));  // * distributes over +
    KALDI_ASSERT(l1.Member() && l2.Member() && l3.Member() && l4.Member() &&
                 l5.Member() && l6.Member());
    if (l2 != CompactLatticeWeight::Zero()) {
      KALDI_ASSERT(ApproxEqual(Divide(Times(l1, l2), l2, DIVIDE_RIGHT),
                               l1));  // (a*b) / b = a if b != 0
      KALDI_ASSERT(ApproxEqual(Divide(Times(l2, l1), l2, DIVIDE_LEFT),
                               l1));  // (a*b) / b = a if b != 0
    }
    KALDI_ASSERT(ApproxEqual(l1, l1.Quantize()));
