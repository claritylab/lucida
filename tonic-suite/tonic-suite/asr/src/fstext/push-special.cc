...
/*
  In pushing algorithm:
    Each state gets a potential, say p(s).  Initial state must have potential
zero.
    The states have transition probabilities to each other, call these w(s, t),
and
    final-probabilities f(s).  One special state is the initial state.
    These are real probabilities, think of them like HMM transition
probabilities; we'll
    ...
    Let us define a kind of normalizer for each state s as:
       n(s) = f(s) + \sum_t w(s, t),
    or taking into account the potentials, and treating the self-loop as
special,
    ...
    in "PushSpecial" we settle for a different condition: that all the n(s) be
the
    same.  This means that the non-sum-to-one-ness of the FST is somehow smeared
    out throughout the FST.  We want an algorithm that makes all the n(s) the
same,
    and we formulate it in terms of iteratively improving objective function.
The
    objective function will be the sum-of-squared deviation of each of the n(s)
from
    their overall mean, i.e.
       \sum_s  (n(s) - n_{avg})^2
    where n_avg is the average of the n(s).  When we choose an s to optimize its
p(s),
    we'll minimize this function, but while minimizing it we'll treat n_{avg} as
    a fixed quantitiy.  We can show that even though this is not 100% accurate,
we
    still end up minimizing the objective function (i.e. we still decrease the
    sum-of-squared difference).

    Suppose we choose some s for which to tweak p(s). [naturally s cannot be the
start
    state].  Firstly, we assume n_{avg} is a known and fixed quantity.  When we
    change p(s) we alter n(s) and also n(t) for all states t != s that have a
transition
    ...
    the n(s).  We'll write out the part of the objective function that involves
p(s),
    and this is:
...
+ (\sum_t 2(j(t) - n_{avg})(w(t, s)/p(t))  p(s)
    ...
    comforting because we want to minimize F.  Let us write the four
coefficients
    above as c_{-2}, c_{-1}, c_1 and c_2.   The minimum of F can be found where
    the derivative of F w.r.t. p(s) is zero.  Here, let's just call it p for
short.
    This will be where:
     d/dp  c_{-2} p^{-2} + c_{-1} p^{-1} + c_1 p + c_2 p^2  = 0
            -2 c_{-2} p^{-3} - c_{-1} p^{-2} + c_1 + c_2 p  = 0 .
    Technically we can solve this type of formula by means of the quartic
equations,
    but these take up pages and pages.  Instead we'll use a one-dimensional form
    of Newton's method, computing the derivative and second derivative by
differentiating the
    formula.
 ...
 private:
  ...
  void Iterate(float delta) {
    ...
    for (iter = 0; iter < 2000; iter++) {
      ...
      for (int i = 0; i < num_states_; i++) new_occ[i] = 0.1 * occ_[i];
...
void PushSpecial(VectorFst<StdArc> *fst, float delta) {
  if (fst->NumStates() > 0) PushSpecialClass c(fst, delta);  // all the work
