...
/* PreDeterminize inserts extra symbols on the input side of an FST as necessary
   to
   ensure that, after epsilon removal, it will be compactly determinizable by
   the
...
/* CreateNewSymbols is a helper function used inside PreDeterminize, and is also
   useful
   when you need to add a number of extra symbols to a different vocabulary from
   the one
   modified by PreDeterminize. */

template <class Label>
void CreateNewSymbols(SymbolTable *inputSymTable, int nSym, std::string prefix,
                      vector<Label> *syms_out);

/** AddSelfLoops is a function you will probably want to use alongside
   PreDeterminize,
    to add self-loops to any FSTs that you compose on the left hand side of the
   one
    modified by PreDeterminize.

    This function inserts loops with "special symbols" [e.g. \#0, \#1] into an
   FST.
    This is done at each final state and each state with non-epsilon output
   symbols on
    at least one arc out of it.  This is to ensure that these symbols, when
   inserted into
    the input side of an FST we will compose with on the right, can "pass
   through" this
    FST.
   ...
   has
    non-epsilon symbols on the output side of arcs leaving it, or which is a
   final state,
...
/* CreateSuperFinal takes an FST, and creates an equivalent FST with a single
   final
   state with no transitions out and unit final weight, by inserting epsilon
   transitions
   as necessary. */
template <class Arc>
