...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Take a union of the indexed lattices. The input index is in the T*T*T "
        "semiring and\n"
        "the output index is also in the T*T*T semiring. At the end of this "
        "program, encoded\n"
    ...
    TableWriter<VectorFstTplHolder<KwsLexicographicArc> > index_writer(
        index_wspecifier);
