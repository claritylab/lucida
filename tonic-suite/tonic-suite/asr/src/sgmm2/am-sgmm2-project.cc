...
void Sgmm2Project::ComputeProjection(const AmSgmm2 &sgmm,
  ...
  int32 model_dim = sgmm.FeatureDim(), full_dim = inv_lda_mllt.NumRows();
  ...
  // feature space.
  for (int32 i = 0; i < B.NumRows(); i++) B(i, i) += diag_term;
  ...
  for (int32 i = model_dim; i < full_dim; i++) W(i, i) = 1.0;
  ...
  B_orig.AddMat2Sp(1.0, inv_lda_mllt_dbl, kNoTrans, B,
                   0.0);  // B_orig <-- inv_lda_mllt B inv_lda_mllt^T
  W_orig.AddMat2Sp(1.0, inv_lda_mllt_dbl, kNoTrans, W,
                   0.0);  // W_orig <-- inv_lda_mllt W inv_lda_mllt^T

  // Now get versions of B_orig and W_orig that are limited to the
  // dimension range that we wanted.
  Matrix<double> B_orig_mat(B_orig),
      W_orig_mat(W_orig);  // Get them as full matrices...
  SpMatrix<double> B_orig_limit(B_orig_mat.Range(
      start_dim, end_dim - start_dim, start_dim, end_dim - start_dim)),
      W_orig_limit(W_orig_mat.Range(start_dim, end_dim - start_dim, start_dim,
                                    end_dim - start_dim));
  ...
  projection->Resize(
      retained_dim,
      full_dim);  // This projection (which we output) will project from
  // full_dim to retained_dim; it goes from the pre-LDA+MLLT space to
  // "retained_dim" which
  // is <= model_dim.

  // Copy the relevant dimensions of "projection" from the "proj" matrix that
  // we just computed.  The rest remain zero (corresponding to discarded
  // dimensions).
  projection->Range(0, retained_dim, start_dim, end_dim - start_dim)
      .CopyFromMat(proj);
}
...
void Sgmm2Project::ComputeLdaTransform(const SpMatrix<double> &B,
  ...
  // Now B_proj = P diag(s) P^T, with P orthogonal.  It's both SVD and
  // eigenvalue
  // decomposition.
  // So P^{-1}, which equals P^T, is the transformation that
  // will make B_proj diagonal (with eigenvalues equal to s).

  P.Resize(dim, dim_to_retain,
           kCopyData);  // keep only rows of P^T that we want.
  Projection->Resize(dim_to_retain, dim);
  // The next line sets "Projection" to the LDA matrix, which is (part of P^T) *
  // T^{-1}
...
void Sgmm2Project::ApplyProjection(const Matrix<BaseFloat> &total_projection,
  ...
  SubMatrix<BaseFloat> small_projection(total_projection, 0, retained_dim, 0,
                                        dim);
...
void Sgmm2Project::ProjectVariance(const Matrix<double> &total_projection,
                                   bool inverse, SpMatrix<double> *variance) {
  if (inverse) {
...
void Sgmm2Project::ProjectVariance(const Matrix<double> &total_projection,
                                   bool inverse, SpMatrix<float> *variance) {
