...
void EbwAmSgmm2Updater::Update(const MleAmSgmm2Accs &num_accs,
  ...
  if (flags & kSgmmPhoneProjections)
    tot_impr +=
        UpdateM(num_accs, den_accs, Q_num, Q_den, gamma_num, gamma_den, model);
  ...
  if (flags & kSgmmCovarianceMatrix)
    tot_impr +=
        UpdateVars(num_accs, den_accs, gamma_num, gamma_den, S_means, model);
  ...
  if (fabs(num_accs.total_frames_ - den_accs.total_frames_) >
    ...
    KALDI_WARN << "Num and den frame counts differ, " << num_accs.total_frames_
               << " vs. " << den_accs.total_frames_;

  BaseFloat like_diff = num_accs.total_like_ - den_accs.total_like_;

  KALDI_LOG << "***Averaged differenced likelihood per frame is "
            << (like_diff / num_accs.total_frames_) << " over "
            << (num_accs.total_frames_) << " frames.";
...
class EbwUpdatePhoneVectorsClass
 ...
 public:
  ...
  ~EbwUpdatePhoneVectorsClass() { *auxf_impr_ptr_ += auxf_impr_; }
...
void EbwAmSgmm2Updater::ComputePhoneVecStats(
    const MleAmSgmm2Accs &accs, const AmSgmm2 &model,
    const std::vector<SpMatrix<double> > &H, int32 j1, int32 m,
    const Vector<double> &w_jm_in, double gamma_jm, Vector<double> *g_jm,
...
void EbwAmSgmm2Updater::UpdatePhoneVectorsInternal(
    const MleAmSgmm2Accs &num_accs, const MleAmSgmm2Accs &den_accs,
    const std::vector<SpMatrix<double> > &H, AmSgmm2 *model, double *auxf_impr,
  ...
  for (int32 j1 = j1_start; j1 < j1_end; j1++) {
    double num_state_count = 0.0, state_auxf_impr = 0.0;
    ...
    for (int32 m = 0; m < model->NumSubstatesForGroup(j1); m++) {
      ...
      Vector<double> g_jm_num(
          S);  // computed using eq. 58 of SGMM paper [for numerator stats]
      SpMatrix<double> H_jm_num(
          S);  // computed using eq. 59 of SGMM paper [for numerator stats]
      ...
      // SSGMM techreport.  But in this code, it gets done inside
      ...
      Vector<double> local_derivative(
          S);  // difference of derivative of numerator
      ...
      double auxf_impr =
          ((gamma_jm_num + gamma_jm_den == 0)
               ? 0.0
               : SolveQuadraticProblem(quadratic_term, local_derivative, opts,
                                       &delta_v_jm));
    ...
    *auxf_impr += state_auxf_impr;
    if (j1 < 10 && thread_id == 0) {
      KALDI_LOG << "Objf impr for group j = " << j1 << "  is "
                << (state_auxf_impr / (num_state_count + 1.0e-10)) << " over "
                << num_state_count << " frames";
    }
  }
}
...
double EbwAmSgmm2Updater::UpdatePhoneVectors(const MleAmSgmm2Accs &num_accs,
  ...
  KALDI_LOG << "**Overall auxf improvement for v is " << auxf_impr << " over "
            << count << " frames";
  return auxf_impr;
}
...
double EbwAmSgmm2Updater::UpdateM(const MleAmSgmm2Accs &num_accs,
                                  ...
                                  AmSgmm2 *model) const {
  int32 S = model->PhoneSpaceDim(), D = model->FeatureDim(),
        I = model->NumGauss();
  ...
  for (int32 i = 0; i < I; i++) {
    ...
    // represents the linear term in the objf on M-- except that we make it the
    // local
    // derivative about the current value, instead of the derivative around
    // zero.
    // But it's not exactly the derivative w.r.t. M, due to the factor of
    // Sigma_i.
    ...
    SpMatrix<double> Q(S);  // This is a combination of the Q's for the
    ...
    double impr = SolveQuadraticMatrixProblem(
        Q, L, SpMatrix<double>(model->SigmaInv_[i]), opts, &deltaM);
...
double EbwAmSgmm2Updater::UpdateW(const MleAmSgmm2Accs &num_accs,
  ...
  Matrix<double> w(model->w_);
  {
    ...
    double garbage;
    UpdateWClass c_num(num_accs, *model, w, log_a_num, &F_i_num, &g_i_num,
                       &garbage);
  ...
  }
  {
    ...
    double garbage;
    UpdateWClass c_den(den_accs, *model, w, log_a_den, &F_i_den, &g_i_den,
                       &garbage);
  ...
  for (int32 i = 0; i < I; i++) {
    ...
    tmp_F.CopyFromVec(
        F_i_den.Row(i));  // tmp_F is used for Vector->SpMatrix conversion.
    quadratic_term.AddSp(1.0, tmp_F);

    double state_count = gamma_num(i) + gamma_den(i);

    quadratic_term.Scale((state_count + options_.tau_w) /
                         (state_count + 1.0e-10));
    ...
    if (i < 10 || objf_impr / (gamma_num(i) + 1.0e-10) > 2.0) {
      KALDI_LOG << "Predicted objf impr for w per frame is "
                << (objf_impr / (gamma_num(i) + 1.0e-10)) << " over "
                << gamma_num(i) << " frames.";
    }
  ...
  KALDI_LOG << "**Overall objf impr for w per frame is " << tot_impr << " over "
            << tot_num_count << " frames.";
  return tot_impr;
}
...
double EbwAmSgmm2Updater::UpdateU(const MleAmSgmm2Accs &num_accs,
  ...
  for (int32 i = 0; i < num_accs.num_gaussians_; i++) {
    if (gamma_num(i) < 200.0) {
      KALDI_LOG << "Numerator count is small " << gamma_num(i)
                << " for gaussian " << i << ", not updating u_i.";
      continue;
    }
...
double EbwAmSgmm2Updater::UpdateN(const MleAmSgmm2Accs &num_accs,
  ...
  for (int32 i = 0; i < I; i++) {
    ...
    double impr = SolveQuadraticMatrixProblem(
        R, L, SpMatrix<double>(model->SigmaInv_[i]), opts, &deltaN);
    impr_vec(i) = impr;
  ...
  tot_impr /= (tot_count + 1.0e-20);
  KALDI_LOG << "**Overall auxf impr for N is " << tot_impr << " over "
            << tot_count << " frames";
  return tot_impr;
}

double EbwAmSgmm2Updater::UpdateVars(
    const MleAmSgmm2Accs &num_accs, const MleAmSgmm2Accs &den_accs,
    const Vector<double> &gamma_num, const Vector<double> &gamma_den,
    const std::vector<SpMatrix<double> > &S_means, AmSgmm2 *model) const {
  ...
  for (int32 i = 0; i < I; i++) {
    ...
    // SigmaStats now contain the stats for estimating Sigma (as in the main
    // SGMM paper),
    // differenced between num and den.
    SpMatrix<double> SigmaInvOld(model->SigmaInv_[i]),
        SigmaOld(model->SigmaInv_[i]);
    SigmaOld.Invert();
    ...
    double smoothing_count =
        (options_.tau_Sigma *
         inv_lrate) +          // multiply tau_Sigma by inverse-lrate
    ...
    SpMatrix<double> SigmaInv(
        SigmaStats);  // before floor and ceiling.  Currently sigma,
    // not its inverse.
    bool verbose = false;
    bool is_psd =
        false;  // we cannot guarantee that Sigma Inv is positive semidefinite.
    int n_floor =
        SigmaInv.ApplyFloor(SigmaOld, options_.cov_min_value, verbose, is_psd);
    SigmaInv.Invert();  // make it inverse variance.
    int n_ceiling = SigmaInv.ApplyFloor(SigmaInvOld, options_.cov_min_value,
                                        verbose, is_psd);

    // this auxf_change.
    double auxf_change =
        -0.5 * count *
        (TraceSpSp(SigmaInv, SigmaStats) - TraceSpSp(SigmaInvOld, SigmaStats) -
         SigmaInv.LogDet() + SigmaInvOld.LogDet());

    model->SigmaInv_[i].CopyFromSp(SigmaInv);
    impr_vec(i) = auxf_change;
    if (i < 10 || auxf_change / (num_count + den_count + 1.0e-10) > 2.0 ||
        n_floor + n_ceiling > 0) {
      KALDI_LOG << "Updating variance: Auxf change per frame for Gaussian " << i
                << " is " << (auxf_change / num_count) << " over " << num_count
                << " frames "
                << "(den count was " << den_count << "), #floor,ceil was "
                << n_floor << ", " << n_ceiling;
    }
  }
  ...
  double tot_count = gamma_num.Sum(), tot_impr = impr_vec.Sum();
  tot_impr /= tot_count + 1.0e-20;
  KALDI_LOG << "**Overall auxf impr for Sigma is " << tot_impr << " over "
            << tot_count << " frames";
  return tot_impr;
}

double EbwAmSgmm2Updater::UpdateSubstateWeights(const MleAmSgmm2Accs &num_accs,
                                                const MleAmSgmm2Accs &den_accs,
                                                AmSgmm2 *model) {
  ...
  for (int32 j2 = 0; j2 < num_accs.num_pdfs_; j2++) {
    ...
    Vector<double> num_occs(M), den_occs(M), orig_weights(model->c_[j2]),
        weights(model->c_[j2]);

    for (int32 m = 0; m < M; m++) {
      num_occs(m) = num_accs.gamma_c_[j2](m) + options_.tau_c * weights(m);
    ...
    if (weights.Dim() > 1) {
      ...
      for (int32 m = 0; m < M; m++)
        weights(m) = std::max(
            weights(m), static_cast<double>(options_.min_substate_weight));
  ...
  KALDI_LOG << "**Overall auxf impr for c is " << tot_impr << " over "
            << tot_count << " frames";
  return tot_impr;
}
