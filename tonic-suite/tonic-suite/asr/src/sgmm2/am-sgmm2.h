...
/*
 ...
 Recognition", by D. Povey,
  L. Burget et. al (Computer Speech and Language, 2011), and
  "The Symmetric Subspace Gaussian Mixture Model": Microsoft Research technical
 report MSR-TR-2010-138.
  We will refer to these as "the paper" [or "the CSL paper"] and "the
 techreport".
  ...
  extension to the "state-clustered tied mixture" [SCTM] system-- a bit like
 BBN's
  style of system, except for SGMMs not Gaussians, at the sub-state not Gaussian
 level.
  We build a first
  ...
  We use the index j2 for indexing 0...NumPdfs()-1 [as it's the "2nd level" of
 the tree],
  and j1 for indexing 0...NumPdfGroups()-1 [as it's the "1st level" of the
 tree].
  The weights are stored as c[j2][m].  There is a mapping Pdf2Group(j2) which
 returns
  the corresponding j1 for a given j2, and Group2PdfList(j1) which returns a
 vector<int32>
  ...
  inefficient.  Instead we store them separately as gamma1[j1][m][i] and
 gamma2[j2][m],
  so each count gets stored in two separate places; this makes the stats more
 compact.
  ...
  with this vector of pseudo-posteriors.  The log of this dot-product gets added
 to the
  original log-sum.
*/
...
struct Sgmm2SplitSubstatesConfig {
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("min-count", &min_count,
                 "Minimum allowed count, used in allocating "
                 "sub-states to state in mixture splitting.");
  }
};

// Caution: this config is probably not used in most of the setups, we generally
// do the Gaussian
...
struct Sgmm2PerFrameDerivedVars {
  ...
  void Resize(int32 ngauss, int32 feat_dim,
              int32 phn_dim) {  // resizes but does
    ...
    if (nti.Dim() != ngauss) nti.Resize(ngauss);
  }
};
...
class Sgmm2PerSpkDerivedVars {
 ...
 protected:
  ...
  Vector<BaseFloat> b_is;  /// < [SSGMM]: Eq. (22) in techreport, b_i^{(s)} =
                           /// \exp(\u_i^T \v^{(s)})
  Vector<BaseFloat>
      log_b_is;  /// < [SSGMM] log of the above (more efficient to store both).
  std::vector<Vector<BaseFloat> >
      log_d_jms;  ///< [SSGMM] normalizers per-speaker and per-substate;
                  ///< indexed [j1][m].
};
...
class AmSgmm2 {
 public:
  ...
  void CopyFromSgmm2(
      const AmSgmm2 &other, bool copy_normalizers,
  ...
  BaseFloat LogLikelihood(const Sgmm2PerFrameDerivedVars &per_frame_vars,
                          int32 j2,                     // pdf_id
                          Sgmm2LikelihoodCache *cache,  // be careful to call
                                                        // NextFrame() when
                                                        // needed!
  ...
  /// pdfs. [it's less necessary, as most of the time we're doing this from
  /// alignments,
  ...
  BaseFloat ComponentPosteriors(const Sgmm2PerFrameDerivedVars &per_frame_vars,
                                int32 j2, Sgmm2PerSpkDerivedVars *spk_vars,
                                Matrix<BaseFloat> *post) const;

  /// Increases the total number of substates based on the state occupancies.
  void SplitSubstates(
      const Vector<BaseFloat> &state_occupancies,  // [indexed by pdf-id j2]
  ...
  template <typename Real>
  void GetVarScaledSubstateSpeakerMean(int32 j1, int32 substate, int32 gauss,
                                       const Sgmm2PerSpkDerivedVars &spk,
                                       VectorBase<Real> *mean_out) const;
 ...
 private:
  ...
  void SplitSubstatesInGroup(const Vector<BaseFloat> &pdf_occupancies,
                             ...
                             const SpMatrix<BaseFloat> &sqrt_H_sm, int32 j1,
                             int32 M);
  ...
  void InitializeNu(int32 spk_subspace_dim, const Matrix<BaseFloat> &norm_xform,
                    bool speaker_dependent_weights);
  ...
  void ComputeHsmFromModel(const std::vector<SpMatrix<BaseFloat> > &H,
                           ...
                           SpMatrix<BaseFloat> *H_sm, BaseFloat max_cond) const;
...
template <typename Real>
void AmSgmm2::GetVarScaledSubstateSpeakerMean(
    int32 j1, int32 m, int32 i, const Sgmm2PerSpkDerivedVars &spk,
    VectorBase<Real> *mean_out) const {
...
typedef RandomAccessTableReader<Sgmm2GauPostHolder>
