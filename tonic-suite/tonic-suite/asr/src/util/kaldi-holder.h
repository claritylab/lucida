...
class GenericHolder {
 public:
  ...
  /// returns true, and text mode otherwise) whether the actual data is binary
  /// or
  ...
  /// function (rather than making the binary mode an argument to this
  /// function),
  /// so that for non-Kaldi binary files we don't have to write the header,
  /// which
...
/// KaldiObjectHolder works for Kaldi objects that have the "standard" Read and
/// Write
...
/// Class TokenVectorHolder is a Holder class for vectors of Tokens (T ==
/// std::string).
