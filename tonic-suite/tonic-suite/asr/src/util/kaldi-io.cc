...
// could replace basic_pipebuf<char> with stdio_filebuf<char>, on some
// platforms.
...
OutputType ClassifyWxfilename(const std::string &filename) {
  ...
  if (*c == '\0' || (*c == '-' && c[1] == '\0'))
    return kStandardOutput;  // "" or "-".
  else if (*c == '|')
    return kPipeOutput;  // An output pipe like "|blah".
  else if (isspace(*c) || isspace(c[filename.length() - 1]))
    return kNoOutput;  // Leading or
  // trailing space: can't interpret this.
    ...
    if (*d == '|' || isspace(*d))
      return kNoOutput;  // An input pipe (not allowed in
    // this context) or trailing space which is just wrong.
      ...
      if (*d == ':')
        return kNoOutput;  // Filename is like some_file:12345; not allowed,
      else
        return kFileOutput;
      ...
      // we
      // check for '|' as it's a common source of errors to have pipe commands
      // without
...
InputType ClassifyRxfilename(const std::string &filename) {
  const char *c = filename.c_str();
  if (*c == '\0' || (*c == '-' && c[1] == '\0'))
    return kStandardInput;  // "" or "-".
  else if (*c == '|')
    return kNoInput;  // An output pipe like "|blah": not valid for input.
  else if (isspace(*c) || isspace(c[filename.length() - 1]))
    return kNoInput;  // Leading or
  // trailing space.
  else if ((*c == 't' || *c == 'b') && c[1] == ',') {
    // We have detected that the user has supplied a wspecifier
    // or rspecifier (as in kaldi-table.h) where a wxfilename was
    ...
    if (isspace(*d))
      return kNoInput;  // trailing space which is never valid.
    else if (isdigit(*d)) {
      // OK, it could be an offset into a file
      // which is not allowed.
      while (isdigit(*d) && d > c) d--;
      if (*d == ':')
        return kOffsetFileInput;  // Filename is like some_file:12345
      else
        return kFileInput;
    } else {
      // at this point it matched no other pattern so we assume a filename, but
      ...
      // check for '|' as it's a common source of errors to have pipe commands
      // without
      // the pipe in the right place.  Say that it can't be classified in this
      // case.
      if (strchr(c, '|') != NULL) {
...
class FileOutputImpl : public OutputImplBase {
 public:
  virtual bool Open(const std::string &filename, bool binary) {
    ...
    filename_ = filename;
    os_.open(filename_.c_str(), binary
...
class PipeOutputImpl : public OutputImplBase {
 public:
  ...
  virtual bool Open(const std::string &wxfilename, bool binary) {
    ...
    KALDI_ASSERT(wxfilename.length() != 0 &&
                 wxfilename[0] == '|');  // should start with '|'
...
#ifndef _MSC_VER
      fb_ = new PipebufType(
          f_,  // Using this constructor won't make the destructor
  ...
  virtual std::ostream &Stream() {
    if (os_ == NULL)
      KALDI_ERR << "PipeOutputImpl::Stream(), object not initialized.";
    // I believe this error can only arise from coding error.
    return *os_;
  }
...
class InputImplBase {
 public:
  ...
  virtual InputType
  MyType() = 0;  // Because if it's kOffsetFileInput, we may call Open twice
  ...
  virtual bool Open(const std::string &filename, bool binary) {
    if (is_.is_open())
      KALDI_ERR << "FileInputImpl::Open(), "
                << "open called on already open file.";
    is_.open(filename.c_str(), binary
...
class OffsetFileInputImpl : public InputImplBase {
 ...
 public:
  ...
  static void SplitFilename(const std::string &rxfilename,
    ...
    KALDI_ASSERT(pos !=
                 std::string::npos);  // would indicate error in calling code,
...
Output::Output(const std::string &rxfilename, bool binary, bool write_header)
    : impl_(NULL) {
...
std::ostream &
Output::Stream() {  // will throw if not open; else returns stream.
  if (!impl_) KALDI_ERR << "Output::Stream() called but not open.";
  return impl_->Stream();
}

bool Output::Open(const std::string &wxfn, bool binary, bool header) {
  // Consolidate all the types of Open calls here, since they're basically doing
  // the
  // same thing.
  ...
  if (IsOpen()) {
    if (!Close()) {  // Throw here rather than return status, as it's an error
                     // about
      // something else: if the user wanted to avoid the exception he/she could
      // have
      // called Close().
...
bool Input::OpenInternal(const std::string &rxfilename, bool file_binary,
  ...
  if (IsOpen()) {
    ...
    if (type == kOffsetFileInput && impl_->MyType() == kOffsetFileInput) {
      ...
      if (!impl_->Open(
              rxfilename,
              file_binary)) {  // true is binary mode-- always open in binary.
        delete impl_;
        impl_ = NULL;
        return false;
      }
  ...
  if (!impl_->Open(
          rxfilename,
          file_binary)) {  // true is binary mode-- always read in binary.
    delete impl_;
    impl_ = NULL;
    return false;
  }
