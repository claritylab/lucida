...
int32 LevenshteinEditDistance(const std::vector<T> &ref,
  ...
  for (size_t hyp_index = 1; hyp_index <= hyp.size(); hyp_index++) {
    ...
    for (size_t ref_index = 1; ref_index <= ref.size(); ref_index++) {
      ...
      if (hyp[hyp_index - 1] != ref[ref_index - 1]) sub_err++;
  ...
  *ins = e[ref_index].ins_num, *del = e[ref_index].del_num,
...
int32 LevenshteinAlignment(const std::vector<T> &a, const std::vector<T> &b,
  ...
  while (m != 0 || n != 0) {
    ...
    if (m == 0) {
      ...
      int32 ins = e[m][n - 1] + 1;
      if (sub_or_ok <=
          std::min(del, ins)) {  // choose sub_or_ok if all else equal.
        last_m = m - 1;
        last_n = n - 1;
      } else {
