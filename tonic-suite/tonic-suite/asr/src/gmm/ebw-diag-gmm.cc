...
static bool EBWUpdateGaussian(BaseFloat D, GmmFlagsType flags,
                              const VectorBase<double> &orig_mean,
                              const VectorBase<double> &orig_var,
                              const VectorBase<double> &x_stats,
                              const VectorBase<double> &x2_stats, double occ,
                              VectorBase<double> *mean, VectorBase<double> *var,
                              double *auxf_impr) {
  ...
  KALDI_ASSERT(!((flags & kGmmVariances) && !(flags & kGmmMeans)) &&
               "We didn't make the update cover this case sensibly (update "
               "vars not means)");
  ...
  if (var->Min() > 0.0) {
    if (auxf_impr != NULL) {
      ...
      for (int32 i = 0; i < dim; i++) {
        ...
        old_auxf += (occ + D) * -0.5 *
                    (log(orig_var(i)) +
                     ((*var)(i) + mean_diff * mean_diff) / orig_var(i));
...
void UpdateEbwDiagGmm(
    const AccumDiagGmm &num_stats,  // with I-smoothing, if used.
    const AccumDiagGmm &den_stats, GmmFlagsType flags, const EbwOptions &opts,
    DiagGmm *gmm, BaseFloat *auxf_change_out, BaseFloat *count_out,
  ...
  for (int32 g = 0; g < num_comp; g++) {
    ...
    if (num_count == 0.0 && den_count == 0.0) {
      KALDI_VLOG(2) << "Not updating Gaussian " << g
    ...
    for (iter = 0; iter < max_iter;
         iter++) {  // will normally break from the loop
      // the first time.
      if (EBWUpdateGaussian(D, flags, diaggmmnormal.means_.Row(g),
                            diaggmmnormal.vars_.Row(g), mean_stats, var_stats,
                            num_count - den_count, &mean, &var, NULL)) {
        // Succeeded in getting all +ve vars at this value of D.
        // So double D and commit changes.
        D *= 2.0;
        double auxf_impr = 0.0;
        bool ans =
            EBWUpdateGaussian(D, flags, diaggmmnormal.means_.Row(g),
                              diaggmmnormal.vars_.Row(g), mean_stats, var_stats,
                              num_count - den_count, &mean, &var, &auxf_impr);
        if (!ans) {
          KALDI_WARN
              << "Something went wrong in the EBW update. Check that your"
                 "previous update phase looks reasonable, probably your model "
                 "is "
                 "already ruined.  Reverting to the old values";
        } else {
          if (auxf_change_out) *auxf_change_out += auxf_impr;
          if (count_out)
            *count_out += den_count;  // The idea is that for MMI, this will
          // reflect the actual #frames trained on (the numerator one would be
          // I-smoothed).
    ...
    if (iter == max_iter)
      KALDI_WARN << "Dropped off end of loop, recomputing D. (unexpected.)";
  }
...
void UpdateEbwWeightsDiagGmm(
    const AccumDiagGmm &num_stats,  // should have no I-smoothing
    const AccumDiagGmm &den_stats, const EbwWeightOptions &opts, DiagGmm *gmm,
  ...
  Vector<double> weights(diaggmmnormal.weights_),
      num_occs(num_stats.occupancy()), den_occs(den_stats.occupancy());
  if (opts.tau == 0.0 &&
    ...
    if (count_out) *count_out += num_occs.Sum();
    return;
  }
  num_occs.AddVec(opts.tau, weights);
  KALDI_ASSERT(weights.Dim() == num_occs.Dim() &&
               num_occs.Dim() == den_occs.Dim());
  ...
  for (int32 g = 0; g < num_comp;
       g++) {  // c.f. eq. 4.32 in Dan Povey's thesis.
  ...
  for (int32 g = 0; g < num_comp;
       g++) {  // c.f. eq. 4.32 in Dan Povey's thesis.
    weight_auxf_at_end += num_occs(g) * log(weights(g)) -
                          den_occs(g) * weights(g) / diaggmmnormal.weights_(g);
  }
...
void UpdateEbwAmDiagGmm(
    const AccumAmDiagGmm &num_stats,  // with I-smoothing, if used.
    const AccumAmDiagGmm &den_stats, GmmFlagsType flags, const EbwOptions &opts,
    AmDiagGmm *am_gmm, BaseFloat *auxf_change_out, BaseFloat *count_out,
    int32 *num_floored_out) {
  ...
  for (int32 pdf = 0; pdf < num_stats.NumAccs(); pdf++)
    UpdateEbwDiagGmm(num_stats.GetAcc(pdf), den_stats.GetAcc(pdf), flags, opts,
                     &(am_gmm->GetPdf(pdf)), auxf_change_out, count_out,
                     num_floored_out);
}

void UpdateEbwWeightsAmDiagGmm(
    const AccumAmDiagGmm &num_stats,  // with I-smoothing, if used.
    const AccumAmDiagGmm &den_stats, const EbwWeightOptions &opts,
    AmDiagGmm *am_gmm, BaseFloat *auxf_change_out, BaseFloat *count_out) {
  KALDI_ASSERT(num_stats.NumAccs() == den_stats.NumAccs() &&
               num_stats.NumAccs() == am_gmm->NumPdfs());
  ...
  for (int32 pdf = 0; pdf < num_stats.NumAccs(); pdf++)
    UpdateEbwWeightsDiagGmm(num_stats.GetAcc(pdf), den_stats.GetAcc(pdf), opts,
                            &(am_gmm->GetPdf(pdf)), auxf_change_out, count_out);
}

void IsmoothStatsDiagGmm(const AccumDiagGmm &src_stats, double tau,
...
void DiagGmmToStats(const DiagGmm &gmm, GmmFlagsType flags, double state_occ,
                    AccumDiagGmm *dst_stats) {
...
void IsmoothStatsAmDiagGmm(const AccumAmDiagGmm &src_stats, double tau,
...
void IsmoothStatsAmDiagGmmFromModel(const AmDiagGmm &src_model, double tau,
                                    AccumAmDiagGmm *dst_stats) {
