...
void GetSingleStatsDerivative(double ml_count, double ml_x_stats,
                              double ml_x2_stats, double disc_count,
                              double disc_x_stats, double disc_x2_stats,
                              double model_mean, double model_var,
                              BaseFloat min_variance, double *ml_x_stats_deriv,
  ...
  double diff_wrt_model_mean =
             (1.0 / model_var) * (disc_x_stats - model_mean * disc_count),
         diff_wrt_model_var =
             0.5 * ((disc_x2_stats - 2 * model_mean * disc_x_stats +
                     disc_count * model_mean_sq) *
                        model_inv_var_sq -
                    disc_count * model_inv_var);

  double stats_mean = ml_x_stats / ml_count,
         stats_var = ml_x2_stats / ml_count -
                     (ml_x_stats / ml_count) * (ml_x_stats / ml_count);
  ...
  // DoRescalingUpdate(), which takes two sets of ML accs (old and new).  The
  // old ML
  // accs given to the update will be the current ml accumulators we have here
  // in
  // this function, and the new ML accs will be affected by change in fMPE
  // transform.
  // The update in DoRescalingUpdate() will preserve any current difference
  // between
  // the ml stats and the model [represented as a shift in mean and factor in
  // variance].
  // Concretely: the update in DoRescalingUpdate() will do:
  //
  // new_model_mean := old_model_mean + new_stats_mean - old_stats_mean   (eq.
  // 1)
  // new_model_var := max(min_variance, old_model_var * new_stats_var /
  // old_stats_var).  (eq. 2)
  //
  // We're differentiating back through this process to new_stats_mean.
  // If the model and the stats were actually the same (e.g. we had been doing
  // ML updates),
  // then all this is equivalent to what was in the original fMPE paper.  It's
  // just
  // extended to make sense outside of that scenario where you're doing ML.

  double diff_wrt_stats_mean =
      diff_wrt_model_mean;  // This comes from eq. 1 above.
  ...
  if (model_var <= min_variance * 1.01) {
    ...
    // the factor "model_var / stats_var" comes from "old_model_var /
    // old_stats_var" in eq. 2.
    // Also note: the {old_,new_} versions of variables are numerically the same
    // here, at the
    // point where we're differentiating.
  }

  // The next equations don't appear in the paper but represent the
  // backpropagation
  ...
  *ml_x_stats_deriv = diff_wrt_stats_mean / ml_count -
                      2 * diff_wrt_stats_var *stats_mean / ml_count;
...
void GetStatsDerivative(const DiagGmm &gmm, const AccumDiagGmm &num_acc,
                        const AccumDiagGmm &den_acc, const AccumDiagGmm &ml_acc,
  ...
  for (int32 gauss = 0; gauss < num_gauss; gauss++) {
    ...
    if (ml_count <= min_gaussian_occupancy) {
      ...
      KALDI_WARN
          << "Skipping Gaussian because very small ML count: (num,den,ml) = "
          << num_count << ", " << den_count << ", " << ml_count;
    } else {
      double disc_count = num_count - den_count;
      for (int32 d = 0; d < dim; d++) {
        double disc_x_acc =
                   num_acc.mean_accumulator()(gauss, d) -
                   (have_den_stats ? den_acc.mean_accumulator()(gauss, d)
                                   : 0.0),
               disc_x2_acc =
                   num_acc.variance_accumulator()(gauss, d) -
                   (have_den_stats ? den_acc.variance_accumulator()(gauss, d)
                                   : 0.0),
        ...
        GetSingleStatsDerivative(ml_count, ml_x_acc, ml_x2_acc, disc_count,
                                 disc_x_acc, disc_x2_acc, model_mean, model_var,
                                 min_variance, &x_acc_deriv, &x2_acc_deriv);
...
void GetStatsDerivative(
  ...
  for (int32 pdf = 0; pdf < num_pdfs; pdf++)
    GetStatsDerivative(gmm.GetPdf(pdf), num_accs.GetAcc(pdf),
                       den_accs.GetAcc(pdf), ml_accs.GetAcc(pdf), min_variance,
                       min_gaussian_occupancy, &(out_accs->GetAcc(pdf)));
}

void DoRescalingUpdate(const AccumDiagGmm &old_ml_acc,
                       const AccumDiagGmm &new_ml_acc, BaseFloat min_variance,
                       BaseFloat min_gaussian_occupancy, DiagGmm *gmm,
                       double *tot_count, double *tot_divergence) {
  int32 num_gauss = gmm->NumGauss(), dim = gmm->Dim();
  KALDI_ASSERT(old_ml_acc.NumGauss() == num_gauss && old_ml_acc.Dim() == dim);
  KALDI_ASSERT(new_ml_acc.NumGauss() == num_gauss && new_ml_acc.Dim() == dim);
  ...
  for (int32 gauss = 0; gauss < num_gauss; gauss++) {
    ...
    if (old_ml_count <= min_gaussian_occupancy ||
      ...
      KALDI_WARN
          << "Gaussian being skipped because it has small count: (old,new) = "
    ...
    *tot_count += new_ml_count;
    for (int32 d = 0; d < dim; d++) {
      double old_model_mean = gmm_normal.means_(gauss, d),
             ...
             old_ml_mean =
                 old_ml_acc.mean_accumulator()(gauss, d) / old_ml_count,
             old_ml_var =
                 old_ml_acc.variance_accumulator()(gauss, d) / old_ml_count -
                 old_ml_mean * old_ml_mean,
             new_ml_mean =
                 new_ml_acc.mean_accumulator()(gauss, d) / new_ml_count,
             new_ml_var =
                 new_ml_acc.variance_accumulator()(gauss, d) / new_ml_count -
      ...
      double divergence = 0.5 * (((new_model_mean - old_model_mean) *
                                      (new_model_mean - old_model_mean) +
                                  new_model_var - old_model_var) /
                                     old_model_var +
                                 log(old_model_var / new_model_var));
      if (divergence < 0.0) KALDI_WARN << "Negative divergence " << divergence;
...
void DoRescalingUpdate(const AccumAmDiagGmm &old_ml_accs,
  ...
  for (int32 pdf = 0; pdf < num_pdfs; pdf++)
    DoRescalingUpdate(old_ml_accs.GetAcc(pdf), new_ml_accs.GetAcc(pdf),
                      ...
                      &am_gmm->GetPdf(pdf), &tot_count, &tot_divergence);
  KALDI_LOG << "K-L divergence from old to new model is "
            << (tot_divergence / tot_count) << " over " << tot_count
            << " frames.";
