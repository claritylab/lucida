...
void AccumFullGmm::AccumulateForComponent(const VectorBase<BaseFloat> &data,
  ...
  while (token != "</GMMACCS>") {
    if (token == "<OCCUPANCY>") {
      ...
      for (int32 i = 0; i < num_components; i++) {
        ...
        if (tmp_occs(i) != 0)
          tmp_acc.AddVec2(1.0 / tmp_occs(i), tmp_means.Row(i));
...
void AccumFullGmm::Write(std::ostream &out_stream, bool binary) const {
  ...
  if (num_comp_ != 0)
    KALDI_ASSERT(((flags_ & kGmmVariances) != 0) ==
  ...
  if (covariance_accumulator_.size() != 0) {
    ...
    for (int32 i = 0; i < num_comp_; i++) {
      ...
      if (occupancy_(i) != 0)
        tmp_acc.AddVec2(-1.0 / occupancy_(i), mean_accumulator_.Row(i));
    ...
    if (occ > static_cast<double>(config.min_gaussian_occupancy) &&
      ...
      if (config.remove_low_count_gaussians &&
        ...
        KALDI_WARN << "Too little data - removing Gaussian (weight "
                   ...
                   << fullgmm_acc.occupancy()(i) << ", vector size "
                   << gmm->Dim() << ")";
