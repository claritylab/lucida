...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "Usage:  gmm-acc-hlda [options] <model-in> <orig-transform-in> "
        "<orig-feature-rspecifier> <posteriors-rspecifier> <stats-out>\n"
        "Note: orig-transform-in must be the current truncated HLDA transform "
        "(e.g. from LDA)."
        "e.g.: \n"
        " gmm-acc-hlda 1.mdl 1.hlda \"ark:splice-feats scp:train.scp |\" "
        "ark:1.post 1.hacc\n";
    ...
    po.Register("speedup", &speedup,
                "Proportion of data to accumulate full HLDA stats with");
    po.Read(argc, argv);
    ...
    Matrix<BaseFloat> cur_transform;
    {
      ...
      KALDI_ASSERT(
          cur_transform.NumRows() == am_gmm.Dim() &&
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!posteriors_reader.HasKey(key)) {
        ...
        transformed_mat.AddMatMat(1.0, mat, kNoTrans, cur_transform, kTrans,
                                  0.0);
        ...
        for (size_t i = 0; i < posterior.size(); i++) {
          for (size_t j = 0; j < pdf_posterior[i].size(); j++) {
            ...
            tot_like_this_file +=
                weight *
                gmm.ComponentPosteriors(transformed_mat.Row(i), &posteriors);
            tot_weight += weight;
            posteriors.Scale(weight);
            hlda_accs.AccumulateFromPosteriors(pdf_id, gmm, mat.Row(i),
                                               posteriors);
          }
        }
