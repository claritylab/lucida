...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Set one of the transforms in lvtln to the minimum-squared-error "
        "solution\n"
        "to mapping feats-untransformed to feats-transformed; posteriors may\n"
        "optionally be used to downweight/remove silence.\n"
        "Usage: gmm-train-lvtln-special [options] class-index <lvtln-in> "
        "<lvtln-out> "
        " <feats-untransformed-rspecifier> <feats-transformed-rspecifier> "
        "[<posteriors-rspecifier>]\n"
        "e.g.: \n"
        " gmm-train-lvtln-special 5 5.lvtln 6.lvtln scp:train.scp "
        "scp:train_warp095.scp ark:nosil.post\n";
    ...
    po.Register("warp", &warp,
                "If supplied, can be used to set warp factor"
                "for this transform");
    po.Register("normalize-var", &normalize_var,
                "Normalize diagonal of variance "
                "to be the same before and after transform.");
    po.Register("normalize-covar", &normalize_covar,
    ...
    if (!normalize_covar) {
      ...
      // Ignoring weighting (which is a straightforward extension), the problem
      // is this:
      // we have original features x(t) and transformed features y(t) [both x(t)
      // and y(t)
      // are vectors of size D].  We are training an affine transform to
      // minimize the sum-squared
      // error between A x(t) + b and y(t).  Let x(t)^+ be x(t) with a 1
      // appended, and let
      ...
      // Note that for lvtln purposes we throw away the "offset" element (i.e.
      // the last
      ...
      for (; !x_reader.Done(); x_reader.Next()) {
        ...
        if (x_feats.NumRows() != y_feats.NumRows() ||
            ...
            x_feats.NumCols() != dim) {
          KALDI_ERR << "Number of rows and/or columns differs in features, or "
                       "features have different dim from lvtln object";
        }
        ...
        for (int32 i = 0; i < x_feats.NumRows(); i++) {
          ...
          for (int32 j = 0; j < dim; j++) xplus_row_dbl(j) = x_row(j);
          ...
          for (int32 j = 0; j < dim; j++) {
            ...
            sumsq_diff(j) +=
                weight * (x_row(j) - y_row(j)) * (x_row(j) - y_row(j));
      ...
      for (int32 i = 0; i < dim; i++) {
        ...
        BaseFloat error =
                      (VecSpVec(w_i, Q, w_i) - 2.0 * VecVec(w_i, l_i) + c(i)) /
                      beta,
                  sqdiff = sumsq_diff(i) / beta, scatter = sumsq_x(i) / beta;

        KALDI_LOG << "For dimension " << i
                  << ", sum-squared error in linear approximation is " << error
                  << ", versus feature-difference " << sqdiff
                  << ", orig-sumsq is " << scatter;
        if (normalize_var) {  // add a scaling to normalize the variance.
          ...
          KALDI_LOG << "For dimension " << i
                    << ", variance of original and transformed data is "
                    << x_var << " and " << y_var
                    << " respectively; scaling matrix row by " << scale
                    << " to make them equal.";
          A.Row(i).Scale(scale);
        }
      }
      ...
      // see the document "Notes for affine-transform-based VTLN" for
      // explanation,
      ...
      tmp.AddMatMat(1.0, N, kNoTrans, Cinv, kNoTrans,
                    0.0);  // tmp := N * C^{-1}
      M.AddMatMat(1.0, C, kNoTrans, tmp, kNoTrans,
                  0.0);  // M := C * tmp = C * N * C^{-1}
    ...
    if (warp >= 0.0) lvtln.SetWarp(class_idx, warp);
