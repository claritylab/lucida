...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Decode features using GMM-based model, producing N-best lattice "
        "output.\n"
        "Note: this program was mainly intended to validate the lattice "
        "generation\n"
        ...
        "<nbest-lattice-wspecifier> <words-wspecifier> "
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (decoder.GetNBestLattice(&decoded, &was_final, &nbest, &nbest_beam)) {
        ...
        num_success++;
        KALDI_LOG << "retrieved:" << nbest
                  << " tokens, effective beam:" << nbest_beam;

        //        std::cout << "n-best paths:\n";
        //        fst::FstPrinter<CompactLatticeArc> fstprinter(decoded, NULL,
        //        NULL, NULL, false, true);
        //        fstprinter.Print(&std::cout, "standard output");

        if (acoustic_scale !=
            0.0)  // We'll write the lattice without acoustic scaling
        ...
        // KALDI_LOG << "final weight:" << weight.Value1() << "," <<
        // weight.Value2();
        tot_like += like;
