...
struct DecodeInfo {
 public:
  DecodeInfo(const kaldi::AmDiagGmm &am, const kaldi::TransitionModel &tm,
             kaldi::FasterDecoder *decoder, BaseFloat scale, bool allow_partial,
      ...
      : acoustic_model(am),
        ...
        words_writer(wwriter),
        alignment_writer(awriter),
        word_syms(wsyms) {}
...
bool DecodeUtterance(kaldi::FasterDecoder *decoder,
                     kaldi::DecodableInterface *decodable, DecodeInfo *info,
                     const string &uttid, int32 num_frames,
...
int main(int argc, char *argv[]) {
  try {
    ...
    std::string model_in_filename = po.GetArg(1),
                fst_in_filename = po.GetArg(2), regtree_filename = po.GetArg(3),
    ...
    RandomAccessRegtreeFmllrDiagGmmReaderMapped fmllr_reader(
        xforms_rspecifier, utt2spk_rspecifier);
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!fmllr_reader.HasKey(utt)) {  // Decode without FMLLR if none found
        ...
        if (DecodeUtterance(&decoder, &gmm_decodable, &decode_info, utt,
                            features.NumRows(), &tot_like)) {
      ...
      if (fmllr.NumRegClasses() == 1) {
        ...
        kaldi::DecodableAmDiagGmmScaled gmm_decodable(
            am_gmm, trans_model, xformed_features, acoustic_scale);

        if (DecodeUtterance(&decoder, &gmm_decodable, &decode_info, utt,
                            xformed_features.NumRows(), &tot_like)) {
          frame_count += gmm_decodable.NumFrames();
          num_success++;
        } else {
          num_fail++;
        ...
        kaldi::DecodableAmDiagGmmRegtreeFmllr gmm_decodable(
            am_gmm, trans_model, features, fmllr, regtree, acoustic_scale);
        if (DecodeUtterance(&decoder, &gmm_decodable, &decode_info, utt,
                            features.NumRows(), &tot_like)) {
          frame_count += gmm_decodable.NumFrames();
          num_success++;
