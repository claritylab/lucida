...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Accumulate stats for training a diagonal-covariance GMM, two-feature "
        "version\n"
        ...
        "e.g.: gmm-global-acc-stats-twofeats 1.mdl scp:train.scp "
        "scp:train2.scp 1.acc\n";
      ...
      if (gselect_rspecifier != "") {
        ...
        for (int32 i = 0; i < file_frames; i++) {
          ...
          for (int32 j = 0; j < loglikes.Dim(); j++)
            gmm_accs.AccumulateForComponent(data2, this_gselect[j],
                                            loglikes(j));
        }
      } else {  // no gselect..
        ...
        for (int32 i = 0; i < file_frames; i++) {
          ...
          file_weight += weight;
          file_like +=
              weight * gmm.ComponentPosteriors(mat1.Row(i), &posteriors);
