...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: \n"
        " gmm-gselect [options] <model-in> <feature-rspecifier> "
        "<gselect-wspecifier>\n"
        "The --gselect option (which takes an rspecifier) limits selection to "
        "a subset\n"
        "of indices:\n"
        "e.g.: gmm-gselect \"--gselect=ark:gunzip -c bigger.gselect.gz|\" "
        "--n=20 1.gmm \"ark:feature-command |\" \"ark,t:|gzip -c "
        ">gselect.1.gz\"\n";
    ...
    po.Register("write-likes", &likelihood_wspecifier,
                "rspecifier for likelihoods per "
                "utterance");
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (gselect_rspecifier !=
        ...
        if (preselect.size() != static_cast<size_t>(mat.NumRows())) {
          KALDI_WARN << "Input gselect for utterance " << utt
                     << " has wrong size " << preselect.size() << " vs. "
                     << mat.NumRows();
          num_err++;
          continue;
        }
        for (int32 i = 0; i < mat.NumRows(); i++)
          tot_like_this_file += gmm.GaussianSelectionPreselect(
              mat.Row(i), preselect[i], num_gselect, &(gselect[i]));
