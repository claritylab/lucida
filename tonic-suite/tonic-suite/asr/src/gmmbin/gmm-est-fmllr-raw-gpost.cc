...
void AccStatsForUtterance(const TransitionModel &trans_model,
                          const AmDiagGmm &am_gmm, const GaussPost &gpost,
  ...
  for (size_t t = 0; t < gpost.size(); t++) {
    for (size_t i = 0; i < gpost[t].size(); i++) {
      ...
      accs->AccumulateFromPosteriors(am_gmm.GetPdf(pdf), feats.Row(t),
                                     posterior);
    }
  }
}
      ...
      for (; !spk2utt_reader.Done(); spk2utt_reader.Next()) {
        ...
        for (size_t i = 0; i < uttlist.size(); i++) {
          ...
          if (!gpost_reader.HasKey(utt)) {
            KALDI_WARN << "Gaussian-level posteriors not found for utterance "
                       << utt;
            num_err++;
            continue;
          }
