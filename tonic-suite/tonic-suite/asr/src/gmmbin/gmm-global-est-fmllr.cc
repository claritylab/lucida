...
namespace kaldi {
bool AccumulateForUtterance(const Matrix<BaseFloat> &feats, const DiagGmm &gmm,
    ...
    for (int32 t = 0; t < num_frames; t++) {
      ...
      if (weight != 0.0) {
        ...
        for (size_t i = 0; i < this_gselect.size(); i++)
          fullcov_stats->AccumulateForComponent(feats.Row(t), this_gselect[i],
                                                post(i));
      }
    }
  } else {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "supplied\n"
        "set of speakers (spk2utt option).  Reads features, and (with "
        "--weights option)\n"
        "weights for each frame (also see --gselect option)\n"
        "Usage: gmm-global-est-fmllr [options] <gmm-in> <feature-rspecifier> "
    ...
    po.Register("align-model", &alignment_model,
                "rxfilename for a model in the "
