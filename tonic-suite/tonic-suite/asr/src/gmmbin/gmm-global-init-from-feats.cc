...
void TrainOneIter(const Matrix<BaseFloat> &feats,
                  const MleDiagGmmOptions &gmm_opts, int32 iter,
  ...
  KALDI_LOG << "Likelihood per frame on iteration " << iter << " was "
            << (tot_like / feats.NumRows()) << " over " << feats.NumRows()
            << " frames.";
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "This program initializes a single diagonal GMM and does multiple "
        "iterations of\n"
        "training from features stored in memory.\n"
        "Usage:  gmm-global-init-feats [options] <feature-rspecifier> "
        "<model-out>\n"
    ...
    po.Register("num-gauss-init", &num_gauss_init,
      ...
      for (int32 t = 0; t < this_feats.NumRows(); t++) {
        ...
        if (num_read <= num_frames) {
          ...
          if (WithProb(keep_prob)) {  // With probability "keep_prob"
            feats.Row(RandInt(0, num_frames - 1))
                .CopyFromVec(this_feats.Row(t));
          }
        }
      }
    }
