...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Generate lattices using GMM-based model, using arc lattices from "
        "forward path.\n"
        "Usage: gmm-latgen-tracking [options] model-in "
        "(fst-in|fsts-rspecifier) features-rspecifier"
        " arcs-rspecifier lattice-wspecifier [ words-wspecifier "
    ...
    if (ClassifyRspecifier(fst_in_str, NULL, NULL) == kNoRspecifier) {
...

      {
        ...
        for (; !feature_reader.Done(); feature_reader.Next()) {
          ...
          if (DecodeUtteranceLatticeTracking(
                  decoder, gmm_decodable, trans_model, arc_graph, word_syms,
      ...
      for (; !fst_reader.Done(); fst_reader.Next()) {
        ...
        double like;
        if (DecodeUtteranceLatticeTracking(
                decoder, gmm_decodable, trans_model, arc_graph, word_syms, utt,
