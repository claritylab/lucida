...
class CuVectorBase {
 public:
  ...
  /// as you would expect.
  void AddDiagMatMat(Real alpha, const CuMatrixBase<Real> &M,
                     MatrixTransposeType transM, const CuMatrixBase<Real> &N,
...
class CuVector : public CuVectorBase<Real> {
 ...
 public:
  ...
  template <typename OtherReal>
  explicit CuVector(const CuVectorBase<OtherReal> &v)
      : CuVectorBase<Real>() {
    Resize(v.Dim(), kUndefined);
    this->CopyFromVec(v);
  }
  ...
  template <typename OtherReal>
  explicit CuVector(const VectorBase<OtherReal> &v)
      : CuVectorBase<Real>() {
    Resize(v.Dim(), kUndefined);
...
class CuSubVector : public CuVectorBase<Real> {
 public:
  ...
  CuSubVector(const Real *data, MatrixIndexT length) : CuVectorBase<Real>() {
    // Yes, we're evading C's restrictions on const here, and yes, it can be
    // used
...
template <typename Real>
bool ApproxEqual(const CuVectorBase<Real> &a, const CuVectorBase<Real> &b,
                 Real tol = 0.01) {
