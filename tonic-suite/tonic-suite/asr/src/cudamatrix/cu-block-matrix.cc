...
CuBlockMatrix<Real>::CuBlockMatrix(const std::vector<CuMatrix<Real> > &data) {
  ...
  for (int32 b = 0; b < NumBlocks(); b++) Block(b).CopyFromMat(data[b]);
  SetCudaData();
}
...
const CuSubMatrix<Real> CuBlockMatrix<Real>::Block(int32 b) const {
  ...
  return CuSubMatrix<Real>(data_, 0, block_data.num_rows, block_data.col_offset,
                           block_data.num_cols);
}
...
template <class Real>
CuBlockMatrix<Real> &CuBlockMatrix<Real>::operator=(
    const CuBlockMatrix<Real> &other) {
  FreeCudaData();
  data_ = other.data_;
  block_data_ = other.block_data_;
  num_rows_ = other.num_rows_;
...
void CuBlockMatrix<Real>::SetCudaData() {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cu_data_ =
        static_cast<CuBlockMatrixData *>(CuDevice::Instantiate().Malloc(size));
    CU_SAFE_CALL(
        cudaMemcpy(cu_data_, &(tmp_cu_data[0]), size, cudaMemcpyHostToDevice));
...
void CuBlockMatrix<Real>::Write(std::ostream &os, bool binary) const {
  ...
  for (int32 b = 0; b < num_blocks; b++) this->Block(b).Write(os, binary);
  WriteToken(os, binary, "</CuBlockMatrix>");
}
...
void CuBlockMatrix<Real>::Read(std::istream &is, bool binary) {
  ...
  if (i != static_cast<int>('<')) {
    ...
    KALDI_ASSERT(size >= 0);
    data.resize(size);
    for (int32 i = 0; i < size; i++) data[i].Read(is, binary);
  } else {
  ...
  CuBlockMatrix<Real> block_mat(
      data);  // initializer from std::vector<CuMatrix<Real> > does
...
template <class Real>
void CuBlockMatrix<Real>::AddMatMat(BaseFloat alpha, const CuMatrix<Real> &A,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    // we assume (which we can, safely) that CU1DBLOCK is <= the max threads per
    // block.
    int32 x_blocksize =
        std::min(CU1DBLOCK, NumBlocks());  // x dim corresponds to block-idx.
    ...
    while (z_blocksize * x_blocksize * y_blocksize > CU1DBLOCK ||
           z_blocksize > CU2DBLOCK)
      z_blocksize--;
    ...
    cuda_block_add_mat_mat(dimGrid, dimBlock, cu_data_, NumBlocks(), A.Data(),
                           A_num_cols, A_row_stride, A_col_stride, B.Data(),
                           B_row_stride, B_col_stride, alpha, beta);
...
#endif
  {
    ...
    for (MatrixIndexT b = 0; b < NumBlocks(); b++) {
      ...
      CuSubMatrix<Real>
          A_part = (transA == kNoTrans
                        ? A.Range(row_offset, this_num_rows, 0, A.NumCols())
                        : A.Range(0, A.NumRows(), row_offset, this_num_rows)),
          B_part = (transB == kNoTrans
                        ? B.Range(0, B.NumRows(), col_offset, this_num_cols)
                        : B.Range(col_offset, this_num_cols, 0, B.NumCols()));
...
void CuBlockMatrix<Real>::CopyFromMat(const CuMatrix<Real> &M) {
  ...
  for (MatrixIndexT b = 0; b < NumBlocks(); b++) {
    ...
    MatrixIndexT this_num_rows = this_block.NumRows(),
                 this_num_cols = this_block.NumCols();
    const CuSubMatrix<Real> src(M, row_offset, this_num_rows, col_offset,
                                this_num_cols);
