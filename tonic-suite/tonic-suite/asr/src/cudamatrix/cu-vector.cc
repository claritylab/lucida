...
template float VecVec(const CuVectorBase<float> &a,
                      const CuVectorBase<float> &b);
template double VecVec(const CuVectorBase<double> &a,
...
template <typename Real>
void CuVectorBase<Real>::CopyColFromMat(const CuMatrixBase<Real> &mat,
                                        MatrixIndexT col) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_copy_col_from_mat(dimGrid, dimBlock, data_, col, mat.Data(), mat.Dim(),
                           dim_);
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuVectorBase::CopyColFromMat",
                                        tim.Elapsed());
  } else
#endif
  {
...
template <>
template <>
void CuVectorBase<double>::CopyColFromMat(const CuMatrixBase<float> &mat,
                                          MatrixIndexT col) {
  KALDI_ASSERT(col < mat.NumCols());
  KALDI_ASSERT(dim_ == mat.NumRows());
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_copy_col_from_mat_df(dimGrid, dimBlock, data_, col, mat.Data(),
                              mat.Dim(), dim_);
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuVectorBase::CopyColFromMat",
                                        tim.Elapsed());
...
template <>
template <>
void CuVectorBase<float>::CopyColFromMat(const CuMatrixBase<double> &mat,
                                         MatrixIndexT col) {
  KALDI_ASSERT(col < mat.NumCols());
  KALDI_ASSERT(dim_ == mat.NumRows());
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_copy_col_from_mat_fd(dimGrid, dimBlock, data_, col, mat.Data(),
                              mat.Dim(), dim_);
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuVectorBase::CopyColFromMat",
                                        tim.Elapsed());
...
void MatrixBase<Real>::CopyRowsFromVec(const CuVectorBase<Real> &v) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    Timer tim;
    if (Stride() == NumCols()) {
      CU_SAFE_CALL(cudaMemcpy(data_, v.Data(), sizeof(Real) * v.Dim(),
                              cudaMemcpyDeviceToHost));
    } else {
      const Real *vec_data = v.Data();
      for (MatrixIndexT r = 0; r < NumRows(); r++) {
        CU_SAFE_CALL(cudaMemcpy(RowData(r), vec_data, sizeof(Real) * NumCols(),
                                cudaMemcpyDeviceToHost));
        vec_data += NumCols();
      }
    }
...
template void MatrixBase<double>::CopyRowsFromVec(
    const CuVectorBase<double> &v);
...
void CuVectorBase<Real>::ApplySoftMax() {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    Timer tim;
    size_t dimBlock =
        dim_ > CU1DBLOCK ? CU1DBLOCK : dim_;  // for cuda_softmax_reduce
                                              // function, dimBlock value is
                                              // fixed min(CU1DBLOCK, dim) ,
                                              // represent CU1DBLOCK threads
    ...
    cuda_softmax_reduce(dimGrid, dimBlock, data_, data_, dim,
                        this->dim_);  // actually dim is not stride...
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
#endif
...
MatrixIndexT CuVectorBase<Real>::ApplyFloor(Real floor_val) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_vec_apply_floor(dimGrid, dimBlock, data_, floor_val, count_vec.Data(),
                         dim_);
    CU_SAFE_CALL(cudaGetLastError());
    num_floored = count_vec.Sum();
    CuDevice::Instantiate().AccuProfile("CuVectorBase::ApplyFloor",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuVectorBase<Real>::ApplyExp() {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuVectorBase::ApplyExp",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuVectorBase<Real>::ApplyLog() {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cudaGetLastError());
    if (flag(0) > 0) KALDI_ERR << "Trying to take log of a negative number.";
    CuDevice::Instantiate().AccuProfile("CuVectorBase::ApplyLog",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuVectorBase<Real>::AddMatVec(const Real alpha,
                                   ...
                                   const Real beta) {
  KALDI_ASSERT(
      (trans == kNoTrans && M.NumCols() == v.dim_ && M.NumRows() == dim_) ||
...
template <typename Real>
void CuVectorBase<Real>::AddSpVec(const Real alpha, const CuSpMatrix<Real> &M,
                                  const CuVectorBase<Real> &v,
                                  const Real beta) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    // Note: in our opinion the CuSpMatrix represents a lower-triangular matrix,
    // but
...
void CuVectorBase<Real>::AddVecVec(Real alpha, const CuVectorBase<Real> &v,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_add_vec_vec(dimGrid, dimBlock, alpha, data_, v.Data(), r.Data(), beta,
                     dim_);
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuVectorBase::AddVecVec",
                                        tim.Elapsed());
  } else
#endif
  {
...
template <typename Real>
bool CuVectorBase<Real>::ApproxEqual(const CuVectorBase<Real> &other,
                                     float tol) const {
  if (dim_ != other.dim_)
    KALDI_ERR << "ApproxEqual: size mismatch " << dim_ << " vs. " << other.dim_;
  KALDI_ASSERT(tol >= 0.0);
  CuVector<Real> tmp(*this);
  tmp.AddVec(-1.0, other);
  BaseFloat tmp_norm = sqrt(VecVec(tmp, tmp)),
            this_norm = sqrt(VecVec(*this, *this));
...
void CuVectorBase<Real>::AddDiagMatMat(Real alpha, const CuMatrixBase<Real> &M,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    int dimGridLimit =
        (transM == kNoTrans && transN == kTrans
             ? 64
             : (transM == kTrans && transN == kNoTrans ? 16 : 32));

    while (M_col_dim > 10 * threads_per_element && dimGrid < dimGridLimit &&
           threads_per_element < 256) {
      threads_per_element *= 2;
      dimGrid = n_blocks(dim * threads_per_element, CU1DBLOCK);
    }

    cuda_add_diag_mat_mat(dimGrid, dimBlock, alpha, data_, dim, M.Data(),
                          M_col_dim, M_row_stride, M_col_stride, N.Data(),
                          N_row_stride, N_col_stride, threads_per_element,
                          beta);
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
...
template <typename Real>
void CuVectorBase<Real>::MulTp(const CuTpMatrix<Real> &M,
                               const MatrixTransposeType trans) {
...
void CuVectorBase<Real>::MulElements(const CuVectorBase<Real> &v) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuVectorBase::MulElements",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuVectorBase<Real>::CopyFromVec(const VectorBase<OtherReal> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    if (sizeof(Real) != sizeof(OtherReal)) {
      ...
      if (dim_ == 0) return;
      Timer tim;
      CU_SAFE_CALL(cudaMemcpy(data_, src.Data(), src.Dim() * sizeof(Real),
                              cudaMemcpyHostToDevice));
      CuDevice::Instantiate().AccuProfile("CuVector::CopyFromVecH2D",
                                          tim.Elapsed());
    }
  } else
#endif
...
// Instantiate the template above.
template void CuVectorBase<float>::CopyFromVec(const VectorBase<float> &src);
template void CuVectorBase<double>::CopyFromVec(const VectorBase<float> &src);
template void CuVectorBase<float>::CopyFromVec(const VectorBase<double> &src);
template void CuVectorBase<double>::CopyFromVec(const VectorBase<double> &src);
...
void CuVectorBase<Real>::CopyToVec(VectorBase<OtherReal> *dst) const {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    if (sizeof(Real) != sizeof(OtherReal)) {
      ...
      if (dim_ == 0) return;
      Timer tim;
      CU_SAFE_CALL(cudaMemcpy(dst->Data(), this->data_, sizeof(Real) * dim_,
                              cudaMemcpyDeviceToHost));
      CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
    }
  } else
...
void CuVector<Real>::Resize(MatrixIndexT dim, MatrixResizeType t) {
  ...
  if (this->dim_ != 0) this->Destroy();
  if (dim == 0) return;
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    Timer tim;
    this->data_ =
        static_cast<Real *>(CuDevice::Instantiate().Malloc(dim * sizeof(Real)));
    this->dim_ = dim;
...
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    if (this->data_ != NULL) CuDevice::Instantiate().Free(this->data_);
  } else
#endif
  {
...
void CuVectorBase<Real>::CopyFromVec(const CuVectorBase<Real> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    if (dim_ == 0) return;
    Timer tim;
    CU_SAFE_CALL(cudaMemcpy(data_, src.data_, src.dim_ * sizeof(Real),
                            cudaMemcpyDeviceToDevice));
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
#endif
...
// instantiate the template above.
template void CuVectorBase<float>::AddVec(float alpha,
                                          const CuVectorBase<double> &vec,
                                          float beta);
template void CuVectorBase<double>::AddVec(double alpha,
                                           const CuVectorBase<float> &vec,
                                           double beta);
...
void CuVectorBase<Real>::AddRowSumMat(Real alpha, const CuMatrixBase<Real> &mat,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    for (int32 block = 0; (block + 1) * CU1DBLOCK <= mat.NumRows(); block++) {
      ...
      cuda_add_row_sum_mat(dimGrid, dimBlock, mat.data_ + offset, temp.data_,
                           d);
    }
...
void CuVectorBase<Real>::AddColSumMat(Real alpha, const CuMatrixBase<Real> &mat,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    for (int32 block = 0; (block + 1) * CU1DBLOCK <= mat.NumCols(); block++) {
      ...
      cuda_add_col_sum_mat(dimGrid, dimBlock, mat.data_ + offset, temp.data_,
                           d);
    }
...
template void CuVectorBase<float>::CopyToVec(VectorBase<float> *dst) const;
template void CuVectorBase<double>::CopyToVec(VectorBase<float> *dst) const;
template void CuVectorBase<float>::CopyToVec(VectorBase<double> *dst) const;
template void CuVectorBase<double>::CopyToVec(VectorBase<double> *dst) const;
