...
inline void cuda_copy_col_from_vec(int Gr, int Bl, float *mat, const float *v,
                                   int col, MatrixDim d) {
  cudaF_copy_col_from_vec(Gr, Bl, mat, v, col, d);
}
...
inline void cuda_scale_diag(int Gr, int Bl, float *mat, float value, int dim) {
  cudaF_scale_diag(Gr, Bl, mat, value, dim);
}
...
inline void cuda_add_mat_mat_div_mat(dim3 Gr, dim3 Bl, const float *A,
                                     ...
                                     MatrixDim d) {
  cudaF_add_mat_mat_div_mat(Gr, Bl, A, B, C, dst, d);
}
...
inline void cuda_transpose_matrix(dim3 Gr, dim3 Bl, float *mat, MatrixDim d) {
  cudaF_transpose_matrix(Gr, Bl, mat, d);
}
...
inline void cuda_copy_from_vec_df(int Gr, int Bl, double *v_out,
                                  const float *v_in, int dim) {
  cudaF_copy_from_vec_df(Gr, Bl, v_out, v_in, dim);
}
inline void cuda_copy_from_vec_fd(int Gr, int Bl, float *v_out,
                                  const float *v_in, int dim) {
  cudaF_copy_from_vec_fd(Gr, Bl, v_out, v_in, dim);
}
...
inline void cuda_vec_min(const float *v, float *value, int dim) {
  cudaF_vec_min(v, value, dim);
}
inline void cuda_vec_max(const float *v, float *value, int dim) {
  cudaF_vec_max(v, value, dim);
}
inline void cuda_trace_mat_mat_trans(const float *A, const float *B,
                                     MatrixDim dA, int B_stride, float *value) {
  cudaF_trace_mat_mat_trans(A, B, dA, B_stride, value);
}
inline void cuda_trace_mat_mat(const float *A, const float *B, MatrixDim dA,
                               int B_stride, float *value) {
  cudaF_trace_mat_mat(A, B, dA, B_stride, value);
}
inline void cuda_add_diag_mat_mat(int Gr, int Bl, float alpha, float *v,
                                  int v_dim, const float *M, int M_cols,
                                  int M_row_stride, int M_col_stride,
                                  const float *N, int N_row_stride,
                                  int N_col_stride, int threads_per_element,
                                  float beta) {
  cudaF_add_diag_mat_mat(Gr, Bl, alpha, v, v_dim, M, M_cols, M_row_stride,
                         M_col_stride, N, N_row_stride, N_col_stride,
...
inline void cuda_copy_col_from_mat(int Gr, int Bl, float *v, int col,
                                   const float *mat, MatrixDim dmat, int dim) {
  cudaF_copy_col_from_mat(Gr, Bl, v, col, mat, dmat, dim);
}
...
inline void cuda_pvec_sum(int Gr, int Bl, float *vec, float *pvec_sum, int dim,
                          int size) {
  cudaF_pvec_sum(Gr, Bl, vec, pvec_sum, dim, size);
}
...
inline void cuda_add_row_sum_mat(dim3 Gr, dim3 Bl, const float *mat,
                                 float *vec_sum, MatrixDim d) {
  cudaF_add_row_sum_mat(Gr, Bl, mat, vec_sum, d);
}
inline void cuda_add_col_sum_mat(dim3 Gr, dim3 Bl, const float *mat,
                                 float *vec_sum, MatrixDim d) {
  cudaF_add_col_sum_mat(Gr, Bl, mat, vec_sum, d);
}
...
inline void cuda_diff_sigmoid(dim3 Gr, dim3 Bl, float *eout, const float *e,
  ...
  cudaF_diff_sigmoid(Gr, Bl, eout, e, y, d, src_stride);
}
...
inline void cuda_diff_tanh(dim3 Gr, dim3 Bl, float *eout, const float *e,
  ...
  cudaF_diff_tanh(Gr, Bl, eout, e, y, d);
}
inline void cuda_softmax(size_t Gr, size_t Bl, float *y, const float *x,
                         MatrixDim d) {
  cudaF_softmax(Gr, Bl, y, x, d);
}
/*
Bl: dimBlock value is fixed min(d.col, CU1DBLOCK), represent CU1DBLOCK threads
reduce a row at the same time.
...
inline void cuda_softmax_part(dim3 Gr, dim3 Bl, const float *X,
                              const int32_cuda *vec_ids, float *Y,
                              MatrixDim d) {
  cudaF_softmax_part(Gr, Bl, X, vec_ids, Y, d);
}

inline void cuda_regularize_l1(dim3 Gr, dim3 Bl, float *wei, float *grad,
                               float l1, float lr, MatrixDim d) {
  cudaF_regularize_l1(Gr, Bl, wei, grad, l1, lr, d);
}
inline void cuda_find_row_max_id(dim3 Gr, dim3 Bl, const float *mat,
                                 float *vec_val, int32_cuda *vec_id,
                                 int32_cuda voff, MatrixDim d) {
  cudaF_find_row_max_id(Gr, Bl, mat, vec_val, vec_id, voff, d);
}
...
inline void cuda_matrix_add_elements(dim3 Gr, dim3 Bl, float *data,
                                     ...
                                     MatrixElement<float> *x, int s) {
  cudaF_matrix_add_elements(Gr, Bl, data, dim, alpha, x, s);
}
...
inline void cuda_copy_col_from_vec(int Gr, int Bl, double *mat, const double *v,
                                   int col, MatrixDim d) {
  cudaD_copy_col_from_vec(Gr, Bl, mat, v, col, d);
}
...
inline void cuda_scale_diag(int Gr, int Bl, double *mat, double value,
                            int dim) {
  cudaD_scale_diag(Gr, Bl, mat, value, dim);
}
...
inline void cuda_add_mat_mat_div_mat(dim3 Gr, dim3 Bl, const double *A,
                                     ...
                                     double *dst, MatrixDim d) {
  cudaD_add_mat_mat_div_mat(Gr, Bl, A, B, C, dst, d);
}
...
inline void cuda_transpose_matrix(dim3 Gr, dim3 Bl, double *mat, MatrixDim d) {
  cudaD_transpose_matrix(Gr, Bl, mat, d);
}
...
inline void cuda_copy_from_vec_df(int Gr, int Bl, double *v_out,
                                  const double *v_in, int dim) {
  cudaD_copy_from_vec_df(Gr, Bl, v_out, v_in, dim);
}
inline void cuda_copy_from_vec_fd(int Gr, int Bl, float *v_out,
                                  const double *v_in, int dim) {
  cudaD_copy_from_vec_fd(Gr, Bl, v_out, v_in, dim);
}
...
inline void cuda_vec_min(const double *v, double *value, int dim) {
  cudaD_vec_min(v, value, dim);
}
inline void cuda_vec_max(const double *v, double *value, int dim) {
  cudaD_vec_max(v, value, dim);
}
inline void cuda_trace_mat_mat_trans(const double *A, const double *B,
                                     MatrixDim dA, int B_stride,
                                     double *value) {
  cudaD_trace_mat_mat_trans(A, B, dA, B_stride, value);
}
inline void cuda_trace_mat_mat(const double *A, const double *B, MatrixDim dA,
                               int B_stride, double *value) {
  cudaD_trace_mat_mat(A, B, dA, B_stride, value);
}
inline void cuda_add_diag_mat_mat(int Gr, int Bl, double alpha, double *v,
                                  int v_dim, const double *M, int M_cols,
                                  int M_row_stride, int M_col_stride,
                                  const double *N, int N_row_stride,
                                  int N_col_stride, int threads_per_element,
                                  double beta) {
  cudaD_add_diag_mat_mat(Gr, Bl, alpha, v, v_dim, M, M_cols, M_row_stride,
                         M_col_stride, N, N_row_stride, N_col_stride,
                         threads_per_element, beta);
}
...
inline void cuda_copy_col_from_mat(int Gr, int Bl, double *v, int col,
                                   const double *mat, MatrixDim dmat, int dim) {
  cudaD_copy_col_from_mat(Gr, Bl, v, col, mat, dmat, dim);
}
inline void cuda_copy_col_from_mat_df(int Gr, int Bl, double *v, int col,
...
inline void cuda_pvec_sum(int Gr, int Bl, double *vec, double *pvec_sum,
                          int dim, int size) {
  cudaD_pvec_sum(Gr, Bl, vec, pvec_sum, dim, size);
}
...
inline void cuda_add_row_sum_mat(dim3 Gr, dim3 Bl, const double *mat,
                                 double *vec_sum, MatrixDim d) {
  cudaD_add_row_sum_mat(Gr, Bl, mat, vec_sum, d);
}
inline void cuda_add_col_sum_mat(dim3 Gr, dim3 Bl, const double *mat,
                                 double *vec_sum, MatrixDim d) {
  cudaD_add_col_sum_mat(Gr, Bl, mat, vec_sum, d);
}
...
inline void cuda_diff_sigmoid(dim3 Gr, dim3 Bl, double *eout, const double *e,
  ...
  cudaD_diff_sigmoid(Gr, Bl, eout, e, y, d, src_stride);
}
...
inline void cuda_diff_tanh(dim3 Gr, dim3 Bl, double *eout, const double *e,
  ...
  cudaD_diff_tanh(Gr, Bl, eout, e, y, d);
}
inline void cuda_softmax(size_t Gr, size_t Bl, double *y, const double *x,
                         MatrixDim d) {
  cudaD_softmax(Gr, Bl, y, x, d);
}
...
inline void cuda_softmax_part(dim3 Gr, dim3 Bl, const double *X,
                              const int32_cuda *vec_ids, double *Y,
                              MatrixDim d) {
  cudaD_softmax_part(Gr, Bl, X, vec_ids, Y, d);
}

inline void cuda_regularize_l1(dim3 Gr, dim3 Bl, double *wei, double *grad,
                               double l1, double lr, MatrixDim d) {
  cudaD_regularize_l1(Gr, Bl, wei, grad, l1, lr, d);
}
inline void cuda_find_row_max_id(dim3 Gr, dim3 Bl, const double *mat,
                                 double *vec_val, int32_cuda *vec_id,
                                 int32_cuda voff, MatrixDim d) {
  cudaD_find_row_max_id(Gr, Bl, mat, vec_val, vec_id, voff, d);
}
inline void cuda_diff_xent(dim3 Gr, dim3 Bl, const int32_cuda *vec_tgt,
...
inline void cuda_matrix_add_elements(dim3 Gr, dim3 Bl, double *data,
                                     ...
                                     MatrixElement<double> *x, int s) {
  cudaD_matrix_add_elements(Gr, Bl, data, dim, alpha, x, s);
}
...
inline void cuda_axpy(int n, float alpha, const float *x, int incx, float *y,
                      int incy) {
  cublasSaxpy(n, alpha, x, incx, y, incy);
}
inline void cuda_axpy(int n, double alpha, const double *x, int incx, double *y,
                      int incy) {
