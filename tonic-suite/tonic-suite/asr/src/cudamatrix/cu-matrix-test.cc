...
static void RandZeroToOneMatrix(MatrixBase<Real> *mat) {
  for (int32 r = 0; r < mat->NumRows(); r++)
    for (int32 c = 0; c < mat->NumCols(); c++) (*mat)(r, c) = RandUniform();
}
...
static void UnitTestCuMatrixCopyRowsFromVec() {
  for (MatrixIndexT p = 0; p < 2; p++) {
    ...
    if (p <= 2)
      ...
      num_cols = 100 + Rand() % 200;

    int32 vec_dim;
    if (p % 2 == 0)
      vec_dim = num_cols;
    else
      vec_dim = num_cols * num_rows;
  ...
  for (MatrixIndexT p = 0; p < 2; p++) {
    ...
    for (MatrixIndexT i = 0; i < num_cols2; i++) {
      ...
      KALDI_ASSERT(indices[i].second >= indices[i].first &&
                   indices[i].second <= num_cols1 && indices[i].first >= 0);
    ...
    for (MatrixIndexT i = 0; i < num_rows; i++) {
      for (MatrixIndexT j = 0; j < num_cols2; j++) {
        ...
        Real sum = 0.0;
        for (MatrixIndexT j2 = start; j2 < end; j2++) sum += src(i, j2);
        dst(i, j) = sum;
      }
    }
...
static void UnitTestCuMatrixSymInvertPosDef() {
  for (int32 i = 0; i < 2; i++) {
    ...
    M_prod.AddMatMat(Real(1.0), M_orig, kNoTrans, M_inverted, kNoTrans,
                     Real(0.0));
...
static void UnitTestCuMatrixTranspose() {
  for (MatrixIndexT i = 1; i < 10; i++) {
    MatrixIndexT dimM = 5 * i + Rand() % 10, dimN = dimM;
    if (i % 2 == 0) dimN += 5;
...
static void UnitTestCuSubMatrix() {
  for (int32 iter = 0; iter < 10; iter++) {
    int32 M1 = 1 + rand() % 10, M2 = 1 + Rand() % 1, M3 = 1 + Rand() % 10,
          M = M1 + M2 + M3, N1 = 1 + rand() % 10, N2 = 1 + Rand() % 1,
          N3 = 1 + Rand() % 10, N = N1 + N2 + N3, m = Rand() % M2,
          n = Rand() % N2;
    CuMatrix<Real> mat(M, N);
    mat.SetRandn();
    CuSubMatrix<Real> submat1(mat, M1, M2, N1, N2),
...
static void UnitTestCuMatrixEqualElementMask() {
  ...
  m1.Range(1, 5, 2, 5)
      .EqualElementMask(m3.Range(0, 5, 0, 5), &m4);  // strides 9, 6, 5
