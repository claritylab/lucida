...
void CuMatrix<Real>::Resize(MatrixIndexT rows, MatrixIndexT cols,
  ...
  if (this->num_rows_ != 0) this->Destroy();
  if (rows == 0) return;
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    ...
    size_t pitch;
    this->data_ = static_cast<Real *>(
        CuDevice::Instantiate().MallocPitch(row_bytes, rows, &pitch));
    this->num_rows_ = rows;
    this->num_cols_ = cols;
...
void CuMatrixBase<Real>::CopyFromBlock(const CuBlockMatrix<Real> &B,
  ...
  if (trans == kNoTrans) {
    ...
    for (int32 b = 0; b < B.NumBlocks(); b++) {
      ...
      CuSubMatrix<Real> this_block(*this, row_offset, num_rows, col_offset,
                                   num_cols);
...
void CuMatrixBase<Real>::CopyFromMat(const CuMatrixBase<OtherReal> &M,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    if (sizeof(Real) == sizeof(OtherReal) && Trans == kNoTrans) {
      ...
      CU_SAFE_CALL(cudaMemcpy2D(data_, dst_pitch, M.data_, src_pitch, width,
                                M.num_rows_, cudaMemcpyDeviceToDevice));
    } else {
    ...
    CuDevice::Instantiate().AccuProfile(
        "CuMatrixBase::CopyFromMat(from other CuMatrixBase)", tim.Elapsed());
  } else
#endif
  {
...
template void CuMatrixBase<float>::CopyFromMat<float>(
    const CuMatrixBase<float> &M, MatrixTransposeType Trans);
template void CuMatrixBase<float>::CopyFromMat<double>(
    const CuMatrixBase<double> &M, MatrixTransposeType Trans);
template void CuMatrixBase<double>::CopyFromMat<float>(
    const CuMatrixBase<float> &M, MatrixTransposeType Trans);
template void CuMatrixBase<double>::CopyFromMat<double>(
    const CuMatrixBase<double> &M, MatrixTransposeType Trans);
...
void CuMatrixBase<Real>::CopyFromMat(const MatrixBase<Real> &src,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    if (trans == kNoTrans) {
      ...
      CU_SAFE_CALL(cudaMemcpy2D(data_, dst_pitch, src.Data(), src_pitch, width,
                                src.NumRows(), cudaMemcpyHostToDevice));

      CuDevice::Instantiate().AccuProfile("CuMatrixBase::CopyFromMat(from CPU)",
                                          tim.Elapsed());
    } else {
...
template <typename Real>
CuMatrix<Real>::CuMatrix(const CuMatrixBase<Real> &other,
                         MatrixTransposeType trans) {
  if (trans == kNoTrans)
    this->Resize(other.NumRows(), other.NumCols(), kUndefined);
  else
...
template <typename Real>
template <typename OtherReal>
CuMatrix<Real>::CuMatrix(const MatrixBase<OtherReal> &other,
                         MatrixTransposeType trans) {
  if (trans == kNoTrans)
    this->Resize(other.NumRows(), other.NumCols(), kUndefined);
  else
...
// Instantiate the template above.
template CuMatrix<float>::CuMatrix(const MatrixBase<float> &other,
                                   MatrixTransposeType trans);
template CuMatrix<double>::CuMatrix(const MatrixBase<float> &other,
                                    MatrixTransposeType trans);
template CuMatrix<float>::CuMatrix(const MatrixBase<double> &other,
                                   MatrixTransposeType trans);
template CuMatrix<double>::CuMatrix(const MatrixBase<double> &other,
                                    MatrixTransposeType trans);
  ...
  if (CuDevice::Instantiate().Enabled()) {
    if (trans == kTrans || sizeof(OtherReal) != sizeof(Real)) {
      ...
      CU_SAFE_CALL(cudaMemcpy2D(dst->Data(), dst_pitch, this->data_, src_pitch,
                                width, this->num_rows_,
                                cudaMemcpyDeviceToHost));

      CuDevice::Instantiate().AccuProfile("CuMatrix::CopyToMatD2H",
                                          tim.Elapsed());
    }
  } else
#endif
...
// instantiate the template above.
template void CuMatrixBase<float>::CopyToMat(MatrixBase<float> *dst,
                                             MatrixTransposeType trans) const;
template void CuMatrixBase<double>::CopyToMat(MatrixBase<float> *dst,
                                              MatrixTransposeType trans) const;
template void CuMatrixBase<float>::CopyToMat(MatrixBase<double> *dst,
                                             MatrixTransposeType trans) const;
template void CuMatrixBase<double>::CopyToMat(MatrixBase<double> *dst,
                                              MatrixTransposeType trans) const;
...
void CuMatrixBase<Real>::SetZeroAboveDiag() {
...
#endif
  {
    ...
    for (int32 r = 0; r + 1 < num_rows; r++) {
      SubVector<Real> vec(mat, r), vec_part(vec, r + 1, num_cols - (r + 1));
      vec_part.SetZero();
    }
  }
}
...
bool CuMatrixBase<Real>::IsUnit(Real tol) const {
  ...
  return (TraceMatMat(*this, *this, kTrans) + this->NumRows() -
              2.0 * this->Trace() <=
          tol * this->NumRows());
}
...
template <typename Real>
void CuMatrixBase<Real>::AddVecToCols(Real alpha, const CuVectorBase<Real> &col,
                                      Real beta) {
  if (col.Dim() != NumRows()) {
    KALDI_ERR << "Non matching dimensions: Rows:" << NumRows()
              << " VectorDim:" << col.Dim();
  }
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_add_vec_to_cols(dimGrid, dimBlock, alpha, col.data_, beta, data_,
                         Dim());
    CU_SAFE_CALL(cudaGetLastError());
...
template <typename Real>
void CuMatrixBase<Real>::AddVecToRows(Real alpha, const CuVectorBase<Real> &row,
                                      Real beta) {
  if (row.Dim() != NumCols()) {
    KALDI_ERR << "Non matching dimensions: Cols:" << NumCols()
              << " VectorDim:" << row.Dim();
  }
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_add_vec_to_rows(dimGrid, dimBlock, alpha, row.data_, beta, data_,
                         Dim());
    CU_SAFE_CALL(cudaGetLastError());
 ...
 */
template <typename Real>
void CuMatrixBase<Real>::AddMatMat(Real alpha, const CuMatrixBase<Real> &A,
                                   MatrixTransposeType transA,
                                   const CuMatrixBase<Real> &B,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cublas_gemm((transB == kTrans ? 'T' : 'N'), (transA == kTrans ? 'T' : 'N'),
                m, n, k, alpha, B.data_, B.Stride(), A.data_, A.Stride(), beta,
                data_, Stride());
...
template <typename Real>
void CuMatrixBase<Real>::SymAddMat2(Real alpha, const CuMatrixBase<Real> &A,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cublas_syrk('U', trans, num_rows_, A_other_dim, alpha, A.Data(), A.Stride(),
...
template <typename Real>
void CuMatrixBase<Real>::AddDiagVecMat(const Real alpha, CuVectorBase<Real> &v,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_add_diag_vec_mat(dimGrid, dimBlock, alpha, data_, Dim(), v.Data(),
                          M.Data(), M_row_stride, M_col_stride, beta);
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
#endif
...
void CuMatrixBase<Real>::Sigmoid(const CuMatrixBase<Real> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
    dim3 dimGrid(n_blocks(src.NumCols(), CU2DBLOCK),
                 n_blocks(src.NumRows(), CU2DBLOCK));
...
an element in "this" matrix = (the element in matrix sv_labels) divided by (the
element in matrix output)
*/
template <typename Real>
void CuMatrix<Real>::CompObjfAndDeriv(
    const std::vector<MatrixElement<Real> > &sv_labels,
    const CuMatrix<Real> &output, Real *tot_objf, Real *tot_weight) {
  {  // check the input.
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    void *addr = CuDevice::Instantiate().Malloc(sv_labels.size() *
                                                sizeof(MatrixElement<Real>));
    CU_SAFE_CALL(cudaMemcpy(addr, sv_labels.data(),
                            sv_labels.size() * sizeof(MatrixElement<Real>),
...
void CuMatrixBase<Real>::Tanh(const CuMatrixBase<Real> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_tanh(dimGrid, dimBlock, this->data_, src.data_, this->Dim(),
              src.Stride());
    CU_SAFE_CALL(cudaGetLastError());
...
void CuMatrixBase<Real>::FindRowMaxId(CuArray<int32> *id) const {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    for (int32 block = 0; (block + 1) * 256 <= num_cols_; block++) {
      ...
      cuda_find_row_max_id(dimGrid, dimBlock, data_ + offset, max.data_,
                           id->Data(), offset, d);
    }
...
void CuMatrixBase<Real>::DiffXent(const CuArray<int32> &tgt,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_diff_xent(dimGrid, dimBlock, tgt.Data(), data_, log_post_tgt->data_,
                   Dim());
...
void CuMatrixBase<Real>::Cholesky(CuMatrixBase<Real> *inv_cholesky) {
  ...
  CuSubMatrix<Real> this_11(*this, 0, dim1, 0, dim1),
      this_12(*this, 0, dim1, dim1, dim2), this_21(*this, dim1, dim2, 0, dim1),
  ...
  /*
    Here is the math on block-wise Cholesky.  We'll use a Matlab-like notation
    for blocks of a matrix,
    e.g. [ A B; C D ], and also for transposes, e.g. A' is the transpose of A.
    Let A be the input matrix; we want to compute both its Cholesky L and its
    inverse Cholesky, which
    we'll call M.
    OK. let  L = [ L11 0; L21 L22 ] be the Cholesky factor of A.
    We have A = L L' = [ L11 0; L21 L22 ] * [ L11' L21'; 0 L22' ].  Multiplying
    it out,
    ...
    We can work out M21 as follows.  We know that [ L11 0; L21 L22 ] [ M11 0;
    M21 M22 ] = [ I 0; 0 I ].
    Considering the zero on the bottom of the rhs, we have: L21 M11 + L22 M21 =
    0, which gives us:
    M21 = - L22^{-1} L21 M11 = - M22 L21 M11.

    Next, we want expressions for L21 and L22.  From the equation A21 = L21
    L11', we have:
    L21 = A21 inv(L11') = A21 M11'
    We can compute L22 and M22 recursively by doing Cholesky (and computing the
    inverse Cholesky)
    on the quantity T = (A22 - L21 L21').   [we give it the name T just for easy
    reference.]
    ...
    Next, we have to consider the in-place nature of the computation, since L
    overwrites A
    ...
    (5):(a)  we first compute the transpose of (L21 M11) is done in the upper
    part of A/L,
  ...
  // (2) compute L21 = A21 M11'.  For now it's in the "wrong place", where M21
  // should be.
  inv_21.AddMatMat(1.0, this_21, kNoTrans, inv_11, kTrans, 0.0);
  // (3) compute T = A22 - L21 L21'.  Note: only the lower triangle of T will be
  // valid, but
  ...
  // (5)(a) compute U = M11' L21'.  We use the storage of this_12 for this.
  // Note that L21 is
...
Real TraceMatMat(const CuMatrixBase<Real> &A, const CuMatrixBase<Real> &B,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    if (trans == kNoTrans) {
      ...
      cuda_trace_mat_mat(A.Data(), B.Data(), A.Dim(), B.Stride(),
                         result_vec.Data());
    } else {
      KALDI_ASSERT(A.NumRows() == B.NumRows() && A.NumCols() == B.NumCols());
      cuda_trace_mat_mat_trans(A.Data(), B.Data(), A.Dim(), B.Stride(),
                               result_vec.Data());
    }
    CU_SAFE_CALL(cudaGetLastError());
    Vector<Real> result_cpu(
        result_vec);  // copying from CUDA faster than summing in CUDA.
...
template float TraceMatMat(const CuMatrixBase<float> &A,
                           const CuMatrixBase<float> &B,
                           MatrixTransposeType trans);
template double TraceMatMat(const CuMatrixBase<double> &A,
    ...
    if (v.Dim() == num_rows_ * num_cols_) {
      if (stride_ == num_cols_) {
        ...
        CU_SAFE_CALL(cudaMemcpy2D(
            data_, stride_ * sizeof(Real), v.Data(), num_cols_ * sizeof(Real),
            num_cols_ * sizeof(Real), num_rows_, cudaMemcpyDeviceToDevice));
      }
...
void CuMatrixBase<Real>::CopyRowsFromVec(const VectorBase<Real> &v) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    if (v.Dim() == num_rows_ * num_cols_) {
      if (stride_ == num_cols_) {
        const Real *v_data = v.Data();
        cudaMemcpy(data_, v_data, sizeof(Real) * num_rows_ * num_cols_,
                   cudaMemcpyHostToDevice);
      } else {
        const Real *v_data = v.Data();
        for (MatrixIndexT r = 0; r < num_rows_; r++) {
          Real *row_data = RowData(r);
          cudaMemcpy(row_data, v_data, sizeof(Real) * num_cols_,
                     cudaMemcpyHostToDevice);
          v_data += num_cols_;
        }
      }
      ...
      dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
      // This is a newer kernel where x corresponds to NumRows() and y to
      // NumCols().
      dim3 dimGrid(n_blocks(NumRows(), CU2DBLOCK),
                   n_blocks(NumCols(), CU2DBLOCK));

      cuda_copy_rows_from_vec(dimGrid, dimBlock, this->data_, this->Dim(),
                              v.Data());
      CU_SAFE_CALL(cudaGetLastError());

      /*      const Real *v_data = v.Data();
      for (MatrixIndexT r = 0; r < num_rows_; r++)
      cudaMemcpy(RowData(r), v_data, sizeof(Real)*num_cols_,
      cudaMemcpyHostToDevice); */
    } else {
      KALDI_ERR << "Wrong sized arguments";
    }
...
void VectorBase<Real>::CopyRowsFromMat(const CuMatrixBase<Real> &mat) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    if (mat.Stride() == mat.NumCols()) {
      cudaMemcpy(data_, mat.Data(), sizeof(Real) * dim_,
                 cudaMemcpyDeviceToHost);
    } else {
      Real *vec_data = data_;
      for (MatrixIndexT r = 0; r < mat.NumRows(); r++) {
    ...
    CuDevice::Instantiate().AccuProfile("CuVectorBase::CopyRowsFromMat",
                                        tim.Elapsed());
  } else
#endif
  {
...
// Instantiate the template above.
template void VectorBase<float>::CopyRowsFromMat(
    const CuMatrixBase<float> &mat);
template void VectorBase<double>::CopyRowsFromMat(
...
void CuMatrixBase<Real>::CopyCols(const CuMatrixBase<Real> &src,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimGrid(n_blocks(NumRows(), CU2DBLOCK),
                 n_blocks(NumCols(), CU2DBLOCK));
    cuda_copy_cols(dimGrid, dimBlock, data_, src.Data(), cuda_reorder.Data(),
                   Dim(), src.Stride());
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
...
void CuMatrixBase<Real>::CopyCols(const CuMatrixBase<Real> &src,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimGrid(n_blocks(NumRows(), CU2DBLOCK),
                 n_blocks(NumCols(), CU2DBLOCK));
    cuda_copy_cols(dimGrid, dimBlock, data_, src.Data(), reorder.Data(), Dim(),
                   src.Stride());
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
...
void CuMatrixBase<Real>::CopyRows(const CuMatrixBase<Real> &src,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimGrid(n_blocks(NumRows(), CU2DBLOCK),
                 n_blocks(NumCols(), CU2DBLOCK));
    cuda_copy_rows(dimGrid, dimBlock, data_, src.Data(), cuda_reorder.Data(),
                   Dim(), src.Stride());
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
...
void CuMatrixBase<Real>::CopyLowerToUpper() {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimGrid(n_blocks(dim, CU2DBLOCK), n_blocks(dim, CU2DBLOCK));
...
template <typename Real>
void Matrix<Real>::Swap(CuMatrix<Real> *mat) {
  mat->Swap(this);
}
// instantiate the template above.
...
template CuMatrix<float>::CuMatrix(const CuMatrixBase<double> &M,
                                   MatrixTransposeType trans);
template CuMatrix<double>::CuMatrix(const CuMatrixBase<float> &M,
                                    MatrixTransposeType trans);
...
template <typename Real>
void CuMatrixBase<Real>::AddMatBlock(Real alpha, const CuMatrixBase<Real> &A,
                                     MatrixTransposeType transA,
                                     const CuBlockMatrix<Real> &B,
                                     MatrixTransposeType transB, Real beta) {
  // Check dimensions
  int32 A_num_rows = A.NumRows(), A_num_cols = A.NumCols(),
        A_row_stride = A.Stride(), A_col_stride = 1, B_num_rows = B.NumRows(),
        B_num_cols = B.NumCols();
  if (transA == kTrans) {
...
#endif
  {
    ...
    for (int32 b = 0; b < B_num_blocks; b++) {
      ...
      CuSubMatrix<Real> this_part(*this, 0, num_rows_, col_offset,
                                  this_num_cols);
      CuSubMatrix<Real> A_part =
          (transA == kNoTrans
               ? CuSubMatrix<Real>(A, 0, num_rows_, row_offset, this_num_rows)
               : CuSubMatrix<Real>(A, row_offset, this_num_rows, 0, num_rows_));
...
template <typename Real>
void CuMatrixBase<Real>::AddElements(
    Real alpha, const std::vector<MatrixElement<Real> > &input) {
...
#if HAVE_CUDA == 1
  if (CuDevice::Instantiate().Enabled()) {
    void *addr = CuDevice::Instantiate().Malloc(input.size() *
                                                sizeof(MatrixElement<Real>));
    ...
    cuda_matrix_add_elements(dimGrid, dimBlock, this->data_, this->Dim(), alpha,
                             (MatrixElement<Real> *)addr, input.size());
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().Free(addr);
    CuDevice::Instantiate().AccuProfile(__func__, tim.Elapsed());
  } else
...
template <typename Real>
void CuMatrixBase<Real>::EqualElementMask(const CuMatrixBase<Real> &mat,
                                          CuMatrix<Real> *mask) const {
  // Check the inputs:
