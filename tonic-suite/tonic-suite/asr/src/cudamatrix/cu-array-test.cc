...
void AssertEqual(const std::vector<T> &vec1, const std::vector<T> &vec2) {
  ...
  if (vec1.size() > 0) {
    ...
    for (size_t i = 0; i < size; i++) KALDI_ASSERT(p1[i] == p2[i]);
  }
}
...
static void UnitTestCuArray() {
  for (int32 i = 0; i < 30; i++) {
    int32 size = Rand() % 5;
    size = size * size *
           size;  // Have a good distribution of sizes, including >256.
    int32 size2 = Rand() % 4;
    ...
    for (size_t i = 0; i < byte_size; i++) rand_c[i] = Rand() % 256;
    if (!vec.empty()) {
      std::memcpy((void *)&(vec[0]), (void *)&(rand_c[0]), byte_size);
    }
    ...
    {  // test resize with resize_type = kSetZero.
      ...
      if (!vec2.empty()) std::memset(&(vec2[0]), 0, vec2.size() * sizeof(T));
      std::vector<T> vec3;
    ...
    if (sizeof(T) == sizeof(int32) &&
        size > 0) {  // test Set for type int32, or same size.
      CuArray<T> cu_vec(vec);
      cu_vec.Set(vec[0]);
