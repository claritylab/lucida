...
void TestCuMatrixMatMat(int32 dim) {
  ...
  KALDI_LOG << "For CuMatrix::AddMatMat" << NameOf<Real>()
...
void TestSymInvertPosDef(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::TestCuInvertPosDef" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
static void TestCuMatrixCompObjfAndDeriv(int32 dim) {
  ...
  KALDI_LOG << "For CuMatrix::CompObjfAndDeriv" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
...
static void TestCuFindRowMaxId(int32 dim) {
  ...
  int iter = 0;
  Timer tim;
  for (; tim.Elapsed() < time_in_secs; iter++) Di.FindRowMaxId(&Dmax);

  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::FindRowMaxId" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
...
void TestCuMatrixSigmoid(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::Sigmoid" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixMulRowsGroupMat(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops =
      (fdim * fdim * group_size * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::MulRowsGroupMat" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixSoftmax(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::Softmax" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixGroupPnorm(int32 dim) {
  ...
  Timer tim;
  int32 iter = 0;
  for (; tim.Elapsed() < time_in_secs; iter++) N.GroupPnorm(M, 2.0);

  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::GroupPnorm" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}

template <typename Real>
void TestCuMatrixGroupPnormDeriv(int32 dim) {
  BaseFloat time_in_secs = 0.025;
  int32 group_size = 4;
  ...
  for (; tim.Elapsed() < time_in_secs; iter++) O.GroupPnormDeriv(M, N, 2.0);

  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::GroupPnormDeriv" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixCholesky(int32 dim) {
  ...
  Timer tim;
  int32 iter = 0;
  for (; tim.Elapsed() < time_in_secs; iter++) M.Cholesky();
...
void TestCuMatrixCopyLowerToUpper(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::CopyLowerToUpper" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixCopyFromTp(int32 dim, MatrixTransposeType trans) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::CopyFromTp"
            << (trans == kNoTrans ? "[NoTrans]" : "[Trans]") << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixCopyFromSp(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::CopyFromSp" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixCopyUpperToLower(int32 dim) {
  ...
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::CopyUpperToLower" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
...
void TestCuMatrixSetZeroAboveDiag(int32 dim) {
  ...
  Timer tim;
  int32 iter = 0;
  for (; tim.Elapsed() < time_in_secs; iter++) M.SetZeroAboveDiag();
  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::SetZeroAboveDiag" << NameOf<Real>()
            << ", for dim = " << dim << ", speed was " << gflops
            << " gigaflops.";
}
template <typename Real>
void TestCuMatrixLookup(int32 dim) {
  ...
  Timer tim;
  int32 iter = 0;
  for (; tim.Elapsed() < time_in_secs; iter++) H.Lookup(indices, &output);

  BaseFloat fdim = dim;
  BaseFloat gflops = (fdim * fdim * iter) / (tim.Elapsed() * 1.0e+09);
  KALDI_LOG << "For CuMatrix::Lookup" << NameOf<Real>() << ", for dim = " << dim
...
void CudaMatrixSpeedTest() {
  ...
  int32 ns = sizes.size();
  for (int32 s = 0; s < ns; s++) TestCuMatrixMatMat<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) {
    TestCuMatrixAddDiagVecMat<Real>(sizes[s], kNoTrans);
    TestCuMatrixAddDiagVecMat<Real>(sizes[s], kTrans);
  }
  for (int32 s = 0; s < ns; s++) TestSymInvertPosDef<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixCholesky<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixSigmoid<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuFindRowMaxId<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixCompObjfAndDeriv<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixMulRowsGroupMat<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixSoftmax<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixGroupPnorm<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixGroupPnormDeriv<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixTraceMatMat<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixCopyLowerToUpper<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++)
    TestCuMatrixCopyFromTp<Real>(sizes[s], kNoTrans);
  for (int32 s = 0; s < ns; s++) TestCuMatrixCopyFromTp<Real>(sizes[s], kTrans);
  for (int32 s = 0; s < ns; s++) TestCuMatrixCopyFromSp<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixCopyUpperToLower<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixSetZeroAboveDiag<Real>(sizes[s]);
  for (int32 s = 0; s < ns; s++) TestCuMatrixLookup<Real>(sizes[s]);
}
