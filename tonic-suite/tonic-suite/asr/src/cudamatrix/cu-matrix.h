...
class CuMatrixBase {
 public:
  ...
  friend void cu::RegularizeL1<Real>(CuMatrixBase<Real> *weight,
                                     CuMatrixBase<Real> *grad, Real l1,
                                     Real lr);
  ...
  /// sigmoid output.  Does, element-by-element, *this = diff * value * (1 -
  /// value).
  ...
  /// inverse.  *this is treated as a symmetric matrix but only the lower
  /// triangle
  /// is accessed.
  ...
  /// for each row, the max value is first subtracted for good numerical
  /// stability
  ...
  // void CopyRowsFromMat(int32 r, const CuMatrixBase<Real> &src, int32 src_ro,
  int32 dst_ro);
  */
  ...
  /// divide each row into src.NumCols() groups, and then scale i'th row's jth
  /// group of elements by src[i, j].
  ...
  void AddMatMat(Real alpha, const CuMatrixBase<Real> &A,
  ...
  void AddMatMatDivMat(const CuMatrixBase<Real> &A, const CuMatrixBase<Real> &B,
  ...
  void AddMatBlock(Real alpha, const CuMatrixBase<Real> &A,
                   MatrixTransposeType transA, const CuBlockMatrix<Real> &B,
                   MatrixTransposeType transB, Real beta);
  ...
  void AddMatSp(const Real alpha, const CuMatrixBase<Real> &A,
                MatrixTransposeType transA, const CuSpMatrix<Real> &B,
                const Real beta) {
    CuMatrix<Real> M(B);
  ...
  void AddSpMat(const Real alpha, const CuSpMatrix<Real> &A,
  ...
  /// this <-- beta*this + alpha*A*B.
  void AddTpMat(const Real alpha, const CuTpMatrix<Real> &A,
                MatrixTransposeType transA, const CuMatrixBase<Real> &B,
  ...
  /// this <-- beta*this + alpha*A*B.
  void AddMatTp(const Real alpha, const CuMatrixBase<Real> &A,
                MatrixTransposeType transA, const CuTpMatrix<Real> &B,
                MatrixTransposeType transB, const Real beta) {
    CuMatrix<Real> M(B);
  ...
  inline CuSubMatrix<Real> Range(const MatrixIndexT row_offset,
    ...
    return CuSubMatrix<Real>(*this, row_offset, num_rows, col_offset, num_cols);
  }
  inline CuSubMatrix<Real> RowRange(const MatrixIndexT row_offset,
                                    const MatrixIndexT num_rows) const {
    return CuSubMatrix<Real>(*this, row_offset, num_rows, 0, num_cols_);
  }
  ...
  /// Return the trace. If check_square = true, will crash if matrix is not
  /// square.
  ...
  // This function, adds a list of MatrixElements (scaled by alpha) to
  // corresponding locations to
  // (*this).
 ...
 protected:
  ...
  /// Get raw row pointer
  inline const Real *RowData(MatrixIndexT r) const {
  ...
  CuMatrixBase<Real>(Real *data, MatrixIndexT num_rows, MatrixIndexT num_cols,
                     MatrixIndexT stride)
      : data_(data),
        num_cols_(num_cols),
        num_rows_(num_rows),
...
class CuMatrix : public CuMatrixBase<Real> {
 public:
  ...
  /// Here, A is interpreted as a matrix of probabilities, and "elements" as a
  /// list
  ...
  void CompObjfAndDeriv(const std::vector<MatrixElement<Real> > &elements,
                        const CuMatrix<Real> &A, Real *tot_objf,
                        Real *tot_weight);
...
class CuSubMatrix : public CuMatrixBase<Real> {
 public:
  inline CuSubMatrix(const CuMatrixBase<Real> &mat,
                     const MatrixIndexT row_offset, const MatrixIndexT num_rows,
                     const MatrixIndexT col_offset,
...
template <typename Real>
bool ApproxEqual(const CuMatrixBase<Real> &A, const CuMatrixBase<Real> &B,
                 Real tol = 0.01) {
...
template <typename Real>
bool SameDimAndStride(const CuMatrixBase<Real> &M,
...
Matrix<Real>::Matrix(const CuMatrixBase<OtherReal> &M,
                     MatrixTransposeType trans) {
  if (trans == kNoTrans)
    Init(M.NumRows(), M.NumCols());
  else
    Init(M.NumCols(), M.NumRows());
