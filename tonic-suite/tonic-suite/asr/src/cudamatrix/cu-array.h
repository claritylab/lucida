...
class CuArray {
 ...
 public:
  ...
  explicit CuArray<T>(const std::vector<T> &src) : dim_(0), data_(NULL) {
    CopyFromVec(src);
  }

  explicit CuArray<T>(const CuArray<T> &src) : dim_(0), data_(NULL) {
    CopyFromArray(src);
  }
  ...
  /// Set to a constant value.  Note: any copying is done as if using memcpy,
  /// and
  /// assignment operators or destructors are not called.  This is NOT
  /// IMPLEMENTED
  /// YET except for T == int32 (the current implementation will just crash).
  void Set(const T &value);

  CuArray<T> &operator=(const CuArray<T> &in) {
    this->CopyFromArray(in);
    return *this;
  }
