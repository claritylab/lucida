...
void CuPackedMatrix<Real>::CopyFromPacked(const CuPackedMatrix<Real> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(
        cudaMemcpy(data_, src.data_, num_bytes, cudaMemcpyDeviceToDevice));
...
void CuPackedMatrix<Real>::CopyFromPacked(const PackedMatrix<Real> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::CopyFromPacked2",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuPackedMatrix<Real>::CopyToPacked(PackedMatrix<Real> *dst) const {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(
        cudaMemcpy(dst->data_, data_, num_bytes, cudaMemcpyDeviceToHost));
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::CopyToPackedD2H",
                                        tim.Elapsed());
  } else
#endif
  {
...
/*
template<typename Real>
void CuPackedMatrix<Real>::CopyRowsFromPacked(int32 r, const
CuPackedMatrix<Real> &src, int32 src_ro, int32 dst_ro) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cudaMemcpy2D(p_dst, dst_pitch, p_src, src_pitch, width, r,
cudaMemcpyDeviceToDevice));
  ...
  #endif
  {
    memcpy(Data()+dst_ro*stride_, src.Data()+src_ro*src.Stride(),
r*stride_*sizeof(Real));
  }
} */
...
void CuPackedMatrix<Real>::SetZero() {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(
        cudaMemset(reinterpret_cast<void *>(this->data_), 0, num_bytes));
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::SetZero",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuPackedMatrix<Real>::SetDiag(Real alpha) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::SetDiag",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuPackedMatrix<Real>::ScaleDiag(Real alpha) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::ScaleDiag",
                                        tim.Elapsed());
  } else
#endif
  {
...
template <typename Real>
void CuPackedMatrix<Real>::AddPacked(const Real alpha,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::AddPacked",
                                        tim.Elapsed());
  } else
#endif
  {
...
void CuPackedMatrix<Real>::AddToDiag(Real r) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    CU_SAFE_CALL(cudaGetLastError());
    CuDevice::Instantiate().AccuProfile("CuPackedMatrix::AddToDiag",
                                        tim.Elapsed());
  } else
#endif
  {
