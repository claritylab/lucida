...
void CuArray<T>::Resize(MatrixIndexT dim, MatrixResizeType resize_type) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    Timer tim;
    this->data_ =
        static_cast<T *>(CuDevice::Instantiate().Malloc(dim * sizeof(T)));
    this->dim_ = dim;
...
void CuArray<T>::CopyFromVec(const std::vector<T> &src) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    Timer tim;
    CU_SAFE_CALL(cudaMemcpy(data_, &src.front(), src.size() * sizeof(T),
...
void CuArray<T>::CopyToVec(std::vector<T> *dst) const {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    Timer tim;
    CU_SAFE_CALL(cudaMemcpy(&dst->front(), Data(), dim_ * sizeof(T),
...
#endif
  {
    for (int32 i = 0; i < dim_; i++) data_[i] = value;
  }
}
