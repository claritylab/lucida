...
void CuRand<Real>::SeedBuffer(MatrixIndexT state_size, uint32 **tgt) {
  ...
  if (device.Enabled()) {
    ...
    *tgt = static_cast<uint32 *>(device.Malloc(state_size_in_bytes));
    CU_SAFE_CALL(cudaMemcpy(*tgt, &(temp_rand_data[0]), state_size_in_bytes,
...
void CuRand<Real>::RandUniform(CuMatrixBase<Real> *tgt) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    dim3 dimGrid(n_blocks(tgt->num_cols_, CU2DBLOCK),
                 n_blocks(tgt->num_rows_, CU2DBLOCK));
...
void CuRand<Real>::RandGaussian(CuMatrixBase<Real> *tgt) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_gauss_rand(dimGrid, dimBlock, tgt->data_, z1_, z2_, z3_, z4_,
                    tgt->Dim());
    CU_SAFE_CALL(cudaGetLastError());
...
void CuRand<Real>::RandGaussian(CuVectorBase<Real> *tgt) {
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    cuda_vec_gauss_rand(dimGrid, dimBlock, tgt->Data(), z1_, z2_, z3_, z4_,
                        tgt->Dim());
...
template <typename Real>
void CuRand<Real>::BinarizeProbs(const CuMatrix<Real> &probs,
  ...
  if (CuDevice::Instantiate().Enabled()) {
    ...
    // (this is done in host, for good performance it is better to avoid
    // re-seeding)
    ...
    dim3 dimBlock(CU2DBLOCK, CU2DBLOCK);
    dim3 dimGrid(n_blocks(states->num_cols_, CU2DBLOCK),
                 n_blocks(states->num_rows_, CU2DBLOCK));

    cuda_binarize_probs(dimGrid, dimBlock, states->data_, probs.data_,
                        tmp_.data_, states->Dim());
    CU_SAFE_CALL(cudaGetLastError());
...
#endif
  {
    for (int32 r = 0; r < states->num_rows_; r++) {
      for (int32 c = 0; c < states->num_cols_; c++) {
        states->Mat()(r, c) =
            ((kaldi::RandUniform() < probs.Mat()(r, c)) ? 1 : 0);
      }
    }
  }
}
