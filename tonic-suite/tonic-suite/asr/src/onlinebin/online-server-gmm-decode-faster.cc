...
void SendPartialResult(const std::vector<int32> &words,
                       const fst::SymbolTable *word_syms, const bool line_break,
                       const int32 serv_sock, const sockaddr_in &client_addr) {
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Decode speech, using feature batches received over a network "
        "connection\n\n"
    ...
    std::cerr << std::endl
              << "Listening on UDP port " << udp_port << " ... " << std::endl;
      ...
      if (dstate & (decoder.kEndFeats | decoder.kEndUtt)) {
        ...
        if (decoder.PartialTraceback(&out_fst)) {
          fst::GetLinearSymbolSequence(out_fst, static_cast<vector<int32> *>(0),
                                       &word_ids,
                                       static_cast<LatticeArc::Weight *>(0));
          SendPartialResult(word_ids, word_syms, false, udp_input.descriptor(),
                            udp_input.client_addr());
