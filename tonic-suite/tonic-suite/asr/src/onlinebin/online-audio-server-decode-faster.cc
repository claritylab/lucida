...
int32 main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        "Starts a TCP server that receives RAW audio and outputs aligned "
        "words.\n"
        "A sample client can be found in: onlinebin/online-audio-client\n\n"
        "Usage: ./online-audio-server-decode-faster [options] model-in "
        "fst-in word-symbol-table silence-phones word_boundary_file tcp-port "
        "[lda-matrix-in]\n\n"
        "example: online-audio-server-decode-faster --verbose=1 --rt-min=0.5 "
        "--rt-max=3.0 --max-active=6000\n"
        "--beam=72.0 --acoustic-scale=0.0769 final.mdl graph/HCLG.fst "
        "graph/words.txt '1:2:3:4:5'\n"
    ...
    if (po.NumArgs() == 7) lda_mat_rspecifier = po.GetOptArg(7);
    ...
    if (!tcp_server.Listen(port)) return 0;
    ...
    while (true) {
      ...
      Mfcc mfcc(mfcc_opts);
      FeInput fe_input(
          au_src, &mfcc, frame_length * (16000 / 1000),
          mfcc_frame_shift *
              (16000 / 1000));  // we always assume 16 kHz Fs on input
      ...
      while (1) {
        ...
        if (dstate & (decoder.kEndFeats | decoder.kEndUtt)) {
          ...
          if (words_num > 0) {
            ...
            for (size_t i = 0; i < word_ids.size(); i++) {
              if (word_ids[i] == 0) continue;  // skip silences...
