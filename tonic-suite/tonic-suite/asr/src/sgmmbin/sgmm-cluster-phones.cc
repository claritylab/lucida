...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        ...
        "Usage: sgmm-cluster-phones [options] <sgmm-in> <tree-stats-in> "
        "<phone-sets-in> <clustered-phones-out>\n"
    ...
    // 5 6
    // ...
    // Format of questions.txt output is similar, but with more lines (and the
    // same phone
    // may appear on multiple lines).

    // bool binary = true;
    int32 P = 1, N = 3;  // Note: N does not matter.
    std::string pdf_class_list_str =
        "1";  // 1 is just the central position of 3.
    ...
    po.Register("pdf-class-list", &pdf_class_list_str,
                "Colon-separated list of HMM positions to consider [Default = "
                "1: just central position for 3-state models].");
    po.Register("mode", &mode,
                "Mode of operation: \"questions\"->sets suitable for decision "
                "trees; \"k-means\"->k-means algorithm, output k classes (set "
                "num-classes options)\n");
    ...
    std::string sgmm_rxfilename = po.GetArg(1), stats_rxfilename = po.GetArg(2),
    ...
    {  // Read tree stats.
      bool binary_in;
      SgmmClusterable sc(am_sgmm,
                         H);  // dummy needed to provide type and sgmm ref.
      Input ki(stats_rxfilename, &binary_in);
      ReadBuildTreeStats(ki.Stream(), binary_in, sc, &stats);
    }
    KALDI_LOG << "Number of separate states in stats is " << stats.size();

    std::vector<int32> pdf_class_list;
    if (!SplitStringToIntegers(pdf_class_list_str, ":", false,
                               &pdf_class_list) ||
        pdf_class_list.empty()) {
      KALDI_ERR << "Invalid pdf-class-list string [expecting colon-separated "
                   "list of integers]: " << pdf_class_list_str;
    }
    ...
    if (mode == "questions") {
      if (num_classes != -1)
        KALDI_ERR << "num-classes option is not (currently) compatible "
                     "with \"questions\" mode.";
      AutomaticallyObtainQuestions(stats, phone_sets, pdf_class_list, P,
                                   &phone_sets_out);
      ...
      KMeansClusterPhones(stats, phone_sets, pdf_class_list, P, num_classes,
                          &phone_sets_out);
    }
