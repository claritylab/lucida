...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "e.g.: sgmm-acc-stats2 1.mdl 1.ali scp:train.scp ark:1.posts num.acc "
        "den.acc\n";
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!posteriors_reader.HasKey(utt)) {
          ...
          for (size_t j = 0; j < posterior[i].size(); j++) {
            ...
            if (acc_flags & kaldi::kSgmmTransitions) {
              trans_model.Accumulate(
                  abs_weight, tid,
                  weight > 0 ? &num_transition_accs : &den_transition_accs);
            }
            tot_like_this_file +=
                (weight > 0 ? num_sgmm_accs : den_sgmm_accs)
                    .Accumulate(am_sgmm, per_frame_vars, spk_vars.v_s, pdf_id,
        ...
        num_sgmm_accs.CommitStatsForSpk(
