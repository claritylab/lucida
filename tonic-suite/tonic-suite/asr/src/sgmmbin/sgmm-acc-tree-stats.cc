...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        ...
        "Usage:  sgmm-acc-tree-stats [options] sgmm-model-in "
        "features-rspecifier "
        "alignments-rspecifier [tree-accs-out]\n"
        "e.g.: sgmm-acc-tree-stats --ci-phones=48:49 1.mdl scp:train.scp "
        "ark:1.ali 1.tacc\n";
    ...
    std::string sgmm_filename = po.GetArg(1), feature_rspecifier = po.GetArg(2),
    ...
    std::vector<SpMatrix<double> >
        H;  // Not initialized in this program-- not needed
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!alignment_reader.HasKey(utt)) {
        ...
        if (!AccumulateSgmmTreeStats(trans_model, am_sgmm, H, N, P, ci_phones,
                                     alignment, gselect, spk_vars, mat,
                                     &tree_stats)) {
          num_err++;
        } else {
          num_done++;
    ...
    for (std::map<EventType, SgmmClusterable *>::const_iterator iter =
             tree_stats.begin();
         iter != tree_stats.end(); iter++) {
      stats.push_back(std::make_pair(iter->first,
                                     static_cast<Clusterable *>(iter->second)));
    }
    tree_stats.clear();
    ...
    KALDI_LOG << "Accumulated stats for " << num_done << " files, " << num_err
              << " failed.";
