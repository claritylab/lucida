...
namespace kaldi {
void InitAndOutputSgmm(const HmmTopology &topo, const AmSgmm &am_sgmm,
                       const ContextDependency &ctx_dep,
                       const std::vector<SpMatrix<double> > &H,
                       const BuildTreeStatsType &stats,
                       const std::string &sgmm_wxfilename, bool binary) {
  ...
  for (size_t i = 0; i < split_stats.size(); i++)
    KALDI_ASSERT(!split_stats[i].empty() &&
                 "Tree has leaves with no stats."
  ...
  for (int32 iter = 0; iter < 5; iter++) {  // Update for
    ...
    updater.UpdatePhoneVectorsCheckedFromClusterable(summed_sgmm_stats, H,
                                                     &am_sgmm_out);
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: sgmm-init-from-tree-stats [options] <old-sgmm> <tree> "
        "<sgmm-tree-stats> <sgmm-out>\n";
    ...
    InitAndOutputSgmm(topo, am_sgmm, ctx_dep, H, stats, sgmm_out_filename,
                      binary);
