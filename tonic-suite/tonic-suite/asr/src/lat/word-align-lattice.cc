...
// Copyright 2011-2012  Microsoft Corporation  Johns Hopkins University (Author:
// Daniel Povey)
 ...
 public:
  ...
  class ComputationState {  /// The state of the computation in which,
   ...
   public:
    ...
    void Advance(const CompactLatticeArc &arc, LatticeWeight *weight) {
      ...
      transition_ids_.insert(transition_ids_.end(), string.begin(),
                             string.end());
    ...
    /// a warning.  In this case it may or may not [output an arc and return
    /// true],
    ...
    /// yourself.
    bool OutputArc(const WordBoundaryInfo &info, const TransitionModel &tmodel,
    ...
    bool operator==(const ComputationState &other) const {
      return (transition_ids_ == other.transition_ids_ &&
              word_labels_ == other.word_labels_ && weight_ == other.weight_);
    }
  ...
  StateId GetStateForTuple(const Tuple &tuple, bool add_to_queue) {
    ...
    if (iter == map_.end()) {  // not in map.
      ...
      if (add_to_queue) queue_.push_front(std::make_pair(tuple, output_state));
      return output_state;
    } else {
      return iter->second;
    }
  ...
  void ProcessFinal(Tuple tuple, StateId output_state) {
    ...
    if (tuple.comp_state.IsEmpty()) {  // computation state doesn't have
      ...
      lat_arc.nextstate =
          GetStateForTuple(tuple, true);  // true == add to queue.
      ...
      // lattice will have no output arcs (and unit final-prob), so there will
      // be
  ...
  void ProcessQueueElement() {
    ...
    if (tuple.comp_state.OutputArc(info_, tmodel_, &lat_arc, &error_)) {
      ...
      // when there's nothing to output, we'll process arcs from the
      // input-state.
      ...
      if (lat_.Final(tuple.input_state) != CompactLatticeWeight::Zero()) {
        KALDI_ASSERT(lat_.Final(tuple.input_state) ==
      ...
      for (fst::ArcIterator<CompactLattice> aiter(lat_, tuple.input_state);
        ...
        StateId next_output_state =
            GetStateForTuple(next_tuple, true);  // true == add to queue,
        // if not already present.
        ...
        lat_out_->AddArc(
            output_state,
            CompactLatticeArc(
                0, 0, CompactLatticeWeight(weight, std::vector<int32>()),
                next_output_state));
      }
    }
  }

  LatticeWordAligner(const CompactLattice &lat, const TransitionModel &tmodel,
    ...
    if (info_.partial_word_label == 0 || info_.silence_label == 0) {
      ...
      if (info_.silence_label == 0) info_.silence_label = unused_label;
    }
  }
  ...
  bool AlignLattice() {
    ...
    StateId start_state =
        GetStateForTuple(initial_tuple, true);  // True = add this to queue.
...
bool LatticeWordAligner::ComputationState::OutputSilenceArc(
  ...
  for (i = 1; i < len; i++) {
    ...
    if (this_phone != phone && !*error) {  // error condition: should have
      ...
      *error = true;
      KALDI_WARN
          << "Phone changed before final transition-id found "
             "[broken lattice or mismatched model or wrong --reorder option?]";
    }
    if (tmodel.IsFinal(tid)) break;
  }
  if (i == len) return false;  // fell off loop.
  i++;               // go past the one for which IsFinal returned true.
  if (info.reorder)  // we have to consume the following self-loop
                     // transition-ids.
    while (i < len && tmodel.IsSelfLoop(transition_ids_[i])) i++;
  if (i == len)
    return false;  // we don't know if it ends here... so can't output arc.
...
bool LatticeWordAligner::ComputationState::OutputOnePhoneWordArc(
  ...
  *arc_out = CompactLatticeArc(
      word, word, CompactLatticeWeight(weight_, tids_out), fst::kNoStateId);
  transition_ids_.erase(transition_ids_.begin(),
                        transition_ids_.begin() + i);  // delete these
  word_labels_.erase(word_labels_.begin(),
                     word_labels_.begin() + 1);  // remove the word we output.
  weight_ = LatticeWeight::One();                // we just output the weight.
  return true;
}
...
bool LatticeWordAligner::ComputationState::OutputNormalWordArc(
  ...
  if (info.reorder)  // Skip over any reordered self-loops for this
                     // final-transition
  ...
  for (; i < len; i++) {
    ...
    if (info.TypeOfPhone(this_phone) == WordBoundaryInfo::kWordEndPhone) break;
  ...
  if (tmodel.TransitionIdToPhone(transition_ids_[i - 1]) != final_phone &&
    ...
    *error = true;
    KALDI_WARN
        << "Phone changed while following final self-loop "
           "[broken lattice or mismatched model or wrong --reorder option?]";
  }
...
static bool IsPlausibleWord(const WordBoundaryInfo &info,
  ...
  if ((info.TypeOfPhone(first_phone) ==
           WordBoundaryInfo::kWordBeginAndEndPhone &&
...
void LatticeWordAligner::ComputationState::OutputArcForce(
  ...
  if (!word_labels_.empty() &&
    ...
    // output, and some transition-ids.  We assume that the normal OutputArc was
    // called
    ...
    *arc_out = CompactLatticeArc(0, 0, cw, fst::kNoStateId);
    ...
    if (info.TypeOfPhone(first_phone) == WordBoundaryInfo::kNonWordPhone) {
      ...
      if (!*error) {  // Check that it ends at the end state of silence; error
                      // otherwise.
      ...
      CompactLatticeWeight cw(weight_, transition_ids_);
      *arc_out = CompactLatticeArc(info.silence_label, info.silence_label, cw,
                                   fst::kNoStateId);
    } else {
      ...
      CompactLatticeWeight cw(weight_, transition_ids_);
      *arc_out =
          CompactLatticeArc(info.partial_word_label, info.partial_word_label,
                            cw, fst::kNoStateId);
    }
    transition_ids_.clear();
    weight_ = LatticeWeight::One();
...
void WordBoundaryInfo::SetOptions(const std::string int_list,
                                  PhoneType phone_type) {
  KALDI_ASSERT(!int_list.empty() && phone_type != kNoPhone);
  std::vector<int32> phone_list;
  if (!kaldi::SplitStringToIntegers(int_list, ":", false, &phone_list) ||
      phone_list.empty())
...
WordBoundaryInfo::WordBoundaryInfo(const WordBoundaryInfoOpts &opts) {
  ...
  SetOptions(
      opts.silence_phones,
      (opts.silence_has_olabels ? kWordBeginAndEndPhone : kNonWordPhone));
...
void WordBoundaryInfo::Init(std::istream &stream) {
  ...
  while (std::getline(stream, line)) {
    ...
    SplitStringToVector(line, " \t\r", true,
                        &split_line);  // split the line by space or tab
    int32 p = 0;
    if (split_line.size() != 2 || !ConvertStringToInteger(split_line[0], &p))
    ...
    if (t == "nonword")
      phone_to_type[p] = kNonWordPhone;
    else if (t == "begin")
      phone_to_type[p] = kWordBeginPhone;
    else if (t == "singleton")
      phone_to_type[p] = kWordBeginAndEndPhone;
    else if (t == "end")
      phone_to_type[p] = kWordEndPhone;
    else if (t == "internal")
      phone_to_type[p] = kWordInternalPhone;
    else
      KALDI_ERR << "Invalid line in word-boundary file: " << line;
  }
  if (phone_to_type.empty()) KALDI_ERR << "Empty word-boundary file";
}

bool WordAlignLattice(const CompactLattice &lat, const TransitionModel &tmodel,
                      const WordBoundaryInfo &info, int32 max_states,
                      CompactLattice *lat_out) {
...
class WordAlignedLatticeTester {
 public:
  ...
  void Test() {
    ...
    for (StateId s = 0; s < aligned_lat_.NumStates(); s++) {
      for (fst::ArcIterator<CompactLattice> iter(aligned_lat_, s); !iter.Done();
           iter.Next()) {
        TestArc(iter.Value());
      }
 ...
 private:
  void TestArc(const CompactLatticeArc &arc) {
    if (!(TestArcSilence(arc) || TestArcNormalWord(arc) ||
          TestArcOnePhoneWord(arc) || TestArcEmpty(arc)))
      KALDI_ERR << "Invalid arc in aligned CompactLattice: " << arc.ilabel
                << " " << arc.olabel << " " << arc.nextstate << " "
                << arc.weight;
  }
  bool TestArcEmpty(const CompactLatticeArc &arc) {
    if (arc.ilabel != 0)
      return false;  // Check there is no label.  Note, ilabel==olabel.
  ...
  bool TestArcSilence(const CompactLatticeArc &arc) {
    ...
    if (!info_.reorder)
      return tmodel_.IsFinal(tids.back());
    else {
      for (size_t i = 0; i < tids.size(); i++) {
        if (tmodel_.IsFinal(tids[i])) {  // got the "final" transition, which is
          ...
          for (size_t j = i + 1; j < tids.size(); j++) {
            if (!(tmodel_.TransitionIdToTransitionState(tids[j]) ==
                  tmodel_.TransitionIdToTransitionState(tids[i])))
              return false;
          }
          return true;
        }
  ...
  bool TestArcOnePhoneWord(const CompactLatticeArc &arc) {
    if (arc.ilabel == 0)
      return false;  // Check there's a label.  Note, ilabel==olabel.
    const std::vector<int32> &tids = arc.weight.String();
    if (tids.empty()) return false;
    int32 first_phone = tmodel_.TransitionIdToPhone(tids.front());
  ...
  bool TestArcNormalWord(const CompactLatticeArc &arc) {
    ...
    for (size_t j = i; j < tids.size();
         j++)  // make sure only this final phone till end.
  ...
  void TestEquivalent() {
    ...
    if (!RandEquivalent(lat_, aligned_lat, 5 /*paths*/, 1.0e+10 /*delta*/,
                        Rand() /*seed*/, 200 /*path length (max?)*/))
      KALDI_ERR
          << "Equivalence test failed (testing word-alignment of lattices.) "
