  ...
  for (size_t counter = 0;; counter++) {
    ...
    for (size_t q = 0; q < R_.size(); q++) {
      if (do_mbr_) {  // This loop updates R_ [indexed same as gamma_].
        ...
        int32 rq = R_[q],
              rhat = this_gamma[0].first;  // rq: old word, rhat: new.
        for (size_t j = 0; j < this_gamma.size(); j++)
          if (this_gamma[j].first == rq) old_gamma = this_gamma[j].second;
        delta_Q +=
            (old_gamma - new_gamma);  // will be 0 or negative; a bound on
        // change in error.
        if (rq != rhat)
...
double MinimumBayesRisk::EditDistance(int32 N, int32 Q, Vector<double> &alpha,
  ...
  for (int32 n = 2; n <= N; n++) {
    ...
    for (size_t i = 0; i < pre_[n].size(); i++) {
      ...
      for (int32 q = 0; q <= Q; q++) {
        ...
        // line 19:
        alpha_dash(n, q) +=
            exp(alpha(s_a) + p_a - alpha(n)) * alpha_dash_arc(q);
      }
    }
  }
...
void MinimumBayesRisk::AccStats() {
  ...
  if (L_ != 0 && Ltmp > L_) {  // L_ != 0 is to rule out 1st iter.
    KALDI_WARN << "Edit distance increased: " << Ltmp << " > " << L_;
  }
  L_ = Ltmp;
  KALDI_VLOG(2) << "L = " << L_;
      ...
      for (int32 q = Q; q >= 1; q--) {
        ...
        switch (static_cast<int>(b_arc[q])) {  // lines 22 and 23:
          ...
          case 3:
            ...
            // the instructions there the next line would say state_times_[sa],
            // but
...
MinimumBayesRisk::MinimumBayesRisk(const CompactLattice &clat_in, bool do_mbr)
  ...
  for (int32 n = 1; n <= N; n++) {
    for (fst::ArcIterator<CompactLattice> aiter(clat, n - 1); !aiter.Done();
         aiter.Next()) {
      ...
      arc.loglike =
          -(carc.weight.Weight().Value1() + carc.weight.Weight().Value2());
