...
/// of the lattice.  The Posterior quantities contain pairs of (transition-id,
/// weight)
/// on each frame.
...
/// -b*[number of frame errors] to the graph-component of the cost of each
/// arc/path.
/// There is a frame error if a particular transition-id on a particular frame
/// corresponds to a phone not matching transcription's alignment for that
/// frame.
...
/**
   ...
   contribution
   for the numerator and a negative one for the denominator.  This function is
   not actually
   used in our normal MMI training recipes, where it's instead done using
   various command
   line programs that each do a part of the job.  This function was written for
   use in
   neural-net MMI training.
   If drop_frames is true, it will not compute any posteriors on frames where
   the num and
   den have disjoint pdf-ids.
   If "convert_to_pdf_ids" is true, it will convert the output to be at the
   level of pdf-ids,
   not transition-ids.
