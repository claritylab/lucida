...
class LatticeDeterminizerPruned {
 public:
  ...
  // correspondence
  // between our states and the states in ofst.  If destroy == true, release
  // memory as we go
  ...
  void Output(MutableFst<Arc> *ofst, bool destroy = true) {
    ...
    ofst->SetStart(0);
    for (OutputStateId this_state_id = 0; this_state_id < nStates;
         this_state_id++) {
      OutputState &this_state = *(output_states_[this_state_id]);
      vector<TempArc> &this_vec(this_state.arcs);
  ...
  LatticeDeterminizerPruned(const ExpandedFst<Arc> &ifst, double beam,
  ...
  void FreeOutputStates() {
    for (size_t i = 0; i < output_states_.size(); i++) delete output_states_[i];
  ...
  void RebuildRepository() {  // rebuild the string repository,
    ...
    {  // the queue doesn't allow us access to the underlying vector,
      ...
      for (size_t i = 0; i < tasks.size(); i++) queue_.push(tasks[i]);
    }
  ...
  bool CheckMemoryUsage() {
    ...
    if (opts_.max_mem > 0 &&
      ...
      KALDI_VLOG(2) << "Rebuilt repository in determinize-lattice: repository "
                       "shrank from " << repo_size << " to " << new_repo_size
                    << " bytes (approximately)";
      ...
      if (new_total_size > static_cast<int32>(opts_.max_mem * 0.8)) {
        ...
        if (!queue_
                 .empty()) {  // Note: queue should probably not be empty; we're
          // just being paranoid here.
          Task *task = queue_.top();
          double total_weight =
              backward_costs_[ifst_->Start()];  // best weight of FST.
  ...
  bool Determinize(double *effective_beam) {
    ...
    while (!queue_.empty()) {
      ...
      if ((opts_.max_states > 0 && num_states > opts_.max_states) ||
          ...
          (num_states % 10 == 0 &&
           !CheckMemoryUsage())) {  // note: at some point
        // it was num_states % 100, not num_states % 10, but I encountered an
        // example
        ...
        break;
        // we terminate the determinization here-- whatever we already expanded
        // is
    ...
    if (effective_beam != NULL) {
      if (queue_.empty())
        *effective_beam = beam_;
      else
        *effective_beam =
            queue_.top()->priority_cost - backward_costs_[ifst_->Start()];
    }
    return (
        queue_
            .empty());  // return success if queue was empty, i.e. we processed
    // all tasks and did not break out of the loop early due to reaching a
    // memory,
    // arc or state limit.
  }
 ...
 private:
  ...
  struct Element {
    StateId
        state;  // use StateId as this is usually InputStateId but in one case
                // OutputStateId.
    StringId string;
    Weight weight;
  ...
  // is
  // order-dependent.  However the weights are not included in the hashing
  // function--
  // we hash subsets that differ only in weight to the same key.  This is not
  // optimal
  // in terms of the O(N) performance but typically if we have a lot of
  // determinized
  // states that differ only in weight then the input probably was pathological
  // in some way,
  ...
  class SubsetKey {
   public:
    size_t operator()(const vector<Element> *subset)
  ...
  typedef unordered_map<const vector<Element> *, OutputStateId, SubsetKey,
                        SubsetEqual> MinimalSubsetHash;
  ...
  typedef unordered_map<const vector<Element> *, Element, SubsetKey,
                        SubsetEqual> InitialSubsetHash;
  ...
  OutputStateId MinimalToStateId(const vector<Element> &subset,
    ...
    if (iter != minimal_hash_.end()) {  // Found a matching subset.
      ...
      if (forward_cost < state.forward_cost - 0.1) {
        ...
        KALDI_WARN << "New cost is less (check the difference is small) "
                   << forward_cost << ", " << state.forward_cost;
      }
    }
    ...
    // Note: in the previous algorithm, we pushed the new state-id onto the
    // queue
    // at this point.  Here, the queue happens elsewhere, and we directly
    // process
    // the state (which result in stuff getting added to the queue).
    ProcessFinal(state_id);  // will work out the final-prob.
    ProcessTransitions(
        state_id);  // will process transitions and add stuff to the queue.
    return state_id;
  }
  ...
  OutputStateId InitialToStateId(const vector<Element> &subset_in,
                                 double forward_cost, Weight *remaining_weight,
    ...
    if (iter != initial_hash_.end()) {  // Found a matching subset.
      ...
      *common_prefix = elem.string;
      if (elem.weight == Weight::Zero()) KALDI_WARN << "Zero weight!";
      return elem.state;
    }
  ...
  // Has no side effects except on the string repository.  The "output_subset"
  // is not
  ...
  void EpsilonClosure(vector<Element> *subset) {
...

    {
      ...
      for (size_t i = 0; i < subset->size(); i++) {
        std::pair<const InputStateId, Element> pr((*subset)[i].state,
                                                  (*subset)[i]);
...
#endif
        // By providing iterator where we inserted last one, we make insertion
    ...
    while (queue.size() != 0) {
      ...
      if (opts_.max_loop > 0 && counter++ > opts_.max_loop) {
        ...
        throw std::runtime_error(
            "looped more than max-arcs times in lattice determinization");
      }
      for (ArcIterator<ExpandedFst<Arc> > aiter(*ifst_, elem.state);
        ...
        if (arc.ilabel == 0 &&
          ...
          typename unordered_map<InputStateId, Element>::iterator iter =
              cur_subset.find(next_elem.state);
          if (iter == cur_subset.end()) {
            // was no such StateId: insert and add to queue.
            next_elem.string =
                (arc.olabel == 0 ? elem.string : repository_.Successor(
                                                     elem.string, arc.olabel));
  ...
  void ProcessFinal(OutputStateId output_state_id) {
    ...
    // compiler happy; if it doesn't get set in the loop, we won't use the value
    // anyway.
    ...
    for (; iter != end; ++iter) {
      ...
      if (this_final_weight != Weight::Zero() &&
          (!is_final ||
           Compare(this_final_weight, this_final_string, final_weight,
                   final_string) == 1)) {  // the new
    ...
    if (is_final &&
      ...
      // Note: we only store the final-weight if it's inside the pruning beam,
      // hence
  ...
  void NormalizeSubset(vector<Element> *elems, Weight *tot_weight,
    ...
    KALDI_ASSERT(weight !=
                 Weight::Zero());  // we made sure to ignore arcs with zero
  ...
  void MakeSubsetUnique(vector<Element> *subset) {
    ...
    // This KALDI_ASSERT is designed to fail (usually) if the subset is not
    // sorted on
    // state.
    KALDI_ASSERT(subset->size() < 2 ||
                 (*subset)[0].state <= (*subset)[1].state);
  ...
  // processes a transition from state "ostate_id".  The set "subset" of
  // Elements
  ...
  void ProcessTransition(OutputStateId ostate_id, Label ilabel,
    ...
    OutputStateId nextstate;
    {
      ...
      nextstate = InitialToStateId(*subset, forward_cost, &next_tot_weight,
                                   &next_common_str);
  ...
  void ProcessTransitions(OutputStateId output_state_id) {
    const vector<Element> &minimal_subset =
    ...
    while (cur != end) {
      ...
      while (cur != end && cur->first == ilabel) {
        ...
        // priority_cost.
        task->priority_cost =
            std::min(task->priority_cost, ConvertToCost(element.weight) +
      ...
      if (task->priority_cost > cutoff_) {
        ...
        MakeSubsetUnique(
            &(task->subset));  // remove duplicate Elements with the same state.
    ...
    all_elems.clear();  // as it's a reference to a class variable; we want it
                        // to stay
    // empty.
  }
  ...
  bool IsIsymbolOrFinal(InputStateId state) {  // returns true if this state
    ...
    for (ArcIterator<ExpandedFst<Arc> > aiter(*ifst_, state); !aiter.Done();
         aiter.Next()) {
      const Arc &arc = aiter.Value();
  ...
  void ComputeBackwardWeight() {
    ...
    for (StateId s = ifst_->NumStates() - 1; s >= 0; s--) {
      ...
      for (ArcIterator<ExpandedFst<Arc> > aiter(*ifst_, s); !aiter.Done();
           aiter.Next()) {
        const Arc &arc = aiter.Value();
        cost = std::min(
            cost, ConvertToCost(arc.weight) + backward_costs_[arc.nextstate]);
      }
    }
  ...
  void InitializeDeterminization() {
    ...
    if (ifst_->Properties(kExpanded, false) != 0) {  // if we know the number of
      ...
      StateId num_states = down_cast<const ExpandedFst<Arc> *, const Fst<Arc> >(
                               ifst_)->NumStates();
    ...
    if (start_id != kNoStateId) {
      /* Create determinized-state corresponding to the start state....
         ...
         of this determinized-state before we put it into minimal_hash_.  This
         is actually
         what we want, as otherwise we'd have problems dealing with any extra
         weight
         and string and might have to create a "super-initial" state which would
         make
         ...
         correctness.
         Note, we don't put anything in the initial_hash_.  The initial_hash_ is
         only
         a lookaside buffer anyway, so this isn't a problem-- it will get
         populated
  ...
  struct OutputState {
    ...
    vector<TempArc>
        arcs;  // arcs out of the state-- those that have been processed.
    // Note: the final-weight is included here with kNoStateId as the state id.
    // We
    // always process the final-weight regardless of the beam; when producing
    // the
    ...
    // Note: we know this minimal cost from when we first create the
    // OutputState;
    ...
    OutputState(const vector<Element> &minimal_subset, double forward_cost)
        : minimal_subset(minimal_subset), forward_cost(forward_cost) {}
  };

  vector<OutputState *>
      output_states_;  // All the info about the output states.

  int num_arcs_;   // keep track of memory usage: number of arcs in
                   // output_states_[ ]->arcs
  ...
  std::vector<double>
      backward_costs_;  // This vector stores, for every state in ifst_,
  // the minimal cost to the end-state (i.e. the sum of weights; they are
  // guaranteed to
  // have "take-the-minimum" semantics).  We get the double from the
  // ConvertToCost()
  ...
  struct Task {
    ...
    vector<Element>
        subset;  // Weighted subset of states (with strings)-- not normalized.
    double priority_cost;  // Cost used in deciding priority of tasks.  Note:
    // we assume there is a ConvertToCost() function that converts the semiring
    // to double.
  };
...
bool DeterminizeLatticePruned(
    const ExpandedFst<ArcTpl<Weight> > &ifst, double beam,
...
bool DeterminizeLatticePruned(const ExpandedFst<ArcTpl<Weight> > &ifst,
                              double beam, MutableFst<ArcTpl<Weight> > *ofst,
                              DeterminizeLatticePrunedOptions opts) {
  ...
  for (int32 iter = 0; iter < max_num_iters; iter++) {
    ...
    if (effective_beam >= beam * opts.retry_cutoff ||
      ...
      // If effective_beam is very small, we want to reduce by a lot.
      // But never change the beam by more than a factor of two.
      if (effective_beam < 0) effective_beam = 0;
      double new_beam = beam * sqrt(effective_beam / beam);
      if (new_beam < 0.5 * beam) new_beam = 0.5 * beam;
      KALDI_WARN << "Effective beam " << effective_beam
                 << " was less than beam " << beam << " * cutoff "
                 << opts.retry_cutoff << ", pruning raw "
    ...
    for (MutableArcIterator<MutableFst<Arc> > aiter(fst, state); !aiter.Done();
      ...
      if ((arc.olabel != 0) &&
        ...
        if (arc.ilabel == 0) {
          ...
          fst->AddArc(additional_state, Arc(first_phone_label + phone, 0,
...
void DeterminizeLatticeDeletePhones(
  ...
  for (StateIterator<MutableFst<Arc> > siter(*fst); !siter.Done();
    ...
    for (MutableArcIterator<MutableFst<Arc> > aiter(fst, state); !aiter.Done();
      ...
      if (arc.ilabel >= first_phone_label) arc.ilabel = 0;
...
bool DeterminizeLatticePhonePrunedFirstPass(
    const kaldi::TransitionModel &trans_model, double beam,
    MutableFst<ArcTpl<Weight> > *fst,
...
bool DeterminizeLatticePhonePruned(
    const kaldi::TransitionModel &trans_model,
    MutableFst<ArcTpl<Weight> > *ifst, double beam,
    MutableFst<ArcTpl<CompactLatticeWeightTpl<Weight, IntType> > > *ofst,
  ...
  if (opts.phone_determinize) {
    ...
    ans = DeterminizeLatticePhonePrunedFirstPass<Weight, IntType>(
              trans_model, beam, ifst, det_opts) &&
          ans;
  ...
  if (opts.word_determinize) {
    ...
    ans = DeterminizeLatticePruned<Weight, IntType>(*ifst, beam, ofst,
                                                    det_opts) &&
          ans;
  }
...
bool DeterminizeLatticePhonePruned(
  ...
  return DeterminizeLatticePhonePruned(trans_model, &temp_fst, beam, ofst,
                                       opts);
}

bool DeterminizeLatticePhonePrunedWrapper(
    const kaldi::TransitionModel &trans_model,
    MutableFst<kaldi::LatticeArc> *ifst, double beam,
...
template bool DeterminizeLatticePruned<kaldi::LatticeWeight>(
    const ExpandedFst<kaldi::LatticeArc> &ifst, double prune,
    MutableFst<kaldi::CompactLatticeArc> *ofst,
    DeterminizeLatticePrunedOptions opts);

template bool DeterminizeLatticePruned<kaldi::LatticeWeight>(
    const ExpandedFst<kaldi::LatticeArc> &ifst, double prune,
    MutableFst<kaldi::LatticeArc> *ofst, DeterminizeLatticePrunedOptions opts);

template bool DeterminizeLatticePhonePruned<kaldi::LatticeWeight, kaldi::int32>(
    const kaldi::TransitionModel &trans_model,
    const ExpandedFst<kaldi::LatticeArc> &ifst, double prune,
    MutableFst<kaldi::CompactLatticeArc> *ofst,
    DeterminizeLatticePhonePrunedOptions opts);
