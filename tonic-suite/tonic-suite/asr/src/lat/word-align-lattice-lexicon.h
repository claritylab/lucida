...
/** Read the lexicon in the special format required for word alignment.  Each
   ...
   alignment, and
   <new-word-id> is the word-is that should appear in the lattice after
   alignment.  This
   is mainly useful when the lattice may have no symbol for the optional-silence
   arcs
   (so <old-word-id> would equal zero), but we want it to be output with a
   symbol on those
   arcs (so <new-word-id> would be nonzero).
   If the silence should not be added to the lattice, both <old-word-id> and
   <new-word-id>
   may be zero.
...
class WordAlignLatticeLexiconInfo {
 public:
  ...
  /// This
  /// function maps from each word-id to the lowest member of its equivalence
  /// class.
...
struct WordAlignLatticeLexiconOpts {
  ...
  WordAlignLatticeLexiconOpts()
      : partial_word_label(0), reorder(true), test(false), max_expand(-1.0) {}

  void Register(OptionsItf *po) {
    po->Register("partial-word-label", &partial_word_label,
                 "Numeric id of "
    ...
    po->Register("test", &test,
                 "If true, testing code will be activated "
...
void TestWordAlignedLatticeLexicon(
