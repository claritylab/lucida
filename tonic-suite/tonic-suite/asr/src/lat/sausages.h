...
///  "Minimum Bayes Risk decoding and system combination based on a recursion
///  for
///  edit distance", Haihua Xu, Daniel Povey, Lidia Mangu and Jie Zhu, Computer
///  Speech and Language, 2011
/// This is a slightly more principled way to do Minimum Bayes Risk (MBR)
/// decoding
...
class MinimumBayesRisk {
 public:
  ...
  MinimumBayesRisk(const CompactLattice &clat,
                   bool do_mbr = true);  // if do_mbr == false,
  // it will just use the MAP recognition output, but will get the MBR stats for
  // things
  // like confidences.

  const std::vector<int32> &GetOneBest()
      const {  // gets one-best (with no epsilons)
    return R_;
  }

  const std::vector<std::pair<BaseFloat, BaseFloat> > GetSausageTimes() const {
    return times_;  // returns average (start,end) times for each bin (each
                    // entry
    // of GetSausageStats()).  Note: if you want the times for the one best,
    // you can work out the one best yourself from the sausage stats and get the
    // times
    // at the same time.
  }

  const std::vector<std::pair<BaseFloat, BaseFloat> > &GetOneBestTimes() const {
    return one_best_times_;  // returns average (start,end) times for each bin
                             // corresponding
  ...
  const std::vector<std::vector<std::pair<int32, BaseFloat> > > &
  GetSausageStats() const {
    return gamma_;
  }
 ...
 private:
  ...
  double EditDistance(int32 N, int32 Q, Vector<double> &alpha,
  ...
  static inline void AddToMap(int32 i, double d,
  ...
  /// Boolean configuration parameter: if true, we actually update the
  /// hypothesis
