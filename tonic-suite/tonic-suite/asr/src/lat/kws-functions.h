...
class Interval {
 public:
  ...
  Interval(const Interval &interval)
      : start_(interval.Start()), end_(interval.End()) {}
  int32 Overlap(Interval interval) {
    return std::max<int32>(
        0, std::min(end_, interval.end_) - std::max(start_, interval.start_));
  }
...
// 0 1 a a (0.1s ~ 0.5s) and 7 8 a a (0.9s ~ 1.4s) are also in different
// clusters.
...
// epsilon removal, determinization and minimization.  Modifies
// factor_transducer.
