...
class CompactLatticePusher {
 public:
  ...
  static void GetString(const ExpandedFst<CompactArc> &clat, StateId state,
                        size_t arc_idx,
    ...
    if (arc_idx != -1) aiter.Seek(arc_idx);
    KALDI_ASSERT(
        !aiter.Done() &&
    ...
    if (arc_len >= len) {
      std::copy(arc.weight.String().begin(), arc.weight.String().begin() + len,
                begin);
    } else {
  ...
  void CheckForConflict(const CompactWeight &final, StateId state,
                        int32 *shift) {
    ...
    if (num_arcs + (is_final ? 1 : 0) > 1 && shift > 0) {
      ...
      for (; arc < num_arcs; arc++) {  // for the other arcs..
        GetString(*clat_, state, arc, compare_string.begin(),
                  compare_string.end());
        ...
        if (pr.first !=
            string.end()) {  // There was a mismatch.  Reduce the shift
  ...
  void ApplyShifts() {
    ...
    for (StateId state = 0; state < num_states; state++) {
      ...
      for (MutableArcIterator<MutableFst<CompactArc> > aiter(clat_, state);
        ...
        GetString(*clat_, arc.nextstate, -1, string.begin() + orig_len,
                  string.end());
        // Remove the first "shift" elements of this string and set the
        // arc-weight string to this.
        arc.weight.SetString(
            std::vector<IntType>(string.begin() + shift, string.end()));
        aiter.SetValue(arc);
      }
...
bool PushCompactLatticeWeights(
  ...
  for (StateId s = num_states - 1; s >= 0; s--) {
    ...
    for (ArcIterator<MutableFst<CompactArc> > aiter(*clat, s); !aiter.Done();
         aiter.Next()) {
      const CompactArc &arc = aiter.Value();
      KALDI_ASSERT(arc.nextstate > s && "Cyclic lattices not allowed.");
      this_weight_to_end = Plus(
          this_weight_to_end,
          Times(aiter.Value().weight.Weight(), weight_to_end[arc.nextstate]));
    }
  ...
  for (StateId s = 0; s < num_states; s++) {
    ...
    if (this_weight_to_end == Weight::Zero()) continue;
      ...
      if (next_weight_to_end != Weight::Zero()) {
        arc.weight.SetWeight(
            Times(arc.weight.Weight(),
                  Divide(next_weight_to_end, this_weight_to_end)));
        aiter.SetValue(arc);
      }
    }
...
template bool PushCompactLatticeStrings<kaldi::LatticeWeight, kaldi::int32>(
    MutableFst<kaldi::CompactLatticeArc> *clat);

template bool PushCompactLatticeWeights<kaldi::LatticeWeight, kaldi::int32>(
    MutableFst<kaldi::CompactLatticeArc> *clat);
