...
class LatticeLexiconWordAligner {
 public:
   ...
   */
  typedef enum { kNotFresh, kFresh, kAllFresh } Freshness;
  ...
  class ComputationState {
   ...
   public:
    ...
    void Advance(const CompactLatticeArc &arc, const TransitionModel &tmodel,
    ...
    bool TakeTransition(const LexiconMap &lexicon_map, int32 word_id,
                        int32 num_phones, ComputationState *next_state,
                        CompactLatticeArc *arc_out) const;
    ...
    size_t Hash() const {
      ...
      ans += p2 * vh(words_) + static_cast<int32>(word_fresh_) * p3 +
    ...
    bool operator==(const ComputationState &other) const {
      ...
      return (transition_ids_ == other.transition_ids_ &&
              words_ == other.words_ && weight_ == other.weight_ &&
    ...
    ComputationState(const ComputationState &other)
        : phones_(other.phones_),
          ...
          phone_fresh_(other.phone_fresh_),
          word_fresh_(other.word_fresh_),
          transition_ids_(other.transition_ids_),
   ...
   private:
    ...
    std::vector<std::vector<int32> >
        transition_ids_;  // sequence of transition-ids for each phone..
  ...
  void ProcessTransition(
      StateId prev_output_state,  // state-id of from-state in output lattice
      const Tuple &next_tuple,
      CompactLatticeArc *
          arc) {  // arc to add (must first modify it by adding "nextstate")
  ...
  // Process any non-epsilon transitions out of this state in the output
  // lattice.
  ...
  /// lattice has arcs out that have either a non-epsilon symbol or
  /// transition-ids
  ...
  // Process all final-probs -- a wrapper function that handles the forced-out
  // case.
  void ProcessFinalWrapper() {
    ...
    error_ = true;
    KALDI_WARN << "Word-aligning lattice: lattice was forced out, will have "
                  "partial words at end.";

    ProcessFinalForceOut();

    if (ProcessFinal()) return;
    KALDI_WARN
        << "Word-aligning lattice: had no final-states even after forcing out "
  ...
  LatticeLexiconWordAligner(const CompactLattice &lat,
                            ...
                            int32 max_states, int32 partial_word_label,
                            CompactLattice *lat_out)
      : lat_in_(lat),
        tmodel_(tmodel),
        lexicon_info_(lexicon_info),
        max_states_(max_states),
        lat_out_(lat_out),
        partial_word_label_(partial_word_label == 0 ? kTemporaryEpsilon
                                                    : partial_word_label),
        error_(false) {
    // lat_in_ is after PhoneAlignLattice, it is not deterministic and contains
    // epsilons
  ...
  // just contains states that may have final-probs to process.  We process
  // these
...
void LatticeLexiconWordAligner::ProcessEpsilonTransitions(
  ...
  if (comp_state.PhoneFreshness() == kAllFresh) {
    ...
    if (num_phones >= iter->second.first && num_phones <= iter->second.second) {
  ...
  for (int32 num_phones = min_num_phones; num_phones <= max_num_phones;
       num_phones++) {
    Tuple next_tuple;
    next_tuple.input_state =
        input_state;  // We're not taking a transition in the
    // input FST so this stays the same.
    CompactLatticeArc arc;
    if (comp_state.TakeTransition(lexicon_info_.lexicon_map_, zero_word,
                                  num_phones, &next_tuple.comp_state, &arc)) {
      ProcessTransition(output_state, next_tuple, &arc);
    }
  }
}

void LatticeLexiconWordAligner::ProcessWordTransitions(const Tuple &tuple,
  ...
  if (comp_state.NumWords() > 0) {
    ...
    if (comp_state.WordFreshness() == kFresh ||
      ...
      return;  // Nothing to do, since neither the word nor the phones are
               // fresh.
    }
    ...
    for (int32 num_phones = min_num_phones; num_phones <= max_num_phones;
      ...
      // input FST so this stays the same.
      CompactLatticeArc arc;
      if (comp_state.TakeTransition(lexicon_info_.lexicon_map_, word_id,
                                    num_phones, &next_tuple.comp_state, &arc)) {
        ProcessTransition(output_state, next_tuple, &arc);
      }
    }
...
void LatticeLexiconWordAligner::PossiblyAdvanceArc(const Tuple &tuple,
                                                   StateId output_state) {
  if (tuple.comp_state.ViableIfAdvanced(lexicon_info_.viability_map_)) {
    for (fst::ArcIterator<CompactLattice> aiter(lat_in_, tuple.input_state);
      ...
      CompactLatticeArc arc_out(
          0, 0, CompactLatticeWeight(arc_weight, std::vector<int32>()),
          next_output_state);
      lat_out_->AddArc(output_state, arc_out);
    }
  }
}
...
bool LatticeLexiconWordAligner::ProcessFinal() {
  ...
  for (size_t i = 0; i < final_queue_.size(); i++) {
    ...
    KALDI_ASSERT(lat_in_.Final(tuple.input_state) ==
...
void LatticeLexiconWordAligner::ProcessFinalForceOut() {
  ...
  for (size_t i = 0; i < final_queue_.size();
       i++) {  // note: all the states will
...
void LatticeLexiconWordAligner::ComputationState::Advance(
    const CompactLatticeArc &arc, const TransitionModel &tmodel,
  ...
  *weight =
      Times(weight_, arc.weight.Weight());  // will go on arc in output lattice
...
void LatticeLexiconWordAligner::ComputationState::TakeForcedTransition(
    int32 partial_word_label, ComputationState *next_state,
    CompactLatticeArc *arc_out) const {
  ...
  if (words_.size() >= 1) {
    ...
    if (words_.size() > 1)
      KALDI_WARN
          << "Word-aligning lattice: discarding extra word at end of lattice"
          << "(forced-out).";
  } else {
    word_id = partial_word_label;
  }
  std::vector<int32> appended_transition_ids;
  AppendVectors(transition_ids_.begin(), transition_ids_.end(),
                &appended_transition_ids);
  arc_out->ilabel = word_id;
  arc_out->olabel = word_id;
  arc_out->weight = CompactLatticeWeight(weight_, appended_transition_ids);
...
bool LatticeLexiconWordAligner::ComputationState::TakeTransition(
  ...
  lexicon_key.insert(lexicon_key.end(), phones_.begin(),
                     phones_.begin() + num_phones);
  ...
  if (iter == lexicon_map.end()) {  // no such entry
    ...
    next_state->phone_fresh_ = (next_state->phones_.empty() || num_phones == 0)
                                   ? kNotFresh
                                   : kAllFresh;
    next_state->weight_ = LatticeWeight::One();
    ...
    std::vector<int32> appended_transition_ids;
    AppendVectors(transition_ids_.begin(), transition_ids_.begin() + num_phones,
                  &appended_transition_ids);
    arc_out->ilabel = word_id;
    arc_out->olabel = word_id;
    arc_out->weight = CompactLatticeWeight(weight_, appended_transition_ids);
...
static bool IsPlausibleWord(const WordAlignLatticeLexiconInfo &lexicon_info,
                            const TransitionModel &tmodel, int32 word_id,
  ...
  if (!lexicon_info.IsValidEntry(lexicon_entry)) {
    ...
    KALDI_WARN
        << "Invalid arc in aligned lattice (code error?) lexicon-entry is "
        << ostr.str();
    return false;
  } else {
    return true;
...
void WordAlignLatticeLexiconInfo::UpdateLexiconMap(
  ...
  if (lexicon_map_.count(key) != 0) {
    if (lexicon_map_[key] == new_word)
      KALDI_WARN << "Duplicate entry in lexicon map for word "
                 << lexicon_entry[0];
    else
...
bool WordAlignLatticeLexiconInfo::IsValidEntry(
    const std::vector<int32> &entry) const {
...
/// Testing code; map word symbols in the lattice "lat" using the
/// equivalence-classes
/// obtained from the lexicon, using the function EquivalenceClassOf in the
/// lexicon_info
/// object.
static void MapSymbols(const WordAlignLatticeLexiconInfo &lexicon_info,
  ...
  for (StateId s = 0; s < lat->NumStates(); s++) {
    for (fst::MutableArcIterator<CompactLattice> aiter(lat, s); !aiter.Done();
         aiter.Next()) {
...
bool TestWordAlignedLattice(const WordAlignLatticeLexiconInfo &lexicon_info,
  ...
  {  // We test whether the forward-backward likelihoods differ; this is
     // intended
    // to detect when we have duplicate paths in the aligned lattice, for some
    // path
    ...
    if (fabs(like_before - like_after) >
      ...
      KALDI_WARN
          << "Forward-backward likelihoods differ in word-aligned lattice "
  ...
  for (CompactLattice::StateId s = 0; s < aligned_clat.NumStates(); s++) {
    for (fst::ArcIterator<CompactLattice> aiter(aligned_clat, s); !aiter.Done();
         aiter.Next()) {
      const CompactLatticeArc &arc(aiter.Value());
      KALDI_ASSERT(arc.ilabel == arc.olabel);
      ...
      if (num_err < max_err)
        if (!IsPlausibleWord(lexicon_info, tmodel, word_id, tids)) num_err++;
  ...
  FLAGS_v = GetVerboseLevel();  // set the OpenFst verbose level to the Kaldi
                                // verbose level.
  if (!RandEquivalent(clat, aligned_clat, num_paths, delta, seed,
                      max_path_length)) {
...
bool WordAlignLatticeLexicon(const CompactLattice &lat,
  ...
  bool ans =
      PhoneAlignLattice(lat, tmodel, phone_align_opts, &phone_aligned_lat);
  ...
  if (opts.max_expand <= 0) {
    ...
    // small inputs.
    max_states =
        kNumStatesOffset + opts.max_expand * phone_aligned_lat.NumStates();
  }
  ...
  /*  if (ans && opts.test) {
    ...
    int32 num_paths = 5, seed = Rand(), max_path_length = -1;
    BaseFloat delta = 0.2; // some lattices have large costs -> use large delta.
    if (!RandEquivalent(lat, phone_aligned_lat, num_paths, delta, seed,
    max_path_length)) {
      KALDI_WARN << "Equivalence test failed during lattice alignment
    (phone-alignment stage)";
      return false;
    }
    } */
  ...
  LatticeLexiconWordAligner aligner(phone_aligned_lat, tmodel, lexicon_info,
                                    max_states, opts.partial_word_label,
                                    lat_out);
  // We'll let the calling code warn if this is false; it will know the
  // utterance-id.
