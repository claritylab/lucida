...
namespace kaldi {
void FindQuietestSegment(const Vector<BaseFloat> &wav_in, BaseFloat samp_rate,
                         Vector<BaseFloat> *wav_sil, BaseFloat search_dur = 0.5,
                         BaseFloat seg_dur = 0.1,
                         BaseFloat seg_shift_dur = 0.05);

void ExtendWaveWithSilence(const Vector<BaseFloat> &wav_in, BaseFloat samp_rate,
                           Vector<BaseFloat> *wav_out, BaseFloat sil_search_len,
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Extend wave data with a fairly long silence at the end (e.g. 5 "
        "seconds).\n"
        "The input waveforms are assumed having silences at the begin/end and "
        "those\n"
        "segments are extracted and appended to the end of the utterance.\n"
        "Note this is for use in testing endpointing in decoding.\n"
        "\n"
        "Usage: extend-wav-with-silence [options] <wav-rspecifier> "
        "<wav-wspecifier>\n";

    ParseOptions po(usage);
    BaseFloat sil_len = 5.0, sil_search_len = 0.5, sil_extract_len = 0.05,
              sil_extract_shift = 0.025;
    po.Register("extra-silence-length", &sil_len,
                "the length of silence that will be "
                "appended to the end of each waveform, in seconds.");
    po.Register("silence-search-length", &sil_search_len,
                "the length at the beginning "
                "or end of each waveform in which to search for the quietest "
                "segment of "
                "silence, in seconds.");
    po.Register(
        "silence-extract-length", &sil_extract_len,
        "the length of silence segments "
        "to be extracted from the waveform, which must be smaller than silence-"
        "search-length, in seconds.");
    po.Register("silence-extract-shift", &sil_extract_shift,
                "the shift length when searching "
                "for segments of silences, typically samller than "
                "silence-extract-length, "
                "in seconds.");
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      int32 num_chan = wave_data.NumRows(),  // number of channels in recording
          num_ext_samp = (int32)(
              samp_freq * sil_len);  // number of samples that will be extended
      KALDI_ASSERT(num_ext_samp > 0);
      Matrix<BaseFloat> new_wave(wave_data.NumRows(),
                                 wave_data.NumCols() + num_ext_samp);
      for (int32 i = 0; i < num_chan; i++) {
        ...
        ExtendWaveWithSilence(wav_this_chan, samp_freq, &wav_extend,
                              sil_search_len, sil_extract_len,
                              sil_extract_shift);
...
void ExtendWaveWithSilence(const Vector<BaseFloat> &wav_in, BaseFloat samp_rate,
  ...
  FindQuietestSegment(wav_in, samp_rate, &quietest_seg, sil_search_len,
                      sil_extract_len, sil_extract_shift);

  int32 window_size = quietest_seg.Dim(), window_size_half = window_size / 2;
  ...
  for (int32 i = 0; i < window_size_half;
       i++)  // windowing the first half window
...
void FindQuietestSegment(const Vector<BaseFloat> &wav_in, BaseFloat samp_rate,
                         Vector<BaseFloat> *wav_sil, BaseFloat search_dur,
                         BaseFloat seg_dur, BaseFloat seg_shift_dur) {
  KALDI_ASSERT(seg_dur < search_dur);

  int32 search_len = (int32)(search_dur * samp_rate),
        seg_len = (int32)(seg_dur * samp_rate),
        seg_shift = (int32)(seg_shift_dur * samp_rate), start = 0;
  double min_energy;
