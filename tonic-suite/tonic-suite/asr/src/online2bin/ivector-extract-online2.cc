...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Extract iVectors for utterances every --ivector-period frames, using "
        "a trained\n"
        "iVector extractor and features and Gaussian-level posteriors.  "
        "Similar to\n"
        "ivector-extract-online but uses the actual online decoder code to do "
        "it,\n"
        "and does everything in-memory instead of using multiple processes.\n"
        "Note: the value of the --use-most-recent-ivector config variable is "
        "ignored\n"
        "it's set to false.  The <spk2utt-rspecifier> is mandatory, to "
        "simplify the code;\n"
        "if you want to do it separately per utterance, just make it of the "
        "form\n"
        "<utterance-id> <utterance-id>.\n"
        "The iVectors are output as an archive of matrices, indexed by "
        "utterance-id;\n"
        "each row corresponds to an iVector.  If --repeat=true, outputs the "
        "whole matrix\n"
        "of iVectors, not just every (ivector-period)'th frame\n"
        "The input features are the raw, non-cepstral-mean-normalized "
        "features, e.g. MFCC.\n"
        "\n"
        "Usage:  ivector-extract-online2 [options] <spk2utt-rspecifier> "
        "<feature-rspecifier> <ivector-wspecifier>\n"
        "e.g.: \n"
        "  ivector-extract-online2 "
        "--config=exp/nnet2_online/nnet_online/conf/ivector_extractor.conf \\\n"
        "    ark:data/train/spk2utt scp:data/train/feats.scp "
        "ark,t:ivectors.1.ark\n";
