...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Demonstrating how to use the Speex wrapper in Kaldi by compressing "
        "input waveforms \n"
        "chunk by chunk and then decompressing them.\n"
        "\n"
        "Usage: compress-uncompress-speex [options] <wav-rspecifier> "
        "<wav-wspecifier>\n";
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      for (int32 i = 0; i < num_chan; i++) {
        ...
        while (samp_offset < wav_this_chan.Dim()) {
          ...
          if (this_num_samp ==
              wav_this_chan.Dim() - samp_offset)  // no more input.
