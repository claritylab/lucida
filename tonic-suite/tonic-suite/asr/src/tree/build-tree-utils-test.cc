...
void TestPossibleValues() {
  ...
  for (size_t i = 0; i < 20; i++) {
    ...
    stats.push_back(
        std::pair<EventType, Clusterable *>(evec, (Clusterable *)NULL));
  }
  for (std::map<EventKeyType, std::set<EventValueType> >::iterator iter =
           all_key_vals.begin();
...
void TestSplitStatsByKey() {
  {
    ...
    for (int32 i = 0; i < 100; i++) {
      EventType evec;
      if (Rand() % 2) evec.push_back(std::make_pair(12, Rand() % 10));
      evec.push_back(std::make_pair(10, Rand() % 10));
      if (Rand() % 2) evec.push_back(std::make_pair(8, Rand() % 10));
...
void TestFindAllKeys() {
  for (size_t iter = 0; iter < 10; iter++) {
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      if (i == 0) {
        all_keys_union = s;
        all_keys_intersection = s;
      } else {
        std::set<EventKeyType> new_intersection;
        for (std::set<EventKeyType>::iterator iter = s.begin(); iter != s.end();
             iter++) {
          all_keys_union.insert(*iter);
          if (all_keys_intersection.count(*iter) != 0)
            new_intersection.insert(*iter);
        }
    ...
    {  // test in insist-same mode.
      ...
      CopySetToVector(all_keys_intersection, &keys1);
      try {
        FindAllKeys(stats, kAllKeysInsistIdentical,
                    &keys2);  // note, it SHOULD throw an exception here.
        // This is for testing purposes.  It gets caught.
        KALDI_ASSERT(keys1 == keys2);
        KALDI_ASSERT(all_keys_union == all_keys_intersection);
      } catch (
          ...) {  // it should throw exception if all keys are not the same.
...
void TestDoTableSplit() {
  ...
  for (size_t iter = 0; iter < 10; iter++) {
    ...
    for (size_t i = 0; i < 10; i++) {
      size_t idx1 = RandInt(0, stats.size() - 1),
             idx2 = RandInt(0, stats.size() - 1);
...
void TestClusterEventMapGetMappingAndRenumberEventMap() {
  ...
  for (size_t iter = 0; iter < 1; iter++) {  // in loop anyway.
    ...
    int32 num_clust = 10;
    for (int32 i = 0; i < num_clust;
         i++) {  // this will correspond to the "cluster".
      size_t n = 1 + Rand() % 3;
      for (size_t j = 0; j < n; j++) {
        ...
        stats.push_back(std::make_pair(
            evec, static_cast<Clusterable *>(new ScalarClusterable(scalar))));
      }
    }
    ...
    int32 num_reduced =
        ClusterEventMapGetMapping(*table_map, stats, 0.1, &mapping);

    std::cout << "TestCluster(): num_reduced = " << num_reduced
              << ", expected: " << cur_value << " - " << num_clust << " = "
              << (cur_value - num_clust) << '\n';
    ...
    std::vector<EventAnswerType> orig_answers, clustered_answers,
        renumbered_answers;
...
void TestClusterEventMapGetMappingAndRenumberEventMap2() {
  ...
  for (size_t iter = 0; iter < 1; iter++) {  // in loop anyway.
    ...
    std::vector<EventMap *> mapping;
    int32 num_reduced =
        ClusterEventMapGetMapping(*table_map, stats_reduced, 0.1, &mapping);

    std::cout << "TestCluster(): num_reduced = " << num_reduced
              << ", expected [ignoring gaps]: " << cur_value << " - "
              << num_clust << " = " << (cur_value - num_clust) << '\n';
    // KALDI_ASSERT(num_reduced == cur_value - num_clust);
...
void TestClusterEventMap() {
  // This second testing routine checks that ClusterEventMap does not renumber
  // leaves whose stats
  // we exclude.
  ...
  for (size_t iter = 0; iter < 1; iter++) {  // in loop anyway.
    ...
    for (size_t i = 0; i < stats.size(); i++) {
      if (exclude_leaves.count(stats[i].first[0].second) !=
          0) {  // this code relies on the fact that there is just one event in
                // the EventVector.
    ...
    KALDI_ASSERT(!stats_excluded.empty() && !stats_included.empty() &&
                 stats_excluded.size() + stats_included.size() == stats.size());

    int32 num_reduced;
    EventMap *clustered_map =
        ClusterEventMap(*table_map, stats_included, 0.1, &num_reduced);
    ...
    for (size_t i = 0; i < stats_excluded.size(); i++) {
      ...
      EventAnswerType ans;
      table_map->Map(evec, &ans);
      EventAnswerType ans2;
      clustered_map->Map(evec, &ans2);
...
void TestClusterEventMapRestricted() {
  ...
  for (size_t i = 0; i < (size_t)n_stats; i++) {
    ...
    stats.push_back(std::make_pair(
        evec, static_cast<Clusterable *>(new ScalarClusterable(RandGauss()))));
  }
  ...
  EventMap *table_split_map =
      DoTableSplitMultiple(*trivial_map, special_keys, stats, &nleaves);
  int32 nleaves_after_table_split = nleaves;
  std::cout << "TestClusterEventMapRestricted: after splitting on "
            << special_keys.size() << " keys, nleaves = " << nleaves << '\n';
  ...
  BaseFloat impr;
  EventMap *split_tree =
      SplitDecisionTree(*table_split_map, stats, qo, thresh, max_leaves,
                        &nleaves, &impr, &smallest_split);
  KALDI_ASSERT(
      (nleaves <= max_leaves || nleaves == nleaves_after_table_split) &&
      smallest_split >= thresh);

  std::cout << "TestClusterEventMapRestricted: after building decision tree, "
            << nleaves << '\n';

  thresh = 1000;  // will ensure everything is combined.
  {
    int32 num_removed;
    EventMap *map_clustered =
        ClusterEventMap(*split_tree, stats, thresh, &num_removed);
...

  {
    ...
    if (test_by_key)
      map_clustered = ClusterEventMapRestrictedByKeys(
          *split_tree, stats, thresh, special_keys, &num_removed);
    else
      map_clustered = ClusterEventMapRestrictedByMap(
          *split_tree, stats, thresh, *table_split_map, &num_removed);
...
void TestShareEventMapLeaves() {
  ...
  std::cout << "TestShareEventMapLeaves: after building decision tree, "
            << nleaves << '\n';

  if (special_keys.size() == 0) {
    KALDI_WARN << "TestShareEventMapLeaves(): could not test since key not "
                  "always defined.";
    delete split_tree;
    delete trivial_map;
    delete table_split_map;
...
void TestQuestionsInitRand() {
  // testing Questions::InitRand() function.  Also testing I/O of Questions
  // class.
  for (int32 p = 0; p < 10; p++) {
    ...
    BuildTreeStatsType dummy_stats;  // dummy because the Clusterable *pointers
                                     // are actually NULL.
    ...
    for (size_t i = 0; i < n_stats; i++) {  // Create stats...
      ...
      for (size_t j = 0; j < keys_all.size(); j++) {
        evec.push_back(
            std::make_pair(keys_all[j], (EventValueType)(Rand() % 10)));
        keys_all_saw_set.insert(keys_all[j]);
      }
      for (size_t j = 0; j < keys_some.size(); j++)
        if (Rand() % 2 == 0) {  // randomly w.p. 1/2
          evec.push_back(
              std::make_pair(keys_some[j], (EventValueType)(Rand() % 10)));
    ...
    int32 num_quest = Rand() % 10, num_iters = rand() % 5;
    qo.InitRand(dummy_stats, num_quest, num_iters,
                intersection ? kAllKeysIntersection : kAllKeysUnion);
    ...
    for (int i = 0; i < 2; i++) {
      ...
      if (oss.str() != oss2.str()) {
        KALDI_ERR << "Questions I/O failure: " << oss.str() << " vs. "
                  << oss2.str();
      }
    }
    ...
    if (n_stats > 0) {
      ...
      if (intersection) {
        ...
        for (std::set<int32>::iterator iter = keys_all_saw_set.begin();
             iter != keys_all_saw_set.end(); iter++) {
...
void TestSplitDecisionTree() {
  // part of the code is the same as the code for testing Questions::InitRand()
  // function.
  for (int32 p = 0; p < 4; p++) {
    std::vector<EventKeyType> keys_all, keys_some;
    {
    ...
    // We now create the Clusterable* stuff.
    BuildTreeStatsType
        stats;  // dummy because the Clusterable *pointers are actually NULL.
    size_t n_stats = Rand() % 100;
    // make sure we sometimes have empty or just one stats: may find extra bugs.
    if (n_stats > 90) n_stats = 0;
    if (n_stats > 80) n_stats = 1;
    ...
    for (size_t i = 0; i < n_stats; i++) {  // Create stats...
      ...
      EventMap::Check(evec);
      stats.push_back(std::make_pair(
          evec, (Clusterable *)new ScalarClusterable(RandGauss())));
    }
    Questions qo;                // all default.
    BaseFloat thresh = 0.00001;  // these stats have a count of 1... need v.
                                 // small thresh to get them to merge.
    ...
    if (n_stats > 0) {
      ...
      EventMap *split_tree =
          SplitDecisionTree(*trivial_tree, stats, qo, thresh, max_leaves,
                            &num_leaves, &impr, &smallest_split);
      KALDI_ASSERT(num_leaves <= max_leaves && smallest_split >= thresh);

      {
        BaseFloat impr_check = ObjfGivenMap(stats, *split_tree) -
                               ObjfGivenMap(stats, *trivial_tree);
        std::cout << "Objf impr is " << impr
      ...
      for (size_t i = 0; i < mapped_stats.size(); i++) {
        ...
        for (size_t j = 0; j < mapped_stats[i].size(); j++) {
          std::cout << ((ScalarClusterable *)(mapped_stats[i][j].second))
                           ->Info() << " ";
        }
        std::cout << '\n';
      }
  ...
  for (int32 p = 0; p < 10; p++) {
    ...
    for (size_t i = 0; i < num_stats; i++) {
      ...
      for (int32 i = 0; i < ev_sz; i++) {
        EventKeyType key = (i == 0 ? 0 : ev[i - 1].first) + Rand() % 2,
                     value = Rand() % 10;
...

    {
      ...
      ReadBuildTreeStats(ki.Stream(), binary_in, gc, &stats2);
