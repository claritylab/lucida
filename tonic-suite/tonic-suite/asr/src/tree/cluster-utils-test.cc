...
namespace kaldi {
static void TestClusterUtils() {  // just some very basic tests of the
                                  // GaussClusterable class.
  ...
  for (size_t i = 0; i + 1 < nGauss; i++) {
    ...
    KALDI_LOG << "Like_before = " << like_before << ", after = " << like_after
...
static void TestClusterUtilsVector() {  // just some very basic tests of the
                                        // VectorClusterable class.
  ...
  for (size_t i = 0; i < num_vectors; i++) delete v[i];
}
...
static void TestObjfPlus() {
  ...
  AssertEqual(a.ObjfPlus(b),
              -0.5 * (1.0 - 2.5) * (1.0 - 2.5));  // 0.5 because half-distance,
                                                  // squared = 1/4, times two
                                                  // points...
...
static void TestDistance() {
  ...
  AssertEqual(a.Objf(), 0.0);
  AssertEqual(b.Objf(), 0.0);
  AssertEqual(a.ObjfPlus(b), -a.Distance(b));  // since distance is negated
                                               // objf-change, and original
                                               // objfs were zero.
}  // end namespace kaldi
...
static void TestEnsureClusterableVectorNotNull() {
  ...
  KALDI_ASSERT(vec[0] != NULL && vec[2] != NULL && vec[0]->Objf() == 0 &&
               vec[2]->Objf() == 0 && vec[0] != vec[2] && vec[0] != vec[1]);
  DeletePointers(&vec);
}
...
static void TestAddToClusters() {
  ...
  KALDI_ASSERT(clusters[0] == NULL && clusters[1] != NULL &&
               clusters[4] != NULL);
  ...
  for (size_t p = 0; p < 100; p++) {
    ...
    for (size_t i = 0; i < n_stats; i++) {
      if (Rand() % 5 < 4) {
        ...
        if (Rand() % 2 == 0)
          ptr->Add(*ptr);  // make count equal 2.  for more randomness.
        stats[i] = ptr;
      } else
        stats[i] = NULL;  // make some zero. supposed to be robust to this.
    }
  ...
  for (size_t i = 0; i < 10; i++) {
    ...
    for (size_t j = 0; j < n_clust; j++) {
      ...
      for (size_t k = 0; k < n_points; k++)
        points.push_back(
            new ScalarClusterable(clust_center + RandUniform() * 0.01));
    }
    ...
    float ans = ClusterBottomUp(points, max_merge_thresh, min_clust, &clusters,
                                &assignments);

    KALDI_ASSERT(ans < 0.000001);  //  objf change should be negative.
    KALDI_LOG << "Objf change from bottom-up clustering is " << ans << '\n';

    ClusterBottomUp(points, max_merge_thresh, min_clust, NULL,
                    NULL);  // make sure no crash.

    if (0) {  // for debug if it breaks.
      for (size_t i = 0; i < points.size(); i++) {
        KALDI_LOG << "point " << i << ": "
                  << ((ScalarClusterable *)points[i])->Info() << " -> "
                  << assignments[i];
      }
      for (size_t i = 0; i < clusters.size(); i++) {
        KALDI_LOG << "clust " << i << ": "
                  << ((ScalarClusterable *)clusters[i])->Info();
      }
    }

    KALDI_ASSERT(clusters.size() ==
                 std::max(n_clust, std::min(points.size(), min_clust)));
...
static void TestRefineClusters() {
  for (size_t n = 0; n < 4; n++) {
    // Test it by creating a random clustering and verifying that it does not
    // make it worse, and
    ...
    for (size_t i = 0; i < clusters.size(); i++)
      clusters[i] = new ScalarClusterable();
    ...
    KALDI_LOG << "TestRefineClusters: objfs are: " << points_objf << " "
              << clust_objf_before << " " << clust_objf_after
              << ", impr = " << impr << '\n';
...
static void TestClusterKMeans() {
  ...
  for (size_t n = 0; n < 3; n++) {
    ...
    BaseFloat ans =
        ClusterKMeans(points, n_clust, &clusters, &assignments, kcfg);

    if (n < 3)
      ClusterKMeans(points, n_clust, NULL, NULL, kcfg);  // make sure no crash.

    BaseFloat clust_objf = SumClusterableObjf(clusters);

    KALDI_LOG << "TestClusterKmeans: objf after clustering is: " << clust_objf
              << ", impr is: " << ans << '\n';

    if (clusters.size() != n_clust) {
      KALDI_LOG << "Warning: unexpected number of clusters " << clusters.size()
                << " vs. " << n_clust << "";
    }
  ...
  if (n_wrong_tot * 4 > n_points_tot) {
    KALDI_LOG << "Got too many wrong in k-means test [may not be fatal, but "
                 "check it out.";
    KALDI_ASSERT(0);
  }
}
...
static void TestTreeCluster() {
  ...
  for (size_t n = 0; n < 10; n++) {
    ...
    int32 num_leaves = 0;
    BaseFloat ans = TreeCluster(points, n_clust, &clusters_ext, &assignments,
                                &clust_assignments, &num_leaves, tcfg);

    if (n < 3)
      TreeCluster(points, n_clust, NULL, NULL, NULL, NULL,
                  tcfg);  // make sure no crash
    ...
    KALDI_LOG << "TreeCluster: objf after clustering is: " << clust_objf
              << ", impr is: " << ans << '\n';

    if (n < 2)  // avoid generating too much output.
      KALDI_LOG << "Num nodes is " << clusters_ext.size() << ", leaves "
                << num_leaves;
    for (int32 i = 0; i < static_cast<int32>(clusters_ext.size()); i++) {
      if (n < 2)  // avoid generating too much output.
        KALDI_LOG << "Cluster " << i << ": "
                  << ((ScalarClusterable *)clusters_ext[i])->Info()
                  << ", parent is: " << clust_assignments[i] << "";
      KALDI_ASSERT(clust_assignments[i] > i ||
                   (i + 1 == static_cast<int32>(clusters_ext.size()) &&
                    clust_assignments[i] == i));
...
static void TestClusterTopDown() {
  ...
  for (size_t n = 0; n < 10; n++) {
    ...
    BaseFloat ans =
        ClusterTopDown(points, n_clust, &clusters, &assignments, tcfg);

    if (n < 3)
      ClusterTopDown(points, n_clust, NULL, NULL,
                     tcfg);  // make sure doesn't crash.

    BaseFloat clust_objf = SumClusterableObjf(clusters);

    KALDI_LOG << "ClusterTopDown: objf after clustering is: " << clust_objf
              << ", impr is: " << ans << '\n';
