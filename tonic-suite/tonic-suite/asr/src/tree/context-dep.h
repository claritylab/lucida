...
/*
  This header provides the declarations for the class ContextDependency, which
  inherits
  from the interface class "ContextDependencyInterface" in
  itf/context-dep-itf.h.
  This is basically a wrapper around an EventMap.  The EventMap
  (tree/event-map.h) declares most of the internals of the class, and the
...
class ContextDependency : public ContextDependencyInterface {
 public:
  ...
  ~ContextDependency() {
    if (to_pdf_ != NULL) delete to_pdf_;
  }
...
/// GenRandContextDependency is mainly of use for debugging.  Phones must be
/// sorted and uniq
/// on input.
/// @param phones [in] A vector of phone id's [must be sorted and uniq].
/// @param ensure_all_covered [in] boolean argument; if true,
/// GenRandContextDependency
///        generates a context-dependency object that "works" for all phones [no
///        gaps].
/// @param num_pdf_classes [out] outputs a vector indexed by phone, of the
/// number
///          of pdf classes (e.g. states) for that phone.
/// @return Returns the a context dependency object.
ContextDependency *GenRandContextDependency(
    const std::vector<int32> &phones, bool ensure_all_covered,
    std::vector<int32> *num_pdf_classes);

/// GenRandContextDependencyLarge is like GenRandContextDependency but generates
/// a larger tree
/// with specified N and P for use in "one-time" larger-scale tests.
ContextDependency *GenRandContextDependencyLarge(
    const std::vector<int32> &phones, int N, int P, bool ensure_all_covered,
    std::vector<int32> *num_pdf_classes);
...
// of a single
// phone.)  Each element of phone_classes is a set of phones that are in that
// class.
ContextDependency *MonophoneContextDependencyShared(
    const std::vector<std::vector<int32> > phone_classes,
    const std::vector<int32> phone2num_pdf_classes);
...
// See build-tree.h for functions relating to actually building the decision
// trees.
