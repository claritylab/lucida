...
ContextDependency *GenRandContextDependency(const std::vector<int32> &phone_ids,
  ...
  qopts.InitRand(stats, num_quest, num_iters,
  ...
  tree = BuildTree(qopts, phone_sets, *hmm_lengths, share_roots, do_split,
                   stats, thresh, 1000, 0.0, P);
  DeleteBuildTreeStats(&stats);
  return new ContextDependency(N, P, tree);
}

ContextDependency *GenRandContextDependencyLarge(
    const std::vector<int32> &phone_ids, int N, int P, bool ensure_all_covered,
    std::vector<int32> *hmm_lengths) {
  KALDI_ASSERT(IsSortedAndUniq(phone_ids));
  int32 num_phones = phone_ids.size();
...
ContextDependency *MonophoneContextDependencyShared(
    ...
    const std::vector<int32> phone2num_pdf_classes) {
  std::vector<bool> share_roots(phone_sets.size(),
                                false);  // don't share roots.
  // N is context size, P = position of central phone (must be 0).
  int32 num_leaves = 0, P = 0, N = 1;
  EventMap *pdf_map = GetStubMap(P, phone_sets, phone2num_pdf_classes,
                                 share_roots, &num_leaves);
