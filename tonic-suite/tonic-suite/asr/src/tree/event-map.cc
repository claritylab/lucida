...
EventMap *EventMap::Read(std::istream &is, bool binary) {
  ...
  if (c == 'N') {
    ...
    KALDI_ERR << "EventMap::read, was not expecting character "
              << CharToString(c) << ", at file position " << is.tellg();
  ...
  for (EventValueType value = 0; value < size; value++) {
    if (table_[value] != NULL) {
      ...
      if (keys_to_map.count(key_) == 0)
        ...
        unordered_map<EventValueType, EventValueType>::const_iterator iter =
            value_map.find(value);
        if (iter == value_map.end()) {
          KALDI_ERR << "Could not map value " << value << " for key " << key_;
        }
...
EventMap *SplitEventMap::MapValues(
  ...
  if (keys_to_map.count(key_) == 0) {
    ...
    for (ConstIntegerSet<EventValueType>::iterator iter = yes_set_.begin();
         iter != yes_set_.end(); ++iter) {
      EventValueType value = *iter;
      unordered_map<EventValueType, EventValueType>::const_iterator map_iter =
          value_map.find(value);
      if (map_iter == value_map.end())
        KALDI_ERR << "Value " << value << ", for key " << key_
                  << ", cannot be mapped.";
...
SplitEventMap *SplitEventMap::Read(std::istream &is, bool binary) {
  ...
  if (yes == NULL || no == NULL)
    KALDI_ERR << "SplitEventMap::Read, NULL pointers.";
...
TableEventMap::TableEventMap(EventKeyType key,
  ...
  if (map_in.size() == 0)
    ...
    std::map<EventValueType, EventMap *>::const_iterator iter = map_in.begin(),
                                                         end = map_in.end();
    for (; iter != end; ++iter) {
...
TableEventMap::TableEventMap(
  ...
  if (map_in.size() == 0)
    ...
    EventValueType highest_val = map_in.rbegin()->first;
    table_.resize(highest_val + 1, NULL);
    std::map<EventValueType, EventAnswerType>::const_iterator
        iter = map_in.begin(),
        end = map_in.end();
    for (; iter != end; ++iter) {
      KALDI_ASSERT(iter->first >= 0 && iter->first <= highest_val);
    ...
    for (size_t i = 0; i < children.size(); i++) {
      ...
      if (IsLeafNode(child)) {
        ...
        if ((*leaf_parents)[leaf] != NULL) {
          KALDI_WARN << "Repeated leaf! Did you suppress leaf clustering when "
                        "building the tree?";
...
bool GetTreeStructure(const EventMap &map, int32 *num_leaves,
  ...
  *num_leaves = leaf_parents.size();
  ...
  for (int32 i = 0; i < num_nodes; i++)
    KALDI_ASSERT((*parents)[i] > i ||
                 (i + 1 == num_nodes && (*parents)[i] == i));
  return true;
}
