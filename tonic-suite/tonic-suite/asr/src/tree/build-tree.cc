...
void GenRandStats(int32 dim, int32 num_stats, int32 N, int32 P,
  ...
  for (int32 i = 0; i < max_phone + 1; i++)
    for (int32 j = 0; j < dim; j++)
      phone_vecs(i, j) = RandGauss() * (2.0 / (j + 1));
  ...
  for (int32 i = 0;
       i < num_stats || (ensure_all_phones_covered && !all_covered); i++) {
    // decide randomly on a phone-in-context.
    std::vector<int32> phone_vec(N);
    for (size_t i = 0; i < (size_t)N; i++)
      phone_vec[i] = phone_ids[(Rand() % num_phones)];
    ...
    for (int32 j = 0; j < hmm_length; j++) {
      ...
      event_vec.push_back(std::make_pair(
          kPdfClass, (EventValueType)j));  // record the position.
      for (size_t pos = 0; pos < (size_t)N; pos++) {
        if (pos == (size_t)(P) || is_ctx_dep[phone_vec[P]])
          event_vec.push_back(std::make_pair((EventKeyType)pos,
                                             (EventValueType)phone_vec[pos]));
        // The if-statement above ensures we do not record the context of
        // "context-free"
        // phone (e.g., silence).
      }

      Vector<BaseFloat> mean(dim);  // mean of Gaussian.
      GaussClusterable *this_stats =
          new GaussClusterable(dim, 0.1);  // 0.1 is var floor.
      {  // compute stats; this block attempts to simulate the process of "real"
         // data
        // collection and does not correspond to any code you would write in a
        // real
        // scenario.
        Vector<BaseFloat> weights(N);  // weight of each component.
        for (int32 k = 0; k < N; k++) {
          BaseFloat k_pos =
              (N - 0.5 - k) / N;  // between 0 and 1, less for lower k...
    ...
    for (size_t i = 0; i < num_phones; i++)
      if (!covered[phone_ids[i]]) all_covered = false;
  }
...
EventMap *BuildTree(Questions &qopts,
  ...
  KALDI_ASSERT(!phone_sets.empty() && phone_sets.size() == share_roots.size() &&
               do_split.size() == phone_sets.size());

  // the inputs will be further checked in GetStubMap.
  int32 num_leaves = 0;  // allocator for leaves.

  EventMap *tree_stub = GetStubMap(P, phone_sets, phone2num_pdf_classes,
                                   share_roots, &num_leaves);
  KALDI_LOG << "BuildTree: before building trees, map has " << num_leaves
            << " leaves.";
  ...
  for (size_t i = 0; i < phone_sets.size(); i++)
    if (!do_split[i])
      nonsplit_phones.insert(nonsplit_phones.end(), phone_sets[i].begin(),
                             phone_sets[i].end());
  ...
  EventMap *tree_split =
      SplitDecisionTree(*tree_stub, filtered_stats, qopts, thresh, max_leaves,
                        &num_leaves, &impr, &smallest_split);

  if (cluster_thresh < 0.0) {
    KALDI_LOG << "Setting clustering threshold to smallest split "
              << smallest_split;
  ...
  KALDI_VLOG(1) << "Including just phones that were split, improvement is "
                << impr_normalized_filt << " per frame over " << normalizer_filt
                << " frames.";
  ...
  if (cluster_thresh != 0.0) {  // Cluster the tree.
    ...
    int32 num_removed = 0;
    EventMap *tree_clustered = ClusterEventMapRestrictedByMap(
        *tree_split, stats, cluster_thresh, *tree_stub, &num_removed);
    ...
    KALDI_VLOG(1) << "Normalizing over only split phones, this is: "
                  << ((objf_after_cluster - objf_before_cluster) /
                      normalizer_filt) << " per frame.";
...
static void ComputeTreeMapping(const EventMap &small_tree,
  ...
  std::vector<BuildTreeStatsType>
      split_stats_small;  // stats split by small tree
  ...
  std::vector<int32>
      small_leaves_unseen;  // a list of small leaves that had no stats..
    ...
    if (static_cast<size_t>(i) >= split_stats_small.size() ||
      ...
      for (size_t j = 0; j < split_stats_small[i].size(); j++) {
        ...
        if (leaf < 0 || leaf >= num_leaves_big)
          KALDI_ERR << "Leaf out of range: " << leaf << " vs. "
                    << num_leaves_big;
        if ((*leaf_map)[leaf] != -1 && (*leaf_map)[leaf] != i)
          KALDI_ERR << "Inconsistent mapping for big tree: " << i << " vs. "
                    << (*leaf_map)[leaf];
        (*leaf_map)[leaf] = i;
      }
    }
  ...
  // happen and if it does it's due to trying to assign tree roots to unseen
  // phones,
...
EventMap *BuildTreeTwoLevel(Questions &qopts,
                            ...
                            const std::vector<bool> &do_split,
                            const BuildTreeStatsType &stats,
                            int32 max_leaves_first, int32 max_leaves_second,
                            bool cluster_leaves, int32 P,
                            std::vector<int32> *leaf_map) {
  KALDI_LOG << "****BuildTreeTwoLevel: building first level tree";
  EventMap *first_level_tree =
      BuildTree(qopts, phone_sets, phone2num_pdf_classes, share_roots, do_split,
                stats, 0.0, max_leaves_first, 0.0, P);
  ...
  BaseFloat impr;
  EventMap *tree =
      SplitDecisionTree(*first_level_tree, filtered_stats, qopts, 0.0,
                        max_leaves_second, &num_leaves, &impr, &smallest_split);
  ...
  KALDI_LOG << "After second decision tree split, num-leaves = " << num_leaves
            << ", like-impr = " << impr_normalized << " per frame over "
            << normalizer << " frames.";
  ...
  if (cluster_leaves) {  // Cluster the leaves of the tree.
    ...
    int32 num_removed = 0;
    EventMap *tree_clustered = ClusterEventMapRestrictedByMap(
        *tree, stats, smallest_split, *first_level_tree, &num_removed);
  ...
  ComputeTreeMapping(*first_level_tree, *tree, stats, leaf_map);
  ...
  {  // Next do another renumbering of "tree" so that leaves with the
    ...
    for (size_t i = 0; i < leaf_map->size(); i++)
      leaf_pairs.push_back(
          std::make_pair((*leaf_map)[i], static_cast<int32>(i)));
...
void ReadSymbolTableAsIntegers(std::string filename, bool include_eps,
                               std::vector<int32> *syms) {
  std::ifstream is(filename.c_str());
  if (!is.good())
    KALDI_ERR << "ReadSymbolTableAsIntegers: could not open symbol table "
              << filename;
  std::string line;
  ...
  while (getline(is, line)) {
    ...
    ss >> sym >> index >> std::ws;
    if (ss.fail() || !ss.eof()) {
      KALDI_ERR << "Bad line in symbol table: " << line
                << ", file is: " << filename;
    }
    if (include_eps || index != 0) syms->push_back(index);
    if (index == 0 && sym != "<eps>") {
      KALDI_WARN << "Symbol zero is " << sym << ", traditionally <eps> is "
                                                "used.  Make sure this is not "
                                                "a \"real\" symbol.";
    }
  }
  size_t sz = syms->size();
  SortAndUniq(syms);
  if (syms->size() != sz)
    KALDI_ERR << "Symbol table " << filename
...
static void ObtainSetsOfPhones(
    const std::vector<std::vector<int32> > &
        phone_sets,  // the original phone sets, may
  ...
  for (int32 j = 0; j < static_cast<int32>(clust_assignments.size()); j++) {
    ...
    if (parent <
        static_cast<int32>(clust_assignments.size()) -
            1) {  // parent is not out of range [i.e. not the top one]...
      // add all j's phones to its parent.
      raw_sets[parent].insert(raw_sets[parent].end(), raw_sets[j].begin(),
                              raw_sets[j].end());
    }
  }
  ...
  // are empty, but we want to ensure they are all there regardless).  note
  // these
...
void AutomaticallyObtainQuestions(
    ...
    const std::vector<int32> &all_pdf_classes_in, int32 P,
  ...
  for (size_t i = 0; i < phone_sets.size(); i++) {
    ...
    if (!IsSortedAndUniq(phone_sets[i]))
      KALDI_ERR << "Phone set in AutomaticallyObtainQuestions contains "
                   "duplicate phones";
    for (size_t j = 0; j < phone_sets[i].size(); j++)
  ...
  if (phones.empty()) KALDI_ERR << "No phones provided.";
  ...
  if (retained_stats.size() * 10 < stats.size()) {
    ...
    KALDI_WARN
        ...
        << "stats disappeared: the size changed from " << stats.size() << " to "
        << retained_stats.size() << ".  You might be using "
  ...
  for (int32 i = 0; static_cast<size_t>(i) < summed_stats.size();
       i++) {  // A check.
    if (summed_stats[i] != NULL &&
        !binary_search(phones.begin(), phones.end(), i)) {
      KALDI_WARN << "Phone " << i << " is present in stats but is not in phone "
                                     "list [make sure you intended this].";
    }
  }

  EnsureClusterableVectorNotNull(
      &summed_stats);  // make sure no NULL pointers in summed_stats.
  // will replace them with pointers to empty stats.

  std::vector<Clusterable *> summed_stats_per_set(phone_sets.size(),
                                                  NULL);  // summed up by set.
  for (size_t i = 0; i < phone_sets.size(); i++) {
  ...
  std::vector<int32> assignments;  // assignment of phones to clusters. dim ==
                                   // summed_stats.size().
  std::vector<int32>
      clust_assignments;  // Parent of each cluster.  Dim == #clusters.
  ...
  TreeCluster(summed_stats_per_set,
              ...
              &assignments, &clust_assignments, &num_leaves, topts);

  // process the information obtained by TreeCluster into the
  // form we want at output.
  ObtainSetsOfPhones(phone_sets, assignments, clust_assignments, num_leaves,
                     questions_out);
...
void KMeansClusterPhones(BuildTreeStatsType &stats,
  ...
  ClusterKMeansOptions
      opts;  // Just using the default options which are a reasonable
  // compromise between speed and accuracy.

  std::vector<int32> assignments;
  BaseFloat objf_impr = ClusterKMeans(summed_stats_per_set, num_classes, NULL,
                                      &assignments, opts);
...
void ReadRootsFile(std::istream &is,
  ...
  while (!getline(is, line).fail()) {
    ...
    if (ss.fail() && shared != "shared" && shared != "not-shared")
      KALDI_ERR << "Bad line in roots file: line " << line_number << ": "
                << line;
  ...
  if (phone_sets->empty()) KALDI_ERR << "Empty roots file ";
}
