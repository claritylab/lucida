...
void ReadBuildTreeStats(std::istream &is, bool binary,
  ...
  for (size_t i = 0; i < size; i++) {
    ...
    if (nonNull)
      (*stats)[i].second = example.ReadNew(is, binary);
    else
      (*stats)[i].second = NULL;
  }
}
...
bool PossibleValues(EventKeyType key, const BuildTreeStatsType &stats,
  ...
  if (ans) CopySetToVector(values, ans);
  return all_present;
}

static void GetEventKeys(const EventType &vec,
...
void FindAllKeys(const BuildTreeStatsType &stats, AllKeysType keys_type,
  ...
  for (; iter != end; ++iter) {
    ...
    if (keys_type == kAllKeysInsistIdentical) {
      if (keys2 != keys)
        KALDI_ERR << "AllKeys: keys in events are not all the same [called "
                     "with kAllKeysInsistIdentical and all are not identical.";
      ...
      std::vector<EventKeyType>::iterator end_iter =
          std::set_intersection(keys.begin(), keys.end(), keys2.begin(),
                                keys2.end(), new_keys.begin());
      ...
      // following line relies on sorted order of event keys.
      std::vector<EventKeyType>::iterator end_iter =
          std::set_union(keys.begin(), keys.end(), keys2.begin(), keys2.end(),
                         new_keys.begin());
      new_keys.erase(end_iter, new_keys.end());
      keys = new_keys;
    }
...
EventMap *DoTableSplit(const EventMap &orig, EventKeyType key,
  ...
  // Now for each leaf that has stats in it, do the table split according to the
  // given name.
  std::vector<EventMap *> splits(split_stats.size(), NULL);
  for (EventAnswerType leaf = 0; leaf < (EventAnswerType)split_stats.size();
       leaf++) {
    if (!split_stats[leaf].empty()) {
      ...
      KALDI_ASSERT(
          all_present);  // currently do not support mapping undefined values.
      KALDI_ASSERT(!vals.empty() &&
                   vals.front() >= 0);  // don't support mapping negative values
      // at present time-- would need different EventMap type, not
      // TableEventMap.
      std::vector<EventMap *> table(vals.back() + 1, (EventMap *)NULL);
      for (size_t idx = 0; idx < vals.size(); idx++) {
        EventValueType val = vals[idx];
        if (idx == 0)
          table[val] = new ConstantEventMap(leaf);  // reuse current leaf.
        else
          table[val] =
              new ConstantEventMap((*num_leaves)++);  // else take new leaf id.
      }
...
EventMap *DoTableSplitMultiple(const EventMap &orig,
  ...
  if (keys.empty())
    ...
    for (size_t i = 0; i < keys.size(); i++) {
      EventMap *next =
          DoTableSplit((cur ? *cur : orig), keys[i], stats, num_leaves);
...
void SplitStatsByMap(const BuildTreeStatsType &stats, const EventMap &e,
  ...
  for (iter = stats.begin(); iter != end; ++iter) {
    ...
    if (!e.Map(evec, &ans))  // this is an error--could not map it.
      KALDI_ERR << "SplitStatsByMap: could not map event vector "
                << EventTypeToString(evec)
...
void FilterStatsByKey(
  ...
  for (iter = stats_in.begin(); iter != end; ++iter) {
    ...
    if (in_values == include_if_present) stats_out->push_back(*iter);
  }
}

Clusterable *SumStats(const BuildTreeStatsType &stats_in) {
  Clusterable *ans = NULL;
  BuildTreeStatsType::const_iterator iter = stats_in.begin(),
                                     end = stats_in.end();
  for (; iter != end; ++iter) {
...
void SumStatsVec(const std::vector<BuildTreeStatsType> &stats_in,
  ...
  for (size_t i = 0; i < stats_in.size(); i++)
    (*stats_out)[i] = SumStats(stats_in[i]);
}
...
BaseFloat ComputeInitialSplit(const std::vector<Clusterable *> &summed_stats,
  ...
  if (total == NULL)
    return 0.0;  // because there were no stats or non-NULL stats.
  BaseFloat unsplit_objf = total->Objf();

  const std::vector<std::vector<EventValueType> > &questions_of_this_key =
      key_opts.initial_questions;
  ...
  for (size_t i = 0; i < questions_of_this_key.size(); i++) {
    ...
    std::vector<int32> assignments(summed_stats.size(),
                                   0);       // 0 is index of "no".
    std::vector<Clusterable *> clusters(2);  // no and yes clusters.
    for (std::vector<EventValueType>::const_iterator iter = yes_set.begin();
    ...
    if (this_objf <
        unsplit_objf -
            0.001 *
                std::abs(unsplit_objf)) {  // got worse; should never happen.
      // of course small differences can be caused by roundoff.
      KALDI_WARN << "Objective function got worse when building tree: "
                 << this_objf << " < " << unsplit_objf;
      KALDI_ASSERT(!(this_objf <
                     unsplit_objf -
                         0.01 * (200 + std::abs(unsplit_objf))));  // do assert
                                                                   // on more
                                                                   // stringent
                                                                   // check.
    }
  ...
  delete total;
  if (best_idx != -1) *yes_set = questions_of_this_key[best_idx];
...
BaseFloat FindBestSplitForKey(const BuildTreeStatsType &stats,
  ...
  if (stats.size() <= 1)
    return 0.0;  // cannot split if only zero or one instance of stats.
  if (!PossibleValues(key, stats, NULL)) {
    yes_set_out->clear();
    return 0.0;  // Can't split as key not always defined.
  }
  std::vector<Clusterable *>
      summed_stats;  // indexed by value corresponding to key. owned here.
  ...
  BaseFloat improvement =
      ComputeInitialSplit(summed_stats, q_opts, key, &yes_set);
  // find best basic question.

  std::vector<int32> assignments(summed_stats.size(),
                                 0);  // assigns to "no" (0) by default.
  for (std::vector<EventValueType>::const_iterator iter = yes_set.begin();
       iter != yes_set.end(); iter++) {
    KALDI_ASSERT(*iter >= 0);
  ...
  // even if improvement == 0 we continue; if we do RefineClusters we may get
  // further improvement.
  // now do the RefineClusters stuff.  Note that this is null-op if
  // q_opts.GetQuestionsOf(key).refine_opts.num_iters == 0.  We could check for
  ...
  if (q_opts.GetQuestionsOf(key).refine_opts.num_iters > 0) {
    ...
    // zero iters.
    BaseFloat refine_impr =
        RefineClusters(summed_stats, &clusters, &assignments,
    ...
    for (size_t i = 0; i < assignments.size(); i++)
      if (assignments[i] == 1) yes_set.push_back(i);
  }
  *yes_set_out = yes_set;
  ...
  {  // Check the "ans" is correct.
    ...
    if (clusters[0] == NULL || clusters[1] == NULL)
      ...
      impr = clusters[0]->Distance(*(clusters[1]));
...
class DecisionTreeSplitter {
 public:
  ...
  void DoSplit(int32 *next_leaf) {
    if (!yes_) {  // not already split; we are a leaf, so split.
      ...
      best_split_impr_ =
          std::max(yes_->BestSplit(), no_->BestSplit());  // may have changed.
    }
  }
  DecisionTreeSplitter(EventAnswerType leaf, const BuildTreeStatsType &stats,
                       const Questions &q_opts)
      : q_opts_(q_opts), yes_(NULL), no_(NULL), leaf_(leaf), stats_(stats) {
    // not, this must work when stats is empty too. [just gives zero
    // improvement, non-splittable].
    FindBestSplit();
  }
 ...
 private:
  void DoSplitInternal(int32 *next_leaf) {
    ...
    yes_stats.reserve(stats_.size());
    no_stats.reserve(
        stats_.size());  //  probably better than multiple resizings.
    ...
    {  // Check objf improvement.
      ...
      // this is a negated objf improvement from merging (== objf improvement
      // from splitting).
      if (!ApproxEqual(impr_check, best_split_impr_, 0.01)) {
        KALDI_WARN << "DoSplitInternal: possible problem: " << impr_check
                   << " != " << best_split_impr_;
      }
      delete yes_clust;
      delete no_clust;
  ...
  void FindBestSplit() {
    ...
    if (all_keys.size() == 0) {
      KALDI_WARN << "DecisionTreeSplitter::FindBestSplit(), no keys available "
                    "to split on (maybe no key covered all of your events, or "
                    "there was a problem with your questions configuration?)";
    }
    best_split_impr_ = 0;
    for (size_t i = 0; i < all_keys.size(); i++) {
      if (q_opts_.HasQuestionsForKey(all_keys[i])) {
        std::vector<EventValueType> temp_yes_set;
        BaseFloat split_improvement =
            FindBestSplitForKey(stats_, q_opts_, all_keys[i], &temp_yes_set);
  ...
  BuildTreeStatsType
      stats_;  // vector of stats.  pointers inside there not owned here.
...
EventMap *SplitDecisionTree(const EventMap &input_map,
                            const BuildTreeStatsType &stats, Questions &q_opts,
                            BaseFloat thresh,
                            int32 max_leaves,  // max_leaves<=0 -> no maximum.
                            int32 *num_leaves, BaseFloat *obj_impr_out,
                            BaseFloat *smallest_split_change_out) {
  KALDI_ASSERT(num_leaves != NULL &&
               *num_leaves > 0);  // can't be 0 or input_map would be empty.
  ...
  {  // set up "builders" [one for each current leaf].  This array is never
     // extended.
    // the structures generated during splitting remain as trees at each array
    // location.
    std::vector<BuildTreeStatsType> split_stats;
  ...
  {  // Do the splitting.
    int32 count = 0;
    std::priority_queue<std::pair<BaseFloat, size_t> >
        queue;  // use size_t because logically these
    // are just indexes into the array, not leaf-ids (after splitting they are
    // no longer leaf id's).
    // Initialize queue.
    for (size_t i = 0; i < builders.size(); i++)
      queue.push(std::make_pair(builders[i]->BestSplit(), i));
    // Note-- queue's size never changes from now.  All the alternatives leaves
    // to split are
    ...
    while (queue.top().first > thresh &&
      ...
      smallest_split_change =
          std::min(smallest_split_change, queue.top().first);
    ...
    KALDI_LOG << "DoDecisionTreeSplit: split " << count
...
int ClusterEventMapGetMapping(const EventMap &e_in,
  ...
  BaseFloat normalizer = SumClusterableNormalizer(summed_stats_contiguous),
            change;
  change = ClusterBottomUp(
      summed_stats_contiguous, thresh,
  ...
  for (size_t i = 0; i < summed_stats_contiguous.size(); i++) {
    ...
    size_t new_index = indexes[assignments[i]];  // index assigned by
                                                 // clusterig-- map to existing
                                                 // indices in the map,
    // that we clustered from, so we don't conflict with indices in other parts
    // of the tree.
    KALDI_ASSERT(
        (*mapping)[index] == NULL ||
        "Error: Cluster seems to have been "
...
EventMap *RenumberEventMap(const EventMap &e_in, int32 *num_leaves) {
  ...
  EventAnswerType max_leaf_plus_one =
      initial_leaves.back() +
      1;  // will typically, but not always, equal *num_leaves.
  std::vector<EventMap *> mapping(max_leaf_plus_one, (EventMap *)NULL);
  std::vector<EventAnswerType>::iterator iter = initial_leaves.begin(),
                                         end = initial_leaves.end();
...
EventMap *ShareEventMapLeaves(const EventMap &e_in, EventKeyType key,
  ...
  for (size_t i = 0; i < values.size(); i++) {
    ...
    for (size_t j = 0; j < values[i].size(); j++) {
      ...
      e_in.MultiMap(evec,
                    &(pdfs[i]));  // append any corresponding pdfs to pdfs[i].
  ...
  for (size_t i = 0; i < values.size(); i++) {
    if (pdfs[i].empty())
      KALDI_WARN
          << "ShareEventMapLeaves: no leaves in one bucket.";  // not expected.
    else {
...
static int32 ClusterEventMapRestrictedHelper(
    const EventMap &e_in, const BuildTreeStatsType &stats, BaseFloat thresh,
    std::vector<EventKeyType> keys, std::vector<EventMap *> *leaf_mapping) {
  if (keys.size() == 0) {
    ...
    for (size_t i = 0; i < split_stats.size(); i++)
      if (split_stats[i].size() != 0)
        ans += ClusterEventMapRestrictedHelper(e_in, split_stats[i], thresh,
                                               keys, leaf_mapping);
    return ans;
  }
}
...
EventMap *ClusterEventMapRestrictedByKeys(const EventMap &e_in,
                                          ...
                                          int32 *num_removed) {
  std::vector<EventMap *>
      leaf_mapping;  // For output of ClusterEventMapGetMapping.

  int32 nr =
      ClusterEventMapRestrictedHelper(e_in, stats, thresh, keys, &leaf_mapping);
  ...
  {  // Checking inputs.
    ...
    for (size_t i = 0; i < phone_sets.size(); i++) {
      ...
      for (size_t j = 0; j < phone_sets[i].size(); j++) {
        KALDI_ASSERT(all_phones.count(phone_sets[i][j]) ==
                     0);  // check not present.
  ...
  if (phone_sets.size() ==
    ...
    if (share_roots[0]) {  // if "shared roots" return a single leaf.
      ...
      return new TableEventMap(kPdfClass,  // split on hmm-position
             ...
             static_cast<int32>(phone_sets.size()) <=
...
bool ConvertStats(int32 oldN, int32 oldP, int32 newN, int32 newP,
  ...
  if (newN - newP - 1 > oldN - oldP - 1) {
    KALDI_WARN << "Cannot convert stats to have more right-context: "
               << (newN - newP - 1) << " > " << (oldN - oldP - 1);
  }
