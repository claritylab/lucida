...
void TestGenRandStats() {
  for (int32 p = 0; p < 2; p++) {
    ...
    for (size_t i = 0; i < (size_t)num_phones; i++)
      phone_ids[i] =
          (i == 0 ? (Rand() % 2) : phone_ids[i - 1] + 1 + (Rand() % 2));
    ...
    for (int32 i = 0; i <= max_phone; i++) {
      ...
      is_ctx_dep[i] =
          (RandUniform() < ctx_dep_prob);  // true w.p. ctx_dep_prob.
    }
    for (size_t i = 0; i < (size_t)num_phones; i++) {
    ...
    // if it doesn't really ensure that all are covered with true, this will
    // induce
    // failure in the test of context-fst.
    GenRandStats(dim, num_stats, N, P, phone_ids, hmm_lengths, is_ctx_dep,
                 false, &stats);
    ...
    std::cout << "hmm_lengths = ";
    WriteIntegerVector(std::cout, false, hmm_lengths);
    std::cout << "phone_ids = ";
    WriteIntegerVector(std::cout, false, phone_ids);
    std::cout << "Stats are: \n";
...
void TestBuildTree() {
  for (int32 p = 0; p < 3; p++) {
    ...
    int32 num_stats =
        1 + (Rand() % 15) * (Rand() % 15);  // up to 14^2 + 1 separate stats.
    int32 N = 2 + Rand() % 2;               // 2 or 3.
    int32 P = Rand() % N;
    float ctx_dep_prob = 0.5 + 0.5 * RandUniform();
    ...
    qopts.InitRand(
        stats, num_quest, num_iters,
        kAllKeysUnion);  // This was tested in build-tree-utils-test.cc
...

    {
      ...
      tree = BuildTree(qopts, phone_sets, hmm_lengths, share_roots, do_split,
                       stats, thresh, max_leaves, 0.0, P);
