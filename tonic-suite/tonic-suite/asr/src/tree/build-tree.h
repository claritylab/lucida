 ...
 *                 in the phone (this info is derived from the HmmTopology
 object)
 ...
 * @param cluster_thresh [in] Threshold for clustering leaves after
 decision-tree
 *                  splitting (only within each phone-set); leaves will be
 combined
 *                  if log-likelihood change is less than this.  A value about
 equal
 *                  to "thresh" is suitable
 *                  if thresh != 0; otherwise, zero will mean no clustering is
 done,
 *                  or a negative value (e.g. -1) sets it to the smallest
 likelihood
 *                  change seen during the splitting algorithm; this typically
 causes
...
/**
 *
 *  BuildTreeTwoLevel builds a two-level tree, useful for example in building
 tied mixture
 *  systems with multiple codebooks.  It first builds a small tree by splitting
 to
 *  "max_leaves_first".  It then splits at the leaves of "max_leaves_first"
 (think of this
 *  as creating multiple little trees at the leaves of the first tree), until
 the total
 *  number of leaves reaches "max_leaves_second".  It then outputs the second
 tree, along
 *  with a mapping from the leaf-ids of the second tree to the leaf-ids of the
 first tree.
 *  Note that the interface is similar to BuildTree, and in fact it calls
 BuildTree
 *  internally.
 *
 *  The sets "phone_sets" dictate how we set up the roots of the decision trees.
 ...
 * @param max_leaves_second [in] Maximum number of leaves it will create in
 second
 *                  level of decision tree.  Must be > max_leaves_first.
 * @param cluster_leaves [in] Boolean value; if true, we post-cluster the leaves
 produced
 *                  in the second level of decision-tree split; if false, we
 don't.
 *                  The threshold for post-clustering is the log-like change of
 the last
 *                  decision-tree split; this typically causes about a 20%
 reduction in
 *                  the number of leaves.
 * @param P [in]   The central position of the phone context window, e.g. 1 for
 a
 *                 triphone system.
...
/// It tries to do so in such a way that they mimic "real" stats.  The event
/// keys
...
/// GenRandStats is useful only for testing but it serves to document the format
/// of
/// stats used by BuildTreeDefault.
/// if is_ctx_dep[phone] is set to false, GenRandStats will not define the keys
/// for
/// other than the P-1'th phone.

/// @param dim [in] dimension of features.
/// @param num_stats [in] approximate number of separate phones-in-context
/// wanted.
...
/// @param is_ctx_dep [in] boolean array indexed by phone, saying whether each
/// phone
///     is context dependent.
/// @param ensure_all_phones_covered [in] Boolean argument: if true,
/// GenRandStats
///     ensures that every phone is seen at least once in the central position
///     (P).
 ...
 *      that we consider for clustering.  In the normal case this is the
 *singleton
 ...
 *    @param questions_out [out] The questions (sets of phones) are output to
 *here.
 **/
...
/// This function clusters the phones (or some initially specified sets of
/// phones)
/// into sets of phones, using a k-means algorithm.  Useful, for example, in
/// building
