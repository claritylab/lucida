...
class BottomUpClusterer {
 public:
  BottomUpClusterer(const std::vector<Clusterable *> &points,
      ...
      : ans_(0.0),
        points_(points),
        max_merge_thresh_(max_merge_thresh),
        min_clust_(min_clust),
        clusters_(clusters_out != NULL ? clusters_out : &tmp_clusters_),
        assignments_(assignments_out != NULL ? assignments_out
                                             : &tmp_assignments_) {
...
BaseFloat BottomUpClusterer::Cluster() {
  ...
  while (nclusters_ > min_clust_ && !queue_.empty()) {
    std::pair<BaseFloat, std::pair<uint_smaller, uint_smaller> > pr =
        queue_.top();
...
void BottomUpClusterer::Renumber() {
  ...
  std::vector<uint_smaller> mapping(
      npoints_, static_cast<uint_smaller>(
                    -1));  // mapping from intermediate to final clusters.
  ...
  for (int32 i = 0; i < npoints_; i++) {  // now reprocess assignments_.
    ...
    KALDI_ASSERT((*clusters_)[ii] !=
                 NULL);  // cannot have assignment to nonexistent cluster.
  ...
  for (int32 i = 0; i < npoints_; i++) {
    for (int32 j = 0; j < i; j++) {
      ...
      if (dist <= max_merge_thresh_)
        queue_.push(
            std::make_pair(dist, std::make_pair(static_cast<uint_smaller>(i),
...
bool BottomUpClusterer::CanMerge(int32 i, int32 j, BaseFloat dist) {
  ...
  if ((*clusters_)[i] == NULL || (*clusters_)[j] == NULL) return false;
...
BaseFloat ClusterBottomUp(const std::vector<Clusterable *> &points,
  ...
  BottomUpClusterer bc(points, max_merge_thresh, min_clust, clusters_out,
                       assignments_out);
...
BaseFloat CompartmentalizedBottomUpClusterer::Cluster(
  ...
  while (nclusters_ > min_clust_ && !queue_.empty()) {
    ...
    if (CanMerge(qelem.compartment, qelem.point1, qelem.point2, qelem.dist))
      total_obj_change +=
          MergeClusters(qelem.compartment, qelem.point1, qelem.point2);
  }
  for (int32 comp = 0; comp < ncompartments_; comp++) Renumber(comp);
...
void CompartmentalizedBottomUpClusterer::Renumber(int32 comp) {
  ...
  for (int32 i = 0; i < npoints_[comp]; i++) {
    if (clusters_[comp][i] != NULL) clusts_in_compartment++;
  }
...
void CompartmentalizedBottomUpClusterer::SetInitialDistances() {
  ...
  for (int32 comp = 0; comp < ncompartments_; comp++) {
    ...
    for (int32 i = 0; i < npoints_[comp]; i++)
      for (int32 j = 0; j < i; j++) SetDistance(comp, i, j);
  }
}
...
bool CompartmentalizedBottomUpClusterer::CanMerge(int32 comp, int32 i, int32 j,
  ...
  if (clusters_[comp][i] == NULL || clusters_[comp][j] == NULL) return false;
...
void CompartmentalizedBottomUpClusterer::SetDistance(int32 comp, int32 i,
                                                     int32 j) {
  KALDI_ASSERT(comp < ncompartments_ && i < npoints_[comp] && j < i);
...
BaseFloat ClusterBottomUpCompartmentalized(
  ...
  KALDI_ASSERT(min_clust >=
               points.size());  // Code does not merge compartments.
  int32 npoints = 0;
...
class RefineClusterer {
 public:
  ...
  RefineClusterer(const std::vector<Clusterable *> &points,
                  std::vector<Clusterable *> *clusters,
                  std::vector<int32> *assignments, RefineClustersOptions cfg)
      : points_(points),
        clusters_(clusters),
    ...
    if (cfg_.top_n > (int32)num_clust_)
      cfg_.top_n = static_cast<int32>(num_clust_);
    KALDI_ASSERT(cfg_.top_n ==
                 static_cast<int32>(static_cast<ClustIndexInt>(cfg_.top_n)));
    t_ = 0;
 ...
 private:
  void InitPoint(int32 point) {
    ...
    for (int32 clust = 0; clust < num_clust_; clust++) {
      if (clust != my_clust) {
        ...
        BaseFloat other_clust_plus_me_objf =
            (*clusters_)[clust]->ObjfPlus(*(points_[point]));

        BaseFloat distance =
            other_clust_objf - other_clust_plus_me_objf;  // negated delta-objf,
                                                          // with only "varying"
                                                          // terms.
    ...
    // iterator
    // that points to the end of that range (i.e. not potentially off the end of
    // the array).
  ...
  void Iterate() {
    ...
    for (iter = 0; iter < num_iters; iter++) {
      // do not do more than 100 iterations-- this is not an exact algorithm
      // anyway.
      int32 cur_t = t_;
  ...
  typedef struct {
    LocalInt clust;
    LocalInt time;
    BaseFloat objf;  // Objf of this cluster plus this point (or minus, if own
                     // cluster).
  } point_info;
  ...
  std::vector<ClustIndexInt> my_clust_index_;  // says for each point, which
                                               // index 0...cfg_.top_n-1
                                               // currently
...
#ifndef KALDI_PARANOID  // don't do this check in "paranoid" mode as we want to
                        // expose bugs.
  if (cfg.num_iters <= 0) {
    return 0.0;
  }  // nothing to do.
...
BaseFloat ClusterKMeansOnce(const std::vector<Clusterable *> &points,
  ...
  KALDI_ASSERT(
      clusters_out
          ->empty());  // or we wouldn't know what to do with pointers in there.
  ...
  {  // This block assigns points to clusters.
    ...
    if (num_points == 1) {
      ...
      skip = 1 + (Rand() %
                  (num_points - 1));  // a number between 1 and num_points-1.
      while (Gcd(skip, num_points) !=
             1) {  // while skip is not coprime to num_points...
        if (skip == num_points - 1) skip = 0;
        skip++;  // skip is now still betweeen 1 and num_points-1.  will cycle
                 // through
  ...
  {  // work out initial value of "ans" (objective function improvement).
    ...
    if (ans < -0.01 &&
      ...
      KALDI_WARN << "ClusterKMeans: objective function after random assignment "
  ...
  for (int32 iter = 0; iter < cfg.num_iters; iter++) {
    ...
    ans += impr;
    if (cfg.verbose)
      KALDI_LOG << "ClusterKMeans: on iteration " << (iter)
...
BaseFloat ClusterKMeans(const std::vector<Clusterable *> &points,
  ...
  if (points.size() == 0) {
    if (clusters_out)
      KALDI_ASSERT(
          clusters_out
              ->empty());  // or we wouldn't know whether to free the pointers.
  ...
  if (clusters_out)
    KALDI_ASSERT(
        clusters_out->empty());  // or we wouldn't know whether to deallocate.
  if (cfg.num_tries == 1) {
    std::vector<int32> assignments;
    return ClusterKMeansOnce(
        points, num_clust, clusters_out,
        (assignments_out != NULL ? assignments_out : &assignments), cfg);
  } else {  // multiple tries.
    if (clusters_out) {
      KALDI_ASSERT(clusters_out->empty());  // we don't know the ownership of
    ...
    for (int32 i = 0; i < cfg.num_tries; i++) {
      ...
      BaseFloat ans = ClusterKMeansOnce(points, num_clust, &clusters_tmp,
                                        &assignments_tmp, cfg);
...
class TreeClusterer {
 public:
  TreeClusterer(const std::vector<Clusterable *> &points, int32 max_clust,
  ...
  BaseFloat Cluster(std::vector<Clusterable *> *clusters_out,
    ...
    while (static_cast<int32>(leaf_nodes_.size()) < max_clust_ &&
           !queue_.empty()) {
  ...
  ~TreeClusterer() {
    for (int32 leaf = 0; leaf < static_cast<int32>(leaf_nodes_.size());
         leaf++) {
    ...
    for (int32 nonleaf = 0; nonleaf < static_cast<int32>(nonleaf_nodes_.size());
         nonleaf++) {
      if (nonleaf_nodes_[nonleaf]->node_total)
 ...
 private:
  struct Node {
    ...
    struct {
      ...
      std::vector<Clusterable *>
          clusters;                    // [branch_factor]... if we do split.
      std::vector<int32> assignments;  // assignments of points to clusters.
    } leaf;
    std::vector<Node *>
        children;  // vector of size branch_factor.   if non-leaf.
  ...
  void CreateOutput(std::vector<Clusterable *> *clusters_out,
    ...
    if (assignments_out) CreateAssignmentsOutput(assignments_out);
    if (clust_assignments_out)
      CreateClustAssignmentsOutput(clust_assignments_out);
    if (clusters_out) CreateClustersOutput(clusters_out);
  }

  // This creates the output index corresponding to an index "index" into the
  // array nonleaf_nodes_.
  ...
  void CreateAssignmentsOutput(std::vector<int32> *assignments_out) {
    ...
    for (int32 leaf = 0; leaf < static_cast<int32>(leaf_nodes_.size());
      ...
      for (int32 i = 0; i < static_cast<int32>(indices.size()); i++) {
        ...
        KALDI_ASSERT((*assignments_out)[indices[i]] ==
                     (int32)(-1));  // check we're not assigning twice.
        (*assignments_out)[indices[i]] = leaf;
      }
    }
#ifdef KALDI_PARANOID
    for (size_t i = 0; i < assignments_out->size(); i++)
      KALDI_ASSERT((*assignments_out)[i] != (int32)(-1));
#endif
  }
  void CreateClustAssignmentsOutput(std::vector<int32> *clust_assignments_out) {
    ...
    for (int32 leaf = 0; leaf < static_cast<int32>(leaf_nodes_.size());
      ...
      if (leaf_nodes_[leaf]->parent == NULL) {  // tree with only one node.
        KALDI_ASSERT(leaf_nodes_.size() == 1 && nonleaf_nodes_.size() == 0 &&
                     leaf == 0);
        parent_index = 0;
      } else {
        if (leaf_nodes_[leaf]->parent->is_leaf)
          parent_index = leaf_nodes_[leaf]->parent->index;
        else
          parent_index = NonleafOutputIndex(leaf_nodes_[leaf]->parent->index);
      }
    ...
    for (int32 nonleaf = 0; nonleaf < static_cast<int32>(nonleaf_nodes_.size());
      ...
      int32 parent_index;
      if (nonleaf_nodes_[nonleaf]->parent == NULL)
        parent_index = index;  // top node.  make it own parent.
      else {
        KALDI_ASSERT(!nonleaf_nodes_[nonleaf]->parent->is_leaf);  // parent is
                                                                  // nonleaf
                                                                  // since child
                                                                  // is nonleaf.
        parent_index =
            NonleafOutputIndex(nonleaf_nodes_[nonleaf]->parent->index);
      }
  ...
  void DoSplit(Node *node) {
    KALDI_ASSERT(node->is_leaf &&
                 node->leaf.best_split >
                     cfg_.thresh * 0.999);  // 0.999 is to avoid potential
                                            // floating-point weirdness under
    ...
    for (int32 i = 0; i < cfg_.branch_factor; i++) {
      ...
      if (i == 0) {
        child->index = node->index;  // assign node's own index in leaf_nodes_
                                     // to 1st child.
        leaf_nodes_[child->index] = child;
      } else {
        child->index =
            leaf_nodes_.size();  // generate new indices for other children.
    ...
    for (int32 i = 0; i < static_cast<int32>(node->leaf.points.size()); i++) {
      ...
      node->children[child_index]->leaf.point_indices.push_back(
          node->leaf.point_indices[i]);
    }
  ...
  void FindBestSplit(Node *node) {
    // takes a leaf node that has just been set up, and does ClusterKMeans with
    // k = cfg_branch_factor.
    KALDI_ASSERT(node->is_leaf);
    if (node->leaf.points.size() == 0) {
      KALDI_WARN << "Warning: tree clustering: leaf with no data";
      node->leaf.best_split = 0;
      return;
    }
    if (node->leaf.points.size() <= 1) {
      node->leaf.best_split = 0;
      return;
    } else {
      // use kmeans.
      BaseFloat impr = ClusterKMeans(node->leaf.points, cfg_.branch_factor,
                                     &node->leaf.clusters,
                                     &node->leaf.assignments, cfg_.kmeans_cfg);
  ...
  void Init() {  // Initializes top node.
    ...
    for (size_t i = 0; i < points_.size(); i++)
      top_node->leaf.point_indices[i] = i;
    FindBestSplit(
        top_node);  // this should always be called when new node is created.
  }
  ...
  std::priority_queue<std::pair<BaseFloat, Node *> >
      queue_;  // contains leaves.
...
BaseFloat TreeCluster(const std::vector<Clusterable *> &points,
                      ...
                      std::vector<int32> *assignments_out,
                      std::vector<int32> *clust_assignments_out,
                      int32 *num_leaves_out, TreeClusterOptions cfg) {
  if (points.size() == 0) {
  ...
  BaseFloat ans = tc.Cluster(clusters_out, assignments_out,
...
BaseFloat ClusterTopDown(const std::vector<Clusterable *> &points,
                         ...
                         TreeClusterOptions cfg) {
  int32 num_leaves = 0;
  BaseFloat ans = TreeCluster(points, max_clust, clusters_out, assignments_out,
                              NULL, &num_leaves, cfg);
  if (clusters_out != NULL) {
    for (size_t j = num_leaves; j < clusters_out->size(); j++)
      delete (*clusters_out)[j];
