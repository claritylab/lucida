...
void TestEventMap() {
  ...
  TableEventMap *T1 =
      new TableEventMap(1, tvec);  // takes ownership of C0a, C1b
  ...
  tmap[1] = C1d;
  TableEventMap *T2 =
      new TableEventMap(1, tmap);  // takes ownership of pointers C0c and C1d.
  ...
  for (size_t p = 0; p < 20; p++) {
...

    {
...

      {
        ...
        EventType evec2;
        evec2.push_back(std::pair<EventKeyType, EventValueType>(
            1, 1));  // make it nonempty.
...
EventMap *RandomEventMap(const std::vector<EventKeyType> &keys) {
  ...
  if (f < 0.333) {  // w.p. 0.333, return ConstantEventMap.
    ...
    for (size_t t = 0; t < (size_t)table_size; t++) {
      if (RandUniform() < nonnull_prob)
        table[t] = RandomEventMap(keys);
      else
        table[t] = NULL;
    }
  ...
  // total fertility is 0.3 + 0.666 = 0.9666, hence this will terminate with
  // finite memory (w.p.1)
}
...
void TestEventMapIo(bool binary) {
  for (size_t p = 0; p < 20; p++) {
    ...
    while (key_set.size() < (size_t)num_keys)
      key_set.insert((Rand() % (2 * max_key)) - 5);
...
void TestEventMapPrune() {
  ...
  EventMap *mapped_em = em->Copy(new_leaves), *pruned_em = mapped_em->Prune();
  ...
  for (int32 i = 0; i < 10; i++) {
    ...
    if (em->Map(event, &answer)) {
      ...
      if (pruned_em == NULL)
        ...
        ret = pruned_em->Map(event, &answer2);
