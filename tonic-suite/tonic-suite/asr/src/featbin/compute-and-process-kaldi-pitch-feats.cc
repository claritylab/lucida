...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Apply Kaldi pitch extractor and pitch post-processor, starting from "
        "wav input.\n"
        "Equivalent to compute-kaldi-pitch-feats | process-kaldi-pitch-feats, "
        "except\n"
        "that it is able to simulate online pitch extraction; see options "
        "like\n"
        "--frames-per-chunk, --simulate-first-pass-online, --recompute-frame.\n"
        "\n"
        "Usage: compute-and-process-kaldi-pitch-feats [options...] "
        "<wav-rspecifier> <feats-wspecifier>\n"
        "e.g.\n"
    ...
    int32 channel =
        -1;  // Note: this isn't configurable because it's not a very
    ...
    std::string wav_rspecifier = po.GetArg(1), feat_wspecifier = po.GetArg(2);
    ...
    for (; !wav_reader.Done(); wav_reader.Next()) {
      ...
      try {
        ComputeAndProcessKaldiPitch(pitch_opts, process_opts, waveform,
                                    &features);
      } catch (...) {
