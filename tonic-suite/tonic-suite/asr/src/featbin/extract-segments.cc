...
/*! @brief This is the main program for extracting segments from a wav file
 - usage :
     - extract-segments [options ..]  <scriptfile > <segments-file>
 <wav-written-specifier>
     - "scriptfile" must contain full path of the wav file.
     - "segments-file" should have the information of the segments that needs to
 be extracted from wav file
     - the format of the segments file : speaker_name wavfilename start_time(in
 secs) end_time(in secs) channel(1 or 2)
     - The channel information in the segfile is optional . default value is
 mono(1).
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage:  extract-segments [options] <wav-rspecifier> <segments-file> "
        "<wav-wspecifier>\n"
        "e.g. extract-segments wav.scp segments ark:- | <some other program>\n"
        " segments-file format: segment_id wav_file_name start_time end_time "
        "[channel]\n"
        ...
        "See also: extract-rows, which does the same thing but to feature "
        "files,\n"
    ...
    while (std::getline(ki.Stream(), line)) {
      ...
      if (channel == -1) {
        if (num_chan == 1)
          ...
          KALDI_ERR
              << "If your data has multiple channels, you must specify the"
                 " channel in the segments file.  Processing segment "
              << segment;
        }
      } else {
        if (channel >= num_chan) {
      ...
      /*
       * This function  return a portion of a wav data from the orignial wav
       * data matrix
       */
      SubMatrix<BaseFloat> segment_matrix(wave_data, channel, 1, start_samp,
                                          end_samp - start_samp);
