...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "This is a special-purpose program to be used in \"predictive "
        "SGMMs\".\n"
        "It takes in an LDA+MLLT matrix, and the original \"full\" LDA matrix\n"
        "as output by the --write-full-matrix option of est-lda; and it "
        "writes\n"
        "out a \"full\" LDA+MLLT matrix formed by the LDA+MLLT matrix plus "
        "the\n"
        "remaining rows of the \"full\" LDA matrix; and also writes out its "
        "inverse\n"
        "Usage: get-full-lda-mat [options] <lda-mllt-rxfilename> "
        "<full-lda-rxfilename> "
        "<full-lda-mllt-wxfilename> [<inv-full-lda-mllt-wxfilename>]\n"
        "E.g.: get-full-lda-mat final.mat full.mat full_lda_mllt.mat "
        "full_lda_mllt_inv.mat\n";
    ...
    full_lda_mllt.Range(0, lda_mllt.NumRows(), 0, lda_mllt.NumCols())
        .CopyFromMat(lda_mllt);
