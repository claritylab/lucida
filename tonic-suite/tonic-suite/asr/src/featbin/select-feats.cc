...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Select certain dimensions of the feature file;  think of it as the "
        "unix\n"
        "command cut -f ...\n"
        "Usage: select-feats <selection> <in-rspecifier> <out-wspecifier>\n"
        "  e.g. select-feats 0,24-22,3-12 scp:feats.scp "
        "ark,scp:feat-red.ark,feat-red.scp\n"
    ...
    for (; !kaldi_reader.Done(); kaldi_reader.Next()) {
      ...
      for (int32 i = 0; i < ranges.size(); ++i) {
        ...
        feats.Range(0, feats.NumRows(), offsets[i], ncol)
            .CopyFromMat(
                kaldi_reader.Value().Range(0, feats.NumRows(), f, ncol));
      }
