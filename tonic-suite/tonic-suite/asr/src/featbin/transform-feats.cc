...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: transform-feats [options] "
        "(<transform-rspecifier>|<transform-rxfilename>) <feats-rspecifier> "
        "<feats-wspecifier>\n"
    ...
    if (ClassifyRspecifier(transform_rspecifier_or_rxfilename, NULL, NULL) ==
      ...
      if (!transform_reader.Open(transform_rspecifier_or_rxfilename,
        ...
        KALDI_ERR << "Problem opening transforms with rspecifier " << '"'
                  ...
                  << " and utt2spk rspecifier " << '"' << utt2spk_rspecifier
                  << '"';
      }
    }
    ...
    double tot_t = 0.0,
           tot_logdet = 0.0;  // to compute average logdet weighted by time...
    ...
    for (; !feat_reader.Done(); feat_reader.Next()) {
      ...
      if (!use_global_transform && !transform_reader.HasKey(utt)) {
        KALDI_WARN << "No fMLLR transform available for utterance " << utt
                   << ", producing no output for this utterance";
        num_error++;
        continue;
      }
      const Matrix<BaseFloat> &trans =
          (use_global_transform ? global_transform
                                : transform_reader.Value(utt));
      int32 transform_rows = trans.NumRows(), transform_cols = trans.NumCols(),
      ...
      if (transform_cols == feat_dim) {
        ...
        KALDI_WARN << "Transform matrix for utterance " << utt
                   << " has bad dimension " << transform_rows << "x"
                   << transform_cols << " versus feat dim " << feat_dim;
        if (transform_cols == feat_dim + 2)
          KALDI_WARN
              << "[perhaps the transform was created by compose-transforms, "
      ...
      if (logdet_type == Unknown) {
        if (transform_rows == feat_dim)
          logdet_type = Logdet;  // actual logdet.
        else if (transform_rows < feat_dim)
          logdet_type = PseudoLogdet;  // see below
        else
          logdet_type = DimIncrease;  // makes no sense to have any logdet.
        // PseudoLogdet is if we have a dimension-reducing transform T, we
        // compute
        // 1/2 logdet(T T^T).  Why does this make sense?  Imagine we do MLLT
        // after
        // LDA and compose the transforms; the MLLT matrix is A and the LDA
        // matrix is L,
        // so T = A L.  T T^T = A L L^T A, so 1/2 logdet(T T^T) = logdet(A) +
        // 1/2 logdet(L L^T).
        // since L L^T is a constant, this is valid for comparing likelihoods if
        // we're
        // just trying to see if the MLLT is converging.
      }

      if (logdet_type != DimIncrease) {  // Accumulate log-determinant stats.
        SubMatrix<BaseFloat> linear_transform(trans, 0, trans.NumRows(), 0,
                                              feat_dim);
        ...
        if (use_global_transform) {
          if (cached_logdet == -1) cached_logdet = 0.5 * TT.LogDet(NULL);
    ...
    if (logdet_type != Unknown && logdet_type != DimIncrease)
      KALDI_LOG << "Overall average "
                << (logdet_type == PseudoLogdet ? "[pseudo-]" : "")
                << "logdet is " << (tot_logdet / tot_t) << " over " << tot_t
                << " frames.";
    KALDI_LOG << "Applied transform to " << num_done << " utterances; "
              << num_error << " had errors.";
