...
bool AppendFeats(const std::vector<Matrix<BaseFloat> > &in, std::string utt,
                 int32 tolerance, Matrix<BaseFloat> *out) {
  // Check the lengths
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Paste feature files (assuming they have the same lengths);  think of "
        "the\n"
        "unix command paste a b.\n"
        "Usage: paste-feats <in-rspecifier1> <in-rspecifier2> "
        "[<in-rspecifier3> ...] <out-wspecifier>\n"
        " or: paste-feats <in-rxfilename1> <in-rxfilename2> [<in-rxfilename3> "
        "...] <out-wxfilename>\n"
        " e.g. paste-feats ark:feats1.ark \"ark:select-feats 0-3 "
        "ark:feats2.ark ark:- |\" ark:feats-out.ark\n"
    ...
    if (ClassifyRspecifier(po.GetArg(1), NULL, NULL) != kNoRspecifier) {
      ...
      for (int32 i = 2; i < po.NumArgs(); i++) {
        ...
        RandomAccessBaseFloatMatrixReader *rd =
            new RandomAccessBaseFloatMatrixReader(rspecifier);
        input.push_back(rd);
      }
      ...
      for (; !input1.Done(); input1.Next()) {
        ...
        if (i != static_cast<int32>(input.size())) continue;
      ...
      for (int32 i = 0; i < input.size(); i++) delete input[i];
      input.clear();
