...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage: compose-transforms [options] "
        "(<transform-A-rspecifier>|<transform-A-rxfilename>) "
        "(<transform-B-rspecifier>|<transform-B-rxfilename>) "
        "(<transform-out-wspecifier>|<transform-out-wxfilename>)\n"
        " Note: it does matrix multiplication (A B) so B is the transform that "
        "gets applied\n"
        "  to the features first.  If b-is-affine = true, then assume last "
        "column of b corresponds to offset\n"
        ...
        " See also: transform-feats, transform-vec, extend-transform-dim, "
        "est-lda, est-pca\n";
    ...
    po.Register("utt2spk", &utt2spk_rspecifier,
                "rspecifier for utterance to speaker map (if mixing utterance "
                "and speaker ids)");

    po.Register("b-is-affine", &b_is_affine,
                "If true, treat last column of transform b as an offset term "
                "(only relevant if a is affine)");
    po.Register(
        "binary", &binary,
    ...
    bool a_is_rspecifier =
             (ClassifyRspecifier(transform_a_fn, NULL, NULL) != kNoRspecifier),
         b_is_rspecifier =
             (ClassifyRspecifier(transform_b_fn, NULL, NULL) != kNoRspecifier),
         c_is_wspecifier = (ClassifyWspecifier(transform_c_fn, NULL, NULL,
                                               NULL) != kNoWspecifier);
    ...
    if (a_is_rspecifier || b_is_rspecifier) {
      ...
      if (a_is_rspecifier) {
        ...
        if (b_is_rspecifier) {  // both are rspecifiers.
          ...
          for (; !a_reader.Done(); a_reader.Next()) {
            if (utt2spk_rspecifier !=
                "") {  // assume a is per-utt, b is per-spk.
              ...
              Matrix<BaseFloat> c;
              if (!ComposeTransforms(a_reader.Value(),
                                     b_reader.Value(a_reader.Key()),
                                     b_is_affine, &c))
                continue;  // warning will have been printed already.
              c_writer.Write(utt, c);
            } else {  // Normal case: either both per-utterance or both
                      // per-speaker.
              if (!b_reader.HasKey(a_reader.Key())) {
                KALDI_WARN << "Second table does not have key "
                           << a_reader.Key();
              } else {
                Matrix<BaseFloat> c;
                if (!ComposeTransforms(a_reader.Value(),
          ...
          for (; !a_reader.Done(); a_reader.Next()) {
            Matrix<BaseFloat> c;
            if (!ComposeTransforms(a_reader.Value(), b, b_is_affine, &c))
              continue;  // warning will have been printed already.
            c_writer.Write(a_reader.Key(), c);
          }
        }
        ...
        for (; !b_reader.Done(); b_reader.Next()) {
          Matrix<BaseFloat> c;
          if (!ComposeTransforms(a, b_reader.Value(), b_is_affine, &c))
            continue;  // warning will have been printed already.
      ...
      ReadKaldiObject(transform_b_fn, &b);
      Matrix<BaseFloat> c;
      if (!b_is_affine && a.NumRows() == a.NumCols() + 1 &&
          a.NumRows() == b.NumRows() && a.NumCols() == b.NumCols())
        KALDI_WARN
            << "It looks like you are trying to compose two affine transforms"
