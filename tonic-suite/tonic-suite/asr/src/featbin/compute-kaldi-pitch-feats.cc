...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Apply Kaldi pitch extractor, starting from wav input.  Output is "
        "2-dimensional\n"
        "features consisting of (NCCF, pitch in Hz), where NCCF is between -1 "
        "and 1, and\n"
        "higher for voiced frames.  You will typically pipe this into\n"
        "process-kaldi-pitch-feats.\n"
        "Usage: compute-kaldi-pitch-feats [options...] <wav-rspecifier> "
        "<feats-wspecifier>\n"
        "e.g.\n"
        "compute-kaldi-pitch-feats --sample-frequency=8000 scp:wav.scp ark:- \n"
        "\n"
        "See also: process-kaldi-pitch-feats, "
