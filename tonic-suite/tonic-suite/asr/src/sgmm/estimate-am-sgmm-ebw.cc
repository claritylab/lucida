...
void EbwAmSgmmUpdater::Update(const MleAmSgmmAccs &num_accs,
                              const MleAmSgmmAccs &den_accs, AmSgmm *model,
...
void EbwAmSgmmUpdater::ComputePhoneVecStats(
    ...
    const std::vector<SpMatrix<double> > &H, int32 j, int32 m,
    const Vector<double> &w_jm, double gamma_jm, Vector<double> *g_jm,
...
void EbwAmSgmmUpdater::UpdatePhoneVectorsInternal(
    const MleAmSgmmAccs &num_accs, const MleAmSgmmAccs &den_accs, AmSgmm *model,
    const std::vector<SpMatrix<double> > &H, double *auxf_impr,
...
double EbwAmSgmmUpdater::UpdatePhoneVectors(
    const MleAmSgmmAccs &num_accs, const MleAmSgmmAccs &den_accs, AmSgmm *model,
...
double EbwAmSgmmUpdater::UpdateWParallel(const MleAmSgmmAccs &num_accs,
  ...
  {
    double garbage;
    UpdateWParallelClass c_num(num_accs, *model, w, &F_i_num, &g_i_num,
                               &garbage);
    RunMultiThreaded(c_num);
  }
  {
    double garbage;
    UpdateWParallelClass c_den(den_accs, *model, w, &F_i_den, &g_i_den,
                               &garbage);
  ...
  for (int32 i = 0; i < I; i++) {
    ...
    if (i < 10 || objf_impr / (num_count_vec(i) + 1.0e-10) > 2.0) {
      KALDI_LOG << "Predicted objf impr for w per frame is "
                << (objf_impr / (num_count_vec(i) + 1.0e-10)) << " over "
                << num_count_vec(i) << " frames.";
    }
...
double EbwAmSgmmUpdater::UpdateVars(
    const MleAmSgmmAccs &num_accs, const MleAmSgmmAccs &den_accs,
    const std::vector<SpMatrix<double> > &S_means, AmSgmm *model) const {
...
double EbwAmSgmmUpdater::UpdateSubstateWeights(const MleAmSgmmAccs &num_accs,
                                               const MleAmSgmmAccs &den_accs,
                                               AmSgmm *model) {
  ...
  for (int32 j = 0; j < num_accs.num_states_; j++) {
    ...
    Vector<double> num_occs(M), den_occs(M), orig_weights(model->c_[j]),
        weights(model->c_[j]);

    for (int32 m = 0; m < M; m++) {
      num_occs(m) =
          num_accs.gamma_[j].Row(m).Sum() + options_.tau_c * weights(m);
