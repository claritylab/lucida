...
struct MleAmSgmmOptions {
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("tau-vec", &tau_vec,
                 module + "Smoothing for phone vector estimation.");
    po->Register("tau-c", &tau_c,
                 module + "Smoothing for substate weights estimation.");
    ...
    po->Register("check-v", &check_v,
                 module +
                     "If true, check real auxf "
...
class MleAmSgmmAccs {
 public:
  ...
  BaseFloat AccumulateFromPosteriors(
      ...
      int32 state_index, SgmmUpdateFlagsType flags);
  ...
  void CommitStatsForSpk(const AmSgmm &model, const VectorBase<BaseFloat> &v_s);

  /// Accessors
  void GetStateOccupancies(Vector<BaseFloat> *occs) const;
  const std::vector<Matrix<double> > &GetOccs() const { return gamma_; }
...
class MleAmSgmmUpdater {
 public:
  ...
  BaseFloat Update(const MleAmSgmmAccs &accs, AmSgmm *model,
  ...
  double UpdatePhoneVectorsCheckedFromClusterable(
      ...
      const std::vector<SpMatrix<double> > &H, AmSgmm *model);
 ...
 protected:
  ...
  static void ComputeQ(const MleAmSgmmAccs &accs, const AmSgmm &model,
                       std::vector<SpMatrix<double> > *Q);

  /// Compute the S_means quantities, minus sum: (Y_i M_i^T + M_i Y_I^T).
  static void ComputeSMeans(const MleAmSgmmAccs &accs, const AmSgmm &model,
 ...
 private:
  ...
  void ComputeSmoothingTerms(const MleAmSgmmAccs &accs, const AmSgmm &model,
  ...
  double UpdatePhoneVectors(const MleAmSgmmAccs &accs, AmSgmm *model,
                            const std::vector<SpMatrix<double> > &H,
  ...
  void UpdatePhoneVectorsInternal(const MleAmSgmmAccs &accs, AmSgmm *model,
                                  const std::vector<SpMatrix<double> > &H,
                                  const SpMatrix<double> &H_sm,
                                  const Vector<double> &y_sm, double *auxf_impr,
                                  double *like_impr, int32 num_threads,
  ...
  // and backtracking.
  double UpdatePhoneVectorsChecked(const MleAmSgmmAccs &accs, AmSgmm *model,
                                   const std::vector<SpMatrix<double> > &H);

  // Called (indirectly) from UpdatePhoneVectorsCheckedFromClusterable()
  void UpdatePhoneVectorsCheckedFromClusterableInternal(
      const std::vector<SgmmClusterable *> &stats,
      const std::vector<SpMatrix<double> > &H, AmSgmm *model, double *count_ptr,
  ...
  static void UpdateWParallelGetStats(const MleAmSgmmAccs &accs,
                                      const AmSgmm &model,
  ...
  double UpdateWSequential(const MleAmSgmmAccs &accs, AmSgmm *model);
  double UpdateSubstateWeights(const MleAmSgmmAccs &accs, AmSgmm *model);
...
class UpdateWParallelClass : public MultiThreadable {
 public:
  UpdateWParallelClass(const MleAmSgmmAccs &accs, const AmSgmm &model,
                       const Matrix<double> &w, Matrix<double> *F_i,
  ...
  inline void operator()() {
    ...
    MleAmSgmmUpdater::UpdateWParallelGetStats(
        accs_, model_, w_, &F_i_, &g_i_, &tot_like_, num_threads_, thread_id_);
  }
