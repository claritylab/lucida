...
void AmSgmm::Write(std::ostream &out_stream, bool binary,
  ...
  int32 num_states = NumPdfs(), feat_dim = FeatureDim(), num_gauss = NumGauss();
  ...
  if (write_params & kSgmmNormalizers) {
    ...
    if (n_.empty())
      ...
      for (int32 j = 0; j < num_states; j++) n_[j].Write(out_stream, binary);
  }
...
void AmSgmm::Check(bool show_properties) {
  int32 num_states = NumPdfs(), num_gauss = NumGauss(), feat_dim = FeatureDim(),
        phn_dim = PhoneSpaceDim(), spk_dim = SpkSpaceDim();

  if (show_properties)
    KALDI_LOG << "AmSgmm: #states = " << num_states
              << ", #Gaussians = " << num_gauss
  ...
  {  // check v, c.
    ...
    for (int32 j = 0; j < num_states; j++) {
      ...
      nSubstatesTot += M_j;
      KALDI_ASSERT(M_j > 0 && v_[j].NumRows() == M_j && c_[j].Dim() == M_j &&
                   v_[j].NumCols() == phn_dim);
    }
...
void AmSgmm::InitializeFromFullGmm(const FullGmm &full_gmm, int32 num_states,
...
void AmSgmm::CopyFromSgmm(const AmSgmm &other, bool copy_normalizers) {
...
void AmSgmm::CopyGlobalsInitVecs(const AmSgmm &other, int32 phn_subspace_dim,
                                 int32 spk_subspace_dim, int32 num_pdfs) {
  if (phn_subspace_dim < 1 || phn_subspace_dim > other.PhoneSpaceDim()) {
    KALDI_WARN << "Initial phone-subspace dimension must be in [1, "
               << other.PhoneSpaceDim() << "]. Changing from "
               << phn_subspace_dim << " to " << other.PhoneSpaceDim();
  ...
  // Copy global params
  SigmaInv_ = other.SigmaInv_;
  int32 num_gauss = diag_ubm_.NumGauss(), data_dim = other.FeatureDim();
  M_.resize(num_gauss);
...
void AmSgmm::ComputePerFrameVars(
  ...
  for (int32 ki = 0, last = gselect.size(); ki < last; ki++) {
    ...
    per_frame_vars->nti(ki) =
        -0.5 * VecVec(per_frame_vars->xti.Row(ki), SigmaInv_xt) + logdet_s;
  }
}
...
BaseFloat AmSgmm::ComponentPosteriors(
    const SgmmPerFrameDerivedVars &per_frame_vars, int32 j,
...
void AmSgmm::SplitSubstates(const Vector<BaseFloat> &state_occupancies,
  ...
  if (target_nsubstates <= tot_n_substates_old || tot_n_substates_old == 0) {
    KALDI_WARN << "Cannot split from " << (tot_n_substates_old) << " to "
               << (target_nsubstates) << " substates.";
    return;
  }
  ...
  for (int32 n_states = tot_n_substates_old; n_states < target_nsubstates;
       n_states++) {
  ...
  while (!substate_counts.empty()) {
    ...
    for (; n_substates_j < tgt_n_substates_j; n_substates_j++) {
      int32 split_substate =
          std::max_element(c_[j].Data(), c_[j].Data() + n_substates_j) -
          c_[j].Data();
...
class ComputeNormalizersClass : public MultiThreadable {  // For multi-threaded.
 public:
  ComputeNormalizersClass(AmSgmm *am_sgmm, int32 *entropy_count_ptr,
...
void AmSgmm::ComputeNormalizersInternal(int32 num_threads, int32 thread,
  ...
  int j_start = thread * block_size,
      j_end = std::min(NumPdfs(), j_start + block_size);
...
void AmSgmm::ComputeNormalizersNormalized(
  ...
  {  // Check sets in normalize_sets are disjoint and cover all Gaussians.
    ...
    for (int32 i = 0; i < normalize_sets.size(); i++)
      for (int32 j = 0; static_cast<size_t>(j) < normalize_sets[i].size();
           j++) {
...
void AmSgmm::ComputeFmllrPreXform(const Vector<BaseFloat> &state_occs,
  ...
  int32 num_states = NumPdfs(), num_gauss = NumGauss(), dim = FeatureDim();
...
template void AmSgmm::GetNtransSigmaInv(vector<Matrix<float> > *out) const;
template void AmSgmm::GetNtransSigmaInv(vector<Matrix<double> > *out) const;
...
template void AmSgmm::ComputeH(std::vector<SpMatrix<float> > *H_i) const;
template void AmSgmm::ComputeH(std::vector<SpMatrix<double> > *H_i) const;
...
void AmSgmm::InitializeMw(int32 phn_subspace_dim,
  ...
  for (int32 i = 0; i < num_gauss; i++) {
    ...
    thisM.Range(0, ddim, 1, phn_subspace_dim - 1)
        .CopyFromMat(norm_xform.Range(0, ddim, 0, phn_subspace_dim - 1),
                     kNoTrans);
  }
}
...
BaseFloat AmSgmm::GaussianSelectionPreselect(
    const SgmmGselectConfig &config, const VectorBase<BaseFloat> &data,
    const std::vector<int32> &preselect, std::vector<int32> *gselect) const {
...
void AmSgmmFunctions::ComputeDistances(const AmSgmm &model,
  ...
  int32 num_states = model.NumPdfs(), phn_space_dim = model.PhoneSpaceDim(),
