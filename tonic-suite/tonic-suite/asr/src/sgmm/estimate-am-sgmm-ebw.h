...
struct EbwAmSgmmOptions {
  ...
  BaseFloat
      tau_w;  ///< Tau value for smoothing update of weight projectsions (w)
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register(
        "tau-w", &tau_w,
        module + "Smoothing constant for estimation of weight projections (w)");
    po->Register("lrate-w", &lrate_w,
                 module + "Learning rate constant for weight-projections");
...
class EbwAmSgmmUpdater {
 public:
  ...
  void Update(const MleAmSgmmAccs &num_accs, const MleAmSgmmAccs &den_accs,
              AmSgmm *model, SgmmUpdateFlagsType flags,
 ...
 private:
  ...
  void UpdatePhoneVectorsInternal(const MleAmSgmmAccs &num_accs,
                                  ...
                                  double *auxf_impr, int32 num_threads,
                                  int32 thread_id) const;
  // Called from UpdatePhoneVectorsInternal
  static void ComputePhoneVecStats(const MleAmSgmmAccs &accs,
                                   const AmSgmm &model,
                                   const std::vector<SpMatrix<double> > &H,
                                   int32 j, int32 m, const Vector<double> &w_jm,
                                   double gamma_jm, Vector<double> *g_jm,
                                   SpMatrix<double> *H_jm);

  double UpdateM(const MleAmSgmmAccs &num_accs, const MleAmSgmmAccs &den_accs,
                 const std::vector<SpMatrix<double> > &Q_num,
                 const std::vector<SpMatrix<double> > &Q_den,
                 AmSgmm *model) const;

  double UpdateN(const MleAmSgmmAccs &num_accs, const MleAmSgmmAccs &den_accs,
                 AmSgmm *model) const;
