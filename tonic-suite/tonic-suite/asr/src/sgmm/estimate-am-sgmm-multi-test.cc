...
void TestMultiSgmmEst(const std::vector<AmSgmm *> &models,
  ...
  int32 num_gauss = models[0]->NumGauss(), feat_dim = models[0]->FeatureDim(),
        phn_dim = models[0]->PhoneSpaceDim(),
        spk_dim = models[0]->SpkSpaceDim(), num_models = models.size();
  ...
  for (int32 i = 0; i < num_models; ++i) {
    ...
    for (int32 f = 0; f < feats[i].NumRows(); ++f) {
      ...
      models[i]->ComputePerFrameVars(feats[i].Row(f), gselect, spk_vars, 0.0,
                                     &frame_vars);
      loglike +=
          acc->Accumulate(*models[i], frame_vars, spk_vars.v_s, 0, 1.0, flags);
    }
  ...
  for (int32 i = 0; i < num_models; ++i) {
    ...
    for (int32 f = 0; f < feats[i].NumRows(); ++f) {
      ...
      new_models[i]->ComputePerFrameVars(feats[i].Row(f), gselect, spk_vars,
                                         0.0, &frame_vars);
