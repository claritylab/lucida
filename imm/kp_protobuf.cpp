...
map<string,Mat> read_map(const char * filename)
{
  ...
  for(int i = 0;i<read_stored_data.size();i++){
    ...
    stored_mat.create(read_stored_mat.row(),read_stored_mat.col(),read_stored_mat.type());
...
Mat read_mat(const char * filename)
{
  ...
  Mat descriptors(read_stored_data.row(),read_stored_data.col(),read_stored_data.type());
