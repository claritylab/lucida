...
1. Implement the Thrift interface jointly defined in `lucida/lucidaservice.thrift` and `lucida/lucidatypes.thrift`.
 ...
 2.  `lucida/lucidatypes.thrift`:
    ...
    * If the function call is `infer`:
    ...
    <size of the following list>, <list of integers>]
    ```
    
    , which indicates a node in the service graph. The `host:port` specifies the location of the service,
    the first list specifies the indices of nodes that point to the service,
    and the second list specifies the indices of nodes that the service points to.
    The size matches the length of the list following it, and thus must be a non-negative integer
    (0 indicating an empty list).
    If the first list is empty, the command center points to the node.
    If the second list is empty, the node does not send any further request.
    ...
    tags: ["localhost", "9090", "0", "1", 2"] },
    { type: "image",
    data: [ "1234567...abcdefg" ],
    tags: ["localhost", "9091", "0", "2", "2", "3"] },
    { type: "text",
    data: [ "Which person in my family is in this image?" ],
    tags: ["localhost", "9092", "2", "0", "1", "0"] },
    { type: "image",
    data: [ "" ],
    tags: ["localhost", "9093", "1", "1", "0"] }
    ] }
    ```
    ...
    the configuration file of the command center. See more detais in step 2.
