...
It was originally developed by Facebook and now both developed by the open-source community (Apache Thrift) and Facebook.
...
Java very well. Also, Apache Thrift seems to be more popular. Therefore, we use Apache Thrift for services written in Python and Java,
...
Each `QuerySpec` in `content` corresponds to one service (CMD is not considered to be service)
in the service graph, a directed acyclic graph (DAG) describing all services that are needed for the query.
Thus, for the following graph, only one `QuerySpec` is present in `content`:
...
. As you see, this way of using a tag to describe a node in the service makes you define arbitrarily complicated service graphs.
For example, for the following service graph:
...
. Notice that the command center only sends the image data to YOS0 and YOS1 but not to YOS3,
because YOS3 is not directly pointed to by the command center. However, since YOS2 is the only service that receives
text, it recieves the text no matter whether it is pointed to by the command center or not.

The command center guarantees to send a service graph constructed following the above rules, but each service in the graph
is responsible for parsing the graph, sending the request(s) to the service(s) it points to, and returning the response(s) to
the service(s) it is pointed to by. Each service is also allowed to ignore or modify the graph if that is necessary,
...
. Thus, most current services ignore the `tags` without any problem.
