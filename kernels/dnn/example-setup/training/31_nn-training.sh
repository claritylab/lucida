...
for iter in $(seq $EPOCHS); do
    if [ $iter == 1 ]; then
        ...
        model_old="bin:data/weights-$(($iter-1))"
    fi
    model_new="bin:data/weights-$iter"
    log_tr=log/nn-training.$iter.log
    log_cv=log/nn-training.cv.$iter.log
    ...
    forward_cv  "$model_new" "$log_cv" || exit $?
