...
int main(int argc, char *argv[])
{
    ...
    FILE * f = fopen("/home/jahausw/list","r");
    if (f == 0) { fprintf(stderr,"File %s not found\n",argv[1]); exit(1); }
    int numExps = fill(f, exps, temp);

    FILE * f1 = fopen("/home/jahausw/questions","r");
    ...
    for (int i = 0; i < numExps; ++i){
        for (int k = 0; k < numQs; ++k){
            slre_match(slre[i], bufs[k], buf_len[k], caps);
            // printf("Result: %d\n", res);
        }
    }
    ...
    gettimeofday(&tv1,NULL);
    // ar::simt_tau::par_for(numQs, [&](size_t i)
    // {
    //     for (int k = 0; k < numExps; ++k){
    //         slre_match(slre[k], bufs[i], buf_len[i], caps);
    //         // printf("Result: %d\n", res);
    //     }
    // });
