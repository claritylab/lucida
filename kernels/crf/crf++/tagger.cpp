...
int iterations;
    ...
    int crfpp_test_seq(const Param &param) {
    ...
    int crfpp_test_tau(const Param &param) {
      if (param.get<bool>("version")) {
        std::cout <<  param.version();
        return -1;
      }
...
int crfpp_test(int argc, char **argv) {
  ...
  gettimeofday(&t2, NULL);
  cpu_elapsedTime = calculateMiliseconds(t1,t2);
  printf("\nPThread Time=%4.3f ms\n",  par_elapsedTime);
  return 0;
}

int crfpp_test2(const char *arg) { CRFPP::Param param;
