...
do
	...
	do
		echo $kernel run: $i
		perf stat -e $stats -o ${kernel}_log${i}.txt ./${kernel}_script.sh > /dev/null
		mv ${kernel}_log${i}.txt $RESDIR
		perf stat -e $stats1 -o ${kernel}_log${i}.txt ./${kernel}_script.sh > /dev/null
		mv ${kernel}_log${i}.txt $RESDIR1
		perf stat -e $stats2 -o ${kernel}_log${i}.txt ./${kernel}_script.sh > /dev/null
