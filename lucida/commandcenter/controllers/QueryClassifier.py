...
class DummyClassifier(object):
	...
	# Returns the only possible result.
...
class QueryClassifier(object):
	...
	# Trains a model dataset found in data/*.txt.
	def train(self, input_type, query_classes):
		...
		for query_class_name in query_classes:
			path = 'data/' + query_class_name + '.txt' # e.g. ../data/IMM_QA.txt
			log('Opening ' + path)
		...
		('classifier',         LinearSVC()) # Change it if you like!
		])
		...
		if not os.path.exists('models'):
			os.makedirs('models')
		with open('models/dumped_classifier_' + input_type + '.pkl',
				  'wb') as fid:
			log('Saving model for ' + input_type)
			cPickle.dump(pipeline, fid)
		return pipeline

	# Loads a trained model from file system.
	...
	# Predicts from speech_input and image_input.
