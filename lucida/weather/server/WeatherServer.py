...
class WeatherHandler(LucidaService.Iface):
    ...
    def infer(self, LUCID, query):
        ...
        f = urllib.urlopen(WU_API_URL_BASE + '%s/conditions/q/%s.json' % (WU_API_KEY, url_location))
        json_string = f.read()
        ...
        if 'error' not in parsed_json['response'] and 'current_observation' in parsed_json:
            ...
            f = urllib.urlopen(OWM_API_URL_BASE + 'q=%s&appid=%s&units=imperial&type=like' % (url_location, OWM_API_KEY))
            json_string = f.read()
            parsed_json = json.loads(json_string)
