...
if [ -z $BFW_UID ] && [ $BFW_SKIP_UPDATE -eq 0 ]; then
  ...
  while [ 1 ]; do
    ...
    while [ 1 ]; do
      export BFW_UID
      export BFW_PWD
      phantomjs check_credentials.js | tee phantom.out
      grep phantom.out -e "Logged into Micrsoft Account" > /dev/null
      if [ $? -eq 0 ]; then
        break 2
      fi
      ...
      if [ $? -eq 0 ]; then
        ...
        if [ $counter -gt 1 ]; then
          ...
          printf "Do you want to skip automatic endpoint update? This will require you to update endpoint manually as explained in README (y/n) "
          read response
          if [[ $response == "y" ]] || [[ $response == "Y" ]]; then
            BFW_SKIP_UPDATE=1
            BFW_UID=
